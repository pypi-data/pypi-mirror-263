...
def do_partial_upload(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for file in cerebrium_config.file_list:
            ...
            # if the size of the file is >100MB, use a link instead
            if os.stat(file).st_size > 100 * 1024 * 1024:
                if os.path.isfile(temp_file):
                    os.remove(temp_file)
                if os.path.islink(file):
                    file = os.readlink(file)
                os.symlink(file, temp_file)
            else:
...
def _setup_request(
    ...
    if setup_response.status_code != 200:
        setup_spinner.fail("❌ There was an error deploying your app")
        cerebrium_log(
    ...
    setup_spinner.text = ""
    setup_spinner.ok("✅ App setup complete!")
    setup_spinner.stop()
