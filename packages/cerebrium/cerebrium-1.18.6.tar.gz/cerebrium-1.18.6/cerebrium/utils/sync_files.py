...
def get_md5(file_path: str, max_size_mb: int = 100) -> str:
    ...
    if os.stat(file_path).st_size > max_size_mb * 1024 * 1024:
        cerebrium_log(
            f"File {file_path} too large to hash. Using file info instead",
            level="DEBUG",
        )
        hasher.update(str(os.stat(file_path)).encode())
...
def upload_file(upload_url: str, file_name: str, file_path: str, pbar: tqdm) -> int:
...
def upload_files_to_s3(
    ...
    # Get the working paths in the tempfile dir. Necessary because the file paths in the upload_urls are relative to the working directory
    working_paths = [
        os.path.join(working_dir, file) for file in file_keys if file in upload_urls
    ]
    ...
    with tqdm(
        ...
        if parallel:
            ...
            for key, real_path in real_paths.items():
                ...
                upload_url = upload_urls.get(key)
                if not upload_url:
                    continue  # Skip if no upload URL is provided for this file
                uploaded_count += upload_file(upload_url, key, real_path, pbar)
    ...
    for file_name, reqs in requirements_files:
        if reqs:
            if file_name == "shell_commands.sh":
                shell_commands_to_file(reqs, os.path.join(temp_dir, file_name))
            else:
