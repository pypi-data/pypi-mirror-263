...
def writeAnaHTML(analysis, excl, anaDir, histoList, prediction=None):
    ...
    if prediction is None:
        index.write(' <p>Exclusion with SM as background = {:.2f}<br/>\n'.format(excl[cfg.smbg]))
        index.write(' Expected exclusion  = {:.2f}\n</br>'.format(excl[cfg.expected]))
        index.write(' Exclusion with data taken as background = {:.2f}</p>\n'.format(excl[cfg.databg]))
    else:
...
def writePoolHTML(pool, poolDir, thisPoolMatchedAnalyses, exclusion):
    ...
    for ana, excl in thisPoolMatchedAnalyses.items():
        ...
        index.write(' Exclusion with SM as background = {:.2f}</p>\n'.format(excl[cfg.smbg]))
