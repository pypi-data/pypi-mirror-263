...
class DBSpace:
    ...
    def add_chunk(self, size, offset=0, chunk_name=None):
        """
        添加chunk
        ...
        self.server._add_chunk_file(_chunk_name)
        code, out = self.server.run_cmd(f"onspaces -a {self.name} -p ./{_chunk_name} -o {offset} -s {size}",
...
class Chunk:
    ...
    def __init__(self, chunk_info: dict):
        ...
        self._offset = chunk_info.get('pathname')
