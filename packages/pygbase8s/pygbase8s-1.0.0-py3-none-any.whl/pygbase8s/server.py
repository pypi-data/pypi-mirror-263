...
class Server(Instance):
    ...
    def initialize(self):  # 初始化实例
        ...
        :return:
        """
        _, _ = self.onconfig, self.sqlhosts
        self.run_cmd(f"mkdir -p {self.path};chmod 755 {self.path}")
    ...
    def _add_chunk_file(self, chunk_name):
        """
        实例中创建chunk文件
    ...
    def add_dbspace(self, name, size, type='BA', offset=0, pagesize=2, chunk_name=None):
        """
        添加数据空间
        :param name: str, 空间名称
        ...
        self._add_chunk_file(primary_chunk_name)
        if type.upper() == 'BA':
            code, out = self.run_cmd(f"onspaces -c -d {name} -p ./{primary_chunk_name} -o {offset} -s {size} -k {pagesize}",
                                     cwd=self.path)
        elif type.upper() == 'TBA':
            code, out = self.run_cmd(f"onspaces -c -d {name} -p ./{primary_chunk_name} -o {offset} -s {size} -k {pagesize} -t",
                                     cwd=self.path)
        elif type.upper() == 'BBA':
            code, out = self.run_cmd(f"onspaces -c -b {name} -g {pagesize} -p ./{primary_chunk_name} -o {offset} -s {size}",
                                     cwd=self.path)
        elif type.upper() == 'SBA':
            code, out = self.run_cmd(f"onspaces -c -S {name} -p ./{primary_chunk_name} -o {offset} -s {size}",
                                     cwd=self.path)
        elif type.upper() == 'UBA':
            code, out = self.run_cmd(f"onspaces -c -S {name} -t -p ./{primary_chunk_name} -o {offset} -s {size}",
                                     cwd=self.path)
        else:
