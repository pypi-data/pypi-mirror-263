    ...
    def calc_search_volume(self, keywords_bundle, searchad_data, datalab_data):
        try:
            ...
            for kwd in keywords_bundle:
                target_searchad_datum = next(
                    filter(lambda item: item["relKeyword"] == kwd, searchad_data)
                )
                target_datalab_datum = next(
                    filter(lambda item: item["keyword"] == kwd, datalab_data)
                )

                monthly_pc_volume = target_searchad_datum.get("monthlyPcQcCnt")
                monthly_mo_volume = target_searchad_datum.get("monthlyMobileQcCnt")

                total_volume = monthly_pc_volume + monthly_mo_volume
                total_ratio = sum(
                    [d["ratio"] for d in target_datalab_datum.get("data")]
                )

                volume_per_ratio = total_volume / total_ratio if total_ratio > 0 else 0

                for d in target_datalab_datum.get("data"):
                    new_data = {
                        "keyword_text": kwd,
                        "keyword_date": pendulum.parse(d["period"], tz="Asia/Seoul"),
...
if __name__ == "__main__":
    search_volume_crawler = SearchVolumeCrawler(
        # date_from="20200101",
        keywords=["한국", "중국", "일본", "미국", "영국", "프랑스"],
    )
    search_volume = search_volume_crawler.get_search_volume()
