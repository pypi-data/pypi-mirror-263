...
class CollectionStore:
    ...
    def create_collection(self, shortcode, path, name=None, description=None):
        shortcode = shortcode or str(uuid.uuid4())
        name = name or "Collection: " + os.path.basename(path)
        description = description or "Collection for " + shortcode
    ...
    def get_default_collection(self, shortcode):
        collection = self.get_or_create_collection(shortcode=self.DEFAULT_SHORTCODE,
                                                   path=self.DEFAULT_PATH,
                                                   name=self.DEFAULT_NAME,
                                                   description=self.DEFAULT_DESCRIPTION)
        self.add_appliance_collection(collection.shortcode, shortcode)
        return collection
