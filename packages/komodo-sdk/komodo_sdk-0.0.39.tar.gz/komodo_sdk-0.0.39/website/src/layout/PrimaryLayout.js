...
const PrimaryLayout = ({ children }) => {
  ...
  const handleChatDelete = async (id, i, e) => {
    e.stopPropagation();
    try {
      const agent = await ApiDelete(`${API_Path.getDeleteConversation}${id}`, {
        agent_shortcode: listData?.reactSelect?.shortcode,
      });
      if (agent?.status === 200) {
  ...
  return (
    <div className="flex lg:block">
      <div className="z-[999]">
        <img src={menuIcon} className="hidden lg:flex w-[24px] h-[24px] m-3" onClick={toggleDrawer} alt="" />
      </div>
      {/* lg:hidden */}
      <div className="lg:hidden w-1/5 font-cerebri flex border-r-[0.5px] border-[#CDCDCD]">
        <Sidebar />
        <div className="font-cerebri w-[-webkit-fill-available] flex flex-col justify-between">
          <div>
            ...
            <div className="h-[calc(100vh-180px)] overflow-auto scrollbar">
              <div className="mt-8">
                <div className="border-t-[0.5px] border-[#F6F6F9]">{renderChatItems()}</div>
              </div>
            </div>
          </div>
      ...
      <Drawer open={isDrawerOpen} onClose={toggleDrawer} direction="left" className="chatDrawer">
        <Sidebar />
        <div className="font-cerebri w-[-webkit-fill-available] flex flex-col justify-between">
          <img
            src={close}
      ...
      <div className="w-4/5 lg:w-[100vw] lg:p-2">
        <Header />
        {children}
      </div>
    </div>
