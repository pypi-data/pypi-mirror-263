...
from komodo.core.agents.docsearch_fast_agent import DocSearchFastAgent
...
class SampleAppliance(KomodoApp):
    def __init__(self, config=ApplianceConfig()):
        ...
        self.add_agent(LibrarianAgent.create(rc=RagContext(path=config.data_dir(), collection_name='default')))
        self.add_agent(DocSearchFastAgent.create(rc=RagContext(path=config.data_dir())))
