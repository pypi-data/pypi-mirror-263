...
const Settings = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await ApiGet(API_Path.index);
        if (res?.status === 200) {
  ...
  useEffect(() => {
    const fetchData1 = async () => {
      try {
        const res = await ApiGet(API_Path.reIndex);
        if (res?.status === 200) {
          ...
          <div className="px-6 xs:px-2 py-10 flex-1">
            <div className="grid grid-cols-1 gap-9">
              <div className="flex bg-customBg rounded-[10px] p-2 xs:p-1 w-[335px] xs:w-[290px] sm:w-[310px] h-[58px]">
                <div
                  className={`px-5 xs:px-2 xs:py-0 py-2 sm:px-3 sm:text-[16px] flex gap-2 justify-center items-center cursor-pointer rounded-[10px] ${
                    selectedTab === 0 ? "bg-white text-customColor" : "bg-transparent"
                  }`}
                ...
                <div
                  className={`px-4 xs:px-2 xs:py-0 py-2 sm:px-3 sm:text-[16px] flex gap-2 justify-center items-center cursor-pointer rounded-[10px] ${
                    selectedTab === 1 ? "bg-white text-customColor" : "bg-transparent"
                  }`}
              ...
              {selectedTab === 1 && (
                <div className="grid grid-cols-1 gap-10 w-1/3 lg:w-[100%] font-cerebriregular text-[16px]">
                  ...
                  <div className="flex justify-between">
                    ...
                    <label className="inline-flex items-center cursor-pointer">
                      <input type="checkbox" value="" className="sr-only peer" />
                      <div className="relative w-12 h-7 bg-[#E0E8F8] peer-focus:outline-none rounded-full peer dark:bg-[#E0E8F8] peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white peer-checked:after:bg-white after:content-[''] after:absolute after:top-[4px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                    </label>
                  </div>
