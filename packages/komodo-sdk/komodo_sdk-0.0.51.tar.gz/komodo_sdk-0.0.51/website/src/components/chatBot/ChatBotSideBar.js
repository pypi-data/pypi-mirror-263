...
const ChatBotSideBar = ({
  ...
  const CollectionData = async () => {
    try {
      const collection = await ApiGet(API_Path.addCollection);
  ...
  // const getUserFiles = async (guid) => {
  //   try {
  //     const files = await ApiGet(API_Path.addCollection + guid);
  ...
  const handleChatDelete = async (id, e) => {
    e.stopPropagation();
    try {
      const file = await ApiDelete(`${API_Path.addCollection}${id}`);
      if (file?.status === 200) {
  ...
  const handleFileData = async (guid) => {
    try {
      const file = await ApiGet(`${API_Path.addCollection}${id}/${guid}`);
    ...
    <div className="col-span-1 w-[100%]">
      <div>
        ...
        {/* <div className="font-cerebri w-[-webkit-fill-available] flex flex-col justify-between">
          <div>
            <h1 className="text-[21px] font-cerebri text-[#495057] leading-[27px] mb-9 mt-5 mx-5 border-t-[0.5px] border-[#F6F6F9] pt-6">
              Chat
            </h1>
      ...
      {isCollections ? (
        ...
        <>
          ...
          <div>
            <div className="sidebar h-[calc(100vh-322px)] overflow-auto">
          ...
          <div className={`flex items-center gap-2 cursor-pointer mb-3 px-5`}>
            ...
            >
              ...
              <div>
                <div className="text-blackText font-medium font-cerebriregular text-[16px] w-[110px] whitespace-nowrap overflow-hidden text-ellipsis">
                  Upload File
                </div>
              </div>
            </div>
