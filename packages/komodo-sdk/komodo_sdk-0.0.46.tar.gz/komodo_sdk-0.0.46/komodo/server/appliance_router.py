...
@router.get('/features', response_model=dict, summary='Get appliance features',
            description='Get the features of the appliance.')
def get_appliance_features(appliance=Depends(get_appliance)):
    if not appliance:
        raise HTTPException(status_code=404, detail="Appliance not found")
