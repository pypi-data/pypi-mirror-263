  ...
  useEffect(() => {
    if (prompt !== "" && apiActive) {
      ...
      eventSource = new EventSource(
        `${API_Path.streamedApi}email=${user?.email}&agent_shortcode=${chatContext?.reactSelect?.shortcode}&prompt=${prompt}?&collection_shortcode=${shortcode}&file_guid=${filesId}`
      );
      ...
      // };
      https: eventSource.onmessage = (event) => {
