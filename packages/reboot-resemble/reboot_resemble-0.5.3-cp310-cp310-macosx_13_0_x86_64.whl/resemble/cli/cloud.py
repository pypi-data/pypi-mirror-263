...
async def _user_id_from_api_key(
    ...
    # `mypy` false positive. The `match` above is exhaustive, so we should not be able
    # to reach this.
    return authenticate_response.user_id  # type: ignore[possibly-undefined]
...
async def _maybe_create_application(
    ...
    try:
        await Application(qualified_application_name).Create(workflow)
    except Aborted as aborted:
        match aborted.error:
...
async def cloud_up(args: argparse.Namespace):
    ...
    try:
        ...
        up_response = await Application(qualified_application_name).Up(
            workflow,
...
async def cloud_down(args: argparse.Namespace):
    ...
    try:
        await Application(qualified_application_name).Down(
            workflow,
        )
    except Aborted as aborted:
        match aborted.error:
