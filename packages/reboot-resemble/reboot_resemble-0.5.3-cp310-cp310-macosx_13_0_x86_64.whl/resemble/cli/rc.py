...
class ArgumentParser:
    ...
    def __init__(
        ...
        *,
        ...
        self._argv_after_dash_dash: list[str] = []

        self._help = '-h' in argv or '--help' in argv

        self.expanded_flags: list[Flag]
        ...
        # '--config=' flags.
        initial_namespace, _ = self._parser.parse_known_args(args=argv[1:])
        self._subcommand = self._update_subcommand_in_namespace(
            initial_namespace
        )

        if self.dot_rc is not None:
            self.expanded_flags, argv = self._expand_dot_rc(
                filename,
                self.dot_rc,
                self._parser,
                subcommands,
    ...
    def _read_flags_from_dot_rc(
        ...
        with open(dot_rc, 'r') as file:
            ...
            for line in file.readlines():
                ...
                arguments: list[str] = shlex.split(match.group('arguments'))
    ...
    def _expand_dot_rc(
        ...
        initial_namespace: argparse.Namespace,
        subcommand: str,
        argv: list[str],
        ...
        """
        flags = self._read_flags_from_dot_rc(filename, dot_rc)
        ...
        namespace, _ = parser.parse_known_args(args=argv[1:])
