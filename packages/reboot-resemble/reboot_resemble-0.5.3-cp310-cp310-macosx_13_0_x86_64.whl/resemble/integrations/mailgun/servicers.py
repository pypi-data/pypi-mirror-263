...
from resemble.v1alpha1.errors_pb2 import Unauthenticated
...
class _AbstractMessageServicer(Message.Interface):
        ...
        async def authorize(
            ...
            *,
            ...
            if context.auth is None:
                raise SystemAborted(Unauthenticated())

            # Invariant: our `_AbstractMessageServicer.TokenVerifier`
            # is verfiying that we have the correct API key, which is
            ...
            return MAILGUN_API_KEY_VERIFIED in context.auth.bearer_token.decoded
    ...
    async def SendTask(
        ...
        text: Optional[str] = (
            ...
            if request.send_request.HasField('text') else None
        )

        html: Optional[str] = (
            request.send_request.html
            if request.send_request.HasField('html') else None
        )
...
class MockMessageServicer(_AbstractMessageServicer):
    ...
    # find and read it.
    emails_sent: ClassVar[dict[tuple[str, str], str]] = {}
    ...
    async def send_email(
        ...
        *,
        ...
        if (recipient, idempotency_key) not in MockMessageServicer.emails_sent:
            logger.info(
                f"Pretending to send email with subject {subject} to {recipient}"
            )
            MockMessageServicer.emails_sent[(recipient, idempotency_key)
                                           ] = body
    ...
    async def is_email_with_idempotency_key_sent_to_recipient(
        ...
        *,
        ...
        return list(MockMessageServicer.emails_sent.keys()).count(
