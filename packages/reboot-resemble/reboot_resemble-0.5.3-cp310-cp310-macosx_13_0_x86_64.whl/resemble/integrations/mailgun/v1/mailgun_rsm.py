...
class MessageServicerMiddleware(Middleware):
    ...
    async def dispatch(
        ...
        if 'SendTask' == task.method_name:
            ...
            return await _run_SendTask_task()
    ...
    async def SendTask(
        ...
        raise resemble.aio.aborted.SystemAborted(
            resemble.v1alpha1.errors_pb2.InvalidMethod(),
            message=
                f"Method 'SendTask' is a task method and cannot (yet) be called directly"
        )
...
class MessageTransactionStub(_MessageStub):
...
class Message:
    ...
    async def Send(
        ...
        *,
        ...
        return await self.writer(__context_or_workflow__).Send(
            request,
            idempotency=idempotency,
            metadata=metadata,
        )
    ...
    async def SendTask(
        ...
        *,
        ...
        if __options__ is not None:
            ...
            if __options__.metadata is not None:
                metadata = __options__.metadata
        return await self.writer(__context_or_workflow__).SendTask(
            request,
            idempotency=idempotency,
            metadata=metadata,
        )
...
class MessageNodeAdaptor(Message):
