...
class APIKeyServicerMiddleware(Middleware):
    ...
    async def __Authenticate(
        ...
        try:
            ...
            assert_type(response, [resemble.cloud.v1alpha1.auth.auth_pb2.AuthenticateResponse])
            return response
        except resemble.aio.aborted.Aborted as aborted:
            # Log any _unhandled_ abort stack traces to make it
            # easier for debugging.
...
class APIKeyTransactionStub(_APIKeyStub):
...
resemble.cloud.v1alpha1.auth.auth_pb2.InvalidInputError        ]
    ...
    async def Revoke(
        ...
        if __options__ is not None:
            ...
            if __options__.metadata is not None:
                metadata = __options__.metadata
        return await self.writer(__context_or_workflow__).Revoke(
            request,
            idempotency=idempotency,
            metadata=metadata,
        )
    ...
    async def Authenticate(
        ...
        *,
        ...
        if __options__ is not None:
            if __options__.metadata is not None:
                metadata = __options__.metadata
        return await self.reader(__context_or_workflow__).Authenticate(
            request,
            metadata=metadata,
        )
...
class APIKeyNodeAdaptor(APIKey):
