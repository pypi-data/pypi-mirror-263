    ...
    async def _launch_subprocess_consensus(
        ...
        async def get_port() -> int:
            ...
            if isinstance(port_or_stringified_error, str):
                ...
                # the case of running `rsm dev` that we'd like to help the user
                ...
                # about the `rsm dev` case which always uses subprocesses.
                # TODO: A better way to handle this would be to have a
                # `rsm dev` equivalent of `on_kubernetes`, e.g. `if
                # using_rsm_dev(): ...`.
                if not on_kubernetes():
                    if (
                        ...
                        terminal.warn(
                            '\n'
                            'Did you start another instance of `rsm dev` in another terminal?'
                            '\n'
                        )
