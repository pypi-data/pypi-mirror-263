...
def validate_network(name, network):
    ...
    if "expose" in network:
        ...
        for exposed_port in expose:
            ...
            expose_port_number = int(expose_proto)
...
def validate(manifest_path, app_name):
    ...
    # version is always defined, no need to check
    if "version" in template_vars:
        template_vars.remove("version")

    defined_vars = set(app["vars"].keys())

    if "version" in defined_vars:
        raise ValidationError("The 'version' keyword is reserved and cannot be defined in vars.")
