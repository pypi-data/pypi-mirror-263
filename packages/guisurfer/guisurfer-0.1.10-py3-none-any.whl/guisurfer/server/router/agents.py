...
async def delete_agent(
    ...
    if task_agent.desktop:
        ...
        print(f"finding runtime {runtime_name}...")
        # TODO: yuck
        desktop_runtimes = DesktopRuntime.find(
            name=runtime_name, owner_id=current_user.email
        )
        desktop_runtime = desktop_runtimes[0]
        print("found runtime, deleting vm...")
        if desktop.provider == "gce":
            opts = GCEProviderOptions(
                zone=desktop.metadata["zone"], region=desktop.metadata["region"]
            )
            desktop_runtime.delete_vm(desktop.name, current_user.email, gce_opts=opts)
        elif desktop.provider == "ec2":
            opts = EC2ProviderOptions(region=desktop.metadata["region"])
            desktop_runtime.delete_vm(desktop.name, current_user.email, ec2_opts=opts)
        else:
