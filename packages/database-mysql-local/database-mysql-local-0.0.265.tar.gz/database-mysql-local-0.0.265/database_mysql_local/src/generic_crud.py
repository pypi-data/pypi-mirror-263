...
class GenericCRUD(metaclass=MetaLogger, object=LOGGER_CRUD_CODE_OBJECT):
    ...
    def __add_identifier(self, data_json: dict) -> None:
        ...
        if "identifier_entity_type_id" not in data_json:
            raise Exception("identifier_entity_type_id is missing from data_json")
        ...
        insert_query = "INSERT " + \
                       f"INTO `identifier`.`identifier_table` (identifier, entity_type_id) " \
                       "VALUES (%s, %s);"
        self.cursor.execute(insert_query, (identifier, data_json["identifier_entity_type_id"]))
        self.connection.commit()
        del data_json["identifier_entity_type_id"]
