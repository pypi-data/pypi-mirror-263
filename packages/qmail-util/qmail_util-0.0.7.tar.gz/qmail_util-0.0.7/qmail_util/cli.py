...
@click.option("-f", "--format", type=click.Choice(["json", "text"]), help="output format")
@click.option(
    "--shell-completion",
    is_flag=False,
    flag_value="[auto]",
...
@click.option(
    "-o", "--output", type=click.Path(dir_okay=False, writable=True, path_type=Path), help="output log filename"
)
