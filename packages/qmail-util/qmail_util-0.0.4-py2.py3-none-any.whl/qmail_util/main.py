...
class Session:
    ...
    def __str__(self):
        return f"{self.addr} {self.count} {self.ok} {self.fail}"
...
def get_logs(hostname, pattern, cache, verbose):
    cmd = ["rsync", "-avz", f"{hostname}:{log_dir}/{pattern}", str(cache) + "/"]
...
def log_lines(cache, all):
    ...
    if all:
        pattern += f" {str(cache)}/@*"
    proc = run(
        f"cat {pattern} | tai64nlocal | sort",
        shell=True,
        text=True,
        capture_output=True,
    )
...
def alert(index, message):
    click.echo(f"ALERT line {index}: {message}", err=True)
...
def scan(hostname, verbose=False, quiet=False, all=False, output=None):
    ...
    for line in lines:
        ...
        if m:
            ...
            if err is not None:
                if verbose:
                    click.echo(f"Error: session {err} at line {i}", err=True)
            elif session_lines:
                ...
                if valid:
                    ...
                    if reason is None:
                        if quiet is False:
                            breakpoint()
                            click.echo(f"Unknown Session Result: index={i} addr={addr} pid={pid}", err=True)
    return sessions
