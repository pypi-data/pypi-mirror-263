...
class Corpert:
    ...
    def run(self):
        ...
        if self.mode == "upload":
            ignore_files = set()
            json_obj = None
            json_file = next((os.path.join(self._path, f) for f in os.listdir(self._path) if f.endswith(".json")), "")
            if os.path.isfile(json_file):
                ignore_files.add(json_file)
                with open(json_file, "r") as jsf:
                    json_obj = json.loads( jsf.read() )
            else:
                json_obj = default_json( next(reversed(self._path.split(os.path.sep))) or "Anonymous Project" )
            with open_text("corpert", "lcp_corpus_template.json") as schema_file:
            ...
            for layer, properties in json_obj.get("layer", {}).items():
                ...
                if (
                    ...
                    and properties.get("contains", "") == firstClass['token']
                    and json_obj.get("layer",{}).get(firstClass['token'], {}).get("anchoring", {}).get("stream", False)
                ):
                    layerFile = next((f for f in self._input_files if Path(f).stem.lower() == layer.lower()), "")
                    assert layerFile, FileExistsError(f"Could not find a reference file for entity type '{layer}'")
                    ...
                    with open(layerFile, "r") as f:
                        ...
                        for a in properties.get("attributes", {}):
                            assert a.lower() in cols, ReferenceError(f"No column found for attribute '{a}' in {layerFile}")
                    aligned_entities[layer.lower()] = {'fn': layerFile, 'properties': properties}
            # token_header = [
            #     "token_id",
            #     "form_id",
            #     "lemma_id",
            ...
            for filepath in self._input_files:
                ...
                if fn_before_ext.lower() in {k.lower() for k in json_obj.get("layer",{})}:
                    continue
            ...
            print(f"\n{json_str}\n")
            print(f"A default meta.json file with the structure above was automatically generated at '{json_path}' for the current corpus.")
