...
def get_child_nodes(node_parent, node_tag, node_att_name=None, node_att_val=None, namespace_map=None,
                    filter_comment_nodes=True):
    selector = node_tag
    return find_all_nodes(node_parent, node_tag, node_att_name, node_att_val, recursive=False,
                          namespace_map=namespace_map, filter_comment_nodes=filter_comment_nodes)
...
# will format and save the xml
def save_xml_file(xml_tree, xml_path, add_utf_8_encoding=False):
    from lxml import etree as ET
    from xml.dom import minidom

    # Convert the entire XML tree, including comments, to a string using lxml
    xml_string = ET.tostring(xml_tree.getroot(), encoding='unicode', pretty_print=True)

    # Use minidom to pretty-print the XML (optional if lxml's pretty_print is sufficient)
    dom = minidom.parseString(xml_string)
    formatted_xml = dom.toprettyxml(indent="  ")

    # Remove extra newlines introduced by minidom
    cleaned_xml = '\n'.join(line for line in formatted_xml.split('\n') if line.strip())

    # Save the cleaned and formatted XML back to the file
    with open(xml_path, 'w', encoding="UTF-8" if add_utf_8_encoding else None) as file:
...
def remove_attrib_from_node(node, att):
