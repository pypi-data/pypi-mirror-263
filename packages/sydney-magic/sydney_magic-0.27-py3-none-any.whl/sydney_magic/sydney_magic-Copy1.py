...
class SydneyMagic:
    ...
    async def _readme_command(self, command, **kwargs):
        ...
        if os.path.exists("README.md"):
            if input("File already exists. Overwrite?").lower() != "y":
                return

        contents = result
        try:
            contents = result.split("<sydney_reponse">)[1].split("</sydney_response>")[0]
        except:
            contents = result
    ...
    async def _run_sydney_command(self, command, **kwargs):
        try:
            ...
            adjusted_command = command[len(verb):]
            handler = command.get(verb, "unknown")
            handler(adjusted_command, **kwargs)
