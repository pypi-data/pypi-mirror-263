...
class DeepFuseNMFUnet(nn.Module):
    ...
    def __init__(self,
        ...
        # Initial convolution block for multi-page input
        ...
        self.feature_fusion = nn.Sequential(
            nn.Conv2d(768, 512, kernel_size=3, padding=1, padding_mode="replicate"),
            nn.BatchNorm2d(512),
            nn.ReLU()
        )
        ...
        self.output = nn.Sequential(
            nn.Conv2d(256, self.num_channels, kernel_size=1),
        ...
        self.low_rank = nn.Sequential(
            ...
            nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=2, dilation=2, padding_mode='reflect'),
            nn.BatchNorm2d(128),
            nn.ReLU(),
        ...
        self.image_pred = nn.Sequential(
            nn.Conv2d(self.rank, 256, kernel_size=5, padding=2, padding_mode='reflect'),
            nn.ReLU(),

            nn.Conv2d(256, self.num_channels, kernel_size=1, padding_mode='reflect'),
            nn.BatchNorm2d(self.num_channels),
            nn.Sigmoid()
        )
    ...
    def forward(self, image, feasible_coord=None, vd_score=None, encode_only=False):
