...
class spatial_obj:
    ...
    def __init__(self,
        ...
        self.kernel_size = int(50 / scale_factor)  # Calculate the kernel size based on the scale factor
        ...
        self.spot_coord, self.spot_index, self.row_range, self.col_range = self.load_spot_coord(
            ...
            radius=self.radius)
        self.num_spots = self.spot_coord.shape[0]  # Calculate the number of spots

        # Prepare the feasible domain and load the image
        self.feasible_domain, self.image = self.prepare_feasible_domain(image_path=image_path,
    ...
    def prepare_feasible_domain(self,
                                image_path: str,
                                multi_pages: bool,
                                pages: List[int],
                                scale_factor: float,
        ...
        return feasible_domain, image

    def load_spot_exp(self,
                      spot_exp_path: str,
                      spot_coord: pd.DataFrame,
                      spot_index: pd.Index):
        """
