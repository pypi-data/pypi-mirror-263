...
class AutoSampler(BaseSampler):
  ...
  """
  def __init__(self, configuration, project, num_random_samples=10, next_sample_ratio=0.95, min_delta_score=0.001, optimizer="base", learning_rate=0.01):
    super().__init__(configuration)
    self.min_delta_score = min_delta_score
    self.num_random_samples = num_random_samples
    self.next_sample_ratio = next_sample_ratio
    ...
    self.optimizer = self.optimizers.get(optimizer, "base")(self.configuration.keys(), self.optimize_keys, learning_rate=self.learning_rate)
      ...
      if self.best_random_experiment is None:
        ...
        for k, v in E.data["experiment"]["config"].items():
          if k in self.optimize_keys:
            ...
            new_params[k] = v
        debug(type(self).__name__, "next sample with ratio", next_sample_ratio, new_params)
        ...
        delta_score, new_params = self.optimizer.update(
          self.best_random_experiment.data["experiment"]["config"],
          self.best_random_experiment.data["experiment"]["eval"]["score"],
          self.project.list_experiments[-1].data["experiment"]["config"],
          self.project.list_experiments[-1].data["experiment"]["eval"]["score"]
        )
