    ...
    def __init__(
        ...
        try:
            ...
            self.logger.error(
                ...
                "admin privileges. Exception reason: %s", excpt)
            self.connected = False

        if self.connected:
            self.logger.info("CRONUS-3P control at %s established", self.url)
        ...
        if not with_topas_control:
            if kwargs.get('with_attenuator_control'):
                ...
                print("Attenuator control for this device requires a connection to the underlying Topas4 REST server.")
        ...
        if with_topas_control:
            try:
                ...
                self.tctrl = WinTopas(ip_address, dev_sn=dev_sn, port=8004)
            except Exception as excpt:
                print("Could not connect to Topas4. An incrrect port was likely "
                    "specified (8000-8010). The program will continue, but some "
                    "advanced control features will be unavailable.")
                print("Exception reason: ", excpt)

            if self.tctrl is not None:
                print("Topas4 control at {:} established".format(self.tctrl.url))
