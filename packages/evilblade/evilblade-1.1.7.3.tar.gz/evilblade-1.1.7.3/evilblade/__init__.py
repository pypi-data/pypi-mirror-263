...
def gadget(instruction, mode, index=-1):
    """
    ...
    global binpath
    if mode == 0:
        command = f'ROPgadget --binary {binpath} --only \"{instruction}\" | grep \"0x\"'
        output = subprocess.check_output(command, shell=True)
        gadgets = output.decode().splitlines()
    elif mode == 1:
        command = f'ROPgadget --binary {binpath} | grep \"{instruction}\"'
        output = subprocess.check_output(command, shell=True)
        gadgets = output.decode().splitlines()
    if index == -1:
        print(f"{RED}=============================================={END}",end="")
        ...
        for gadget in gadgets:
            print(gadget.replace(":", f"#{gadgets.index(gadget)}"))
        print(f"{RED}=============================================={END}")
        return 0
    addresses = [int(gadget.split(':')[0], 16) for gadget in gadgets]
    gadgets = [gadget.split(':')[1] for gadget in gadgets]
    print(f"\n{GREEN}[+]{END} Get \b{gadgets[index]} gadgets, its address is {hex(addresses[index])}\n")
