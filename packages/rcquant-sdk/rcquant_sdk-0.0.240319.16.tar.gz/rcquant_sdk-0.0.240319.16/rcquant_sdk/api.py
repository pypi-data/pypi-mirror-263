...
def fin_save_basetick_list(exchange_id: str = '', instrument_id: str = '', base_path: str = '', trading_day: str = '', pre_close_price: float = float("NaN"), pre_settlement_price: float = float("NaN"),
                           pre_open_interest: float = float("NaN"), upper_limit_price: float = float("NaN"), lower_limit_price: float = float("NaN"), action_day_list: List[str] = [],
                           trading_time_list: List[str] = [], update_mill_sec_list: List[int] = [], local_time_list: List[int] = [], last_price_list: List[float] = [], last_volume_list: List[int] = [],
                           bid_price_list: List[float] = [], bid_volume_list: List[int] = [], ask_price_list: List[float] = [], ask_volume_list: List[int] = [], avg_price_list: List[float] = [],
                           open_price_list: List[float] = [], high_price_list: List[float] = [], lower_price_list: List[float] = [], total_turnover_list: List[float] = [], total_volume_list: List[int] = [],
                           open_interest_list: List[float] = [], close_price_list: List[float] = [], settlement_price_list: List[float] = [], total_value_list: List[float] = []):
    """批量按天保存行情数据
    ...
    return FinClient.instance().market_handle().fin_save_basetick_list(
        FinSaveBaseTickListParamData(exchange_id=exchange_id, instrument_id=instrument_id, base_path=base_path, trading_day=trading_day, pre_close_price=pre_close_price,
                                     pre_settlement_price=pre_settlement_price, pre_open_interest=pre_open_interest, upper_limit_price=upper_limit_price, lower_limit_price=lower_limit_price,
                                     action_day_list=action_day_list, trading_time_list=trading_time_list, update_mill_sec_list=update_mill_sec_list, local_time_list=local_time_list, last_price_list=last_price_list,
                                     last_volume_list=last_volume_list, bid_price_list=bid_price_list, bid_volume_list=bid_volume_list, ask_price_list=ask_price_list, ask_volume_list=ask_volume_list,
                                     avg_price_list=avg_price_list, open_price_list=open_price_list, high_price_list=high_price_list, lower_price_list=lower_price_list, total_turnover_list=total_turnover_list,
                                     total_volume_list=total_volume_list, open_interest_list=open_interest_list, close_price_list=close_price_list, settlement_price_list=settlement_price_list, total_value_list=total_value_list))
