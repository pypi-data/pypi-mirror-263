...
class ADI_Process(Process):
    ...
    def __init__(self, pcm_name, site, facility, begin_date, end_date, **kwargs):
        ...
        # TODO: as self._ds_ins has complex datastructure (List[Dict[str, xr.Dataset]]), 
        #   it might be worthy it to make it into a data class, and facilitate filter/query
        ...
        # self.ds_transforms = []  # [{'co2flx25m.b1': ds1, 'dlprofwstats4news.c1': ds2}, {'co2flx25m.b1': ds1, 'dlprofwstats4news.c1': ds2},]
        self._ds_outs = []  # [{'co2flx25m.b1': ds1, 'dlprofwstats4news.c1': ds2}, {'co2flx25m.b1': ds1, 'dlprofwstats4news.c1': ds2},]
    ...
    def post_transform_hook(self, begin_date: int, end_date: int):
        ...
        # Note: auto get transformed_dataset can be complex, skip for now
        # Note: the current PCM use variable-coord paired transform structure,
        # whcih is inconsistent with the (sudo-code:) get_transformed_dataset(s)(datastream, coord) workflow.
        # Propbably will never "auto" PCM-based get_transformed_dataset(s) for that matter, 
        # but might consider manually set datastream-vise transformation rule.
    ...
    def process(self, cached=False):
        ...
        if cached and os.path.isfile(self.ds_ins_full_path) and os.path.isfile(self.ds_outs_full_path):
            return
