...
Datetime = Annotated[
    datetime,
    BeforeValidator(lambda x: parse(x)),
...
def pyd_list(schema, meta, *args, **kw):
    ...
    keys_schema = schema['keys_schema']
    keys_result = pyd_analyze_same(keys_name, keys_schema, *args, **kw)
    if keys_result:
        result[keys_name] = keys_result

    values_name = f"{name}[value]"
    values_schema = schema['values_schema']
    values_result = pyd_analyze_same(values_name, values_schema, *args, **kw)
    if values_result:
        result[values_name] = values_result
    return result
