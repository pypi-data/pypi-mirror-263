...
from ciomaya.lib import layer_utils, validation, scraper_utils
...
def transient_save(filepath, cleanup=True):
    ...
    with scraper_utils.strip_drive(scraper_utils.RENDERMAN_ATTRS | scraper_utils.XGEN_ATTRS):
        pm.saveAs(filepath)
    try:
        yield
    finally:
...
def convert_xgen():
    for xgen_node_cache in mc.ls(exactType="xgmSplineCache"):
        attr_name = "{}.fileName".format(xgen_node_cache)
        attr_value = mc.getAttr(attr_name)
        print(attr_value)
        if attr_value[1:3] == ":/":
            new_attr_value = attr_value[2:]
            print("Changing '{}' to '{}'".format(attr_value, new_attr_value))
            mc.setAttr(attr_name, new_attr_value, type="string")
...
def submit(node):
    ...
    if pm.isModified():
        ...
        if not filepath:
            ...
            return
        with scraper_utils.strip_drive(scraper_utils.RENDERMAN_ATTRS):
            pm.saveAs(filepath)
