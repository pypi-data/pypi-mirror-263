...
from utils.logs import loga
import config.constants as ct
import lxml
from newspaper import Article, Config
from datetime import datetime
from nlpretext import Preprocessor
from utils.helpers import SentimentAnalysis, clean_text, TextChunk
                ...
                if lang_name == 'English':
                    ...
                    try:
                        article_html = self.extract_article_html(i["url"])
                                ...
                                print("Newspaper3k error: {0}, skipping article.".format(e.__class__.__name__))
                                print("Exception details: ", e)
                    i["articleHtml"] = '<div> <center> <h2>'+ i["title"] +' </h2> <center> <h4>Date: '+ i["date"] +'</h4><h4>Sentiment: '+str(i["sentiment"])+'</h4> </div><a></a><hr>' + article_html
                else:
                    continue
