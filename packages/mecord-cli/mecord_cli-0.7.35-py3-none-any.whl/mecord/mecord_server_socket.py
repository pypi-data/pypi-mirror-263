...
def restart_mecord_cli():
    ...
    with open(stop_file, 'w') as f:
        f.write("")
    time.sleep(10)
    while os.path.exists(os.path.join(thisFileDir, "all_stoped.now")) == False:
        time.sleep(1)
    print("stoped")
...
class MecordLongConnectThread(Thread):
    ...
    def __init__(self, country):
        ...
        print(f"~~~~~~~~~~~~~~~~~ {current_thread().name} 启动")
        self.socket_init()
        self.start()
    ...
    def socket_start(self):
        if self.ws:
            self.ws.run_forever()
    def reconnect(self):
        print(f"{current_thread().name}-{self.service_country} === reconnect...")
        self.socket_close()
        self.socket_init()
        ...
        while self.is_running:
            ...
            if cur_pts - last_check_service_work_status_pts > 120 and self.cur_counter == 0:
                ...
                if task_config["last_task_pts"] > 0:
                    if abs(self.last_get_task_end - self.last_get_task_begin) > 60:
