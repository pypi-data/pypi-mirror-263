...
# flyflow-openai._exceptions.NotFoundError -> flyflow-openai.NotFoundError
__locals = locals()
for __name in __all__:
    if not __name.startswith("__"):
        try:
            __locals[__name].__module__ = "flyflow-openai"
...
_ApiType = _te.Literal["flyflow-openai", "azure"]
...
class _AmbiguousModuleClientUsageError(OpenAIError):
    def __init__(self) -> None:
        super().__init__(
            "Ambiguous use of module client; please set `flyflow-openai.api_type` or the `OPENAI_API_TYPE` environment variable to `flyflow-openai` or `azure`"
        )
