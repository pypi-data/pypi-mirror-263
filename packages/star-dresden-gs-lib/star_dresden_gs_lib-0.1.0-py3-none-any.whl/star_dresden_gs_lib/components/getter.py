...
# TODO: No Param            AsyncGetterWidget
...
class GetterWidget(QtWidgets.QWidget):
    ...
    def __init__(self, name: str, endpoint: Callable, button_pos: Tuple[int, int] = (0, 0),
                 text_pos: Tuple[int, int] = (1, 0), execute_func: Callable = None, *args, **kwargs):
        """
        ...
        self.echo_label = QtWidgets.QLabel('None')
        ...
        self.layout.addWidget(self.button, button_pos[0], button_pos[1])
        self.layout.addWidget(self.echo_label, text_pos[0], text_pos[1])

    def __change_text(self, value):
        self.echo_label.setText(f"{self.name}: {value}")

    def __execute(self):
        value = self._endpoint()
        self.__change_text(value)
