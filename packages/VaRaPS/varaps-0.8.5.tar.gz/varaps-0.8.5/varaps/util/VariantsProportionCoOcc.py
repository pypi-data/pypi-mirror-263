...
class VariantsProportionCoOcc:
    ...
    def compute_expectation_helper(self, pMutationsVariantsErr, one_minus_pMutationsVariantsErr):
        self.temp_Tkj = np.zeros((self.nbReads, self.nbVariants))
        for k in range(self.pReadsVariants.shape[0]):
            if self.mutations_data[k]:
                ...
                self.temp_Tkj[k] = np.prod(pMutationsVariantsErr[list(self.mutations_data[k])], axis=0) * np.prod(one_minus_pMutationsVariantsErr[unmuted_idx], axis=0)
            else:
                self.temp_Tkj[k] = np.prod(
                    one_minus_pMutationsVariantsErr[self.starts_idx[k] : self.ends_idx[k]],
                    axis=0,
                )
