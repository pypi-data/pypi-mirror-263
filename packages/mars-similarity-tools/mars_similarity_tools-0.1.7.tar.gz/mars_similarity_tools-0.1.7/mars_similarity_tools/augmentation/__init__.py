...
from mars_similarity_tools.models import SimilarityObject
...
class ItemVectorizer:
    ...
    """
        A vectorizer service that takes a SimilarityObject and returns a VectorGroup.
        In between, the SimilarityObject is parsed using a GroupParser into a Group.
    ...
    def __call__(self, items: List[SimilarityObject]) -> List[VectorGroup]:
        return list(
            map(
...
class FederatedGraphVectorizer(ItemVectorizer):
    ...
    def __call__(self, items: List[SimilarityObject], force_update: bool = False) -> List[Tuple[str, VectorGroup]]:
        return list(
            chain(
                *map(
