...
def cluster_install(
    ...
    try:
        ...
        if config.get("maprrepoislocal", False):
            ...
            commands.insert(
                1,
                f"curl -s -k https://127.0.0.1:9443/ > /dev/null || sudo /tmp/mapr-setup.sh -y -r {repourl.rstrip('/')}",
            )
            ...
            for command in commands:
                ...
                for result in ssh_run_command(
                    ...
                    if (
                        ...
                        logger.info(
                            "Check installation process and results. Installer is running at https://%s:9443/",
                            first_host
                        )
