...
def parse_args():
    ...
    parser.add_argument(
        "--input_model_uri", type=str, default=os.environ.get("INPUT_MODEL_URI")
    )
    parser.add_argument(
        "--input_dataset_uri", type=str, default=os.environ.get("INPUT_DATASET_URI")
    )
...
class AnnotationFormatter(Preprocessor):
    ...
    def _fit_coco(self, ds: "Dataset") -> "Dataset":
        ...
        updated_annoation_ds = merged_annotaion_ds.add_column(
            "image_id", lambda df: self._update_image_id_byMD5(df)
        )
...
def run_testing(args):
    ...
    for image in image_ds.iter_rows():
        infer_dict = infer(triton_client, image["id"], image["file_name"])
        infer_raw.append(infer_dict[0])

    items = gaeainfer_to_vistudioV1(infer_raw, model["artifact"]["name"])
