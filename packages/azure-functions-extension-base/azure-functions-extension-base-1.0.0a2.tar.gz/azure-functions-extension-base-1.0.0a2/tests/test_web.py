    ...
    def test_request_type_not_provided(self):
        ...
        with self.assertRaises(Exception) as context:
            class TestClass(metaclass=RequestTrackerMeta):
                pass
        self.assertEqual(str(context.exception), 'Request type not provided for class TestClass')
...
class TestResponseTrackerMeta(unittest.TestCase):
    ...
    def test_classes_imported_from_same_module(self):
        ...
        self.assertEqual(ResponseTrackerMeta.get_response_type('test_label_1'), self.MockResponse1)
        self.assertEqual(ResponseTrackerMeta.get_response_type('test_label_2'), self.MockResponse2)
    ...
    def test_different_labels(self):
        ...
        self.assertEqual(ResponseTrackerMeta.get_response_type(ResponseLabels.STANDARD), self.MockResponse1)
        self.assertEqual(ResponseTrackerMeta.get_response_type(ResponseLabels.STREAMING), self.MockResponse2)
        self.assertTrue(ResponseTrackerMeta.check_type(self.MockResponse1))
        self.assertTrue(ResponseTrackerMeta.check_type(self.MockResponse2))
