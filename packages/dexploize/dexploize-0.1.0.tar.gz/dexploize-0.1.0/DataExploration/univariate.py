...
def best_split(attr1, attr2):
  ...
  for value in np.unique(attr1):
    ...
    right_entr = compute_entropy(attr2[right_val])
...
def entropy(attr1, attr2, bins):
  ...
  for x in range(bins):
    ...
    if BS is None:
      break
    splits.append(BS)
    info_gains = compute_entropy(attr2) - ((len(attr1[attr1 <= BS]) / len(attr1)) * LE + (len(attr1 > BS) / len(attr1)) * RE)
...
def entropy_based(data1, target, bins):
  ...
  print(f'Entropy: {compute_entropy(target)}') 
  print(f'Entropy given best split: {min(info_gain)}')
  print(f'Information gain: {max(info_gain)}')
  print(f'Best split bins <= {min(splits)} and > {max(splits)}')
...
def target_encoding(data, encoded):
  combine = pd.concat([data, encoded], axis=1)
  return combine
