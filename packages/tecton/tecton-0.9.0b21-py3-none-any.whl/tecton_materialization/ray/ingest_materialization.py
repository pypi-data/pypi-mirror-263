...
def _write_to_offline_store(
    delta_writer: DeltaWriter, materialized_data: pa.Table, ingest_path: str, is_overwrite: bool
) -> List[str]:
    ...
    if transaction_exists:
        print(
            f"Found previous commit with metadata {transaction_metadata} for data with path {ingest_path}. Skipping writing to delta table."
        )
...
def ingest_pushed_df(
    ...
    delta_writer = None
    try:
        if ingest_task_info.ingest_parameters.write_to_offline_feature_store:
            ...
            with offline_stage_monitor() as progress_callback:
                ...
                parts = _write_to_offline_store(delta_writer, table_for_ingest, ingest_path, is_overwrite=True)
        ...
        if ingest_task_info.ingest_parameters.write_to_online_feature_store:
            ...
            with online_stage_monitor() as progress_callback:
                assert parts is not None, "Features must be written to offline store first"
                for uri in parts:
