...
class ParsecCore:
    ...
    async def create_employer(self, dto: BasePerson.Create | Person.Create, department_uuid: Guid):
        dep = await self.get_department(department_uuid=department_uuid)
        if not dep:
            raise ParsecIntegrationError(f"Department with uuid: {department_uuid} not found")
        return self.core.create_person(self.session_id, dto)

    async def update_employer(self, dto: BasePerson.Update | Person.Update, employee_uuid: Guid):
        emp = await self.get_employer(employee_uuid=employee_uuid)
        if not emp:
            raise ParsecIntegrationError(f"Employee with uuid: {employee_uuid} not found")
        edit_session = self.core.open_person_editing_session(self.session_id, employee_uuid)
        if hasattr(dto, "ORG_ID"):
            object_type = self.core.client.get_type('ns0:Person')
            object_wrap = xsd.Element('Person', object_type)
            value = object_wrap(employee_uuid,
