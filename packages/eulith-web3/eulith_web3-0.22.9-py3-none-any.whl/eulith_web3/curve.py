...
class CurveUtils:
    ...
    def get_lp_token_value_denominated_usd(self) -> float:
        ...
        for i, coin in enumerate(self.tokens):
            ...
            price, txs = self.w3.v0.get_swap_quote(
                EulithSwapRequest(sell_token=usdc, buy_token=coin, sell_amount=1)
            )
            dollar_denominated_balance = bal_float * price
            total_usd_value += dollar_denominated_balance

        lp_token = EulithERC20(
            self.w3, self.w3.to_checksum_address(self.curve_pool.token())
        )
