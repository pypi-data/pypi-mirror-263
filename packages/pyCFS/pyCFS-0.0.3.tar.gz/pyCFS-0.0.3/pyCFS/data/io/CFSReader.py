...
class CFSReader:
    ...
    * `get_single_data_steps` `(quantity: str, region: str, entity_id: int) -> np.ndarray`
    ...
    def __init__(self, filename: str, h5driver: str | None = None, verbosity=v_def.release) -> None:
    ...
    @_catch_key_error
    def get_mesh_region_nodes(self, region: str, is_group: bool | None = None):
        if is_group is None:
            is_group = self.check_group_region(region=region)
        with h5py.File(self._filename, driver=self._h5driver) as f:
            if is_group:
                return np.array(f[f"Mesh/Groups/{region}/Nodes"])
            else:
                return np.array(f[f"Mesh/Regions/{region}/Nodes"])

    @_catch_key_error
    def get_mesh_region_elements(self, region: str, is_group: bool | None = None):
        if is_group is None:
            is_group = self.check_group_region(region=region)
        with h5py.File(self._filename, driver=self._h5driver) as f:
            if is_group:
                return np.array(f[f"Mesh/Groups/{region}/Elements"])
            else:
                return np.array(f[f"Mesh/Regions/{region}/Elements"])
