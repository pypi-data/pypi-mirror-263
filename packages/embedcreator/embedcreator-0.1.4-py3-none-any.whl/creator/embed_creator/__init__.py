...
class EmbedCreator(View):
    ...
    async def on_error(self, interaction: Interaction, error: Exception, item: Item[Any]) -> None:
        if isinstance(error, HTTPException) and error.code == 50035: 
            await interaction.followup.send('O URL que você inseriu não é válido. Por favor, verifique se você digitou corretamente o endereço da imagem e tente novamente.')
