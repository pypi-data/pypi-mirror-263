...
class CollectionItem(_ItemMixin):
    ...
    def commit(self) -> "Self":
        ...
        print("COMINT HERE", self.get("datasources"))
        data = self._commiter(item=self, replace_document=True)
        pprint.pprint(data)
        if data:
    ...
    def _update(self, mutations: dict):
        ...
        data = self.to_dict()
        print("DOC-UPD0", data.get("datasources"), mutations)
        ...
        print("DOC-UPD", doc.get("datasources"), mutations, _commit_replace)
        if _commit_replace:
...
class SubCollection(object):
    ...
    def insert(self, data: dict, _key:str=None):
        ...
        """
        data, _, _commit_replace = dict_mutator.mutate(mutations=data.copy(), immuts=self._item._immut_keys, custom_ops=self._custom_ops)
    ...
    def update(self, filters:dict, mutations: dict, upsert:bool=False):
        ...
        if res:
            for item in res:
                ...
                upd, _, _commit_replace = dict_mutator.mutate(mutations=mutations, init_data=item, immuts=self._item._immut_keys, custom_ops=self._custom_ops)
                if _commit_replace:
                    self._commit_replace = True
