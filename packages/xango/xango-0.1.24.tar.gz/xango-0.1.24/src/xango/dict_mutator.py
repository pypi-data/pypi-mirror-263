...
def mutate(mutations: dict, init_data: dict = {}, immuts:list=[], custom_ops:dict={}):
    ...
    _to_delete = set() # list of keys to remove from init_data
    ...
    if _to_delete:
        _replace = True
        for k in list(_to_delete):
            delete_by_dot_notation(init_data, k)
...
def delete_by_dot_notation(dictionary, dot_notation_key):
