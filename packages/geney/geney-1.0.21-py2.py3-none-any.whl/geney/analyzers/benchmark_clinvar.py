...
def run_oncosplice_on_clinvar_interactive():
    now = datetime.now()
...
def aggregate_clinvar_results(benchmark_path, benchmark_feature):
    data = oncosplice_reduced(pd.concat([pd.read_csv(file) for file in benchmark_path.glob('*.csv')]))
