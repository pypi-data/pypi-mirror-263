...
class SurvivalAnalyzer:
    ...
    def perform_cox_analysis(self, dict1):
        affected_cases = dict1['primary']
        muts = list(set(dict1['mut1'] + dict['mut2']))

        df1 = self.clinical_df.query(f"case_id in {affected_cases}")
        df2 = self .clinical_df.query(f"case_id in {muts}")

        df1['group'] = 1
        df2['group'] = 0
        df = pd.concat([df1, df2])
        cph = CoxPHFitter().fit(df, 'duration', 'event')
        self.summary_df = cph.summary
        cph.print_summary()
...
def perform_cox_analysis(dict1, clindf):
    affected_cases = dict1['primary']
    muts = list(set(dict1['mut1'] + dict1['mut2']))
    df1 = clindf.query(f"case_id in {affected_cases}")
    df2 = clindf.query(f"case_id in {muts}")
    df1['group'] = 1
    df2['group'] = 0
    df = pd.concat([df1, df2])
    df = df[['duration', 'event', 'chemotherapy', 'hormone_therapy', 'immunotherapy', 'targeted_molecular_therapy', 'group']]
    ...
    cph.print_summary()
    return summary_df.p.min()



res = perform_cox_analysis(survival_data[mut_id], clindf)