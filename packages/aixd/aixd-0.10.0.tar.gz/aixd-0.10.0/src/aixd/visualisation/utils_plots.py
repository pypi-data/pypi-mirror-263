...
def errors_gen(fig, y_diff, y_diff_best, y_list, y_list_best, y_train, dobj, name_plot, pos, n_bins, offset=0.005, range_bins=None, leg_str="Estimated error", **kwargs):
    ...
    fig = add_trace_error(fig, y_ids, y_mean, y_conf_rel, "de", "{} {}/{}".format(leg_str, n_per_bin, n_per_bin), offset=offset, pos=pos, yaxis=2, symbol=symbol, **kwargs)

    n_per_bin_best = int(len(y_diff_best) / n_bins)
    fig = add_trace_error(
        fig, y_ids, y_mean_best, y_conf_rel_best, "de_s", "{} {}/{}".format(leg_str, n_per_bin_best, n_per_bin), offset=offset, pos=pos, yaxis=2, symbol=symbol, **kwargs
    )
...
def add_trace_error(fig, y_ids, y_mean, y_conf_rel, str_color, name_p, offset=0.005, pos=(1, 1), yaxis=1, symbol="circle", **kwargs):
