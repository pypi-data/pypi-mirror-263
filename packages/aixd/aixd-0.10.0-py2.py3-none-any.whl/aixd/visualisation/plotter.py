...
class Plotter:
    ...
    def evaluate_generation(
        ...
        """
        return self._evaluate_train_gen(
            attributes, normalize, per_column, bottom_top, downsamp, range_bins, n_bins, n_samples_per_bin, oversampling_gen, datamodule, output_name, False, analyzer
        )
            ...
            for cols in column_list:
                ...
                if flag_eval_train:
                    ...
                    dict_out = gen.run(
                        ...
                        request_orig_range=not normalize,
                        flag_peratt=True,
                        nbins=n_bins,
                        analyzer=analyzer,
                        verbose=False,
                    ...
                    if analyzer is not None and gen._check_analyzer_sets(analyzer):
                        y_d_gt, y_d_gt_best = dict_out[name_plot][key_normalization]["y_diff_gt"], dict_out[name_plot][key_normalization]["y_diff_gt_best"]
                        y_samp = dict_out[name_plot][key_normalization]["y_samp"][:, dict_out[name_plot]["ids_att"]].flatten()
                        y_samp_best = dict_out[name_plot][key_normalization]["y_samp_best"][:, dict_out[name_plot]["ids_att"]].flatten()
                        fig = up.errors_gen(
                            fig,
                ...
                for ic, (dobj_c, cols_c) in enumerate(zip(dobj_list, cols_names)):
                    if ic >= ir:
                        ...
                        if name_x in gen["all"]["attributes"] and name_y in gen["all"]["attributes"]:
                            ...
                            x_est, y_est = gen_exp["y_est_best"][:n_samples, x_ind], gen_exp["y_est_best"][:n_samples, y_ind]
                            y_est = [0.15] * len(x_est) if name_x == name_y else y_est

                            if "y_gt_best" in gen_exp.keys():
                                x_gt, y_gt = gen_exp["y_gt_best"][:n_samples, x_ind], gen_exp["y_gt_best"][:n_samples, y_ind]
                                y_gt = [0.3] * len(x_gt) if name_x == name_y else y_gt
                                x_l, y_l = np.vstack([x_est, x_gt]).T, np.vstack([y_est, y_gt]).T
                                tit = ["", ""]
                                for ip, valx in enumerate(x_est):
                                    fig = fig_sc2d(
                                        fig,
                                        x_l[ip],
                                        y_l[ip],
