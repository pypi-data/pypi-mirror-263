                ...
                if not has_error and nn(label_function) and length > 1 or use_free_input:
                    for index, item in enumerate(data):
                        self.output.index(
                            ...
                            item if allow_choose_all and index == 0 else label_function(item, index),
                            max_value,
                        )
...
class PIH:
    ...
    class DATA(
        ...
        class MATERIALIZED_RESOURCES:
            ...
            @staticmethod
            def set_quantity(type_value: MATERIALIZED_RESOURCES.Types, value: int) -> bool:
                return PIH.ACTION.DATA_STORAGE.value(
                    value, type_value.name, MATERIALIZED_RESOURCES.NAME
                )

            @staticmethod
            def add_quantity(type_value: MATERIALIZED_RESOURCES.Types, value: int) -> bool:
                return PIH.ACTION.DATA_STORAGE.value(
