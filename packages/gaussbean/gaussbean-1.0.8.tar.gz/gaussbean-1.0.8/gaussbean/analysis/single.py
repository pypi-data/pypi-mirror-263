...
def single_image_proj(xmargins, ymargins, fwrange=1.3, imgpath='', imgar=[]):
    ...
    yFWHM = calc_utils.find_FWHM(calc_utils.find_proj_y(imgar=finalimg), fwrange=fwrange)[0]
    
    # use the projection along the x-axis to find the FWHM value for the beam along the x-axis
    xFWHM = calc_utils.find_FWHM(calc_utils.find_proj_x(imgar=finalimg), fwrange=fwrange)[0]
...
def single_image_line(xmargins, ymargins, xpixel=0, ypixel=0, toavg=0, fwrange=1.3, imgpath='', imgar=[]):
    ...
    # use the projection along the y-axis to find the FWHM value for the beam along the y-axis
    yFWHM = calc_utils.find_FWHM(calc_utils.find_line_y(xpixel, toavg=toavg, imgar=finalimg), fwrange=fwrange)[0]
    
    # use the projection along the x-axis to find the FWHM value for the beam along the x-axis
    xFWHM = calc_utils.find_FWHM(calc_utils.find_line_x(ypixel, toavg=toavg, imgar=finalimg), fwrange=fwrange)[0]
