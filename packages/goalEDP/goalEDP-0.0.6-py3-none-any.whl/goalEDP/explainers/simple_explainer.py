...
class SimpleExplainer(Explainer):
    ...
    async def similarEventsAsync(self, events: List[Event], minTime: int = 0, maxTime: int = sys.maxsize) -> tuple[List[Event], List[EventHandler]]:
        ...
        for e in events:
            topics.add(e.topic)
            valuesHashes.add(await self.history.hashAsync(e.value))
        similarEvents = await self.history.getEventsAsync({'topics': list(topics), 'valuesHashes': list(valuesHashes), 'minTime': minTime, 'maxTime': maxTime})
        handlers = self.eventBroker.publishers(list(topics))
        return (similarEvents, handlers)
    ...
    async def possibleCausesAsync(self, effects: List[Event], minTime: int = 0, maxTime: int = sys.maxsize) -> dict[str, dict[str, float]]:
        ...
        sEffects, handlers = await self.similarEventsAsync(effects, minTime, maxTime)
    ...
    async def possibleEffectsAsync(self, causes: List[Event], minTime: int = 0, maxTime: int = sys.maxsize) -> dict[str, dict[str, float]]:
        ...
        counts: dict[str, dict[str, int]] = dict()
        similarCauses = (await self.similarEventsAsync(causes, minTime, maxTime))[0]
        effects = await self.effectsOfAsync(similarCauses, minTime, maxTime)
        publishers = (await self.similarEventsAsync(effects, minTime, maxTime))[1]
