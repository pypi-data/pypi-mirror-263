...
class Writer:
    ...
    def add_message(message: str, severity_id: int) -> None:
        ...
        try:
            ...
            query = ("INSERT INTO logger.logger_table (message, record, severity_id) "
                     "VALUES (%s, None, %s)")
    ...
    def add_message_and_payload(message: str = None, **kwargs) -> None:
        ...
        try:
            ...
            listed_values = [str(k) for k in params_to_insert.values()] + [message]
            joined_keys = ','.join(list(params_to_insert.keys())) + (',' if params_to_insert else '') + 'message'
