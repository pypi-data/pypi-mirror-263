...
class FixedIncome:
    ...
    def create_results(self, valuation_results, status):
        ...
        if self.multiple == False:
            ...
            self.results['precio' if self._lang == 'esp' else 'price'] = valuation_results[1] if status else None
            self.results['valor_compra' if self._lang == 'esp' else 'purchase_value'] = valuation_results[2] if status else None
            self.results['precio_limpio' if self._lang == 'esp' else 'clean_price'] = valuation_results[3] if status else None
            self.results['duracion' if self._lang == 'esp' else 'duration'] = valuation_results[4] if status else None
            self.results['duracion_modificada' if self._lang == 'esp' else 'modified_duration'] = valuation_results[5] if status else None
            self.results['dv01'] = valuation_results[6] if status else None
            self.results['convexidad' if self._lang == 'esp' else 'convexity'] = valuation_results[7] if status else None
            self.results['valor_presente_preliminar' if self._lang == 'esp' else 'preliminar_present_value'] = valuation_results[8] if status else None
            self.results['tiempo_calculo' if self._lang == 'esp' else 'process_time'] = self._process_time if status else None
        elif self.multiple == True:
            self.results['Emisor' if self._lang == 'esp' else 'Issuer'] = self.issuer
            self.results['Fecha Vencimiento' if self._lang == 'esp' else 'Maturity Date'] = self.maturity_date
            self.results['Monto' if self._lang == 'esp' else 'Face Value'] = self.face_value
            self.results['Rendimiento' if self._lang == 'esp' else 'Yield'] = self.ytm
            self.results['Cupón Corrido' if self._lang == 'esp' else 'Running Coupon'] = self.gained_coupon if status else None
            self.results['Valor Presente' if self._lang == 'esp' else 'Present Value'] = valuation_results[0] if status else None
            self.results['Precio Sucio' if self._lang == 'esp' else 'Dirty Price'] = valuation_results[1] if status else None
            self.results['Valor Compra' if self._lang == 'esp' else 'Purchase Value'] = valuation_results[2] if status else None
            self.results['Precio Limpio' if self._lang == 'esp' else 'Clean Price'] = valuation_results[3] if status else None
            self.results['Duracion' if self._lang == 'esp' else 'Duration'] = valuation_results[4] if status else None
            self.results['Duracion Modificada' if self._lang == 'esp' else 'Modified Duration'] = valuation_results[5]  if status else None
            self.results['DV01'] = valuation_results[6] if status else None
            self.results['Convexidad' if self._lang == 'esp' else 'Convexity'] = valuation_results[7] if status else None
            self.results['Valor Presente Preliminar' if self._lang == 'esp' else 'Preliminar Present Value'] = valuation_results[8] if status else None
            self.results['Tiempo de Cálculo' if self._lang == 'esp' else 'Process Time'] = self._process_time if status else None
