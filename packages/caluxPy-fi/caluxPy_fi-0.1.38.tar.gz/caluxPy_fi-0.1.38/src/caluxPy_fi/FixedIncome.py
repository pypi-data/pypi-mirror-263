        ...
        try:
            if self.maturity_date >= self.settlement_date:
                ...
                if self.multiple == False:
                    ...
                    if self._lang == 'eng':
                        self.results['Issuer'] = self.issuer
                        ...
                        self.results['Emisor'] = self.issuer
                ...
                #Otras operaciones
                try:
                    self.repo_margin = float(Support.rateConversion(repo_margin))
                    self.repo_rate = float(Support.rateConversion(repo_rate))
                    self.repo_tenure = int(repo_tenure)
                    ...
                    if self.repo_margin != '' and self.repo_rate != '' and self.repo_tenure != '':
                        ...
                        if self.multiple:
                            if self._lang == 'eng':
                                self.results['Inflow'] = resultado_repos[0]
                                self.results['Outflow'] = resultado_repos[1]
                            elif self._lang == 'esp':
                                self.results['Valor Ida'] = resultado_repos[0]
                                self.results['Valor Vuelta'] = resultado_repos[1]
                        else:
                            if self._lang == 'eng':
                ...
                except Exception as e: self._logger.error(f'Error calculating repo => {e}')
            ...
            if self.multiple == False:
                ...
                #FixedIncome.counter += 1
                if self._lang == 'eng':
                    self.results['Maturity Date'] = maturity_date
                    self.results['Face Value'] = face_value
                    self.results['Yield'] = ytm
                    self.results['Running Coupon'] = None
                    self.results['Present Value'] = None
                    self.results['Dirty Price'] = None
                    self.results['Purchase Value'] = None
                    ...
                    self.results['Fecha Vencimiento'] = maturity_date
                    self.results['Monto'] = face_value
                    self.results['Rendimiento'] = ytm
