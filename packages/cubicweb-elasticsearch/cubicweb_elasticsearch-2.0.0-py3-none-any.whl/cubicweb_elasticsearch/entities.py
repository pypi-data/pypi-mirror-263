...
class IFullTextIndexSerializable(entity.EntityAdapter):
    ...
    numeric_indexable = True
    main_attributes = ("cw_etype", "eid", "cwuri")
    ...
    def fulltext_indexable_attributes(self):
        ...
        attrs = ["creation_date", "modification_date"]
        attrs.extend(
            [
                r.type
        ...
        for rschema, tschema in eschema.attribute_definitions():
            if rschema.type in ("eid", "cwuri"):
                continue
            if self.numeric_indexable and tschema.type in ("Int", "Float"):
    ...
    def serialize(self, complete=True):
        ...
        if complete:
            entity.complete()
        data = {}
        for attr in self.main_attributes:
