...
class IFullTextIndexSerializableTC(testlib.CubicWebTC):
    ...
    @patch("elasticsearch.client.IndicesClient.exists")
    @patch("elasticsearch.client.Elasticsearch.index")
    def test_noindex_numeric_es_attr(self, create, exists, index):
        """check numeric attributes are not indexed"""
        with self.admin_access.repo_cnx() as cnx:
            PersonFTIAdapter.numeric_indexable = False
            with self.temporary_appobjects(PersonFTIAdapter):
                cnx.create_entity("Person", age=123456, name="Jean")
                cnx.commit()
                indexer = cnx.vreg["es"].select("indexer", cnx)
                ...
                self.assertTrue(es.index.called)
                args, kwargs = es.index.call_args
                # make sure age is not indexed
                self.assertNotIn("age", kwargs["body"])
                # make sure age is still used as custom es document id
                self.assertEqual(kwargs["id"], 123456)

    @patch("elasticsearch.client.IndicesClient.create")
    @patch("elasticsearch.client.IndicesClient.exists")
    @patch("elasticsearch.client.Elasticsearch.index")
    def test_index_numeric_es_attr(self, create, exists, index):
        """check numeric attributes are indexed by default"""
        with self.admin_access.repo_cnx() as cnx:
            with self.temporary_appobjects(PersonFTIAdapter):
                cnx.create_entity("Person", age=123456, name="Jean")
                cnx.commit()
                ...
                self.assertTrue(es.index.called)
                args, kwargs = es.index.call_args
                # make sure age is indexed
                self.assertEqual(kwargs["body"]["age"], 123456)
