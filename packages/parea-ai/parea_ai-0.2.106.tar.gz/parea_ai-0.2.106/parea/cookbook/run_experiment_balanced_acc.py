...
def is_conciseness(log: Log) -> float:
    # return log.target == log.output
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[{'role': 'system', 'content': 'Evaluate the cociseness of this answer'}],
    )
    # prase the response as a float
...
@trace(eval_funcs=[is_conciseness, ])
