...
class MozartClient(MozartApi):
    ...
    async def check_websocket_connection(self) -> bool:
        ...
        try:
            ...
            logger.debug("%s : %s", self.host, error)
            return False
    ...
    async def check_device_connection(self) -> bool:
        ...
        async with asyncio.TaskGroup() as tg:
            websocket_task = tg.create_task(self.check_websocket_connection())
            api_task = tg.create_task(self.check_api_connection())

        return bool(websocket_task.result() and api_task.result())
