...
@pytest.mark.parametrize("compute_config_version", [None, 1])
...
def test_start(
    ...
    needs_start: bool,
    cluster_exists: bool,
    compute_config_version: Optional[int],
) -> None:
    cluster_controller = ClusterController()
    ...
    with patch.multiple(
        ...
        cluster_controller.start(
            ...
            cluster_compute_version=compute_config_version,
        )
    ...
    mock_get_cluster_compute_from_name.assert_called_once_with(
        "cluster_compute_name", cluster_controller.api_client, compute_config_version
    )
