...
@pytest.mark.parametrize("enable_head_node_fault_tolerance", [True, False])
@pytest.mark.parametrize(
    "cfn_tags", [[], [{"Key": "anyscale:is-anyscale-hosted", "Value": "true"}]]
)
@mock_cloudformation
def test_update_cloud_with_resources_aws(
    enable_head_node_fault_tolerance: bool, cfn_tags: List[Any],
):
    ...
    mock_s3_id = "mock-s3"
    if cfn_tags:
        mock_s3_id = "anyscale-customer-dataplane-data-production"
    ...
    cloud_controller.update_cloud_with_resources(
        ...
        mock_cloud_id,
        enable_head_node_fault_tolerance,
        shared_s3_bucket="anyscale-customer-dataplane-data-production",
    )
