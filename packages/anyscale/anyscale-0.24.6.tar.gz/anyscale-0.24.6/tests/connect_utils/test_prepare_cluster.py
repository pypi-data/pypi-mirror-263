    ...
    if len(cluster_computes) == 0:
        with pytest.raises(ValueError):
            mock_prepare_cluster_block._get_cluster_compute_id_from_name(
                "test_cluster_compute_name", cluster_compute_version
            )
    else:
        assert (
            mock_prepare_cluster_block._get_cluster_compute_id_from_name(
...
def test_validate_new_cluster_compute_and_env_match_existing_cluster(
    ...
    mock_prepare_cluster_block.cluster_compute_version = 2
...
@pytest.mark.parametrize("cluster_compute_version", [0, 2])
@pytest.mark.parametrize(
