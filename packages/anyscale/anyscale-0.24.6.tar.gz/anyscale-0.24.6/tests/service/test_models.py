...
class TestServiceConfig:
    ...
    def test_image_uri(self):
        ...
        config = ServiceConfig(
            applications=[{"import_path": "main:app"}],
            image_uri="user/my-custom-image:1",
        )
    ...
    def test_ray_gcs_external_storage_config(self):
        ...
        assert config.ray_gcs_external_storage_config == {"address": "host:1234"}

        with pytest.raises(
            TypeError, match="'ray_gcs_external_storage_config' must be a dict."
        ):
            ServiceConfig(
                applications=[{"import_path": "main:app"}],
    ...
    def test_image_uri_validation(self, image_uri: str, valid: bool):
        if valid:
            _ = ServiceConfig(
                applications=[{"import_path": "main:app"}], image_uri=image_uri
            )
        else:
            with pytest.raises(
                ValueError,
...
class TestServiceStatus:
    ...
    def test_to_dict_and_back(
        ...
        if "primary_version" in status_dict:
            primary_version = ServiceVersionStatus(
                config=ServiceConfig(**status_dict["primary_version"].pop("config")),
                **status_dict.pop("primary_version"),
            )
        canary_version = None
        if "canary_version" in status_dict:
            canary_version = ServiceVersionStatus(
                config=ServiceConfig(**status_dict["canary_version"].pop("config")),
                **status_dict.pop("canary_version"),
            )
