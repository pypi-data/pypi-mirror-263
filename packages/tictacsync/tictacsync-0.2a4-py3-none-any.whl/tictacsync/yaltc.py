...
class Recording:
    ...
    def get_duration(self):
        ...
        """
        if self.merged_sound:
            val = sox.file_info.duration(_pathname(self.merged_sound))
            logger.debug('duration of merged_sound %f'%val)
            return val
        else:
            if self.probe is None:
                return 0
    ...
    def get_start_time(self, plots=False):
        ...
        if not returns None.
        If successful AND self is audio, sets self.merged_sound
        """
        ...
        if self.is_audio():
            self.merged_sound = self._extract_sound_to_merge()
        return start_UTC

    def _extract_sound_to_merge(self) -> tempfile.NamedTemporaryFile:
        """        
        Returns a tempfile.NamedTemporaryFile of sound to be eventually
        merged with video.

        If the original audio is stereo, this is simply the audio stripped of
        the TicTacCode channel, so this fct returns a mono wav tempfile.

        If the original audio is multitrack, two possilities:

            A- there's a track.txt file declaring a mono 'mix' track or a 'mixL'
            and 'mixR' stereo mix track - then this fct returns a tempfile
            containing this audio.

            B- there's a track.txt but without any mix identifier OR there is
            simply no track.txt - then this fct returns a wav tempfile of a
            programmatically done mix down.


        Notes:
            'track.txt' is defined in device_scanner.TRACKSFN 

            Beware of track numerotation: sox starts indexing tracks at 1 but code
        ...
        if not trax: #TO DO: mix or mixL, mixR or do mixdown
            return self._sox_strip(input_file, [self.TicTacCode_channel])
        else:
        ...
        logger.debug('track_is_active (1st = #1) %s'%track_is_active)
        ...
        if trax:
            excluded_channels = [i for i in range(n_channels)
                if i not in track_is_declared_audio]
        else:
