...
class AudioStitcherVideoMerger:
    ...
    def add_matched_audio(self, audio_rec):
        ...
        """
        self.edited_audio[audio_rec] = audio_rec.merged_sound
        """
        ...
        audio_rec.merged_sound doesn't need to be reinitialized since
        it stays unchanged)
        """
        return
    ...
    def _write_ISOs_for(self, top_dir, synced_clip_file, device, joined_audio):
        """
        ...
        shutil.copy(_pathname(joined_audio), ISO_multi_chan)
        ...
        nchan = sox.file_info.channels(_pathname(joined_audio))
    ...
    def build_audio_and_write_video(self, top_dir, output_dir,
                                    ...
                                    asked_ISOs):
        """
        top_dir: directory where media were looked for
        ...
        if output_dir != None:
            synced_clip_dir = Path(output_dir)
        else:
            synced_clip_dir = Path(top_dir)/OUT_DIR_DEFAULT
        # four possibilities for output:
        # user specified a folder, write_multicam_structure is True
        # user did not specified a folder, write_multicam_structure is True
        # user specified a folder, write_multicam_structure is False
        # user did not specified a folder, write_multicam_structure is False
        if write_multicam_structure:
            device_name = self.ref_recording.device.name
            synced_clip_dir = synced_clip_dir/device_name
        ...
        if len(merged_audio_files_by_device) > 1:
            logger.debug('more than one audio device...  work in progress')
        ...
        for idx, pair in enumerate(merged_audio_files_by_device):
            device, joined_audio = pair
            logger.debug('idx: % i device.folder: %s'%(idx, device.folder))
            logger.debug('joined_audio: %s'%_pathname(joined_audio))
            logger.debug('duration %f s'%
                sox.file_info.duration(_pathname(joined_audio)))
        ...
        if asked_ISOs:
            ...
            for idx, pair in enumerate(merged_audio_files_by_device):
                device, joined_audio = pair
                logger.debug('device %i, joined_audio %s %s'%
                    (idx, device, _pathname(joined_audio)))
                self._write_ISOs_for(top_dir, synced_clip_file,
                    device, joined_audio)
        video_path = self.ref_recording.AVpath
        logger.debug('will merge with %s'%(_pathname(joined_audio)))
        stacked_audio = self._sox_combine([audio for _, audio
                in merged_audio_files_by_device])
        self._merge_audio_and_video(stacked_audio, synced_clip_file)
        self.ref_recording.final_synced_file = synced_clip_file # relative

    def _sox_combine(self, paths) -> Path:

        """
        combines files refered by the list of Path into a new temporary files passed on return
        """
