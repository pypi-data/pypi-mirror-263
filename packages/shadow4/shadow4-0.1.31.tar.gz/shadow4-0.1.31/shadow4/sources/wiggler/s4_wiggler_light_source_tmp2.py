...
class S4WigglerLightSource(S4LightSource):
    ...
    def __calculate_rays(self,user_unit_to_m=1.0,F_COHER=0,EPSI_DX=0.0,EPSI_DZ=0.0,
        ...
        if method == 'new' or method == 'old':
            ...
            AA = numpy.outer(angle_array_reduced, numpy.ones_like(e_over_ec_array))
            ...
            if False:
                ...
                plot_image(fm, angle_array_reduced, e_over_ec_array, title='intensity', xtitle='reduced angle', ytitle='reduced energy', aspect='auto')

            if False:
                from srxraylib.plot.gol import plot_image
                plot_image(fm_pol, angle_array_reduced, e_over_ec_array, title='polarization', xtitle='reduced angle', ytitle='reduced energy', aspect='auto')
            ...
            if True:
                ...
                plot_image(cdf1d, angle_array_reduced, e_over_ec_array, xtitle='reduced angle',
            ...
            # cdf_interpolated = interpolator(angle_array_reduced, eeee)
            # plot(angle_array_reduced, cdf_interpolated, title='E/Ec=%f' % eeee)
            ...
            # eeee = numpy.ones_like(angle_array_reduced) * e_over_ec_array[10]
            # cdf_interpolated = interpolator(angle_array_reduced, eeee)
            #
            # # for i in range(eeee.size):
            # #     print(i, angle_array_reduced[i], eeee[i], cdf_interpolated[i])
            #
            # if True:
            #     from srxraylib.plot.gol import plot, plot_image
            #     plot_image(cdf2, angle_array_reduced, e_over_ec_array, xtitle='reduced angle',
            #                ytitle='reduced energy', title='cdf2', aspect='auto')
            #
            #     plot(angle_array_reduced, cdf_interpolated)
            #
            # print(">>> cdf: ", cdf2.shape, cdf1.shape)
            #
            # plot(angle_array_reduced, fm[:, 5], title='1D PDF')
            # :
            # print(sampler1d.cdf())
            # plot(angle_array_reduced, sampler1d.cdf(), title='1D CDF')
        ...
        for itik in range(NRAYS):
            ...
            if method == 'new':
                ...
                cdf_interpolated = interpolator(angle_array_reduced, sampled_photon_energy / critical_energy_array[itik])
                ...
                cdf_interpolated = interpolator(angle_array_reduced, sampled_photon_energy / critical_energy)
                s = Sampler1Dcdf(cdf_interpolated, angle_array_mrad)
                r = numpy.random.random()
