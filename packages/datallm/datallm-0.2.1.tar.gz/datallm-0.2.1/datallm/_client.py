...
class DataLLM(SyncClient):
    def __init__(self, base_url=None, api_key=None):
        ...
        self.usage = CompletionUsage(completion_tokens=0, prompt_tokens=0, total_tokens=0)
    ...
    def enrich(
        ...
        if model is None:
            model = self.models()[0]
        if dtype is None:
            dtype = DtypeEnum.category if categories else DtypeEnum.string
        ...
        try:
            ...
            for df_chunk in track(
                    df_chunks,
                    description=f"{prompt[:20]:<20}",
                    total=num_chunks,
                    disable=not progress_bar,
            ):
                completion_row_values = [
                    [NamedValue(name=name, value=str(row[name])) for name in row.index]
                    for _, row in df_chunk.iterrows()
                ]

                @retry(
                    retry=retry_if_exception_type(APIStatusError) & retry_if_exception_message(match=r"HTTP error occurred: (408|429|503|504)"),
    ...
    def mock(
        ...
        if isinstance(columns, list):
            columns = {col: {} for col in set(columns)}
