        ...
        *,
        ...
        Args:
            ...
            rag_config:
                ...
                rag_type: str one of
                    :code:`"llm_only"` LLM Only (no RAG) - Generates a response to answer the user's
                        query without any supporting document contexts. Requires 1 LLM call.
                    :code:`"rag"` RAG (Retrieval Augmented Generation) - RAG with neural/lexical hybrid
                        search using the user's query to find relevant contexts from a collection for
                        generating a response. Requires 1 LLM call.
                    :code:`"hyde1"` HyDE RAG (Hypothetical Document Embedding) - Like RAG, but uses the
                        LLM Only response to find relevant contexts from a collection for generating a
                        response. Requires 2 LLM calls.
                    :code:`"hyde2"` HyDE RAG+ (Combined HyDE+RAG) - Like RAG, but uses HyDE RAG response
                        to find relevant contexts from a collection for generating a response. Requires
                        3 LLM calls.
                    :code:`"rag+"` RAG+ - Like RAG, but uses more context and recursive summarization to
                        overcome LLM context limits. Keeps all retrieved chunks, puts them in order,
                        adds neighboring chunks, then uses the summary API to get the answer. Can require
                        several LLM calls.
