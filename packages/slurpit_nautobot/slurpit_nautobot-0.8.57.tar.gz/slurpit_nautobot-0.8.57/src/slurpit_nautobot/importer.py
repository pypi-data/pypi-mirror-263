...
def handle_changed():
    #qs = SlurpitStagedDevice.objects.filter(id__in=RawSQL("SELECT s.* FROM slurpit_nautobot_slurpitstageddevice s INNER JOIN slurpit_nautobot_slurpitimporteddevice i ON s.slurpit_id = i.slurpit_id AND s.changeddate > i.changeddate"))
    ...
    while offset < count:
        ...
        for device in batch_qs:
            ...
            if result.mapped_device:
                ...
                result.mapped_device._custom_field_data.update({
...
def get_dcim_device(staged: SlurpitStagedDevice | SlurpitImportedDevice, **extra) -> Device:
    kw = get_defaults()
    cf = extra.pop('_custom_field_data', {})
    cf.update({
    ...
    device_inventory_status = Status.objects.get_for_model(Device).all()[3]
    kw.setdefault('status', device_inventory_status)
