...
class Backend(abc.ABC):
    ...
    def translate(self, query: "AnyQuery", **kwargs) -> Tuple[str, "ParamManager"]:
        """Converts the Query object into a plain SQL string, with all their
        needed parameters and metadata."""
        raise NotImplementedError()
...
class Session(abc.ABC):
    ...
    @abc.abstractmethod
    def fetch(
        self, query: "AnyQuery", *, size: Optional[int] = None
    ) -> Result[List["AnyTuple"]]:
        raise NotImplementedError()

    @abc.abstractmethod
    def fetch_dataframe(self, query: "AnyQuery") -> Result[pl.DataFrame]:
        raise NotImplementedError()
