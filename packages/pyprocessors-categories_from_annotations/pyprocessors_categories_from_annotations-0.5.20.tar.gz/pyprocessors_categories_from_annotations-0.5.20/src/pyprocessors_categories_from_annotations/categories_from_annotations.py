        ...
        for document in documents:
            with add_logging_context(docid=document.identifier):
                if params.processing_unit == ProcessingUnit.document:
                    document.categories = create_categories_from_annotations(document.annotations, params.multi_label_threshold)
                    ...
                    for isent, sent in enumerate(document.sentences):
                        sent.categories = create_categories_from_annotations(grouped[isent], params.multi_label_threshold)
                    document.annotations = None
                    document.categories = []
        return documents
...
def create_categories_from_annotations(annotations, threshold=0.0):
    categories = []
    if annotations:
