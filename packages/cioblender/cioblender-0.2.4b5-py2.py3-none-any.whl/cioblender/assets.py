    ...
    try:
        ...
        for asset in extra_file_assets_list:
            if asset.file_path and STARTUP_VAL not in asset.file_path:
                ...
                if os.path.exists(asset.file_path) and not is_home_directory_or_drive(asset.file_path):
                    path_list.add(asset.file_path)
                else:
                    logger.debug("Unable to find extra file asset: {}".format(asset.file_path))
    except Exception as e:
