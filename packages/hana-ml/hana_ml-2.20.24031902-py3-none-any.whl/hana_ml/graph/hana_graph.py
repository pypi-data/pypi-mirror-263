...
class Graph(object):  # pylint: disable=too-many-public-methods
    ...
    * edges_hdf: hana_ml.DataFrame of the edges
    ...
    def describe(self) -> pd.Series:
        ...
        desc_series = pd.concat(desc_series, describer.self_loops)
        desc_series = pd.concat(desc_series, describer.degree)
        desc_series = pd.concat(desc_series, describer.density)

        try:
            # Not supported on on-prem, so it won't be added
            desc_series = pd.concat(desc_series, describer.triangles_count)
            desc_series = pd.concat(desc_series, describer.is_connected)
