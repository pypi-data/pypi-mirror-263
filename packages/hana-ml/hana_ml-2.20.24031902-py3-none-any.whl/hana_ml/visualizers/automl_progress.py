...
class AutomaticEmptyObjProxy(object):
    ...
    def switch_to_another_fetch_table(self):
        ...
        new_conn = create_connection_context(self.connection_context)
...
class PipelineProgressStatusMonitor(object):
    ...
    def start(self):
        ...
        """
        self.task_manager = TaskManager(AutomaticObjProxy(self.automatic_obj, self.new_connection_context), self.update_ui_interval, self.fetch_table_interval, self.runtime_platform)
...
class SimplePipelineProgressStatusMonitor(object):
    """
    The instance of this class can monitor the progress of AutoML execution.
    No automatic object is required, only progress_indicator_id is required.
    This means that you can use the instance of this class at any time.
    ...
    """
    def __init__(self, connection_context: ConnectionContext, fetch_table_interval=1, runtime_platform='jupyter'):
        if runtime_platform not in ['jupyter', 'sap_bas', 'vscode']:
    ...
    def start(self, progress_indicator_id, highlight_metric=None):
        ...
        progress_indicator_id : str
            Represents the ongoing automatic task.

        highlight_metric : str, optional
            Specify the metric that need to be displayed on the UI.
        """
        new_connection_context = create_connection_context(self.original_connection_context)
        self.task_manager = TaskManager(AutomaticEmptyObjProxy(progress_indicator_id, new_connection_context, highlight_metric), self.update_ui_interval, self.fetch_table_interval, self.runtime_platform)
        self.task_manager.start()
