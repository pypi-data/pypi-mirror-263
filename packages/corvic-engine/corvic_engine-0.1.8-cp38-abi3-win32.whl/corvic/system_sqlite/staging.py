    ...
    def _update_blobs(self) -> None:
        ...
        for blob, table_name in table_blobs:
            ...
            with blob.open("rb") as stream:
                # Technically, we could set the count now but this class is mainly
                # for testing and we test more by adopting the fiction that table
                # import is eventually consistent.
                self._db_conn.from_arrow(pq.read_table(stream)).create(
                    f'"{table_name}"'
                )

        # Synchronize tables known by client with duckdb state
        next_count: dict[str, int | None] = {}
        for _, table_name in table_blobs:
            next_count[table_name] = self._table_counts.get(table_name, None)
