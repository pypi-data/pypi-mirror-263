...
nround: typing.Callable[[typing.Union[numbers.Real, int, float, complex, typing.SupportsComplex]], int]
'''Round any number to integer.

For production binaries, this is just `int`.
'''
if __debug__:
    def nround(n: typing.Union[numbers.Real, int, complex, typing.SupportsComplex]) -> int:
...
nfloat: typing.Callable[[typing.Union[numbers.Real, int, float, complex, typing.SupportsComplex]], float]
'''Convert any number to floating point.

For production binaries, this is just `float`.
'''
if __debug__:
    def nfloat(n: typing.Union[numbers.Real, int, float, complex, typing.SupportsComplex]) -> float:
...
class Vector:
    ...
    def scale(self, both: typing.Optional[numbers.Real] = None, *,
        ...
        return self.__class__(dx=nround(self.dx * (sx or both or 1)),
                              dy=nround(self.dy * (sy or both or 1)))
...
def VectorPolar(r: numbers.Real, theta: numbers.Real) -> Vector:
    ...
    return Vector(
        nround(r * math.cos(math.radians(theta))),
        -nround(r * math.sin(math.radians(theta))),
    )
...
class GeometryLine:
    ...
    def intersect_y(self, y: int) -> typing.Optional[XY]:
        ...
        if self.m is None:
            ...
            return XY(nround((y - self.c0) / self.m), y)
    ...
    def intersect_x(self, x: int) -> typing.Optional[XY]:
        ...
        if self.m is None:
            ...
            return XY(x, nround(self.m * x + self.c0))
    ...
    def parallel_point(self, p: XY) -> 'GeometryLine':
        ...
        if p in self:
            ...
            return self.__class__(m=self.m, c0=nfloat(self.c0 + hdelta))
    ...
    def perpendicular_point(self, p: XY) -> 'GeometryLine':
        ...
        if self.m is None:
            ...
            pm = math.pow(-self.m, -1.0)  # type: ignore[arg-type]
