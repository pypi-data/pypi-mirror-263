...
class CosimulationEnergyPlusDataDriven(CosimulationEngine):
    ...
    def __init__(self, cfg_file, neighborhood_file, weather_file, max_iter = 100, ddm = 'dummy', training_split_ratio = 0.0,
                 error_function = RootMeanSquareError(), num_processes_bems_pool = 2, num_processes_data_driven_ucm=2):
    ...
    def load_umms(self):
        ...
        for n in range(n_street_canyons):
            ...
            bems_pool = EnergyPlusSimulationPool(EPWDataLoader(self.weather_file, year=self.start_date.year), output_dir=self.building_energy_sim_dir, nproc=self.num_processes_bems_pool)
            ...
            if self.ddm == 'lss':
                ...
                if self.training_split_ratio > 0.0:
                    umm = LinearStateSpaceUrbanCanopyModel(street_canyon_loader, training_split_ratio=self.training_split_ratio, n_processes=self.num_processes_data_driven_ucm)
                else:
                    umm = LinearStateSpaceUrbanCanopyModel(street_canyon_loader, n_processes=self.num_processes_data_driven_ucm)
                    if platform.system() == 'Windows':
        ...
        if self.manager_mementos.is_empty():
            ...
            for umm in self.umms:
                for bem in umm.street_canyon_loader.bems_pool.pool:
                    ...
                    errors_sensible_cooling_load.append(self.error_function.err(bem.building.get_sensible_cooling_load().values,
                                                                  self.manager_mementos.get_building_memento(building_name).average_sensible_cooling_load.values))
                    errors_latent_cooling_load.append(self.error_function.err(bem.building.get_latent_cooling_load().values,
...
class ExclusiveCosimulationEnergyPlusDataDriven(CosimulationEnergyPlusDataDriven):
    ...
    def __init__(self, cfg_file, neighborhood_file, weather_file, max_iter = 100, ddm = 'dummy', training_split_ratio = 0.0, error_function = RootMeanSquareError(), num_processes_bems_pool = 2,  num_processes_data_driven_ucm = 2):
        CosimulationEnergyPlusDataDriven.__init__(self, cfg_file, neighborhood_file, weather_file, max_iter, ddm, training_split_ratio, error_function, num_processes_bems_pool, num_processes_data_driven_ucm)
