...
class PlotConfig:
    ...
    err_style: Literal["band", "unit_traces"] = "unit_traces"
    """the error style of the chart"""
    figlegend: bool = True
    """if toggled, we will use `fig.legend` instead of `ax.legend`"""
    custom_env_id: Optional[str] = None
    """Costa's hack -- allow you to specify the custom env_id to be plotted as title"""
...
def compare(
        ...
        for idx, env_id in enumerate(env_ids):
            ...
            for i in range(len(runsetss[0][idx].metrics)):
                metric_over_step = wb.LinePlot(
                    ...
                    title_x="Steps",
                    title_y=runsets[idx].metric_names[i],
    ...
    for idx, env_id in enumerate(env_ids):
        ...
        for idx_metric, (metric, metric_name) in enumerate(zip(runsetss[0][0].metrics, runsetss[0][0].metric_names)):
            ...
            use_legend = not pc.figlegend and len(env_ids) * idx_metric + idx == 0 and pc.ncols_legend > 0
            ex.plot(
                ax=ax,
                title=env_id,
                x="global_step",
            ...
            ax.set_xlabel("")
            if idx_metric == 0:
                if pc.custom_env_id is not None:
                    ax.set_title(pc.custom_env_id)
                else:
                    ax.set_title(env_id)
            else:
                ax.set_title("")
            ...
            if idx_metric == 0:
                if pc.custom_env_id is not None:
                    ax_time.set_title(pc.custom_env_id)
                else:
...
if __name__ == "__main__":
    ...
    for filters_idx, filters in enumerate(args.filters):
        ...
        metric_names = query["metric_names"] if "metric_names" in query else None
