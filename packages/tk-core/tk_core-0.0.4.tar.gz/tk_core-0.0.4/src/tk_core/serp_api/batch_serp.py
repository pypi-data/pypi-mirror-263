...
from tk_core.serp_api.models import BatchSERPAPIResponse, SERPAPIBatchRequest
...
class BatchSERPAPISerps(BatchRequest):
    ...
    def __init__(self, params: SERPAPIBatchRequest) -> None:
        ...
        self.metadata: TerakeetMetadata = params.tk_metadata
        self.cache_duration = params.tk_metadata.cache_duration
        if self.cache_duration not in [0, 1, 30]:
            raise ValueError("cache_duration (days) must be 0, 1, or 30")
        self.run_date = datecode()
    ...
    def prepare_hash(self, params: dict | None = None) -> None:
        ...
        if params is not None:
            ...
            to_hash = subset_dict(self.params.model_dump(), ["tk_metadata"])
    ...
    def get_serps_new(self) -> BatchSERPAPIResponse:
        """ """
        # Step 1
        ...
        # Substep 1a - write to the audit log
    ...
    def process_one_query(self, hash_: str, query: str) -> None:
        ...
        response["tk_metadata"] = self.metadata.model_dump()
    ...
    def create_requests(self, query: str) -> None:
        ...
        for page_num in range(page_count):
            ...
            loaded_params = extract_serpapi_params(
                query, self.params.search_parameters.model_dump(), per_page, per_page * page_num, self.metadata.model_dump()
            )
    ...
    def format_response_for_snowflake(self, query: str, hash_: str, response: dict) -> list:
        """
        ...
        for p in response.get("pages", []):
            ...
            for org_res in p.get("response", {}).get("organic_results", []):
                # create a new result
                ...
                result = {
                    "METADATA": self.metadata.tags,
                    "PAGE_NUM": p.get("request_metadata", {}).get("page_number", -1),
                    "QUERY": query,
                    "REQUEST_HASH": hash_,
                    "EXECUTION_ID": f"{hash_}/{self.run_date}",
