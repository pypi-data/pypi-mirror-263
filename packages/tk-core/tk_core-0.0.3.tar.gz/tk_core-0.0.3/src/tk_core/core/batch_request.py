...
class BatchRequest(TerakeetRequest):
    ...
    def process_cached(self, cached_hashes: list) -> None:
        ...
        a - batch update snowflake where hashes match
        update tk_metadata with new tags
        """
        self.batch_update_snowflake(cached_hashes)

    def process_needed(self, needed_hashes: list) -> None:
        """
        Step 4
        Process the needed hashes
        This step should be a batch process
        ThreadPool for FastAPI
        Prefect should use Dask

        """
        for hash_ in needed_hashes:
            self.process_one_query(hash_)

    def process_one_query(self, params: dict) -> dict:
        """
        Process one query

        a - make the request
        b - write to API log
        c - write to Snowflake (tabular data for end users)
        d - update Redis hashes
        """
        # a
        response = self._make_api_request(self.params)  # noqa
