...
class SERPAPI(TerakeetRequest):
    ...
    def execute_query(self) -> dict:
        """
        Returns the full response from the serpapi client adding metadata information
        """
        # set all metadata to tk_metadata
        self.json_response["tk_metadata"] = self.output_metadata.model_dump()
        # check for cached results
        cached_result = self.check_for_cached_result(self.metadata.cache_duration)
        if cached_result is not None:
            print(f"Cached result found for [blue]{self.request_hash}")
            self.json_response["response"] = cached_result
            self.json_response["tk_metadata"]["cached"] = True
        else:
            self.json_response["response"] = self._make_request()
            # prepare the tags from metadata
            self.metadata.tags["request_id"] = [self.request_id]
            self.json_response["response"]["tags"] = self.metadata.tags
            # write to the cache in snowflake
            self.write_request_response_to_snowflake(self.json_response["response"], self.params.model_dump())
            self.json_response["tk_metadata"]["cached"] = False
