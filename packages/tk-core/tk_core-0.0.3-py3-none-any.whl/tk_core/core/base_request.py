...
from tk_core.common.dictionary import combine_dicts, subset_dict
...
class TerakeetRequest(metaclass=PostInitMetaclass):
    ...
    def check_for_cached_result(self, cache_duration: int = 24) -> dict:
        """
        Checks for a cached result in Snowflake
        Args:
            cache_duration (int, optional): The duration in hours to check for a cached result. Defaults to 24.
            [0,1,24,168,720,8760]
            [no cache, 1 hour, 1 day, 1 week, 1 month, 1 year]
        """
        with Snowkeet() as snow:
            table = snow.session.table("REQUEST_CACHE")
            # TODO: Make this better?
            df = table.to_pandas()
            results = df.loc[
                (df["REQUEST_HASH"] == self.request_hash)
                & (df["REQUEST_TIME"] > dt.datetime.now() - dt.timedelta(hours=cache_duration))
            ]

            if len(results) > 0:
                cached_df = pd.DataFrame([results.iloc[-1]])
                existing_tags = json.loads(cached_df["TK_METADATA"].iloc[0])
                # add our request ID to the existing tags
                existing_tags["request_id"].append(self.request_id)
                # Combine the tags into one dictionary
                new_metadata = combine_dicts(
                    existing_tags,
                    self.get_tags_as_lists(),
                )
                # cached_df["TK_METADATA"] = new_metadata
                # convert the parameters and JSON into true json from snowpark string
                # cached_df['REQUEST_PARAMETERS'] = [json.loads(cached_df['REQUEST_PARAMETERS'].iloc[0])]
                # cached_df['RAW_JSON'] = [json.loads(cached_df['RAW_JSON'].iloc[0])]
                # we hit a cache so we need to both update the line with the new tags
                update_result = snow.update_table_one_condition(
                    "REQUEST_CACHE",
                    "TK_METADATA",
                    new_metadata,
                    "REQUEST_ID",
                    cached_df["REQUEST_ID"].iloc[0],
                )
                print(update_result)
                # and return the RAW result
                return json.loads(cached_df["RAW_JSON"].iloc[0])
