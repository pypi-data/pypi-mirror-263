...
class BatchSERPAPISerps(BatchRequest):
    ...
    def process_request(self) -> None:
        """ """
        cached, needed = self.separate_hashes_into_cached_vs_needed()
        # save the number of cached results vs needed results
        self.set_cached_results_count(len(cached))
        self.set_needed_results_count(len(needed))
        # for cached results, batch update Snowflake tags
        for hash_ in cached:
            pass
        # for need results, create requests
        for hash_ in needed:
            self.process_one_serp(hash_)
