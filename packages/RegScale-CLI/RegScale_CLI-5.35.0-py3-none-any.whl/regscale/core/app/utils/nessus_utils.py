    ...
    if download:
        ...
        if response.status_code == 200:
            with zipfile.ZipFile(io.BytesIO(response.content), "r") as zip_ref:
                zip_ref.extractall("artifacts")  # You can specify a different folder if you want
...
def get_cpe_data() -> Tuple[etree._ElementTree, List[dict]]:
    ...
    :rtype: Tuple[etree._ElementTree, List[dict]]
    """
    cpe_list = []
...
def validate_nessus_severity(severity: str) -> str:
    ...
    if severity.lower() in ["info", "low", "medium", "high", "critical"]:
        ...
        return "medium"
    raise ValueError(
        "Invalid tenableMinimumSeverityFilter setting in the configuration. Must be one of: low, medium, high, or critical"
    )
