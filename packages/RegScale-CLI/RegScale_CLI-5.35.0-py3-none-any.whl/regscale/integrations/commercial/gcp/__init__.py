...
class GCPScannerIntegration(ScannerIntegration):
        ...
        for control_id, categories in self.gcp_control_tests.items():
            for category, control_test in categories.items():
                if control_test.get("status", "") == "Failed":
                    ...
                    continue
                if ci_id := self.control_implementation_map.get(control_id.lower()):
                    ...
                    logger.warning(f"Control {control_id} not found in RegScale SSP {self.plan_id}.")
    ...
    def parse_finding(self, gcp_finding: Finding) -> Optional[IntegrationFinding]:
        ...
        """
        if any(control_labels := [c.ids for c in gcp_finding.compliances if c.standard == "nist"]):
            control_ids = list(
                filter(
                    None,
                    [self.control_implementation_map.get(str(cid).lower()) for cid in control_labels[0]],
                )
            )
            ...
            for control_id in control_ids:
                ...
                if self.gcp_control_tests.get(control_id_str, {}).get(gcp_finding.category):
                    self.gcp_control_tests[control_id_str][gcp_finding.category]["status"] = "Failed"
