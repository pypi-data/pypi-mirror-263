...
class Conftest:
    def __init__(self, base_path: str, view=pyecharts, driver=...,
                 replace_js='', index_url='please input url here.', user='None', pwd='None'):
        """
        ...
        if report.when == 'call' or report.when == 'setup':
            ...
            if (report.skipped and xfail) or (report.failed and not xfail):
                ...
                if file_name:
                    ...
                    html_ = (
                            '<div><img src="%s" alt="screenshot" style="width:465px;height:236px;" '
    ...
    def pytest_html_results_table_header(self, cells):
        cells[0].attr.width = '12%'
        cells[1].attr.width = '28%'
        cells.insert(2, html.th("Description", width='35%'))
        cells.insert(3, html.th("Time", class_="sortable time", col="time", width='15%'))
        cells[4].attr.width = '10%'
        cells.pop()
        for i in range(len(cells)):
    ...
    def pytest_html_results_summary(self, prefix):
        if 'pyplot' in str(self.view):
            prefix.extend([html.div(html.img(
                src=r'./summary.jpg', alt='', style="width:280px;height:245px;margin:-200px 0px 0px 43%"),
            )])
            prefix.extend([html.div(html.img(
                src=r'./summary2.jpg', alt='', style="width:280px;height:245px;margin:-200px 0px 0px 73%"),
            )])
    ...
    def _sub_html_(self):
        ...
        waste = re.findall(re_waste, res)[0][3:-4]
    ...
    def _replace_js_(self):
        if self.replace_js:
            for i in (self.e1_html_path, self.e2_html_path):
                with open(i, 'r', encoding='utf-8') as f:
                    con = f.read()
                res = re.sub('https://assets.pyecharts.org/assets/v5/echarts.min.js', '../../../common/e.js', con)
        ...
        res = re.sub('seconds. </div>',
                     'seconds. </div>'
                     '<iframe style="width:30%;height:300px;margin:-200px 0px 0px 30.5%" width="100%" height="100%" '
                     'frameborder="no" border="0" src="assets/e1.html"></iframe>'
                     '<iframe style="width:30%;height:300px;margin:-280px 0px 0px 59.5%" width="100%" height="100%" '
    ...
    def _draw_by_echarts2_(self, data):
        ...
        bar = self.view.charts.Bar(init_opts=self.view.options.InitOpts(width="500px", height=f"{140 + len(x) * 20}px"))
        bar.add_xaxis(x)
        bar.add_yaxis(series_name='成功', y_axis=y1, stack="stack1", color=self.__pfs_color__[0], category_gap="33%",
                      itemstyle_opts=radius(0, [0, 20, 20, 0]), label_opts={"show": True, "color": "black"})
        ...
        bar.add_yaxis(series_name='失败', y_axis=y2, stack="stack1", color=self.__pfs_color__[1], category_gap="33%",
                      itemstyle_opts=radius(1, [20, 0, 0, 20]), label_opts={"show": True, "color": "black"})

        max_ = reduce(lambda _self, self_: _self + self_, v) * 1.33
        bar.set_global_opts(
            # min/max 控制长短，这种情况配合inverse使用，调试时把show打开
            xaxis_opts={"show": False, "inverse": True, "min": 0, "max": max_},
            yaxis_opts={"show": True, "splitLine": {"show": False}, "axisLine": {"show": False}, "axisTick": {
                "show": False}, "offset": -90},  # offset 控制偏移多少
