...
def firma_electronica_avanzada(
    ...
    try: 
        ...
        if not models.Ficha.objects.filter(id=ficha_id).exists():
            ...
            raise ObjectDoesNotExist("La ficha no existe")
        ...
        if not models.Evaluacion_Imiv.objects.filter(imiv_id=o_ficha.id, vigente=True).exists():
            ...
            raise ObjectDoesNotExist("La evaluacion no existe")
        ...
        if prorroga:
            ...
            raise ObjectDoesNotExist("El estado de evaluacion no existe")
        ...
        if models.DocumentoEvaluacionImiv.objects.filter(
            ...
            raise ObjectDoesNotExist("No existe documento para enviar a firmar")
