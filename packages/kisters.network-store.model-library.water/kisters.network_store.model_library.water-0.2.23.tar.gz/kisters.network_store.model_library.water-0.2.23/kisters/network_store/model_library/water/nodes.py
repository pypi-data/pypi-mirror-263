...
class Storage(_Node):
    ...
    level_uncontrolled: Optional[List[_StorageLevelFlow]] = Field(
        None,
        description="Optional level-uncontrolled table providing the "
        "uncontrolled flow per level",
        min_items=4,
    )
    level_uncontrolled_interp: Optional[InterpEnum] = Field(
        InterpEnum.bspline, description="Level-uncontrolled flow interpolation approach"
    )
    initial_level: Optional[float] = Field(
        None, description="Initial level for simulation"
    )
