...
class DockerImage:
    ...
    def serialize(self, args):
        repo_url = args.docker_image_repo

        if args.docker_push_repo is not None:
            repo_url = args.docker_push_repo
        return {
            "dockerfile": str(self.dockerfile.absolute()),
            "context": str(self.context.absolute()),
            "url": "%s/%s" % (repo_url, self.image_tail),
            "tags": self.tags,
        }
...
class Container:
    ...
    def patch_build(self, args, challenge_dir: Path, version_tag: str):
        ...
        if len(image_name) > 0:
            ...
            image_tail = normalize(challenge_dir.name)

        image_tail = image_tail.lower()
        image_url = "%s/%s" % (args.docker_image_repo, image_tail)

        logger.info(f"Determined docker image URL {image_tail} for {challenge_dir.name}")
        del self.container_dict["build"]
        self.container_dict["image"] = "%s:%s" % (image_url, version_tag)

        return DockerImage(dockerfile, context, image_tail, [version_tag])
