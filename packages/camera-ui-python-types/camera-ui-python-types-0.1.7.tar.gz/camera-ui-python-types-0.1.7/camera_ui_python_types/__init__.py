...
class DeviceManager(Protocol):  # todo: add methods
    def on(self, event: str, listener: Callable) -> "DeviceManager": ...
    def once(self, event: str, listener: Callable) -> "DeviceManager": ...
    def remove_listener(self, event: str, listener: Callable) -> "DeviceManager": ...
    def remove_all_listeners(self, event: str) -> "DeviceManager": ...


# plugin api
EventType = Literal["finishLaunching", "shutdown"]
...
class PluginAPI(Protocol):
    ...
    def on(self, event: EventType, listener: Callable) -> "PluginAPI": ...
    def once(self, event: EventType, listener: Callable) -> "PluginAPI": ...
    def remove_listener(self, event: EventType, listener: Callable) -> "PluginAPI": ...
    def remove_all_listeners(self, event: EventType) -> "PluginAPI": ...


# base plugin
class BasePlugin(ABC):
    def __init__(self, plugin_id: str, log: PluginLogger, api: PluginAPI) -> None:
        self.id: str = plugin_id
        self.api: PluginAPI = api
        self.logger: PluginLogger = log
