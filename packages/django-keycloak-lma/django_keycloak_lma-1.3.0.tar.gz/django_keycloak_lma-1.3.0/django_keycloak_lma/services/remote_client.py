...
def exchange_remote_token(client, access_token):
    ...
    if not django_keycloak_lma.services.oidc_profile.check_groups_for_client_access(client.client_id, id_token_object['user_group']):
        raise exceptions.AuthenticationFailed(detail=f'The token does not contain a user_group with access to the Client { client.client_id }')
    ...
    try:
        ...
        raise exceptions.AuthenticationFailed(detail=messages.REMOTE_REFRESH_TOKEN_ERROR)
