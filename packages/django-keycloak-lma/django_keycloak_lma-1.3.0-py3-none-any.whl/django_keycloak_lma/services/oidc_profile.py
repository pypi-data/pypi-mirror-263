...
def get_from_id_token(client, id_token, check_client=False):
    """
    Get OpenID Connect profile from given id_token.

    :param django_keycloak_lma.models.Client client:
    :param str id_token:
    :param boolean check_client:
    :rtype: django_keycloak_lma.models.OpenIdConnectProfile
    """
    issuer = django_keycloak_lma.services.realm.get_issuer(client.realm)
    ...
    if check_client:
        if client.client_id != id_token_object['azp']:
            raise exceptions.Conflict(detail=messages.MISMATCH_CLIENT_ERROR)
...
def update_or_create_oidc_profile(client, id_token_object, user=None):
    ...
    if user is None:
        raise exceptions.Conflict(detail='User does not specified')
...
def get_oidc_profile_by_token(client, id_token_object):
    ...
    try:
        ...
        raise exceptions.NotFound(detail=messages.obj_id_not_found(id_token_object['sub'], messages.OIDC_PROFILE_NOT_FOUND))
...
def get_active_access_token(oidc_profile):
    ...
    if oidc_profile.refresh_expires_before is None \
        ...
        raise exceptions.TokensExpired()
...
def refresh_token(client, refresh_token_obj):
    try:
        ...
        raise exceptions.AuthenticationFailed(detail=messages.REFRESH_TOKEN_ERROR)
...
def check_groups_for_client_access(client_id, user_groups):
    if user_groups is None:
        return False

    service_list = [item.split('/')[1] for item in user_groups]

    if client_id not in service_list:
        return False
