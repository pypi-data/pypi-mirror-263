...
class Tracker:
    ...
    def _start_exercise(
        ...
        if DEFAULT_RICH:
            ...
            table.add_row("Exercise #", f"{self.current_exercise_number} of {len(self.config['workouts'][self.current_workout_name]['exercises'])}")
    ...
    def _save_workout_session(self) -> None:
        ...
        if "end_time" not in self.config["current_workout"]:
            self._record_workout_end_time()
