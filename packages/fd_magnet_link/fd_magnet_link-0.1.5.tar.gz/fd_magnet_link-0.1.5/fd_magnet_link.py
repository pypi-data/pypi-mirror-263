...
def main():
    ...
    parser.add_argument('-n', '--number', type=int, default=10, help='The number of results you would like to display.')
    ...
    query_str.append(args.number)
    highlight = highlight_prepare(["file_name"])
    sql = prepare_sql(
        "select {} _id ,category ,file_name ,firstadd_utc_timestamp ,filesize ,total_count from library.dht where query_string_recency(%s) limit 0,%d".format(
            highlight), query_str)
    rst = glitter_db_client.query(sql)
    console = Console()
    table = Table(show_header=True, header_style="bold magenta", expand=True)
    table_centered = Align.center(table)
    link = Link("How to download via magnet link", "https://anybt.eth.limo/#/guide")
