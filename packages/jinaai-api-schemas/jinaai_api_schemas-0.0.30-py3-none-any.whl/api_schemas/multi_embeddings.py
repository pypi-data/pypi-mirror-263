...
class MultiEmbeddingsObject(BaseDocWithoutId):
    ...
    embeddings: Union[NdArray, bytes] = Field(
        ...
        'base64.b64decode(embeddings), dtype=np.float32)`'
    )
...
class LateInteraction(BaseInputModel):
    ...
    query_embeddings: Union[NdArray, bytes] = Field(
        description='The embeddings of the query. It should be of the shape (num_tokens, token_embedding_size). It '
        'can also be a base64 encoded string of the "float32" embeddings array. You can pass them as a '
        'string with `base64.b64encode(nparray.tobytes()).decode()`',
    )
    documents_embeddings: List[Union[NdArray, bytes]] = Field(
        description='The embeddings of all the documents to rerank. Each matrix in the list should be of the shape ('
        'num_tokens, token_embedding_size). It can also be list of base64 encoded strings of the '
