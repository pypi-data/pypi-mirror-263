from PKiL.Utils import NLP,Math,Utils
...
class ProKnow(object):
    ...
    def color_code(self,point_item,annotations):
        ...
        for li in index_list:
            ...
            try:
                # if(li[0]==li[-1]):
                #     color_coded += ' '+color_code+' '.join(chars[li[0]:li[0]-1])
                # else:
    ...
    def get_label(self,annotated_point,temp,pk=True):
        ...
        for item in self.annotate_rules:
            ...
            if bool_annotation == annotation:
                if not pk:
                    return item[-1]
                return (item[0],bool_annotation,temp)
    ...
    def evaluate_pk(self,point,kernel='dot',pk=True,explanation=False):
        ...
        if explanation:
            ...
            print ("="*40)
            print ("Inference: ",self.get_label(annotated_point,color_coded_point,pk=False))
        
        return (self.get_label(annotated_point,color_coded_point,pk=pk))
