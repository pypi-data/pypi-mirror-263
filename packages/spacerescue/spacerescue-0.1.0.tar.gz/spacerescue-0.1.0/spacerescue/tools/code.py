...
class Code:
    ...
    def __init__(self):
        self.brain_module = load_module("brain.py", "brain")
        self.queue = Queue()
...
def _validate_code_proc(queue: Queue):
    result = subprocess.run(["coverage", "run", "-m", "pytest", "test_brain.py"])
    if result.returncode != 0:
        queue.put(result)
        time.sleep(1)
        return

    result = subprocess.run(["coverage", "report", "--show-missing", "--fail-under=80"])
    if result.returncode != 0:
        queue.put(result)
        time.sleep(1)
        return
