import pyray as pr

from spacerescue.constants import SCREEN_HEIGHT, SCREEN_WIDTH
from spacerescue.gameplay import context_manager
from spacerescue.gameplay.root_scene import RootScene
from spacerescue.render import resource_manager
from spacerescue.render.animators.open_vertical import OpenVertical
from spacerescue.render.widgets.button import Button
from spacerescue.render.widgets.console import Console
from spacerescue.render.animators.fade_inout import FadeInOut
from spacerescue.resources import STORY
from spacerescue.tools.util import window_request_close


class Game(RootScene):
    
    CONTEXT = context_manager.get_instance()

    def __init__(self):
        super().__init__()
        Game.CONTEXT.reset_all()

    def enter_scene(self):
        super().enter_scene()
        self.menu = Game.RESOURCE_MANAGER.load_texture("menu")
        self.fade_in = FadeInOut(0, 255, 1.0, pr.Color(*pr.BLACK))
        self.fade_out = FadeInOut(255, 0, 1.0, pr.Color(*pr.BLACK))
        self.state = 0

        self.avatar = Console(
            "widget",
            pr.Vector2(100, 100),
            pr.Vector2(276, 276),
            ["![avatar](resources/images/avatar.png)"],
        )
        self.avatar_animator = OpenVertical(self.avatar, 0.3, Console.BG_COLOR)

        self.console = Console(
            "widget",
            pr.Vector2(400, 100),
            pr.Vector2(SCREEN_WIDTH / 2, SCREEN_HEIGHT - 200),
            [STORY],
        )
        self.console_animator = OpenVertical(self.console, 0.3, Console.BG_COLOR)

        self.buttons = [
            Button(
                "button_start",
                pr.Vector2(SCREEN_WIDTH - 220, 100),
                pr.Vector2(200, 38),
                "START",
                pr.Color(254, 201, 123, 192),
                self.button_is_clicked,
            ),
            Button(
                "button_music_onoff",
                pr.Vector2(SCREEN_WIDTH - 220, 150),
                pr.Vector2(200, 38),
                "MUSIC ON/OFF",
                pr.Color(64, 191, 182, 192),
                self.button_is_clicked,
            ),
            Button(
                "button_quit",
                pr.Vector2(SCREEN_WIDTH - 220, 200),
                pr.Vector2(200, 38),
                "QUIT",
                pr.Color(252, 127, 104, 192),
                self.button_is_clicked,
            ),
        ]

    def button_is_clicked(self, button: Button):
        if button.id == "button_start":
            self.state = 3
        if button.id == "button_music_onoff":
            music = resource_manager.get_instance().load_music("music")
            if pr.is_music_stream_playing(music):
                pr.stop_music_stream(music)
            else:
                pr.play_music_stream(music)
        if button.id == "button_quit":
            self.state = 5
            
    def update(self):
        match self.state:
            case 0:
                if self.fade_in.is_playing():
                    self.fade_in.update()
                else:
                    self.state = 1

            case 1:
                if (
                    self.avatar_animator.is_playing()
                    or self.avatar_animator.is_playing()
                ):
                    self.avatar_animator.update()
                    self.console_animator.update()
                else:
                    self.state = 2

            case 2:
                if pr.is_key_pressed(pr.KeyboardKey.KEY_SPACE):
                    self.state = 3
                else:
                    self.avatar_animator.update()
                    self.console_animator.update()
                    for button in self.buttons:
                        button.update()

            case 3:
                from spacerescue.gameplay.challenges.challenge1 import Challenge1
                self.begin_challenge(Challenge1(self))
                self.state = 4

            case 4:
                self.leave_scene()
                self.enter_scene()
                self.state = 0
                
            case 5:
                if self.fade_out.is_playing():
                    self.fade_out.update()
                else:
                    self.state = 6
            
            case 6:
                self.leave_scene()
                window_request_close()

    def draw(self):
        pr.begin_drawing()

        pos_x = int(SCREEN_WIDTH / 2 - self.menu.width / 2)
        pos_y = int(SCREEN_HEIGHT / 2 - self.menu.height / 2)
        pr.draw_texture(self.menu, pos_x, pos_y, pr.WHITE)  # type: ignore

        if self.state in (1, 2):
            self.avatar_animator.draw()
            self.console_animator.draw()
        
        if self.state == 2:
            for button in self.buttons:
                button.draw()

        if self.state in (0, 3):
            self.fade_in.draw()
            
        if self.state in (5, 6):
            self.fade_out.draw()

        pr.end_drawing()
