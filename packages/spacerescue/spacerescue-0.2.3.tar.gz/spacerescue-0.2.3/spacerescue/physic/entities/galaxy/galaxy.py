...
from spacerescue.physic.entities.galaxy.galaxy_generator import GalaxyGenerator
from spacerescue.physic.entities.galaxy.physic.physical_laws import PhysicalLaws
from spacerescue.physic.entities.galaxy.physic.physical_mapper import PhysicalMapper
...
from spacerescue.physic.entities.galaxy.dust import Dust
...
ALL_SODB = lambda x: isinstance(x, Star) or  isinstance(x, Planet) or isinstance(x, HyperspacePortal)

ALL_STAR_SYSTEM_OBJECTS = lambda y: (
    lambda x: not isinstance(x, Dust) and (isinstance(y, Star) or x.parent == y)
)

ALL_STAR_SYSTEM_PORTALS= lambda y: (
    lambda x: isinstance(x, HyperspacePortal) and (isinstance(y, Star) or x.parent == y)
)

ALL_STAR_SYSTEM_PLANETS = lambda y: (
    lambda x: isinstance(x, Planet) and (isinstance(y, Star) or x.parent == y)
)

ALL_STAR_SYSTEM_HABITABLE_PLANETS = lambda y: (
    lambda x: isinstance(x, Planet) and (isinstance(y, Star) or x.parent == y) and x.is_habitable
)
...
class Galaxy(Universe):
    ...
    def __init__(self):
        ...
        for stellar_object in self.stellar_objects:
            point = self.mapper.real_to_grid(stellar_object.position)
            self.quadtree.insert(point, stellar_object)

        # Lighting effect

        shader_lighting = Galaxy.RESOURCE_MANAGER.load_shader("shader_lighting")
    ...
    def update(self, dt: float, camera: Camera):
        self.quadtree.update(self.mapper.real_to_grid(camera.position), dt)
        super().update(dt)
    ...
    def find_closest_stellar_object(
        ...
        node = self.quadtree.find(self.mapper.real_to_grid(position))
        if node is not None:
            closest_entity = None
            min = 0
    ...
    def find_stellar_object_over_mouse(
        self, camera: Camera, filter_func=None
    ) -> Entity | None:
        ...
        for stellar_object in self.filter_stellar_objects(filter_func):
            hit_info = pr.get_ray_collision_sphere(
                ray,
                stellar_object.to_grid_position(),
                max(stellar_object.to_grid_size(camera.position), 2),
            )
            if hit_info.hit:
    ...
    def _enter_solar_system(self, star: Star):
        ...
        self.star_light = rl.create_light(
            rl.LIGHT_POINT,
            self.mapper.real_to_grid(star.position),
            pr.Vector3(0, 0, 0),
            star.color,
            Galaxy.RESOURCE_MANAGER.load_shader("shader_lighting"),
        )
