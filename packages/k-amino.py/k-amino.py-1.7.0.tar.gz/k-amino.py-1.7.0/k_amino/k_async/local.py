...
__all__ = ('AsyncSubClient',)


class AsyncSubClient(AsyncAcm, AsyncSession):
    ...
    def __init__(
        ...
        if acm:
            AsyncAcm.__init__(self, comId=comId, client=client, proxies=proxies)
        else:
            self.comId = comId
            AsyncSession.__init__(self, client=client, debug=debug, proxies=proxies)
    ...
    async def get_video_rep_info(self, chatId: str) -> RepInfo:
        ...
        """
        return RepInfo(await self.getRequest(f"/x{self.comId}/s/chat/thread/{chatId}/avchat-reputation"))

    async def claim_video_rep(self, chatId: str) -> RepInfo:
        ...
        """
        return RepInfo(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/avchat-reputation"))

    async def join_chat(self, chatId: str) -> Json:
        """Join a chat.
    ...
    async def get_member_following(self, userId: str, start: int = 0, size: int = 25) -> UserProfileList:
        ...
        """
        return UserProfileList((await self.getRequest(f"/x{self.comId}/s/user-profile/{userId}/joined?start={start}&size={size}"))["userProfileList"]).UserProfileList

    async def get_member_followers(self, userId: str, start: int = 0, size: int = 25) -> UserProfileList:
        ...
        """
        return UserProfileList((await self.getRequest(f"/x{self.comId}/s/user-profile/{userId}/member?start={start}&size={size}"))["userProfileList"]).UserProfileList

    async def get_member_visitors(self, userId: str, start: int = 0, size: int = 25) -> VisitorsList:
        ...
        """
        return VisitorsList((await self.getRequest(f"/x{self.comId}/s/user-profile/{userId}/visitors?start={start}&size={size}"))["visitors"]).VisitorsList

    async def get_chat_threads(self, start: int = 0, size: int = 25) -> ThreadList:
        ...
        """
        return ThreadList((await self.getRequest(f"/x{self.comId}/s/chat/thread?type=joined-me&start={start}&size={size}"))["threadList"]).ThreadList

    async def get_chat_messages(self, chatId: str, start: int = 0, size: int = 25) -> MessageList:
        ...
        """
        return MessageList((await self.getRequest(f"/x{self.comId}/s/chat/thread/{chatId}/message?v=2&pagingType=t&start={start}&size={size}"))["messageList"]).MessageList
    ...
    async def get_user_blogs(self, userId: str, start: int = 0, size: int = 25) -> BlogList:
        ...
        """
        return BlogList((await self.getRequest(f"/x{self.comId}/s/blog?type=user&q={userId}&start={start}&size={size}"))["blogList"]).BlogList

    async def get_user_wikis(self, userId: str, start: int = 0, size: int = 25) -> WikiList:
        """Get user wiki list.
        ...
        """
        return WikiList((await self.getRequest(f"/x{self.comId}/s/item?type=user-all&start={start}&size={size}&cv=1.2&uid={userId}"))["itemList"]).WikiList

    async def get_all_users(self, usersType: UserType = "recent", start: int = 0, size: int = 25) -> UserProfileList:
    ...
    async def get_chat_members(self, chatId: str, start: int = 0, size: int = 25) -> UserProfileList:
        ...
        """
        return UserProfileList((await self.getRequest(f"/x{self.comId}/s/chat/thread/{chatId}/member?start={start}&size={size}&type=default&cv=1.2"))["memberList"]).UserProfileList

    async def get_chat_info(self, chatId: str) -> Thread:
        ...
        """
        return Thread((await self.getRequest(f"/x{self.comId}/s/chat/thread/{chatId}"))["thread"]).Thread

    async def get_online_users(self, start: int = 0, size: int = 25) -> UserProfileList:
        ...
        """
        return UserProfileList((await self.getRequest(f"/x{self.comId}/s/live-layer?topic=ndtopic:x{self.comId}:online-members&start={start}&size={size}"))["userProfileList"]).UserProfileList

    async def get_public_chats(self, filterType: FilterType = "recommended", start: int = 0, size: int = 25) -> ThreadList:
        ...
        """
        return ThreadList((await self.getRequest(f"/x{self.comId}/s/chat/thread?type=public-all&filterType={filterType}&start={start}&size={size}"))["threadList"]).ThreadList

    async def full_embed(self, chatId: str, link: str, image: typing.BinaryIO, message: str) -> Json:
    ...
    async def unfollow(self, userId: str) -> Json:
        ...
        """
        return Json(await self.postRequest(f"/x{self.comId}/s/user-profile/{userId}/member/{self.uid}"))

    async def follow(self, userId: typing.Union[typing.List[str], str]) -> Json:
    ...
    async def start_chat(
        ...
        return Thread((await self.postRequest(f"/x{self.comId}/s/chat/thread", data))['thread']).Thread

    async def invite_to_chat(self, chatId: str, userId: typing.Union[typing.List[str], str]) -> Json:
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/member/invite", data=data))
    ...
    async def edit_profile(
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/user-profile/{self.uid}", data))
    ...
    async def edit_chat(
        ...
        if background:
            ...
            res.append(Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/member/{self.uid}/background", data)))
        if announcement:
        ...
        if extensions:
            data['extensions'] = extensions
        res.append(Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}", data)))
        return res
    ...
    async def chat_settings(
        ...
        if isinstance(doNotDisturb, bool):
            ...
            res.append(Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/member/{self.uid}/alert", data)))
        if isinstance(viewOnly, bool):
            view = "enable" if viewOnly else "disable"
            res.append(Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/view-only/{view}")))
        if isinstance(canInvite, bool):
            can = "enable" if canInvite else "disable"
            res.append(Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/members-can-invite/{can}")))
        if isinstance(canTip, bool):
            can = "enable" if canTip else "disable"
            res.append(Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/tipping-perm-status/{can}")))
        if isinstance(pin, bool):
            action = "pin" if pin else "unpin"
            res.append(Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/{action}")))
        if coHosts:
            data = {
                "uidList": coHosts if isinstance(coHosts, list) else [coHosts],
                "timestamp": int(time.time() * 1000)
            }
            res.append(Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/co-host", data)))
        return res

    async def like_blog(self, blogId: typing.Optional[str] = None, wikiId: typing.Optional[str] = None) -> Json:
        """Like a blog or wiki.
    ...
    async def unlike_blog(self, blogId: typing.Optional[str] = None, wikiId: typing.Optional[str] = None):
    ...
    async def change_titles(self, userId: str, titles: typing.List[str], colors: typing.List[str]) -> Json:
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/user-profile/{userId}/admin", data))

    @typing.overload
    async def like_comment(self, commentId: str, blogId: str) -> Json: ...
    @typing.overload
    async def like_comment(self, commentId: str, *, wikiId: str) -> Json: ...
    @typing.overload
    async def like_comment(self, commentId: str, *, userId: str) -> Json: ...
    async def like_comment(self, commentId: str, blogId: typing.Optional[str] = None, wikiId: typing.Optional[str] = None, userId: typing.Optional[str] = None) -> Json:
    ...
    @typing.overload
    async def unlike_comment(self, commentId: str, *, wikiId: str) -> Json: ...
    @typing.overload
    async def unlike_comment(self, commentId: str, *, userId: str) -> Json: ...
    async def unlike_comment(self, commentId: str, blogId: typing.Optional[str] = None, wikiId: typing.Optional[str] = None, userId: typing.Optional[str] = None) -> Json:
    ...
    @typing.overload
    async def comment(self, comment: str, blogId: str, *, replyTo: typing.Optional[str] = None, isGuest: bool = False) -> Json: ...
    @typing.overload
    async def comment(self, comment: str, *, wikiId: str, replyTo: typing.Optional[str] = None, isGuest: bool = False) -> Json: ...
    @typing.overload
    async def comment(self, comment: str, *, userId: str, replyTo: typing.Optional[str] = None, isGuest: bool = False) -> Json: ...
    async def comment(
        self,
        comment: str,
    ...
    @typing.overload
    async def delete_comment(self, commentId: str, blogId: str) -> Json: ...
    @typing.overload
    async def delete_comment(self, commentId: str, *, wikiId: str) -> Json: ...
    @typing.overload
    async def delete_comment(self, commentId: str, *, userId: str) -> Json: ...
    async def delete_comment(self, commentId: str, blogId: typing.Optional[str] = None, wikiId: typing.Optional[str] = None, userId: typing.Optional[str] = None) -> Json:
        """Delete a comment.
    ...
    async def edit_comment(
        ...
        if userId:
            ...
            raise ValueError("Please put blog or wiki or user Id")
        return Comment(await self.postRequest(link, data)).Comments

    @typing.overload
    async def get_comment_info(self, commentId: str, blogId: str, *, isGuest: bool = False) -> Comment: ...
    @typing.overload
    async def get_comment_info(self, commentId: str, *, wikiId: str, isGuest: bool = False) -> Comment: ...
    @typing.overload
    async def get_comment_info(self, commentId: str, *, userId: str, isGuest: bool = False) -> Comment: ...
    async def get_comment_info(
        ...
        if userId:
            ...
            raise ValueError("Please put blog or wiki or user Id")
        return Comment(await self.getRequest(link)).Comments
    ...
    async def get_wall_comments(
        ...
        """
        return CommentList((await self.getRequest(f"/x{self.comId}/s/user-profile/{userId}/comment?sort={sorting}&start={start}&size={size}"))["commentList"]).CommentList
    ...
    async def vote_comment(self, commentId: str, blogId: str, value: bool = True) -> Json:
        """Vote a comment.
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/blog/{blogId}/comment/{commentId}/vote?cv=1.2&value=1", data))

    async def vote_poll(self, optionId: str, blogId: str) -> Json:
        """Vote a poll option.
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/blog/{blogId}/poll/option/{optionId}/vote", data))

    @typing.overload
    async def get_blog_info(self, blogId: str) -> GetInfo: ...
    @typing.overload
    async def get_blog_info(self, *, wikiId: str) -> GetInfo: ...
    @typing.overload
    async def get_blog_info(self, *, folderId: str) -> GetInfo: ...
    async def get_blog_info(self, blogId: typing.Optional[str] = None, wikiId: typing.Optional[str] = None, folderId: typing.Optional[str] = None) -> GetInfo:
        ...
        return GetInfo(await self.getRequest(link)).GetInfo

    async def get_blogs(self, start: int = 0, size: int = 25) -> BlogList:
        ...
        """
        return BlogList((await self.getRequest(f"/x{self.comId}/s/feed/featured?start={start}&size={size}"))["featuredList"]).BlogList

    async def get_blogs_more(self, start: int = 0, size: int = 25) -> BlogList:
        ...
        """
        return BlogList((await self.getRequest(f"/x{self.comId}/s/feed/featured-more?start={start}&size={size}"))["blogList"]).BlogList

    async def get_blogs_all(self, start: int = 0, size: int = 25, pagingType: str = "t") -> BlogList:
    ...
    async def check_in(self, timezone: int = 0) -> Json:
        """Community check-in
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/check-in", data))

    async def check_in_lottery(self, timezone: int = 0) -> Json:
        """Play lottery.
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/check-in/lottery", data))

    @typing.overload
    async def delete_message(self, chatId: str, messageId: str, asStaff: typing.Literal[False] = False, reason: None = None) -> Json: ...
    @typing.overload
    async def delete_message(self, chatId: str, messageId: str, asStaff: typing.Literal[True], reason: str) -> Json: ...
    async def delete_message(self, chatId: str, messageId: str, asStaff: bool = False, reason: typing.Optional[str] = None) -> Json:
        ...
        if asStaff and reason:
            ...
            return Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/message/{messageId}/admin", data))
        else:
            return Json(self.deleteRequest(f"/x{self.comId}/s/chat/thread/{chatId}/message/{messageId}"))

    async def invite_by_host(self, chatId: str, userId: str) -> Json:
        ...
        """
        return Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/member/{userId}/invite-av-chat"))
    ...
    async def strike(
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/notice", data))

    async def ban(self, userId: str, reason: str, banType: typing.Optional[int] = None) -> Json:
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/user-profile/{userId}/ban", data))

    async def unban(self, userId: str, note: str) -> Json:
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/user-profile/{userId}/unban", data))

    @typing.overload
    async def hide(self, blogId: str, *, note: typing.Optional[str] = None) -> Json: ...
    @typing.overload
    async def hide(self, *, wikiId: str, note: typing.Optional[str] = None) -> Json: ...
    @typing.overload
    async def hide(self, *, chatId: str, note: typing.Optional[str] = None) -> Json: ...
    @typing.overload
    async def hide(self, *, userId: str, note: typing.Optional[str] = None) -> Json: ...
    async def hide(
        self,
        blogId: typing.Optional[str] = None,
        wikiId: typing.Optional[str] = None,
    ...
    @typing.overload
    async def unhide(self, blogId: str, *, note: typing.Optional[str] = None) -> Json: ...
    @typing.overload
    async def unhide(self, *, wikiId: str, note: typing.Optional[str] = None) -> Json: ...
    @typing.overload
    async def unhide(self, *, chatId: str, note: typing.Optional[str] = None) -> Json: ...
    @typing.overload
    async def unhide(self, *, userId: str, note: typing.Optional[str] = None) -> Json: ...
    async def unhide(
        self,
        blogId: typing.Optional[str] = None,
        wikiId: typing.Optional[str] = None,
    ...
    async def send_warning(self, userId: str, reason: typing.Optional[str] = None) -> Json:
        """Warn a user.
    ...
    async def invite_to_voice_chat(self, chatId: str, userId: str) -> Json:
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/vvchat-presenter/invite", data))

    async def post_blog(self, title: str, content: str, fansOnly: bool = False) -> Json:
        """Post a blog.
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/blog", data))
    ...
    async def post_wiki(
        ...
        if icon:
            data["icon"] = icon
        return Json(await self.postRequest(f"/x{self.comId}/s/item", data))

    async def delete_blog(self, blogId: str) -> Json:
        """Delete a blog.
    ...
    async def delete_wiki(self, wikiId: str) -> Json:
        """Delete a wiki.
    ...
    async def activate_status(self, status: int = 1) -> Json:
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/user-profile/{self.uid}/online-status", data))

    async def subscribe(self, userId: str, autoRenew: bool = False, transactionId: typing.Optional[str] = None) -> Json:
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/influencer/{userId}/subscribe", data))

    async def submit_wiki(self, wikiId: str, message: typing.Optional[str] = None) -> Json:
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/knowledge-base-request", data))
    ...
    async def get_chat_bubbles(self, start: int = 0, size: int = 25) -> BubbleList:
        ...
        """
        return BubbleList((await self.getRequest(f"/x{self.comId}/s/chat/chat-bubble?type=all-my-bubbles&start={start}&size={size}"))["chatBubbleList"]).BubbleList

    async def select_bubble(self, bubbleId: str, applyToAll: bool = False, chatId: typing.Optional[str] = None) -> Json:
        """Apply a chat bubble.
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/apply-bubble"))

    async def delete_chat_bubble(self, bubbleId: str) -> Json:
    ...
    async def get_chat_bubble_templates(self, start: int = 0, size: int = 25) -> BubbleTemplates:
        ...
        """
        return BubbleTemplates((await self.getRequest(f"/x{self.comId}/s/chat/chat-bubble/templates?start={start}&size={size}"))["templateList"])

    async def upload_custom_bubble(self, templateId: str) -> Json:
        ...
        """
        return Json(await self.postRequest(f"/x{self.comId}/s/chat/chat-bubble/templates/{templateId}/generate"))

    async def kick(self, chatId: str, userId: str, rejoin: bool = True) -> Json:
    ...
    async def block(self, userId: str) -> Json:
        """Block a user.
        ...
        """
        return Json(await self.postRequest(f"/x{self.comId}/s/block/{userId}"))

    @typing.overload
    async def flag(self, reason: str, blogId: str, *, flagType: int = 0) -> Json: ...
    @typing.overload
    async def flag(self, reason: str, *, wikiId: str, flagType: int = 0) -> Json: ...
    @typing.overload
    async def flag(self, reason: str, *, userId: str, flagType: int = 0) -> Json: ...
    async def flag(
        self,
        reason: str,
        blogId: typing.Optional[str] = None,
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/flag", data))

    # thanks to V¡ktor#9475 for ideas
    async def send_active_time(self, tz: int = 0, timers: typing.Optional[typing.List[typing.Dict[typing.Literal["start", "end"], int]]] = None) -> Json:
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/community/stats/user-active-time", data, minify=True))

    async def transfer_host(self, chatId: str, userIds: str) -> Json:
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/transfer-organizer", data))

    async def accept_host(self, chatId: str, requestId: str) -> Json:
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/transfer-organizer/{requestId}/accept", data))

    async def set_cohost(self, chatId: str, userId: typing.Union[str, typing.List[str]]) -> Json:
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/chat/thread/{chatId}/co-host", data))

    async def del_cohost(self, chatId : str, userId : str) -> Json:
    ...
    async def get_quizzes(
        ...
        return BlogList((await self.getRequest(link))["blogList"]).BlogList

    async def get_quiz_questions(self, quizId: str) -> QuizQuestionList:
        ...
        """
        return QuizQuestionList((await self.getRequest(f"/x{self.comId}/s/blog/{quizId}?action=review"))["blog"]["quizQuestionList"]).QuizQuestionList

    async def play_quiz(self, quizId: str, questions: typing.List[str], answers: typing.List[str], mode: typing.Literal[0, 1] = 0) -> Json:
        """Play a quiz.
        ...
        return Json(await self.postRequest(f"/x{self.comId}/s/blog/{quizId}/quiz/result", data))

    async def get_quiz_rankings(self, quizId: str, start: int = 0, size: int = 25) -> QuizRankings:
        ...
        """
        return QuizRankings(await self.getRequest(f"/x{self.comId}/s/blog/{quizId}/quiz/result?start={start}&size={size}")).QuizRankings

    async def search_user(self, username: str, start: int = 0, size: int = 25) -> UserProfileList:
        ...
        """
        return UserProfileList((await self.getRequest(f"/x{self.comId}/s/user-profile?type=name&q={username}&start={start}&size={size}"))["userProfileList"]).UserProfileList

    async def search_blog(self, words: str, start: int = 0, size: int = 25) -> BlogList:
        ...
        """
        return BlogList((await self.getRequest(f"/x{self.comId}/s/blog?type=keywords&q={words}&start={start}&size={size}"))["blogList"]).BlogList

    async def get_notifications(self, start: int = 0, size: int = 25, pagingType: str = "t") -> NotificationList:
        ...
        """
        return NotificationList(await self.getRequest(f"/x{self.comId}/s/notification?pagingType={pagingType}&start={start}&size={size}")).NotificationList

    async def get_notices(self, start: int = 0, size: int = 25, noticeType: str = "usersV2", status: int = 1) -> NoticeList:
        ...
        """
        return NoticeList(await self.getRequest(f"/x{self.comId}/s/notice?type={noticeType}&status={status}&start={start}&size={size}")).NoticeList

    async def accept_promotion(self, requestId: str) -> Json:
        ...
        """
        return Json(await self.postRequest(f"/x{self.comId}/s/notice/{requestId}/accept"))

    async def decline_promotion(self, requestId: str) -> Json:
        ...
        """
        return Json(await self.postRequest(f"/x{self.comId}/s/notice/{requestId}/decline"))

    async def sendWebActive(self) -> Json:
        ...
        return Json(await self.postRequest("/community/stats/web-user-active-time", data, webRequest=True))

    async def get_recent_blogs(self, pageToken: typing.Optional[str] = None, start: int = 0, size: int = 25) -> BlogList:
        ...
        """
        return RecentBlogs((await self.getRequest(f"/x{self.comId}/s/feed/blog-all?pagingType=t&start={start}&size={size}&pageToken={pageToken}"))["BlogList"]).RecentBlogs

    @typing.overload
    async def publish_to_featured(self, blogId: str, *, duration: typing.Literal[1, 2, 3] = 1) -> Json: ...
    @typing.overload
    async def publish_to_featured(self, *, wikiId: str, duration: typing.Literal[1, 2, 3] = 1) -> Json: ...
    @typing.overload
    async def publish_to_featured(self, *, chatId: str, duration: typing.Literal[1, 2, 3] = 1) -> Json: ...
    @typing.overload
    async def publish_to_featured(self, *, userId: str, duration: typing.Literal[1, 2, 3] = 1) -> Json: ...
    ...
    @typing.overload
    async def remove_from_featured(self, blogId: str) -> Json: ...
    @typing.overload
    async def remove_from_featured(self, *, wikiId: str) -> Json: ...
    @typing.overload
    async def remove_from_featured(self, *, chatId: str) -> Json: ...
    @typing.overload
    async def remove_from_featured(self, *, userId: str) -> Json: ...
    ...
    async def get_all_influencers(self) -> UserProfileList:
        ...
        """
        return UserProfileList((await self.getRequest(f"/x{self.comId}/s/influencer"))["userProfileList"])
