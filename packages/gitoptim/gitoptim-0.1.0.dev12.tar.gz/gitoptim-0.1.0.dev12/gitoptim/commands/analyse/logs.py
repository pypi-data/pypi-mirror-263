...
def extract_relevant_logs(api: GitlabAPI, last_section: bool, after_last_section: bool, section_name: str):
    ...
    if after_last_section:
        return extract_after_last_section(logs_before_command)
    if last_section:
        return extract_section(logs_before_command)
    if section_name is not None:
        return extract_section(logs_before_command, section_name)

    return logs_before_command


def remove_command_echo(logs: str):
    command_echo_index = logs.rfind("$ gitoptim")
    return logs[:command_echo_index] if command_echo_index != -1 else logs
...
def extract_section(logs: str, section_name: Optional[str] = None):
    ...
    pattern = regex.compile(EndTag.generate_regex_pattern(name=section_name), regex.REVERSE)
    match = pattern.search(logs)
    end_index = match.start() if match is not None else len(logs)

    return remove_command_echo(logs[start_index:end_index])
