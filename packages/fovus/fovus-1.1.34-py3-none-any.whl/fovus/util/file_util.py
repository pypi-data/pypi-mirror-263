...
class FileUtil:
    ...
    def get_files_in_job_directory(
        ...
        for directory_path, _, filenames in os.walk(root_directory_path):
            ...
            local_relative_folderpath = os.path.relpath(directory_path.replace("\\", "/"), root_directory_path)

            if not is_any_file_included and not FileUtil.include_exclude_allows_path(
                local_relative_folderpath, include_input_list, exclude_input_list
            ):
                continue

            local_folderpath_list.append(local_relative_folderpath)

            if not is_any_file_included:
                empty_folderpath_list.append(local_relative_folderpath)
