...
async def init(
    ...
    async with async_launchflow_client_ctx() as client:
        ...
        if "aws" in project.configured_cloud_providers:
            infra = infra_dot_py_template.aws_template(
                project.name, framework, resources
            )
            requirements = project_gen.requirements(framework, resources, "aws")
        elif "gcp" in project.configured_cloud_providers:
            infra = infra_dot_py_template.gcp_template(
                project.name, framework, resources
            )
            requirements = project_gen.requirements(framework, resources, "gcp")
        else:
        ...
        init_py = os.path.join(app_directory_path, "__init__.py")
        # Top level files
...
async def _deploy_service(
    ...
    deploy_task = progress.add_task(f"Deploying {service_ref}...", total=None)
    ...
    async for status in client.operations.stream_operation_status(operation.id):
        if status.is_error():
