...
def coolmit_probe_worker(args):
    ...
    msg_h = hf(ensure_binary(msg_header))
    for i in range(start_offset + worker_id, start_offset + work_chunk_size, num_workers):
        salt = ''
        remaining = i
        ...
        h = msg_h.copy()
        h.update(ensure_binary(salt))
        h.update(ensure_binary(msg_body))
        h = h.hexdigest()
