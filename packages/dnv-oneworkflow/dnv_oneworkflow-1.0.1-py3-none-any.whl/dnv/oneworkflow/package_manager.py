...
from .user_agreement_manager import UserAgreementManager
...
class ApplicationInfo:
    ...
    Attributes:
        ...
        VersionId (str): The version ID of the application.
        ExecutablePath (str): The path to the executable file of the application.
        ExecutableType (str): The type of the executable file of the application.
    """
    ...
    class Encoder(json.JSONEncoder):
        """
        Custom JSON encoder that can be used to serialize instances of the ApplicationInfo class.
        """
        ...
        def default(self, o: Any):
            ...
            """
            if isinstance(o, ApplicationInfo):
                return asdict(o)
...
class PackageManager:
    ...
    async def _get_package_info_from_server(
        ...
        Returns:
            Optional[ApplicationInfo]: The package information, if available; otherwise, None.
        """
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                return from_dict(ApplicationInfo, data=response.json())
    ...
    def _load_installed_applications_info(self) -> List[ApplicationInfo]:
        """
        ...
        Returns:
            List[ApplicationInfo]: A list of installed application information.
        """
        oc_install_path = PackageManager.get_apps_install_path()
        installed_packages: List[ApplicationInfo] = []
        for path in Path(oc_install_path).rglob("*.ocm"):
            with path.open("r", encoding="utf-8-sig") as ocm_file:
                data = ocm_file.read()
                try:
                    app_info = from_dict(ApplicationInfo, data=json.loads(data))
    ...
    def _update_application_info_cache(self, package_info: ApplicationInfo):
        """
        ...
        Args:
            package_info (ApplicationInfo): The package information to update the cache with.
        """
    ...
    def _write_application_info_to_ocm_file(self, package_info: ApplicationInfo):
        """
        ...
        Args:
            package_info (ApplicationInfo): The package information to write to the OCM file.
        """
        app_name = package_info.ApplicationId
        ...
        with open(file_path, "w", encoding="utf-8") as ocm_file:
            try:
                json.dump(package_info, ocm_file, indent=4, cls=ApplicationInfo.Encoder)
            except Exception as ex:
                raise ex

    def _update_application_info(self, package_info: ApplicationInfo):
        """
        Updates the application information both in the cache and in the OCM file.

        Args:
            package_info (ApplicationInfo): The package information to update with.
        """
