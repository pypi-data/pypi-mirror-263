...
@dataclass
class IgnoreResultError(ExTask):
    task: ExTask

    async def exchange(self, c: Client) -> exc.ResultError:
        try:
            await self.task.exchange(c)
        except exc.ResultError as e:
            return e
...
class ReadAttribute(ExTask):
    ...
    @staticmethod
    async def read_from_obj(c: Client, obj: collection.InterfaceClass, index: int) -> cdt.CommonDataType:
