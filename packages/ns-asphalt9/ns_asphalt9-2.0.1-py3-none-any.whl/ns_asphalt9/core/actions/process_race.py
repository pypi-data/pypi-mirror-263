...
def process_race():
    ...
    while globals.G_RACE_RUN_EVENT.is_set():
        if 0 <= progress < 100:
            ...
            if page.name in [
                ...
                if not navi_data:
                    globals.NOT_SUPPORT_TRACK.append(track_name)
    ...
    if final_pos:
        ...
        if track:
            if track["tracken"] in globals.TRACK_RANK[mode]:
                if final_pos in globals.TRACK_RANK[mode][track["tracken"]]:
                    globals.TRACK_RANK[mode][track["tracken"]][final_pos] += 1
                else:
                    globals.TRACK_RANK[mode][track["tracken"]][final_pos] = 1
            else:
                globals.TRACK_RANK[mode][track["tracken"]] = {final_pos: 1}
        ...
        logger.info(
            f"track rank = {json.dumps(globals.TRACK_RANK[mode], indent=2, ensure_ascii=False)}"
        )
    logger.info(f"Already finished {globals.FINISHED_COUNT} times.")
    logger.info(f"Not recognized track = {globals.NOT_RECOGNIZED_TRACK}")
    logger.info(f"Not support track = {globals.NOT_SUPPORT_TRACK}")
