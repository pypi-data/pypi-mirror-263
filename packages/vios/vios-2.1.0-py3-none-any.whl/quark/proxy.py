...
def setlog():
    logger.remove()
    path = Path.home()/"Desktop/home/log/proxy/{time:%Y-%m-%d}.log"
    level = "INFO"
...
class QuarkProxy(object):
    ...
    def connect(self):
        ...
        with open(router, 'r') as f:
            ...
            for chip, addr in table.items():
                self.router[chip] = connect('QuarkProxy', **addr)
    ...
    def submit(self, task: dict):
        ...
        try:
            ...
            self.dbcon.execute(
                f'INSERT INTO proxy (TID, NAME, SYSTEM, CREATED) VALUES ("{t.tid}","{name}","{chip}","{ctime}")')
            self.dbcon.commit()
            logger.info(f'task will be executed on {chip}!')
        except KeyError as e:
            # by server
            logger.info(f'task will be executed on local machine: {chip}!')
            t: Task = submit(task, block=False)  # local machine
    ...
    def run(self, task: dict):
        ...
        cloud = {'metainfo': {'name': f'{chip}:/{name}',
                              'priority': 15,
                              'mutex': True,
                              'other': {'shots': 1024,
                                        'signal': 'count',
                                        'standby': True,
                                        'quafu': {'tid': tid,  # int('task_id', 16),  # 42229EA0373A3AB2
                                                  'token': token}}
                              },
    ...
    def result(self, tid: int, raw: bool = False):
        ...
        try:
            if self.router:  # proxy
                ...
                result: dict = self.router[record[3]].result(record[1])
