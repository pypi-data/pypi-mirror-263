...
class OpenAIWrapper:
    ...
    def __init__(self, *, llm: BaseLanguageModel, **base_config: Any):
        """
        Args:
        ...
        #     self._clients: List[BaseLanguageModel] = [llm]
        #     self._config_list = [{
        #         **extra_kwargs,
        #         **{
        #             k: v
    ...
    def _separate_openai_config(self, config: Dict[str,
    ...
    def _separate_create_config(self, config: Dict[str,
                                                   Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    def _client(self, config: Dict[str, Any], openai_config: Dict[str, Any]) -> OpenAI:
        ...
        if api_type is not None and api_type.startswith("azure"):
            openai_config["azure_deployment"] = openai_config.get("azure_deployment",
                                                                  config.get("model"))
    ...
    def _construct_create_params(self, create_config: Dict[str, Any],
        ...
        if prompt is not None:
            ...
            params["messages"] = [
                ...
                } if m.get("content") else m for m in messages  # type: ignore [union-attr]
            ]
        return params
    ...
    def create(self, **config: Any) -> ChatCompletion:
        ...
        for i, llm in enumerate(self._clients):
            ...
            if cache_client is not None:
                with cache_client as cache:
                    ...
                    if response is not None:
                        ...
                        # check the filter
                        pass_filter = filter_func is None or filter_func(context=context,
                                                                         response=response)
            ...
            try:
                response = self._completions_create(llm, params)
    ...
    def _completions_create(self, llm: BaseLanguageModel, params: Dict[str,
                                                                       Any]) -> ChatCompletion:
        ...
        index = 0
        for chunk in llm.stream(input=params["messages"]):
            content = chunk.text
            tool_calls_chunks = chunk.additional_kwargs.get("tool_calls")
            ...
            # begin block
            function_call_chunk = chunk.additional_kwargs.get("tool_calls")
            # Handle function call
            if function_call_chunk:
                # Handle function call
                if function_call_chunk:
            ...
            if content is not None:
                ...
                response_contents[index] += content
                finish_reasons[index] = None
                completion_tokens += 1
                index += 1
            else:
                # print()
    ...
    def _update_usage_summary(self, response: Union[ChatCompletion, Completion],
                              use_cache: bool) -> None:
        ...
        def update_usage(usage_summary: Optional[Dict[str, Any]]) -> Dict[str, Any]:
            ...
            usage_summary[response.model] = {
                "cost":
                usage_summary.get(response.model, {}).get("cost", 0) +
                response.cost,  # type: ignore [union-attr]
                "prompt_tokens":
                usage_summary.get(response.model, {}).get("prompt_tokens", 0) +
                usage.prompt_tokens,
                "completion_tokens":
                usage_summary.get(response.model, {}).get("completion_tokens", 0) +
                usage.completion_tokens,
                "total_tokens":
                usage_summary.get(response.model, {}).get("total_tokens", 0) + usage.total_tokens,
            }
    ...
    def cost(self, response: Union[ChatCompletion, Completion]) -> float:
        ...
        if isinstance(tmp_price1K, tuple):
            return (tmp_price1K[0] * n_input_tokens +
                    tmp_price1K[1] * n_output_tokens) / 1000  # type: ignore [no-any-return]
        return tmp_price1K * (n_input_tokens + n_output_tokens) / 1000  # type: ignore [operator]

    @classmethod
    def extract_text_or_completion_object(
        cls, response: Union[ChatCompletion,
                             Completion]) -> Union[List[str], List[ChatCompletionMessage]]:
