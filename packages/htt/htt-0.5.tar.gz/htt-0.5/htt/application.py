...
class Application:
    ...
    @staticmethod
    def set_config_from_environ(
        config: google.protobuf.message,
        environ_definition: dict,
    ):
        """Set protobuf-based config from environ variables

        Args:
            config (google.protobuf.message): Protobuf message to set
            environ_definition (dict): environ definition in { "env_name", ("field_name", "field_type") }
        """
        for env_name, (field_name, field_type) in environ_definition.items():
