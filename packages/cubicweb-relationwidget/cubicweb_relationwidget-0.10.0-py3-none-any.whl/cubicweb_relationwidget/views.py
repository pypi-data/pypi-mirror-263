...
def bootstrap_dialog(w, _, domid):
    w(
        """
<div id="{domid}" class="modal fade rw-modal" tabindex="-1" role="dialog">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary save">{validate}</button>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->""".format(
            domid=xml_escape(domid),
            loading=_("loading..."),
            validate=_("validate"),
            close=_("close"),
        )
...
def _ensure_set(value):
    ...
    assert isinstance(value, (list, tuple)), f"unexpected type for {value!r}"
    return frozenset(value)
...
def _guess_multiple(form, field, targetetype):
    ...
    rdef = eschema.relation_definition(rschema, role=field.role, targettype=targetetype)
...
class RelationFacetWidget(fwdg.Select):
    ...
    * pertinent facets (mixing everything would shut down all
    ...
    def _render(self, form, field, renderer):
        ...
        html = []
        w = html.append
        domid = f"widget-{field.input_name(form, self.suffix).replace(':', '-')}"
        rtype = entity._cw.vreg.schema.relation_schema_for(field.name)
        ...
        bootstrap_dialog(w, _, domid)
        return "\n".join(str(node) for node in html)
    ...
    def _compute_related(self, form, field):
        ...
        if linkedto:
            related += [
                (form._cw.entity_from_eid(eid).view(self.related_vid), str(eid))
                for eid in linkedto
            ]
        if entity.has_eid():
            seen = {str_eid for html, str_eid in related}
            rset = entity.related(field.name, field.role)
            related += [
                (e.view(self.related_vid), str(e.eid))
                for e in rset.entities()
                if str(e.eid) not in seen
            ]
        if not related and callable(field.value):
            entity_from_eid = form._cw.entity_from_eid

            def view(eid):
                return entity_from_eid(int(eid)).view(self.related_vid)

            related = [(view(eid), eid) for eid in field.value(form, field)]
        return related

    def _render_post(self, w, entity, rtype, role, related, domid):
        name = f"{rtype}-{role}:{entity.eid}"
        with div(w, id="inputs-for-" + domid, Class="cw-relationwidget-list"):
        ...
        * a drop-down list of possible target etypes if there are more than 1
        ...
        if not actions:
            w(
                div(
                    xml_escape(
                        _('no available "%s" to relate to')
                        % ", ".join(f"{_(e)}" for e in target_etypes)
                    ),
            ...
            link_title = xml_escape(
                _("link to %(targetetype)s") % {"targetetype": _(target_etype)}
            )
            w(
                p(
                    a(
                ...
                with ul(w, Class="dropdown-menu"):
                    for target_etype, action in actions:
                        w(
                            li(
                                a(
                                    xml_escape(_(target_etype.type)),
                                    Class="btn-link",
                                    onclick=xml_escape(action),
                                )
                            )
    ...
    base_url_params = {"vid": "search_related_entities", "__modal": "1"}
    ...
    def trigger_options(self, form, field, target_etype):
        ...
        if "title" not in dialog_options:
            dialog_options["title"] = _(self.dialog_title) % {
                "target_etype": _(target_etype)
            }
        else:
        ...
        url_params.setdefault("relation", f"{field.name}:{target_etype}:{field.role}")
    ...
    def trigger_search_url(self, entity, url_params):
        ...
        if not entity.has_eid():
            ...
            return entity._cw.build_url(
                "ajax", fname="view", etype=entity.__regid__, **url_params
            )
        else:
            # entity is edited, use its absolute url as base url
            return entity._cw.build_url(
                "ajax", fname="view", eid=entity.eid, **url_params
            )
...
class SearchForRelatedEntitiesView(EntityStartupView):
    ...
    def nav(self):
        ...
        with ul(w, Class=css_class, role="tablist"):
            with li(w, role="presentation", Class="active"):
                w(
                    a(
                        xml_escape(_("Link/unlink entities")),
                        Class="toggle-cw-relationwidget-table",
                        href=f"#cw-relationwidget-table-{eid}",
                        data_toggle="tab",
                        role="tab",
                    )
                )
            if self.has_creation_form:
                with li(w, role="presentation"):
                    w(
                        a(
                            xml_escape(self._cw._("Link to a new entity")),
                            href=f"#rw-creation-form-{eid}",
                            data_toggle="tab",
                            role="tab",
                        )
                    )
    ...
    def selection(self):
        ...
        with div(
            w,
            id=f"cw-relationwidget-table-{eid}",
            Class="tab-pane fade in active cw-relationwidget-table",
        ):
            w(h2(f"{self._cw.__(tetype)} ({_('selection')})"))
            rset = self.linkable_rset()
            with div(
                w, Class="table-wrapper", data_rql=xml_escape(rset.printable_rql())
            ):
            ...
            with div(w, Class="cw-relationwidget-summary"):
                ...
                w(
                    ul(
                        Class=(
                            "cw-relationwidget-linked-summary" " cw-relationwidget-list"
                        )
                    )
                )
    ...
    def creation(self):
        ...
        with div(
            self.w,
            id=f"rw-creation-form-{eid}",
            Class="tab-pane fade rw-creation-form",
            data_etype=tetype,
        ):
    ...
    def linkable_rset(self):
        ...
        rtype, tetype, role = self.rdef
        rql, args = entity.cw_linkable_rql(
            rtype, tetype, role, ordermethod="fetch_order", vocabconstraints=False
        )
...
startup.ManageView.skip_startup_views.add(
    SearchForRelatedEntitiesView.__regid__
)  # noqa
...
class RelationWidgetNavigationMixin:
            ...
            with ul(w, Class="pagination pull-right rw-total-size"):
                with li(w):
                    w(
                        span(
                            xml_escape(
                                self._cw._("total: %s entities") % len(self.cw_rset)
                            )
                        )
                    )
        else:
            super().render_link_display_all(w)


class RelationWidgetPageNavigationSelect(
    RelationWidgetNavigationMixin, PageNavigationSelect
):
    __select__ = PageNavigationSelect.__select__ & _match_rw_view


class RelationWidgetSortedNavigation(RelationWidgetNavigationMixin, SortedNavigation):
...
def rset_main_type(rset):
    ...
    etypes = {sol[varname] for sol in select.solutions}
    if len(etypes) != 1:
        return None
    return etypes.pop()
...
class RelationWidgetCreationFormView(CreationFormView):
    ...
    def form_title(self, entity):
        ...
        self.w(
            '<div class="formTitle"><h2>{} {}</h2></div>'.format(
                entity.dc_type(), ptitle and f"({ptitle})"
            )
        )
    ...
    def init_form(self, form, entity):
        ...
        form.add_hidden("__onsuccess", "window.parent.cw.cubes.relationwidget.created")
        form.add_hidden(
            "__onfailure", "window.parent.cw.cubes.relationwidget.creationFailed"
        )
        form.form_buttons = [
            fwdg.SubmitButton(),
            fwdg.Button(
