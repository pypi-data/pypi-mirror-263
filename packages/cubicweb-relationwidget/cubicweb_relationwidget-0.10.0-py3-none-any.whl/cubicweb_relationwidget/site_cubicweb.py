    ...
    (
        ...
        (
            "display-all-limit",
            {
                ...
                "default": 100,
                "help": _(
                    'Maximum size of the result set the "display all" '
...
if numversion < (3, 19):
    ...
    from cubicweb.entity import Entity, pruned_lt_info, build_cstr_with_linkto_infos
    ...
    def _cw_compute_linkable_rql(
        ...
        if rqlexprs and not rdef.has_perm(self._cw, "add", **sec_check_args):
            # compute a varmap suitable to RQLRewriter.rewrite argument
            varmap = {
                v: v for v in (searchedvar.name, evar.name) if v in select.defined_vars
            }
