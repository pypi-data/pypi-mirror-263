...
class EthTxDecoders:
    ...
    def get_decode_transaction_from_db(
        self, tx_hash: str, chain_id: str = None
    ) -> bool:
        assert_tx_hash(tx_hash)
        return self._decoder_service.get_decode_transaction_from_db(chain_id, tx_hash)
...
class EthTx:
    ...
    def initialize(config: EthTxConfig):
        ...
        try:
            # Verify the connection by trying to retrieve server statistics.
            mongo_client.server_info()
            print("MongoClient connected successfully!")
        except Exception as e:
            print("Failed to connect to MongoDB: %s", str(e))
            raise  # Re-raise the exception so it can be handled by the calling code.
