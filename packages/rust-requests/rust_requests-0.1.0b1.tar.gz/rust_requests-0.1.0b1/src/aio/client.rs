...
struct MyProxyError {
    ...
    // Additional context for handling, if needed
}
    ...
    pub fn new(
        ...
        if let Some(proxy) = proxy {
            match Url::parse(&proxy) {
                Ok(proxy_url) => match reqwest::Proxy::http(proxy_url) {
                    
                    Ok(proxy) => client = client.proxy(proxy),
                    Err(err) => return Err(PyValueError::new_err(format!("Error configuring proxy: {}", err))), // Convert reqwest::Error to PyValueError
                },
                Err(err) => return Err(PyValueError::new_err(format!("Error parsing proxy URL: {}", err))), // Convert ParseError to PyValueError
            }
        }
