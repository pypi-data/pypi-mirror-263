...
api_local="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"
api_client="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"
...
def explain_api_request(code):
    ...
    response = requests.post(CHAT_API_URL, json=payload, headers=HEADERS, auth=('', api_client))
        ...
        with open(file_name, 'r', encoding='utf-8') as file:
            ...
            if private_key == "gc_spgrXSnfZqYSVqqhQiVjlYApSQofFXIUtF":
                ...
                if file_size >= 20000:
                    ...
                    behaviour_type = [{"type": "HappyPath", "number": 6}, {"type": "EdgeCase", "number":10 }, {"type": "NegativeCase", "number": 8}]
                    for obj in behaviour_type:
                        type = obj['type']

                        print(f"Generating tests for '{type}' scenarios for file '{file_name}' ")
                        ...
                        if test_response1:
                            ...
                            test_folder_path = os.path.join(directory_path, 'gocodeo_tests')
                            os.makedirs(test_folder_path,exist_ok=True)
                            # function_test_folder_path = os.path.join(test_folder_path, 'TESTS')
                            # os.makedirs(function_test_folder_path,exist_ok=True)
                            File_name=extract_filename(file_name)
                            # Write API response (test cases) to a test.py file in the same directory
                            output_file_path = os.path.join(test_folder_path, f'test_{File_name}_{type}.spec.ts')
                            
                            if (test_response1.startswith("```typescript") or 
                                test_response1.startswith("```javascript") or 
                                test_response1.startswith("```ts") or 
                                test_response1.startswith("```")):
                            ...
                            with open(output_file_path, 'w') as output_file:
                                ...
                                print(f"Test cases for '{file_name}' written to {output_file_path}")        
                        else:
                            print(f"Failed to fetch test cases from API for function '{function_name}'.") 
                    total_lines=sum(total_line) 
