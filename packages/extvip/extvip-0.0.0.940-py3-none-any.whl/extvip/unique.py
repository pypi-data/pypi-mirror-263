...
class BetaConsole:
    ...
    def alphaPrint(self, type, textvip, increment:bool=False):
        if increment:
            if self.extvipraSpace == 0: self.incrementing = True
            elif self.extvipraSpace == 10: self.incrementing = False
            if self.incrementing: self.extvipraSpace += 1
            else: self.extvipraSpace -= 1

        console_width = get_terminal_size().columns

        other_len = 28 # avg of # len(textvip.split("\x1b[0m] ")[0])
        textvip_length = len(textvip.split("\x1b[0m] ")[1]) # keeps changing cuz of colors
        type_length = len(type)
        
        spaces = max(0, console_width - (textvip_length - type_length + other_len + self.extvipraSpace))
        ...
        output = f"{textvip}{' ' * spaces}{formatted_info}"
