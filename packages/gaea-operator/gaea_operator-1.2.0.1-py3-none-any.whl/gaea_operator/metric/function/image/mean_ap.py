...
from ..tensor_operations import list_round
...
class MeanAveragePrecision(Metric):
    ...
    def __init__(self,
        ...
        self.coco_eval_bbox = DistCocoEval(
            iouThrs=iou_thrs, areaRng=area_rng, maxDets=max_dets, iouType='bbox', categories=categories)
        self.coco_eval_segm = DistCocoEval(
            iouThrs=iou_thrs, areaRng=area_rng, maxDets=max_dets, iouType='segm', categories=categories)
    ...
    def _compute_per_cat_metrics(self, params, precisions, recalls,
        ...
        for idx, category in enumerate(self.categories):
            ...
            # TODO: 增加confidenceMetrics []
            results_per_category.append(dict(categoryName=cate_name, averagePrecision=round(ap, self.decimals),
    ...
    def compute(self) -> Any:
        ...
        eval_results = {'bbox': [], 'segm': [], 'bbox_results_per_category': [], 'segm_results_per_category': []}
        if 'bbox' in _results:
        ...
        if self.classwise:
            ...
            if 'bbox' in _results:
                ...
                eval_results['bbox_results_per_category'] = results_per_category
            ...
            if self.segm:
                ...
                eval_results['segm_results_per_category'] = results_per_category
