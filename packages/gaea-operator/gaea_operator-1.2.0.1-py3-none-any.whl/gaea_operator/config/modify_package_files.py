...
from gaea_operator.utils import find_down_level_folder
...
class ModifyEnsembleFile(object):
    ...
    def __init__(self,
        ...
        self.model_name_pairs = self.get_model_name_pairs(model_config)
    ...
    def modify_ensemble(self):
        ...
        """
        aim_files = self.retrive_path(find_down_level_folder(self.ensemble_model_uri), [KEY_PBTXT_NAME])
        if len(aim_files) > 0:
            ensemble_pbtxt_name = aim_files[0]
            ensemble_config = ModelConfig._create_from_file(ensemble_pbtxt_name)
            ensemble_config.set_ensemble_step_model_name(self.model_name_pairs)
    ...
    def modify_ppyoloe(self, output_uri: str = None):
        ...
        for model_type, models in self.modify_model_names.items():
            if model_type == KEY_PREPROCESS:
                ...
                for m in models:
                    aim_files = self.retrive_path(find_down_level_folder(
