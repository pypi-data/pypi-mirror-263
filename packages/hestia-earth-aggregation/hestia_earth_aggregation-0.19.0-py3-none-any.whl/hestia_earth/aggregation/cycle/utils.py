...
def _format_terms_results(results: dict, include_matrix=True):
    ...
    if len(cycles) > 0:
        ...
        if product_id:
            ...
            same_products = flatten([
                p.get('nodes', []) for p in products if p['node'].get('term').get('@id') == product_id]
            )
            product['value'] = [list_average(flatten([p.get('value', []) for p in same_products]), 1)]
            product['economicValueShare'] = list_average([p.get('economicValueShare') for p in same_products], 100)
            return cycle
    return None
