...
class _Solution(object):
  ...
  def pprint(self, components=None, limit=None, file=_sys.stdout, precision=8):
    ...
    fields_collection = [_shared.as_matrix(getattr(self, _), name=_) for _ in fields]
    ...
    for values in zip(*fields_collection):
      file.write(" ".join(["%s: %s" % (c, nptostr(v)) for c, v in zip(fields, values)]) + '\n')
...
class GTOptAPI:
  ...
  def setup_problem(self, problem, postprocess_satellite_objective):
    ...
    for obj_index, obj_hints in enumerate(combined_resps_hints[:n_obj]):
      ...
      evaluates_map.append(objective_type != "evaluate")
      if not evaluates_map[-1]:
        ...
        if str(obj_hints.get(hint_noise_level, "")).lower() == "fromblackbox":
          raise ValueError('"Evaluate" objective does not support blackbox-based noise level: hints values `@GT/ObjectiveType=Evaluate` and `@GT/NoiseLevel=FromBlackbox` are mutually exclusive.')
  ...
  def get_results(self, in_progress=False, mode=GTOPT_SOLVE, initial_sample=None, c_tol=None):
    ...
    if pointsCount == 0 and mode not in (GTOPT_VALIDATE, GTOPT_VALIDATE_DOE) \
                        and not in_progress \
                        and _status.exception_by_status_id(status.id) is not None \
    ...
    if not in_progress:
      if self._evaluations_map is not None or not self.__compatibility:
      ...
      if self._evaluations_map is not None and designs is not None and "f" in designs[1][2]:
        try:
          ...
          objectivesValue = _designs._fill_sliced_sample_gaps(_shared._pad_columns(variablesValue, size_s, _shared._NONE),
                                                              objectivesValue, designs[0][:, :(size_x+size_s)], designs[0][:, designs[1][2]["f"]])
        except:
          pass
    ...
    if self.__compatibility:
      if pointsCount == 0:
        ...
        solution_optimal   =  _Solution(variablesValue[optimal_points], objectivesValue[optimal_points],
        ...
        solution_converged =  _Solution(variablesValue[converged_points], objectivesValue[converged_points],
        ...
        solution_infeasible = _Solution(variablesValue[infeasible_points], objectivesValue[infeasible_points],
  ...
  def raise_exception(self, status=None, message=None):
    ...
    :param status: GTOptAPI status, if None status will be obtainted from get_status call
    :type status: ``int``
