...
class Solver(object):
  ...
  def _run_solver(self, problem, mode, **kwargs):
    ...
    for k, v in kwargs.items():
      ...
      if k == recognized[0]:
        ...
        sample_f = self._read_nonempty_matrix(v, problem.size_f(), True, sample_f, name="Initial sample of objective function values ('sample_f' argument)")
      elif k == recognized[4]:
  ...
  def _expose_analytic_cache(self, problem):
    ...
    if problem._history_cache:
      try:
        ...
        designs = _designs._fill_gaps_and_keep_dups(designs, slice(input_size)) # fill holes in history
  ...
  def _combine_validation(self, subproblems_name, subproblems_spec, subproblems_result):
    status, diagnostics, misc_payload = _status.SUCCESS, [], {}
    for subproblem_name, subproblem_spec, (subproblem_status, subproblem_diagnostics) in zip(subproblems_name, subproblems_spec, subproblems_result):
      if _status._select_status(subproblem_status, status):
  ...
  def _combine_solutions(self, problem, mode, subproblems_result, invalid_initial_sample, intermediate_result=False):
    ...
    for result in subproblems_result:
      # combine designs
      designs = _designs._combine_designs(input_size, designs, (result._designs_data, result._designs_fields, result._designs_samples))
...
def _unique_points(datasets):
  from ..utils.designs import _unique_rows_indices

  datasets = [_ for _ in datasets if len(_)]
  if not datasets:
    return slice(0)

  datasets = _shared._make_dataset_keys(_numpy.hstack(datasets) if len(datasets) > 1 else datasets)
  unique_points = _unique_rows_indices(datasets)
  return unique_points if len(unique_points) < len(datasets) else slice(len(datasets))
