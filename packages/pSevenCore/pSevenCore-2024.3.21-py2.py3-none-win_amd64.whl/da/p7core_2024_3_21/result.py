...
class Result(object):
  ...
  def _copy_solutions_to_designs(self, problem, options):
    ...
    with make_writeable(self._designs_data):
      solution_keys = _read_dataset_input(self._fields, self._solutions_data)
      designs_keys = _read_dataset_input(design_fields, self._designs_data)
  ...
  def _mark_undefined_designs(self, problem, auto_objective_type):
    ...
    if undefined_objectives.any():
      for kind, undefined in zip(_read_objectives_type(problem, (auto_objective_type or "minimize").lower()), undefined_objectives.T):
        if kind != "evaluate" and undefined.any():
    ...
    if responses_evaluation_allowed:
      try:
        ...
        if evaluation_mask.any():
          ...
          self._solutions_data, self._fields, evaluation_mask = _evaluate_masked_data(problem, self._solutions_data, self._fields, evaluation_mask)
          if evaluation_mask.any():
            modified = True
  ...
  def _reveal_potentially_optimal_solutions(self, pareto_optimal_objectives, deferred_objectives, options):
    ...
    flags_vector = self._solutions_data[:, self._fields.get('flag', slice(0))][:, 0]
  ...
  def _refill_linear_responses(self, problem, evaluation_mask, logger):
    ...
    for resp_index, basic_response_name in enumerate(responses_names):
      ...
      for combination_mask, combination_name in self._enumerate_categorical_samples(problem, categorical_variables):
        ...
        if reconstruction_mask.any():
          ...
          self._solutions_data, self._fields, reconstruction_mask = _evaluate_masked_data(problem, self._solutions_data, self._fields, reconstruction_mask)
  ...
  def _enumerate_categorical_samples(self, problem, categorical_variables):
    ...
    if not categorical_variables.any():
      yield slice(len(self._solutions_data)), ""
      return
    ...
    for unique_combination_index in _unique_rows_indices(categorical_inputs):
      ...
      yield combination_mask, combination_name
  ...
  def _unsafe_read_solutions(self, destination_map, destination_data):
  ...
  def _solutions(self, fields=None, filter_type=None):
    ...
    if fields is None:
      return designs_data.copy() if points_filter is None else designs_data # avoid double copy, avoid return pointer to the origin

    fields_collection = [designs_data[:, (self._fields[field] if field in self._fields else self._extended_fields[field])] for field in fields]

    if not fields_collection:
      return designs_data[:, 0:0] # right shape, empty data
    elif len(fields_collection) == 1:
      return fields_collection[0].copy() if points_filter is None else fields_collection[0] # avoid double copy, avoid return pointer to the origin

    return _numpy.hstack(fields_collection)
  ...
  def _designs(self, fields=None, sample_type=None):
    ...
    if fields is not None:
      ...
      # by defaut use all top-level fields
    ...
    if not fields_collection:
      ...
      return fields_collection[0].copy() if points_mask is None else fields_collection[0]
  ...
  def pprint(self, fields=None, filter_type=None, file=_sys.stdout, precision=8, designs=False, limit=None):
    ...
    if designs:
      ...
      points_filter = self._solutions_filter(filter_type)
      fields_collection = [self._solutions_data[:, (self._fields[field] if field in self._fields else self._extended_fields[field])] for field in fields]
    ...
    named_flags = {
      ...
      GT_SOLUTION_TYPE_POTENTIALLY_FEASIBLE: "potentially feasible",
    }

    flag_format = "%s"
    def flagtostr(xi):
      return flag_format % xi
    ...
    def stringify_flags(flag_code, initial, potentially_optimal):
      ...
      if potentially_optimal:
        result = result + ', potentially optimal'
      return result
    ...
    for idx, field in enumerate(fields):
      if field.lower() == "flag":
        ...
        if fields_collection[idx]:
          flag_format = "%-0" + str(max([len(flag) for flag in fields_collection[idx]])) + "s"
        converter[field] = flagtostr
      else:
        converter[field] = nptostr

    for values in zip(*fields_collection):
      file.write(" ".join(["%s: %s" % (c, converter[c](v)) for c, v in zip(fields, values)]) + '\n')
...
def _evaluate_masked_data(problem, solutions_table, fields_map, evaluation_mask):
  if not evaluation_mask.any():
    return solutions_table, fields_map, evaluation_mask
...
def _read_dataset_input(fields, sample):
...
def _fill_dataset_gaps(fields, source_sample, destination_sample):
  if not _shared._find_holes(destination_sample).any():
    return destination_sample

  assert source_sample.shape[1] == destination_sample.shape[1]

  from .utils.designs import _fill_sliced_sample_gaps

  fields = dict(fields)
  return _fill_sliced_sample_gaps(_read_dataset_input(fields, destination_sample), destination_sample,
                                  _read_dataset_input(fields, source_sample), source_sample)

def _postprocess_solution_samples(default_solution, default_fields, initial_sample, initial_fields):
  ...
  if has_initial_sample and has_default_solution:
    ...
    # fill gaps in the initial sample if any
    initial_sample = _fill_dataset_gaps(default_fields, default_solution, initial_sample)
