...
class SmartSelectionOptimizationProblemGeneric(object):
  ...
  def _build_model_and_estimate_error(self, x, just_submit, alt_initial_model):
    ...
    if technique == "hdagp":
      # very special case: we can use trend from initial model but only if there is not alternative, otherwise we may fail output transformations
      initial_model = None if alt_initial_model[0] is not None else _extract_trend(self.initial_model, None)
    else:
    ...
    try:
      ...
      if not has_test_sample:
        ...
        iv = _IterativeIV(self.x, self.y, options=_merge_dicts(approx_options, iv_options, True),
...
class SmartSelectionOptimizer(object):
  ...
  def _truncate_options_bounds(self, gtapprox_options, x, y, has_test_sample=False,
    ...
    if 'tgp' in self.techniques:
      ...
      if 'gp' in self.techniques and not manual_technique:
        remove_technique(self.techniques.index('gp'), 'TGP is prefered technique')
      if 'sgp' in self.techniques and not manual_technique:
        remove_technique(self.techniques.index('sgp'), 'TGP is prefered technique')
  ...
  def solve(self):
    ...
    try:
      ...
      for i, optimization_problem in enumerate(self.optimization_problems):
        ...
        if isinstance(optimization_problem, SmartSelectionSBO):
        ...
        elif 'gp' in self.techniques[i] and not batch_mode and 'GTApprox/OutputTransformation' not in optimization_problem.opt_options:
          # In batch mode any *GP is just enumeration of all possible options combinations, so non-default handling is needed only in sequential mode
          self._solve_gp(optimization_problem, prefer_linear_trend, self.techniques[i])
          ...
          self._solve_enum(optimization_problem, just_submit=batch_mode)
  ...
  def _solve_gp(self, problem, prefer_linear_trend, technique):
    i_power = problem.get_opt_option_index('GTApprox/GPPower')
    i_trend_type = problem.get_opt_option_index('GTApprox/GPTrendType')
    i_learning_mode = problem.get_opt_option_index('GTApprox/GPLearningMode')
    i_covariance_type = problem.get_opt_option_index('GTApprox/GPType')

    powers_list = problem.opt_options.get_bounds('GTApprox/GPPower', [])
    trend_types_list = _lower_case_list(problem.opt_options.get_bounds('GTApprox/GPTrendType'))
    trend_sort_order = ['none', 'linear', 'quadratic'] if prefer_linear_trend else ['none', 'quadratic', 'linear']
    trend_types_list.sort(key=trend_sort_order.index)
    learning_modes_list = _lower_case_list(problem.opt_options.get_bounds('GTApprox/GPLearningMode'))
    covariance_types_list = _lower_case_list(problem.opt_options.get_bounds('GTApprox/GPType'))

    # robust mode is incompatible with the exact fit requirement
    if "robust" in learning_modes_list and _shared.parse_bool(problem.approx_builder.options.get("GTApprox/ExactFitRequired")):
      learning_modes_list.remove("robust")
      if not learning_modes_list:
        i_learning_mode = -1

    # For 1D sample only Wlp is allowed
    if problem.x.shape[1] == 1 or self.accelerator_options.get('LandscapeAnalysis', False):
      covariance_types_list = [cov for cov in covariance_types_list if cov.lower() == 'wlp']

    if len(covariance_types_list) > 1 and "mahalanobis" in [cov.lower() for cov in covariance_types_list] and \
       ((problem.x.shape[1] * (problem.x.shape[1] + 1) + 3) > problem.x.shape[0] or (problem.x.shape[1] * (problem.x.shape[1] + 1) // 2) > 50):
      # Mahalanobis covariance have dim * (dim + 1) // 2 internal parameters. Let's require n_points >= (2 * n_params + 3)
      # Second case: it's just too slow
      covariance_types_list = [cov for cov in covariance_types_list if cov.lower() != 'mahalanobis']

    if not _shared.parse_bool(problem._read_fixed_option('/GTApprox/SmartSelection/BruteforceGP', False)):
      # Do not vary power if there is no need of smooth model
      if technique != 'tgp' and (not self.accelerator_options.get('LandscapeAnalysis', False) or problem.x.shape[0] > 125) and len(powers_list):
        powers_list = [problem.opt_options.get_init_value('GTApprox/GPPower')]

    # build model with initial value first
    problem.define_objectives(problem.get_optimal_x())
    if problem.dry_run == 'quick':
      return

    # choose learning mode
    test_value = problem.get_optimal_x()
    for learning_mode in learning_modes_list:
      test_value[i_learning_mode] = learning_mode
      problem.define_objectives(test_value)

    # choose trend type with the optimal learning mode fixed
    test_value = problem.get_optimal_x()
    for trend_type in trend_types_list:
      test_value[i_trend_type] = trend_type
      problem.define_objectives(test_value)

    # Vary GPPower first if TGP technique is chosen or GPType is fixed
    if i_covariance_type < 0 and problem.approx_builder.options.get('GTApprox/GPType').lower() in ['additive', 'wlp']:
      test_value = problem.get_optimal_x()
      for power in powers_list:
        test_value[i_power] = power
        problem.define_objectives(test_value)

    # choose covariance function with the optimal trend type and learning mode fixed
    test_value = problem.get_optimal_x()
    for covariance_type in covariance_types_list:
      test_value[i_covariance_type] = covariance_type
      problem.define_objectives(test_value)
      if covariance_type in ['additive', 'wlp']:
        for power in powers_list:
          test_value[i_power] = power
          problem.define_objectives(test_value)
  ...
  def _solve_hdagp(self, problem, opt_data, just_submit):
    ...
    def define_objectives(x):
      ...
      trend_model = opt_data.hda_trend_model.get(output_transform)
      if estimated_p is not None or trend_model is not None:
        ...
        trend_model = getattr(problem, 'hda_trend_model', {}).get(output_transform, None)
  ...
  def _solve_enum(self, problem, just_submit):
    self._filter_options_grid(problem)

    # build model with initial value first
    problem.define_objectives(problem.init_x, just_submit)
  ...
  def _filter_options_grid(self, problem):
    ...
    has_test_sample = not problem.x_test is None and not problem.y_test is None
