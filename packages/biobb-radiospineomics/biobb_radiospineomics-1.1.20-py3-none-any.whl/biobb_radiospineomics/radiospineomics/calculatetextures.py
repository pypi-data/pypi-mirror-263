...
from biobb_radiospineomics.radiospineomics.common import calculate_textures
...
class CalculateTextures(BiobbObject):
    """
    ...
    def __init__(self, input_mask_path1, input_image_path1, texture_params_file, output_file_path, properties=None, **kwargs) -> None:
        ...
        # Input/Output files
        self.io_dict = {
                'in': {'input_mask_path1': input_mask_path1, 'input_image_path1': input_image_path1, 'texture_params_file': texture_params_file},
    ...
    def launch(self) -> int:
        ...
        #shu  til.copy(self.io_dict['in']['input_file_path1'], self.tmp_folder)
    ...
    def run(self):
        ...
        for key, value in self.io_dict['in'].items():
            if 'mask' in key:
                ...
                texture_params_file = value
        
        resamplings = self.properties.get('resamplings')

        if not fsu_mask:
            print("Error: No mask file found in the input dictionary.")
            exit()

        if not fsu_image:
            print("Error: No image file found in the input dictionary.")
        if not texture_params_file:
            print("Error: No image texture file yaml found in the input dictionary.")

        texture_results = None
        try: 
        
            print(f'Error extractions for {fsu_image}')
            print(f'Error extractions for {texture_params_file}')
            texture_results = calculate_textures(fsu_image, fsu_mask, texture_params_file, self.properties.get('resamplings'), self.properties.get('binwidths'))
        
        except Exception as e: 
            
            print(f'Error extracting features for {fsu_mask}: {e}')

        if texture_results is not None:
            #texture_results = pd.DataFrame(texture_results, index=[0])
            #texture_results['id'] = fsu[1].split('/')[-1]
            texture_results.to_csv(self.io_dict['out']['output_file_path'], index=False)
        else:
            print("No se pudo generar texturey_results debido a un error.")



def calculatetextures(input_mask_path1: str, input_image_path1: str,  texture_params_file: str, output_file_path: str, properties: dict = None, **kwargs) -> int:
    ...
    return CalculateTextures(input_mask_path1=input_mask_path1,
                            input_image_path1=input_image_path1,
                            texture_params_file=texture_params_file,
...
def main():
   ...
   # parser.add_argument('--config', required=False, help='Configuration file')
    ...
    required_args.add_argument('--texture_params_file', required=True, help='')
    ...
    calculatetextures(input_mask_path1=args.input_mask_path1,
                     input_image_path1=args.input_image_path1,
                     texture_params_file=args.texture_params_file,
