...
from expressmoney.apps.auth.black_list.models import BlackList
...
class AuthRouter:
    ...
    exclude_models = ('permission', 'group', 'group_permissions')
...
class AppsRouter(AuthRouter):
    def db_for_read(self, model, **_hints):
        db = super().db_for_read(model, **_hints)
        if model in (BlackList,):
            db = 'auth_db'
        return db

    def db_for_write(self, model, **_hints):
        db = super().db_for_write(model, **_hints)
        if model in (BlackList,):
            db = 'auth_db'
        return db
