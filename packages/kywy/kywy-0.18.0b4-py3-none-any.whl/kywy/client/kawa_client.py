...
from kywy.client.computations import KawaComputations
...
class KawaClient:
    ...
    def __init__(self, kawa_api_url=None, verify_certificates=False, tmp_files_directory=None):
        ...
        # APIs
        self.data = KawaData(kawa_client=self)
        self.commands = KawaCommands(kawa_client=self)
        self.entities = KawaEntities(kawa_client=self)
        self.configuration = KawaConfiguration(kawa_client=self)
        self.computations = KawaComputations(kawa_client=self)
        self.data_providers = KawaDataProviders(kawa_client=self)
    ...
    @staticmethod
    def cols(regexp='.*'):
        return KawaColumn(column_name='', column_regexp=regexp)
