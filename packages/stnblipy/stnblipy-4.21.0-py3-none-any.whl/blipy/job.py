...
from blipy.jsonstring import JsonString, TpJson
...
class Job():
    ...
    def importa_json_url(self,
        ...
        cols_entrada:       lista com o índice (zero-based) dos campos do json
                            que serão utilizados. Se None, utiliza todos
                            os campos, na ordem lida. Esse parâmetro é
                            importante tanto para filtrar campos do JSON quanto
                            para o caso de se querer que um único campo do JSON
                            seja fonte de entrada para mais de uma coluna de
                            saída, já que a relação entre cols_saida e
                            cols_entrada tem que ser de 1 para 1. Por exemplo,
                            se o primeiro campo do JSON for fonte de entrada
                            para a primeira e terceira colunas da tabela de
                            saída, esse parâmetro deve ser [0, 1, 0]
        tp_cols_entrada:    tipos dos campos do json, no formato de um array de
                            objetos TpColBD, na ordem de cols_entrada. Se None,
                            o pandas tentará obtê-los automaticamente do JSON
        estrategia:         estratégia de gravação que será utilizada (enum
                            TpEstrategia)
        cols_chave_update:  lista com os nomes das colunas que serão chave de
    ...
    def __importa_json( self,
        ...
        dtype = self.__monta_dtype(cols_saida, False, tp_cols_entrada)

        dados_json = JsonString(fonte_json)
        dados_json.carrega_dados(str_json, tipo_json, campo_de_dados, dtype)

        if cols_entrada is not None:
            dados_json.formata_colunas(cols_entrada)
    ...
    def __monta_dtype(self, cols_saida, usecols, tp_cols_entrada):
        # seta o parâmetro dtype para a importação de dados num dataframe
