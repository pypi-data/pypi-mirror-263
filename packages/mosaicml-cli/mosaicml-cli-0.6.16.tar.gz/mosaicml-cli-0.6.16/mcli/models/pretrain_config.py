...
class PretrainConfig(BaseSubmissionConfig):
    ...
    Required args:
        ...
        train_data_path: The full remote directory of your training data (eg 's3://my-bucket/my-data'). Alternately,
            if you want a mix of datasets, you can use streaming. The format for streaming is a list of
            ``dataset_name`` containing either ``remote`` or ``local`` paths and optionally a ``proportion``
            signifying the proportion of the dataset to use (default 1). For example, ``[{"dataset_name":
            "s3://my-bucket/my-data", "proportion": 0.5}, {"dataset_name": "s3://my-bucket/my-other-data"}]``.
    ...
    model: str
    train_data_path: Union[str, Dict[str, Any]]
    save_folder: str
