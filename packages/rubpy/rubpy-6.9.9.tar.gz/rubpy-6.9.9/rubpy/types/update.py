...
class Update:
    ...
    @property
    def message(self):
        return 'message' in self.original_update and self.find_keys('message')

    @property
    def is_me(self):
        message = self.message
        return message is not None and self.message.is_mine
    ...
    def is_edited(self) -> bool:
        message = self.message
        return message is not None and self.message.is_edited
    ...
    def forward_type_from(self):
        return self.is_forward and self.message.forwarded_from.type_from
    ...
    @property
    def event_data(self):
        return self.is_event and self.message.event_data
    ...
    def file_inline(self):
        return self.is_file_inline and self.message.file_inline

    @property
    def text(self) -> str:
        message = self.message
        return message is not None and message.text

    @property
    def message_id(self):
        message = self.message
        return message is not None and self.find_keys(keys=['message_id', 'pinned_message_id'])

    @property
    def reply_message_id(self):
        message = self.message
        return message is not None and self.find_keys(keys='reply_to_message_id')
    ...
    def author_guid(self):
        message = self.message
        return message is not None and message.author_object_guid
