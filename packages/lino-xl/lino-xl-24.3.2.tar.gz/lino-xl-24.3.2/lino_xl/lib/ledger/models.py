...
class PaymentTerm(BabelNamed, Referrable):
    ...
    worker = dd.ForeignKey(dd.plugins.ledger.worker_model,
                           null=True,
                           blank=True,
...
if has_payment_methods:
    ...
    class PaymentMethod(BabelDesignated):
        ...
        payment_account = dd.ForeignKey('ledger.Account',
...
class PartnerHasOpenMovements(ObservedEvent):
    ...
    def add_filter(self, qs, pv):
        ...
        if pv.end_date:
            qs = qs.filter(
                ledger_movement_set_by_partner__value_date__lte=pv.end_date)
        if pv.start_date:
            qs = qs.filter(
                ledger_movement_set_by_partner__value_date__gte=pv.start_date)
        return qs.distinct()
