...
class ByJournal(dd.Table):
    ...
    def create_journal(cls, trade_type=None, account=None, **kw):
        ...
        return rt.models.ledger.Journal(trade_type=trade_type,
                                        voucher_type=vt,
                                        **kw)
...
class Vouchers(dd.Table):
    ...
    parameters = dict(year=dd.ForeignKey('ledger.FiscalYear', blank=True),
...
class PartnerVouchers(Vouchers):
    ...
    parameters = dict(project=dd.ForeignKey(dd.plugins.ledger.project_model,
                                            blank=True,
                                            null=True),
    ...
    def get_request_queryset(cls, ar, **kwargs):
        ...
        if pv.cleared == dd.YesNo.yes:
            ...
            qs = qs.filter(
                ledger_movement_set_by_voucher__cleared=False,
                ledger_movement_set_by_voucher__account__clearable=True)
        return qs


def mvtsum(**fkw):
    d = rt.models.ledger.Movement.objects.filter(**fkw).aggregate(
        models.Sum('amount'))
...
class MovementsByVoucher(Movements):
    ...
    # auto_fit_column_widths = True
    display_mode = ((None, constants.DISPLAY_MODE_HTML), )
    order_by = dd.plugins.ledger.remove_dummy('seqno', 'value_date',
...
class MovementsByPartner(Movements):
    ...
    order_by = dd.plugins.ledger.remove_dummy('-value_date', 'voucher__id',
...
class MovementsByAccount(Movements):
    ...
    order_by = dd.plugins.ledger.remove_dummy('-value_date', 'partner__name',
                                              'project', 'id')
...
class MovementsByMatch(Movements):
    ...
    variable_row_height = True
    order_by = dd.plugins.ledger.remove_dummy('-value_date', 'account__ref',
                                              'project', 'id')
