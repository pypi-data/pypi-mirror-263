...
def multi_read(
    ...
    results = []
    for wha, kwargs in zip(what, kwargs_list):
        if not hasattr(reader, wha):
            raise AttributeError(
                "Could not understand the what property of read. Possible values include (but not only) data, description, metadata..."
            )
        results.append(eval(rf"reader.{wha}")(**kwargs))
    return results


def read(input_path, what="data", *args, **kwargs):
    reader = Reader(input_path)
    if not hasattr(reader, what):
        raise AttributeError(
            "Could not understand the what property of read. Possible values include (but not only) data, description, metadata..."
        )
    return eval(rf"reader.{what}")(*args, **kwargs)
...
class Reader(ScanImageTiffReader):
    ...
    def data(self, channel=None, plane=None, beg=None, end=None, strictly_positive=True):
        """Channel and plane selection are zero based (channel = 0 : first channel, channel = 1 : second channel, etc...)

        If the data has multiple planes and/or multiple channels, this reader class expects the frames to be interleaved, e.g.
    ...
    def metadata(self, raw=False):
        ...
        # _meta_to_dict converts from string to python dictionnary containing python objects, asserting their types with json
    ...
    def timeline(
        ...
        if (
            relative_to == "trigger"
        ):  # use the value at nanargmax trigger to set the offset more precisely in the timeline, important as there is 33 msec per frame, we want to keep subframe resolution as much as possible
    ...
    def trigger_line(self, channel=None, plane=None, trigger_ids="all", prefix="auxTrigger", beg=None, end=None):
        """
        Parameters:
            ...
            trigger_ids (str or int or list, optional): The ids of the triggers to retrieve. If 'all', all triggers are retrieved.
        ...
        Returns:
            dict or numpy array:
                A dictionary containing the triggers as keys and their corresponding values as values if `return_one` is None.
        ...
        return trigger_lines if return_one is None else trigger_lines[return_one]
    ...
    def channels_colors(self, available=False):
        if available:
    ...
    def channel_nb(self, available=False):
        if available:
    ...
    def plane_nb(self):
        # TODO will probably have to change this function when we work with several planes for real. I.E. : not tested in real multiplane data from scanimage tiffs
        try:
            return self._plane_nb
        except AttributeError:
            pass
        try:
            self._plane_nb = self.metadata()["RoiGroups"]["imagingRoiGroup"]["rois"]["discretePlaneMode"] + 1
        except KeyError:
    ...
    def _start_stop_step(self, channel=None, plane=None):
        ...
        if isinstance(channel, str):
            ...
            if self.channels_save_status()[channel_index] == False:
                raise ValueError(
                    f"The channel color {channel} asked does exist in the setup metadata but such channel wasn't saved in this file (check with channels_colors())"
                )
    ...
    def SI_infos(self, info, section="hChannels"):
        try:
            return self.metadata()["SI"][section][info]
        except KeyError:
    ...
    def _keys_value_from_matobj(matobj):
        ...
        try:
            matkey, matvalue = matobj.split(
                "="
            )  # splits a matlab like string representation of an object into the variable name(key) and content(value), separated by = sign
        except (
            ValueError
        ):  # if the string doesn't contain an equal sign it cannot unpack return into two variables : matkey, matvalue. in that case, we return None as we cannot later add key value pair to the dict
            return None, None
    ...
    def _value_from_matvalue(matvalue):
        ...
        val = Reader._strarray_mat_to_py(
            val
        )  # convert format from matlab string representation of array into python string representation of array. Does nothing and return original if it does not contain such representation.
    ...
    def _strarray_mat_to_py(value, use_c=True):
        ...
        # value = re.sub(r"(?<!\[|;|,)(?<=\b|\B)( +)(?=\b|\B)(?=[^\]\[]*\])(?!(?:,|;)|(?:\w+')|\])",",",value) #find spaces inside brackets but not when near a ; and replace them with ,
        # value = re.sub(r"(?<=\])( +)(?=\[)",",",value) # also add coma between enclosed brackets with a space in between e.g. ; ] [  Simpler to do it in two re patterns than making a super huge and complex one to do this in one pass
        #     #value = re.sub(r"([a-zA-Z0-9',\.-]+)(?=[^\]\[]*\])","[\g<1>]",value)#find content between brackets and ; or between two ; and replace them inside brackets

        #     #in two steps :
        # value = re.sub(r"([^\]\[;]+)(?=;[^\]\[]*\])","[\g<1>]",value)# all nested areas except terminal ones before a ] and separated by ;
        # value = re.sub(r"(?<=;)([^\]\[;]+)(?=[^\]\[]*\])","[\g<1>]",value)#only terminal areas before a ] and behind a ;

        # value = re.sub(r";(?=.*\])",",",value)#find ; before an end bracket and replace them with ,
        #     #for all the above lines, if the string was not a matlab like array, they didn't replaced anything. Then, we try to see if we could make a python like object with it :
        return value
    ...
    def _cast_as_python_type(value, use_c=True):
        ...
        try:
            # val = json.loads('{"_dummy":' + temp + "}")["_dummy"]#just using a fake json representation if it helps. Seems it isn't necessary for scalar objects
            value = json.loads(temp)  # we try to cast it into a python type
        except (
            json.JSONDecodeError
        ):  # we could't make a python object out of it. Try to make apython string out of it (converting single backslash to doulbe ones for json)
            try:
                temp = '"' + temp + '"'
    ...
    def _arrayify_list(alist):
        with warnings.catch_warnings():  # temporarily prevents creating arrays from ragged sequences. If so, raises VisibleDeprecationWarning
