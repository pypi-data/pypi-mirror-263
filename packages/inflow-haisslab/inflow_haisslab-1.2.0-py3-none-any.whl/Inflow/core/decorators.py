...
def return_as_attr_dict(func):
    ...
    Examples:
        The following example shows how to use this decorator to wrap the proj_aliases function:

    ``` python
        @return_as_attr_dict
        def proj_aliases(arguments,...):
            ...

        result = proj_aliases("Adaptation")
    ...
    def wrapper(*args, **kwargs):
        def make_function_specific_kwargs(args_spec):
            ...
            for arg_name in kwargs_names:
                if arg_name in kwargs.keys() and not arg_name in specific_kwargs.keys():
                    specific_kwargs[arg_name] = kwargs[arg_name]
        ...
        if "session_details" in kwargs.keys():
            ...
            with LogSession(session_details):
                ...
                if new_kwargs:  # new_kwargs is not empty
                    local_log.info(
                        f"Found some arguments for the function {func.__name__} in pipelines_arguments.json. Using them."
                    )
                overrides_names = []
