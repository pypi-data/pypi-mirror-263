...
def get_true_parents(parents: dict):
    ...
    for key in keys:
        current_key = key
        parent_key = parents[current_key]
        while parent_key != current_key:
...
def merge_all(threshold, graph: Graph, find_algorithm='min_find'):
    """
    function to merge graph nodes to their neighbors until the threshold weight is reached (if possible). 
    """
    parents = {}
    ...
    for index in indexes:
        # get node id and at the begining each member represents itself 
        key = keys[index]
        parents[key] = key
