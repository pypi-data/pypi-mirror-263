            ...
            if i + 1 < tamanho_texto:
                if not frase_linha[i+1] == '.':
                    if i-7 >= 0:
                        ...
                        parte_inicial = 0
                    if not (verificaAbreviacao(frase_linha[parte_inicial:i+1]) and frase_linha[i+1].isdigit()):
...
def tokenizadorDeFrase(frase : str) -> list:
  lista_de_tokens = []
...
def tokenizadorDeListaDeFrasesRetornoLista(lista_de_frases : list) -> list:
    
    lista_de_frases_tokenizadas = []
    for frase in lista_de_frases:
        lista_de_frases_tokenizadas.append(tokenizadorDeFrase(frase))
    return lista_de_frases_tokenizadas

def tokenizadorDeListaDeFrasesRetornoGerador(lista_de_frases : list) -> Generator:
    for frase in lista_de_frases:
        yield tokenizadorDeFrase(frase)
...
def tokenizarTexto(texto : str,
    ...
    if dividir_frases:
        ...
        if retorno_como_gerador:
            ...
            frases_tokenizadas = tokenizadorDeListaDeFrasesRetornoGerador(lista_de_frases=frases_separadas)
        else:
            frases_tokenizadas = tokenizadorDeListaDeFrasesRetornoLista(lista_de_frases=frases_separadas)
