...
def compute_scores(df_given: pd.DataFrame, no_recall: pd.DataFrame, group = ['group_id', 'project_identifier', 'task_name', 'recalled_brand','total_participants']):
    ...
    pivot_df = df.pivot_table(index=group, values='mrt_score_freerecall', aggfunc='mean').reset_index()
    ...
    result = result[['Age_Group', 'project_identifier',  'task_name', 'group_id', 'recalled_brand', 'total_participants','brand_count',
                      'position_mean',  'recall_%', 'mrt_score_freerecall']]
    return result
