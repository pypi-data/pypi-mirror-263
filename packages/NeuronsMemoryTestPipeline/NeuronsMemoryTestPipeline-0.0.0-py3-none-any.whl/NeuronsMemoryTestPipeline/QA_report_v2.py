...
def percent_calculation(df):
    ...
    if "Above" in df.columns and "Below" in df.columns:
        ...
        print("\nThere are no responses below the minimum.")
    elif "Below" in df.columns:
        df["Sum"] = df["Below"].fillna(0) + df["Good"]
        print ("\nThere are no responses Above the maximum.")
    else:
        df["Sum"] = df["Good"]
        print("\nThere are no responses outside the norm.")
...
for file in args:
    if (os.path.splitext(file)[1] != '.txt' ) & (os.path.splitext(file)[1] != '.csv') & (os.path.splitext(file)[1] != '.xlsx'):
...
for num in range(1, len(args)):
    ...
    if provider_file.endswith('.xlsx'):  # Check if the file is an Excel file
        df_meta = pd.read_excel(provider_file)
        if 'participant_id' in df_meta.columns:
            print('Reading Neurons file: ', provider_file)
            df_meta = df_meta[['participant_id', 'Age', 'Gender', 'provider']].copy()
            df_meta['TargetGroupName'] = 'unknown'
            df_meta['participant_id'] = df_meta['participant_id'].astype(str)
            df_meta['demographic_file'] = provider_file
            final_meta = pd.concat([final_meta, df_meta], axis=0)
        else:
            print(f'WARNING: Unknown format in file {provider_file}. No "participant_id" column found.')
        ...
        if 'RespondentUrlCode' in df_meta.columns:
            ...
            if 'Transaction ID' in df_meta.columns:
                ...
                print(f'WARNING: Unknown Provider in file {provider_file}, names of the columns in file do not match!')
                pass
...
for file in final_meta.demographic_file.unique():
    ...
    print('')
print('Total number of participants in provider files: ', final_meta.participant_id.nunique())
...
print('Total number of ghost participants in provider files: ', ghost_id.participant_id.nunique())

df_merge = df_jatos.merge(final_meta, on="participant_id", how="inner")
print('Total number of matching participants with jatos file: ', final_meta.participant_id.nunique())

df_merge= df_merge[df_merge['reason_to_end_the_behavioral_task_code']==13].copy()
print('Total number of participants with behavioral task code 13: ', df_merge.participant_id.nunique())
...
print('Number of participants who pass the QA with 50% or more good answers:', unique_participant_ids)
