...
from os import system as run, mkdir, getcwd as pwd, chdir, unlink, chmod, rename
from os.path import isdir, isfile, abspath, dirname, basename, join, exists as there, relpath
from tempfile import mkdtemp as mkt
...
def main():
    arguments = argparse.ArgumentParser(description='warlock sfx')
...
"""
    ...
    def _make(self, content: str, outfile: str, _setup: str, _setupargs=(), sha256=False, compress='gz', quiet=False, label=None, extension='.warlock'):
        """Create SFX
        ...
        print(f' -> Copying dist/{_exe_name} <-')
        ...
        print(f' -> Deleting mid file {self._midfile} <-')
    ...
    def _pkg_to_exe(self, _tar: str, outfile: str, _setup: str, _setupargs: tuple, in_content: bool, sha256_sum: str, label: str):
        ...
        if there(_exe_mid):
            print(f' -> removing existing midfile: {relpath(_exe_mid)} <-', file=stderr)
            unlink(_exe_mid)
        
        print(f' -> writing midfile {relpath(_exe_mid)} <-')
...
def _copy_package_files(_pkg: str, _install_src: str, _setup: str, in_content: bool):
    ...
    print(f' -> Packaging from: {_install_src} <-')
    # copy the files
    ...
    if isfile(src_auth_keys):
        print(f' -> packaging only authorized_keys file from {src_dot_ssh} <-')
    ...
    if _setup:
        if in_content:
            print(' -> _setup already included. <-')
        else:
            print(f' -> packaging _setup: {_setup} <-')
            # copy the script
            _setupname = basename(_setup)
...
def _archive_package(_pkg: str, compress: str, sha256: bool):
    ...
    print(f' -> creating tar file: {basename(_tar)}')
    ...
    if sha256:
        ...
        print(f' -> SHA256 {basename(_tar)} = {sha256_sum}')
    else:
        print(' -> skipped SHA256 <-')
