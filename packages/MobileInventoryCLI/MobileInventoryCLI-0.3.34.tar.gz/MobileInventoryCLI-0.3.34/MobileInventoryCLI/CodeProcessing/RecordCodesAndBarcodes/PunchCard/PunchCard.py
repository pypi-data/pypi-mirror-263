...
{Fore.light_green}clrt|clear_today{Style.reset} -{Fore.cyan} clear today's punches{Style.reset}
		...
		while True:
			...
			if cmd.lower() in ['ps','start']:
				with Session(engine) as session:
					s=session.query(Shift).filter(Shift.Date==date.today()).first()
					if s:
						s.duration_completed()
					else:
						pcard=Shift(start=datetime.now())
						session.add(pcard)
						session.commit()
				...
				with Session(engine) as session:
					...
					session.refresh(s)
					s.duration_completed()
			elif cmd.lower() in ['clrt','clear_today',]:
				with Session(engine) as session:
					s=session.query(Shift).filter(Shift.Date==date.today()).delete()
					print(f"deleted: {s}")
					session.commit()
