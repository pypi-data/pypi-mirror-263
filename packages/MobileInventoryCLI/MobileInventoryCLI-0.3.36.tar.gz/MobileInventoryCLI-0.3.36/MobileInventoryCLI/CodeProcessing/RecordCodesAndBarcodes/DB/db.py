...
class Shift(BASE):
    ...
    def __str__(self):
        ...
        for col in self.__table__.columns:
            msg+=f"{col.name}={getattr(self,col.name)},\n"
        msg+=")"
        return msg
    ...
    def duration_completed(self):
        ...
        if self.end:
            try:
                print(f"Shift Has Ended: {(self.end-self.start)-(self.break_end-self.break_start)}")
            except Exception as e:
                print(f"Shift Has Ended: (start:{self.end}-end:{self.start})-(break_end:{self.break_end}-break_start:{self.break_start})")
                print(f"Shift Has Ended: {(self.end-self.start)}")
        else:
            if self.break_start != None:
                if self.break_end != None:
                    #break is done
                    try:
                        print(f"Break Is Completed Duration:{(now-self.start)-(self.break_end-self.break_start)}")
                    except Exception as e:
                        print(f"Break Is Completed Duration:({now}-{self.start})-({self.break_end}-{self.break_start})")

                else:
                    #break is started but not ended
                    print(f"Break Is Started, But Not Ended: {now-self.break_start}")
            elif self.break_start == None:
                #break has not started
                print(f"Shift has Started, But Not Break {now-self.start}")  
