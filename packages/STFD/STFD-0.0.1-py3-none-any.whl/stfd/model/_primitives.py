...
from scatlasvae.utils._definitions import FCDEF
...
class MMDLayerBase:
    ...
    def hierarchical_mmd_loss_1(
        ...
        cat1: np.array, 
        cat2: np.array,
        hierachical_weight: Tuple[float] = (0.5, 1),
        dim=-1,
        min_n_samples: int = 2,
        flavor: Literal['trvae','default'] = 'trvae'
    ) -> torch.Tensor:
        ...
        zs = []
        loss = torch.tensor(0.0, device=z.device)
        cat2to1 = dict(zip(cat2.flatten(),cat1.flatten()))
        for i in np.unique(cat2):
            idx = list(map(lambda z:z[0], filter(lambda x:x[1] == i, enumerate(cat2))))
            zs.append((z[idx], cat2to1[i]))

        for i in range(len(np.unique(cat2))):
            for j in range(i+1,len(np.unique(cat2))):
                if zs[i][0].shape[0] > min_n_samples and zs[j][0].shape[0] > min_n_samples:
                    if flavor == 'trvae':
                        loss += LossFunction.mmd_loss_trvae(
                            zs[i][0], zs[j][0]
                        ) * hierachical_weight[zs[i][1] == zs[j][1]] / len(zs)
                    else:
                        loss += LossFunction.mmd_loss(
                            zs[i][0], zs[j][0], dim=-1
                        ) * hierachical_weight[zs[i][1] == zs[j][1]] / len(zs)
