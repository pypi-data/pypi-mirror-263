...
@pytest.mark.parametrize("tag_value,expected_output",
                         [("normal", "folder/normal"),
                          ("numbers1234", "folder/numbers1234"),
                          ("bad<char", "folder/bad_char"),
                          ('all"<>:"/|?*verybad', "folder/all_________verybad")])
def test_avoid_windows_getting_upset_over_paths(tag_value, expected_output):
    gen = PathGenerator(pattern=DicomPathPattern("folder/(PatientID)"))
    ds = Dataset()
    ds.PatientID = tag_value
    test = gen.generate_by_ds(ds).flatten()
