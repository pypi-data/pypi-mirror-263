...
from dicomsort.parser import (DicomPathPatternParser, DicomPathParseException,
                              DicomTagName, DicomTagNotFoundException,
                              ResolvedPathElement, FolderSeparator, )


class DicomPathPattern:
    """Describes how to generate paths for a dicom file. Can contain dicom tag or tag
    names. For example '/some_folder/(PatientID)/(0008,1030)/' is valid
...
class PathGenerator:
    ...
    # replace the following chars with underscore in paths
    BAD_WIN_PATH_CHARS = '"<>:"/\|?*'
    ...
    @classmethod
    def clean_path_string(cls, string_in):
        """Replaces characters that could be illegal in paths with underscores"""
        for char in cls.BAD_WIN_PATH_CHARS:
            string_in = string_in.replace(char, "_")
        return string_in

    def generate_by_ds(self, ds):
        ...
        for path_element in self.pattern.elements:
            try:
                ...
                resolved.append(ResolvedPathElement(path_element=None,
                    resolved_value=self.UNKNOWN_ELEMENT_NAME))

        # group path elements by full folder or file name
        units = []

        for group in split_list(resolved,
                lambda x: type(x.path_element) == FolderSeparator):
            units.append(group)


        # Clean for pathname usage. Clean only DicomTagName elements to leave elements lik
        # C:/
        for unit in units:
            for element in [x for x in unit if isinstance(x.path_element, DicomTagName)]:
                element.resolved_value = self.clean_path_string(element.resolved_value)

        return TentativePath([PathUnit(x) for x in units])
    ...
    def generate(self, path_in):
        ...
        """
        try:
            return self.generate_by_ds(pydicom.dcmread(str(path_in),
