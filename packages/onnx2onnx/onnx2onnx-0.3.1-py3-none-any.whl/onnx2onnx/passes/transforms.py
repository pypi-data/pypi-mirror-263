...
class ExpandConvChannelRewriter(Rewriter):
    ...
    def rewrite(self, graph: OnnxGraph, nodes: List):
        ...
        graph.inputs.remove(old_input.name)
        self -= weight_node
        self += expand_weight_node


@L2.register(name="eliminate_identity", deps=["initializer_to_constant"])
class EliminateIdentityRewriter(Rewriter):
    ...
    def __init__(self):
        const = SingleNodePattern("Constant")
        identity = SingleNodePattern("Identity")
        pattern = GraphPattern()
        pattern.add_edge(const, identity)
        super().__init__(pattern=pattern)

    def rewrite(self, graph: OnnxGraph, nodes: List[NodeProto]):
        constant_node, identity_node = nodes
        for i, succ in enumerate(graph.onnx_successors(identity_node)):
            pos = list(succ.input).index(identity_node.output[0])
            if i == 0:
                succ.input[pos] = constant_node.output[0]
            else:
                # make a copy of constant
                constant_copy_node = make_constant(
                    constant_node.name + f"_copy{i}", self.get_value(constant_node)
                )
