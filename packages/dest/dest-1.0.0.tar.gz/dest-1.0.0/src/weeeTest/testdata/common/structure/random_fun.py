...
------------------------------------
@File           :  random_fun.py
@Description    :  A function that generates random data
@CreateTime     :  2023/4/6 14:32
@Software       :  PyCharm
------------------------------------
@ModifyTime     :  2023/4/6 14:32
"""
import datetime
import hashlib
import random
...
from weeeTest.testdata.common.structure import en_first_names_female, en_first_names_male, zh_names_female, \
    zh_names_male, en_last_names, zh_last_name, words_str, mobile, unicom, telecom

__all__ = ["first_name", "last_name", "username", "address", "get_email", "get_md5", "get_uuid", "get_int", "get_int32",
           "get_int64", "get_float", "get_digits", "yes", "get_words", "get_word", "get_birthday", "get_past_datetime",
           "get_future_datetime", "get_past_datetime", "get_week",
           "get_future_datetime", "get_past_time", "get_future_time", "get_date", "get_month", "get_year", "get_phone",
           "get_timestamp"]
...
def address(language: str = "en") -> str:
    ...
    """
    if language == "en":
        return Faker().address()[0:-6]

    if language == "zh":
        return Faker("zh_CN").address()[0:-7]

    raise ValueError(f"{language} language is not supported")


def get_email(name: str = "", domain: str = "") -> str:
    """
...
def get_md5(val: str = "") -> str:
    """Return a md5 hash of val, if no val then return a random md5 hash
...
def yes(specifier=0) -> int:
    ...
    :param specifier: int|float, if int, return a value between 1 and specifier.
        if floated, return 1 approximately specifier percent of the time, return 0
...
def get_week(week: int = None) -> str:
    """
    Get week, default to current month.
    :param week:
    :return:
    """
    if week is None:
        ...
        # 显示指定周
        date = int(datetime.datetime.now().strftime("%W")) + week
