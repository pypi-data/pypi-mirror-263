...
def isoparse(isodate: str) -> datetime:
...
class KiWISTimeSeriesClient(TimeSeriesClient):
    ...
    async def _filter_time_series(
        ...
        **kwargs: Any,
        ...
        else:  # Your query is technically correct but produces no results
            return []
    ...
    async def read_coverage(
        ...
        **kwargs: Any,
        ...
        if json:
            if is_single_result:
                return isoparse(json[0]["from"]), isoparse(json[0]["to"])
            else:
                return {
                    TimeSeriesKey(path=ts["ts_path"]): (isoparse(ts["from"]), isoparse(ts["to"]))
                    for ts in json
                }
        else:  # The query is technically correct but produces no results
            return {}
    ...
    async def read_data_frame(
        ...
        **kwargs: Any,
        ...
        if (start is None or isinstance(start, datetime)) and (end is None or isinstance(end, datetime)):
            ...
            raise TimeSeriesUserError(f"No paths to read {','.join(paths)}", ",".join(paths))
    ...
    async def read_comments(
        ...
        **kwargs: Any,
        ...
        if isinstance(start, Iterable) or isinstance(end, Iterable):
            raise NotImplementedError(
                "Read comments for different time ranges for each time series is not yet implemented"
            )
        if start:
            params["from"] = start.isoformat()
        if end:
        ...
        for ts_list in ts_list_result:
            if isinstance(ts_list, Exception):
                raise RuntimeError(f"Reading time series comments failed because {ts_list}") from ts_list
            else:
        ...
        if is_single_result:
            ...
            for comments in comments_results:
                if isinstance(comments, Exception):
                    raise RuntimeError(
                        f"Reading time series comments failed because {comments}"
                    ) from comments
                else:
                    kiwis_comments += [
                        self._parse_kiwis_comment(c) for c in self._session.to_json(comments)
                    ]
