...
class Manager:
    ...
    def plot_fusion_intensity_profile(self,embryo_name,fuse_folder):
        fuse_files = []
        for file in listdir(fuse_folder):
        ...
        for fuse_file in fuse_files:
            ...
            print("Time : " + str(time))
            image_path = join(fuse_folder, fuse_file)
        ...
        plt.savefig(join(folder_out, "fusion_images_intensities.png"))
    ...
    def final_fusion(self, parameters,one_stack_only=False,stack_chosen=0):
        ...
        try :
            fuse_path = "./FUSE/FUSE_"+str(real_parameters["EXP_FUSE"])+"/"
            self.plot_fusion_intensity_profile(embryo_name,fuse_path)
        except Exception as e:
            strlog = traceback.format_exc()
            print("Error during intensities signal plot: "+str(strlog))
