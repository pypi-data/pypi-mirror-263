...
def test_inject_owner_label():
    ...
    assert "pipelines.baler.com/owned-by" in updated_manifest["metadata"]["labels"]
    assert (
        updated_manifest["metadata"]["labels"]["pipelines.baler.com/owned-by"]
        == owner_name
    )
...
def test_operate_on_resource(
    ...
    mock_manage_kubernetes_object.assert_called_with(
        ...
        "default",
        {
            ...
            "metadata": {
                "name": "test-deploy",
                "labels": {"pipelines.baler.com/owned-by": "test-owner"},
            },
        },
    )
