...
from test_tele.bot.bot_header import loop_message, is_bot_in_group #, hentaifox_to_telegraph
...
# Helper (regex) for media downloader
...
async def download_media_url_handler(event, is_premium: bool=False):
    ...
    """
    if not await youtube_url_validation(event.message.text):
        return
    ...
    with yt_dlp.YoutubeDL({'quiet': True, 'format': "mp4", 'outtmpl': f'outputs/{video_title}.%(ext)s', 'postprocessors': []}) as ydl:
        info = ydl.extract_info(event.message.text, download=True)
        title = info['title']
