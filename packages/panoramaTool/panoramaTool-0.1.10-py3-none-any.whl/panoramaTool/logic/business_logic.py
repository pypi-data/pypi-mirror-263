...
class BusinessLogic:
    ...
    def create_security_post_rules(url, api_key, csv_data, device_group):
        ...
        return security_post_rules_manager.create_post_rules(
            device_group=device_group,
            name=csv_data.get('NAME').replace(" ", ""),
            action=csv_data.get('ACTION').replace(" ", "") or 'allow',
            description=csv_data.get('DESCRIPTION') or '',
            from_zone=csv_data.get('FROM').replace(" ", "") or 'any',
            source_group=csv_data.get('SOURCE').replace(" ", "") or 'any',
            to_zone=csv_data.get('TO').replace(" ", "") or 'any',
            destination_group=csv_data.get('DESTINATION').replace(" ", "") or 'any',
            service=csv_data.get('SERVICE').replace(" ", "") or 'application-default',
            profile_type=csv_data.get('PROFILE TYPE').replace(" ", "") or 'group',
            profile=csv_data.get('PROFILE').replace(" ", "") or 'Alert-Only'
        )
