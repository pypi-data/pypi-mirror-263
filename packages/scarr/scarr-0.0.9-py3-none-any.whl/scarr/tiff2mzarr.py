...
def tiff2mzarr(load_dir: str, save_dir: str, is_seg: bool, lossy: bool) -> None:
    """
    Converts a tiff files of an image to a scarr file.

    Args:
        load_dir (str): Path to the tiff directory.
        save_dir (str): Path to the folder where the scarr file should be saved.
    ...
    image = Scarr(image)    
    image.save(join(save_dir, "{}.scarr".format(name)), is_seg=is_seg, lossless=not lossy)
