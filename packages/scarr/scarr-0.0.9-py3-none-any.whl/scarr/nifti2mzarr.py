...
def all_nifti2mzarr(load_dir: str, save_dir: str, is_seg: bool, lossy: bool) -> None:
    """
    Converts all nifti files into scarr files.

    Args:
        load_dir (str): Directory where the nifti files are located.
        save_dir (str): Directory where the scarr files should be saved.
    ...
    for name in tqdm(names, desc="Image conversion"):
        nifti2mzarr(join(load_dir, name + ".nii.gz"), join(save_dir, name + ".scarr"), is_seg, lossy)


def nifti2mzarr(load_filepath: str, save_filepath: str, is_seg: bool, lossy: bool) -> None:
    """
    Converts a single nifti file to a scarr file.

    Args:
        load_filepath (str): Path to the nifti file.
        save_dir (str): Path to where the scarr file should be saved.
                is_seg (bool): Whether the image is a segmentation.
        lossy (bool): Whether lossy JpegXL compression should be used.
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', "--input", required=True,
                        help="Absolute input path to the file(s) that should be converted to from Nifti to Scarr.")
    parser.add_argument('-o', "--output", required=True, help="Absolute output path to the folder that should be used for the Scarr images.")
