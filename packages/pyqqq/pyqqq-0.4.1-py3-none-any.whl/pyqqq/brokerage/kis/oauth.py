...
class KISAuth:
    ...
    def get_token(self, refresh = False) -> str:
        ...
        Returns:
            str: 액세스 토큰
        '''
        token = self.repo.find(self.appkey, self.appsecret)
        if token is None or refresh is True:
            token, expiry = self.issue_access_token()
            self.repo.save(self.appkey, self.appsecret, token, expiry)
...
class KISTokenRepository:
    ...
    def find(self, appkey, appsecret) -> str | None:
        ...
        Returns:
            str|None: 액세스 토큰
        """
        return self.repo.find(appkey, appsecret)

    def save(self, appkey, appsecret, token, expiry):
        """
...
class _LocalTokenRepository:
    ...
    def save(self, appkey, appsecret, token, expiry):
        TokenCache = Query()
        self.db.upsert({'appkey': appkey, 'appsecret': appsecret, 'token': token, 'expiry': expiry}, TokenCache.appkey == appkey and TokenCache.appsecret == appsecret)
