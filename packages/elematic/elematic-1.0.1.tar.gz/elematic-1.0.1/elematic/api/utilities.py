...
def report(material:MatML_api.Material,type:str="full",out:io.TextIOWrapper=sys.stdout):
    ...
    if material.BulkDetails is not None:
        ...
        if bulk_details.Form is not None:
            ...
            if form.Geometry.Shape: out.write(indent[1]+f"Shape: {form.Geometry.Shape}\n")
            if form.Geometry.Dimensions: out.write(indent[1]+f"Dimensions: {form.Geometry.Dimensions}\n")
            if form.Geometry.Orientation: out.write(indent[1]+f"Orientation: {form.Geometry.Orientation}\n")
            if form.Geometry.Notes: out.write(indent[1]+f"{form.Geometry.Notes}\n")
        if bulk_details.ProcessingDetails is not None:
            ...
            for processing_detail in processing_details:
                out.write(indent[0]+f"Processing: {processing_detail.Name.valueOf_}\n")
                for parameter_value in processing_detail.ParameterValue:
                    ...
                    out.write(indent[1]+f"Parameter: {parameter_details.Name.valueOf_}\n")
                    out.write(indent[2]+f"Data: {parameter_value.Data.valueOf_}\n")
                    if parameter_details.Units:
                        out.write(indent[2]+f"Units: {parameter_details.Units.string()}\n")
                ...
                if processing_detail.Notes: out.write(indent[1]+f"{processing_detail.Notes}\n")
        ...
        if bulk_details.PropertyData is not None:
            for property_data in bulk_details.PropertyData:
                ...
                out.write(indent[0]+f"Property data: {property_details.Name.valueOf_}\n")
                ...
                if property_data.test is not None:
                    #TODO implement this
                    pass
                if property_data.Notes: out.write(indent[1]+f"{processing_detail.Notes}\n")
        out.write("\n")
