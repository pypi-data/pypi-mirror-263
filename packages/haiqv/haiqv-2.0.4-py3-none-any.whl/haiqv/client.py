...
def get_notebook_info(client_ip: str) -> Any:
    try:
        ...
        if notebook_info.status_code == 200:
            ...
            __logger.warning(f'notebook info fetch failed - client_ip: {client_ip} -> {notebook_info.text}')
            return HaiqvValueError(notebook_info.text)
    except Exception as e:
        __logger.warning(f'notebook info fetch exception - client_ip: {client_ip}')
        __logger.debug(e)
        return None
...
def create_run(exp_name: str, run_name: str, namespace: str, notebook_id: int, notebook_name: str, owner: str, owner_name: str) -> Any:
    ...
    try:
        ...
        if run_info.status_code == 200:
            __logger.info(f'create_run ok - exp_name: {exp_name}, run_name: {run_name}')
            return run.Run(id=run_info.json()['run_id'], name=run_name)
        else:
            __logger.warning(f'create_run failed - exp_name: {exp_name}, run_name: {run_name} -> {run_info.text}')
            return HaiqvValueError(run_info.text)
    except Exception as e:
        __logger.warning(f'create_run exception - exp_name: {exp_name}, run_name: {run_name}')
        __logger.debug(e)
        return None
...
def update_run(
    ...
    try:
        ...
        if res.status_code == 200:
            __logger.info(f'update_run ok - status: {status}, is_end: {is_end}')
        else:
            __logger.warning(f'update_run failed - status: {status}, is_end: {is_end} -> {res.text}')
            return HaiqvValueError(res.text)
    except Exception as e:
        __logger.warning(f'update_run exception - status: {status}, is_end: {is_end}')
        __logger.debug(e)
        return None
...
def log_param(run_id: str, key: str, value: Any) -> Any:
    ...
    try:
        res = requests.post(f'{os.environ.get("_HAIQV_BASE_URL")}/experiment/logging/batch-params?run_id={run_id}',
                            ...
                            headers={'Content-Type': 'application/json'})
        if res.status_code == 200:
            __logger.info(f'log_param ok - key: {key}, value: {value}')
        else:
            __logger.warning(f'log_param failed - key: {key}, value: {value} -> {res.text}')
            return HaiqvValueError(res.text)
    except Exception as e:
        __logger.warning(f'log_param exception - key: {key}, value: {value}')
        __logger.debug(e)
        return None
...
def log_params(run_id: str, data: Any) -> Any:
    try:
        res = requests.post(f'{os.environ.get("_HAIQV_BASE_URL")}/experiment/logging/batch-params?run_id={run_id}',
                            ...
                            headers={'Content-Type': 'application/json'})
        if res.status_code == 200:
            __logger.info(f'log_params ok - data: {data}')
        else:
            __logger.warning(f'log_params failed - data: {data} -> {res.text}')
            return HaiqvValueError(res.text)
    except Exception as e:
        __logger.warning(f'log_params exception - data: {data}')
        __logger.debug(e)
        return None
...
def log_metric(run_id: str, key: str, value: float, step: int) -> Any:
    ...
    try:
        res = requests.post(f'{os.environ.get("_HAIQV_BASE_URL")}/experiment/logging/batch-metrics?run_id={run_id}',
                            ...
                            headers={'Content-Type': 'application/json'})
        if res.status_code == 200:
            __logger.info(f'log_metric ok - key: {key}, value: {value}, step={step}')
        else:
            __logger.warning(f'log_metric failed - key: {key}, value: {value}, step={step} -> {res.text}')
            return HaiqvValueError(res.text)
    except Exception as e:
        __logger.warning(f'log_metric exception - key: {key}, value: {value}, step={step}')
        __logger.debug(e)
        return None
...
def log_metrics(run_id: str, data: Any) -> Any:
    try:
        res = requests.post(f'{os.environ.get("_HAIQV_BASE_URL")}/experiment/logging/batch-metrics?run_id={run_id}',
                            ...
                            headers={'Content-Type': 'application/json'})
        if res.status_code == 200:
            __logger.info(f'log_metrics ok - data: {data}')
        else:
            __logger.warning(f'log_metrics failed - data: {data} -> {res.text}')
            return HaiqvValueError(res.text)
    except Exception as e:
        __logger.warning(f'log_metrics exception - data: {data}')
        __logger.debug(e)
        return None
...
def log_artifact(run_id: str, local_file: str, artifact_path: str) -> Any:
    try:
        ...
        with open(local_file, 'rb') as f:
            ...
            if res.status_code == 200:
                __logger.info(f'log_artifact ok - local_file: {local_file}, artifact_path: {artifact_path}')
            else:
                __logger.warning(f'log_artifact failed - local_file: {local_file}, artifact_path: {artifact_path} -> {res.text}')
                return HaiqvValueError(res.text)
    except Exception as e:
        __logger.warning(f'log_artifact exception - local_file: {local_file}, artifact_path: {artifact_path}')
        __logger.debug(e)
        return None
...
def log_requirements(run_id: str, text: str, requirement_file: str) -> Any:
    try:
        ...
        __logger.warning('log_requirements exception')
        __logger.debug(e)
        return None
...
def log_dataset_metadata(run_id: str, name: str, path: str, desc: str = None) -> Any:
    try:
        res = requests.post(f'{os.environ.get("_HAIQV_BASE_URL")}/experiment/logging/metadata-dataset?run_id={run_id}&name={name}&path={path}&description={desc}',
                            headers={'Content-Type': 'application/json'})
        if res.status_code == 200:
            __logger.info(f'log_dataset_metadata ok - name: {name}, path: {path}, desc: {desc}')
        else:
            __logger.warning(f'log_dataset_metadata failed - name: {name}, path: {path}, desc: {desc} -> {res.text}')
            return HaiqvValueError(res.text)
    except Exception as e:
        __logger.warning(f'log_dataset_metadata exception - name: {name}, path: {path}, desc: {desc}')
        __logger.debug(e)
        return None
...
def log_model_metadata(run_id: str, name: str, path: str, step: int, volume_name: str, volume_path: str, metric: Optional[dict] = None) -> Any:
    try:
        res = requests.post(f'{os.environ.get("_HAIQV_BASE_URL")}/experiment/logging/metadata-model?run_id={run_id}&name={name}&path={path}&step={step}&volume_name={volume_name}&volume_path={volume_path}',
                            ...
                            headers={'Content-Type': 'application/json'})
        if res.status_code == 200:
            __logger.info(f'log_model_metadata ok - name: {name}, path: {path}, step: {step}, metric: {metric}, volume_name: {volume_name}, volume_path: {volume_path}')
        else:
            __logger.warning(f'log_model_metadata failed - name: {name}, path: {path}, step: {step}, metric: {metric}, volume_name: {volume_name}, volume_path: {volume_path} -> {res.text}')
            return HaiqvValueError(res.text)
    except Exception as e:
        __logger.warning(f'log_model_metadata exception - name: {name}, path: {path}, step: {step}, metric: {metric}, volume_name: {volume_name}, volume_path: {volume_path}')
        __logger.debug(e)
        return None
...
def keepalive(run_id: str) -> Optional[HaiqvValueError]:
    try:
        res = requests.post(f'{os.environ.get("_HAIQV_BASE_URL")}/experiment/run/heartbeat?run_id={run_id}',
                            headers={'Content-Type': 'application/json'})
        if res.status_code != 200:
            __logger.warning(f'keepalive failed -> {res.text}')
            return HaiqvValueError(res.text)
    except Exception as e:
        __logger.warning('keepalive failed')
        __logger.debug(e)
        return None
