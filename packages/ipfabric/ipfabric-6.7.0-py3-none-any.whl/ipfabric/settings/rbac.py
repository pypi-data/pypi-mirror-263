...
class Policies(BaseModel):
    ...
    def get_policies(self, policy_name: str = None):
        """
        Gets all users or filters on one of the options.
        :param policy_name: str: Role Name to filter
        :return: List of roles
        """
        filters = {"name": ["ieq", policy_name]} if policy_name else None
        policies = list()
    ...
    def create_policies(
        ...
        for policy in policies:
            try:
                created[policy["name"]] = self.create_policy(**policy)
    ...
    def delete_policies(self, policies: List[Union[int, str, Policy]]) -> Dict[str, bool]:
        ...
        for policy in policies:
            ...
            try:
                deleted[policy_id] = self.delete_policy(policy_id)
            except HTTPStatusError:
...
class Roles(BaseModel):
    ...
    _policies: Optional[Policy] = None

    def model_post_init(self, __context: Any) -> None:
        try:
            self._policies = Policies(client=self.client)
        except HTTPStatusError:
            logger.warning(self.client._api_insuf_rights + 'on POST "/tables/policies".')
    ...
    def get_roles(self, role_name: str = None) -> List[Role]:
        """
        Gets all users or filters on one of the options.
        ...
        roles = self.client.fetch_all("tables/roles", filters=filters, snapshot=False)
        if not self._policies:
            return [Role(**role) for role in roles]
        return [Role(**role, policies=[self._policies.policies_by_id[_] for _ in role["policyIds"]]) for role in roles]
    ...
    def delete_roles(self, roles: List[Union[int, str, Role]]) -> Dict[str, bool]:
        ...
        for role in roles:
            ...
            try:
                deleted[role_id] = self.delete_role(role_id)
            except HTTPStatusError:
