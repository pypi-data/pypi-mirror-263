...
def create2DPolynomial(pts, u=np.arange(-0.05, 1.01, 0.01), order=3, weights=None, autoFlipXY=False):
    poly = vtkh.createPolynomial(pts, k=order, weights=weights, autoFlipXY=autoFlipXY)
...
def calcPolynomialTangentSlopes(pts, u=np.arange(-0.05, 1.01, 0.01), k=3, weights=None, autoFlipXY=False):
    warnings.simplefilter(action='ignore')
    poly, uPts = vtkh.createPolynomial(pts, k=k, retU=True, weights=weights, autoFlipXY=autoFlipXY)
    if str(u) == 'only_pts':
        u = uPts
