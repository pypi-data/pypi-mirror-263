...
def performSpineAnalysis1(
    ...
    for i in iRange:
        ...
        if not useCustomSpineModelDef:
            ...
            spineLineSagDer = np.zeros((1,2))
            
            for defPW in customSpineModelDef['model_sag']:
                ...
                orderPW = defPW['order']
                autoFlipXYPW = defPW['autoflip_xy']
                sPW = spinePointNamesNewSag.index(pointStartPW)
                ePW = spinePointNamesNewSag.index(pointStopPW) + 1
                nPW = ePW - sPW
                weightsPW = np.ones((nPW,))
                if 'weights' in defPW:
                    for weightPW in defPW['weights']:
                        iPW = spinePointNamesNewSag.index(weightPW['point']) - sPW
                        weightsPW[iPW] = weightPW['value']
                
                spineDataSagPW = spineDataSag[sPW:ePW,:]
                
                spineLineSagPW = spine.create2DPolynomial(spineDataSagPW, order=orderPW, weights=weightsPW, autoFlipXY=autoFlipXYPW)
                spineLineSagDerPW = spine.calcPolynomialTangentSlopes(spineDataSagPW, u='only_pts', k=orderPW, weights=weightsPW, autoFlipXY=autoFlipXYPW)
                
                spineLineSag = np.vstack((spineLineSag[:-1,:], spineLineSagPW))
                spineLineSagDer = np.vstack((spineLineSagDer[:-1,:], spineLineSagDerPW))
...
#        spineDataFro = spineData[i,2:0:-1,:].T # Np x 2
    ...
    #        spineLineFroDer = spine.calcSplineTangentSlopes(spineDataFro, u='only_pts')
            ...
            spineLineFroDer = np.zeros((1,2))
            
            for defPW in customSpineModelDef['model_fro']:
                ...
                if 'weights' in defPW:
                    for weightPW in defPW['weights']:
                        iPW = spinePointNamesNewFro.index(weightPW['point']) - sPW
                        weightsPW[iPW] = weightPW['value']
                
                spineDataFroPW = spineDataFro[sPW:ePW,:]
                
                spineLineFroPW = spine.create2DPolynomial(spineDataFroPW, order=orderPW, weights=weightsPW, autoFlipXY=autoFlipXYPW)
                spineLineFroDerPW = spine.calcPolynomialTangentSlopes(spineDataFroPW, u='only_pts', k=orderPW, weights=weightsPW, autoFlipXY=autoFlipXYPW)
                
                spineLineFro = np.vstack((spineLineFro[:-1,:], spineLineFroPW))
                spineLineFroDer = np.vstack((spineLineFroDer[:-1,:], spineLineFroDerPW))            
