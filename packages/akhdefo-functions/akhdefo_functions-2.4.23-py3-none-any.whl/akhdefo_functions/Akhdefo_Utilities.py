...
def measure_displacement_from_camera(hls_url, alpha=0.1, save_output=False, output_filename=None, ssim_threshold=0.4, 
    ...
    avg_frame = None
    alpha = 0.00002  # Weight for the running average
    ...
    while True:
        ...
        stats_sequence_label=os.path.join(full_path_stats, f"frame_{current_time.strftime('%Y-%m-%d_%H-%M-%S')}_{frame_count}.jpg")
        # Check if the current directory has "mask.png"
        if mask_path.exists():
        ...
        flow = cv2.calcOpticalFlowFarneback(prev_frame_crop_noNan, frame_gray_crop_noNan, None, pyr_scale=0.5, levels=15, winsize=148,iterations= 3, poly_n=5,poly_sigma= 1.5, flags=1)
        ...
        try:  
            ...
            fig.savefig(label_plot[:-4]+'_w.jpg', dpi=150) 
        ...
        try:   
            plt_img=cv2.imread(label_plot[:-4]+'_w.jpg', 1)
       ...
       #######################
        ...
        try:
            ...
            #fig2.savefig(stats_sequence_label, dpi=65 )
        except PermissionError as e:
            print(f"Permission denied when trying to save 'plot.png': {e}")
            # Handle the error, e.g., by using a different file name or directory
        except Exception as e:
        ...
        try:
            # Example: Attempting to save a plot
            image = cv2.imread(label_plot[:-4]+f"_{last_hour}.jpg")
        ...
        if save_output:
            ...
            if len(imagescreenshot_list) in [10, 100, 200, 300, 400, 500,600,700, 800,900, 1000, 2000, 3000, 4000] or last_hour!=current_hour: 
