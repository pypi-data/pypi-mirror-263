...
def measure_displacement_from_camera(hls_url, alpha=0.1, save_output=False, output_filename=None, ssim_threshold=0.4, 
    ...
    backSub1 = cv2.createBackgroundSubtractorMOG2(detectShadows=True)
    ...
    def delete_older_mp4(folder_path, output_video_path, bitrate_kbps):
        ...
        # Check if there are exactly two mp4 files
        if len(mp4_files) == 2:
            ...
            set_video_bitrate(sorted_files[0][0], output_video_path, bitrate_kbps)
            # Delete the older file
            os.remove(sorted_files[0][0])
            print(f"Deleted older mp4 file: {sorted_files[0][0].name}")
        # else:
        #     print(f"Folder does not contain exactly two mp4 files. Found {len(mp4_files)} mp4 files.")
    ...
    while True:
        ...
        label_plot = os.path.join(plot_folder, f"frame_{now.strftime('%Y-%m-%d')}{frame_suffix}.jpg")
        file_path_data_csv = os.path.join(data_folder, f"frame_{now.strftime('%Y-%m-%d')}{frame_suffix}.csv")
        ...
        detected_shift, error, diffphase = phase_cross_correlation(prev_frame_initgray, frame_gray_ini, upsample_factor = 20, normalization='phase' )
        y_shift, x_shift = detected_shift

        shift_list_x.append(x_shift)
        shift_list_y.append(y_shift)
        ...
        flow = cv2.calcOpticalFlowFarneback(prev_frame_crop_noNan, frame_gray_crop_noNan, None, pyr_scale=0.5, levels=15, winsize=120,iterations= 3, poly_n=5,poly_sigma= 1.5, flags=1)
        ...
        end_time = time.time()
        processing_time = (end_time - start_time)+2
       ...
       # Find indices of all non-NaN values
        non_nan_indices = np.argwhere(~np.isnan(vel_2d_crop))

        # Calculate the bounds of the non-NaN values
        x_min, y_min = non_nan_indices.min(axis=0)
        x_max, y_max = non_nan_indices.max(axis=0)
        ...
        fig.colorbar(dd, ax=ax, fraction=0.046, pad=0.04).set_label('Displacement (pixels)', rotation=270, labelpad=15, fontsize=12, fontweight='bold')
        ...
        plt.tight_layout(pad=2)  # Adjust 'pad' as needed to fit the color bar and prevent overlap
        ...
        try:   
            plt_img=cv2.imread('window.png', 1)
        ...
        try:
            ...
            # Attempt to resize the colorbar to match the image height
                AOI_img = cv2.resize(plt_img, (frame_width // 2 , frame_height), interpolation=cv2.INTER_AREA)
       ...
       # Create a figure and two subplots for min and max displacements
        ...
        ax1.plot(datetimes_from_frame, std_frames, label='Standard-Deviation', color='green', linestyle='--', marker='x')
        # Plot minimum displacement with a blue line
        ax1.plot(datetimes_from_frame, min_displacements, label='Min Displacement', color='blue', linestyle='-', marker='o')
        ...
        # Plot maximum displacement with a red line
        ax1.plot(datetimes_from_frame, mean_displacement, label='Mean Displacement', color='k', linestyle='--', marker='+')
        ...
        # Add a title
        ax1.set_title(f'Minimum and Maximum Displacement Over Time in Pixel Unit, timestamp:{timestamp}', fontsize=14, fontweight='bold', color='navy')
        ...
        ax3 = ax1.twinx()
        colorx = 'tab:blue'
        ax3.plot(datetimes_from_frame, shift_list_x, color=colorx, label='Image-Shift-x', linestyle='dotted', marker='d')
        ax3.tick_params(axis='y', labelcolor=colorx)
        ax3.set_ylabel('Image-Shift', fontsize=12, fontweight='bold', color=colorx)
        
        colory = 'tab:purple'
        ax3.plot(datetimes_from_frame, shift_list_y, color=colory, label='Image-Shift-y', linestyle='dashed', marker='^')
       ...
       #######################
        ...
        os.makedirs('plots/GIFS', exist_ok=True)
        os.makedirs('plots/GIFS/converted', exist_ok=True)
        
        save_image='plots/screenshots/'+ label_plot[5:]
        save_gif_statsprofile='plots/GIFS/'+ label_plot[5:-4]+'_profile.gif'
        save_gif_image='plots/GIFS/'+label_plot[5:-4]+'_image.mp4'
        save_gif_converted='plots/GIFS/converted/'+label_plot[5:-4]+'_image.mp4'
        ...
        if save_output:
            ...
            if len(imagescreenshot_list) in [100,200,400,800,1000]:
            ...
            delete_older_mp4('plots/GIFS', save_gif_converted, 1500)
        ######
        ...
        cv2.imwrite(save_image,combined_frame )
