...
class AudioModule:
    ...
    def __init__(self, port, baud, event):
        ...
        self.end_byte_data = self.protocol_head_pack(MAGIC_DATA, CHECK_SUM, TypeCode.PLAY_DATA_END, 0, 0,
                                                     FillCode.MP3_FILL)
        self.stop_byte_data = self.protocol_head_pack(MAGIC_DATA, CHECK_SUM, TypeCode.PLAY_TTS_END, 0, 0,
                                                      FillCode.MP3_FILL)
        ...
        # 订阅aigc发出得事件
        self.aigc_event.subscribe(lambda i: self.event_handler(i), scheduler=pool_scheduler)
    ...
    @staticmethod
    def protocol_head_pack(magic, checksum, head_type, data_len, version, fill_data):
    ...
    def media_end(self, data):
        logger.info("Media end, fileCode: {0}".format(self.file_code))
        if self.file_code == FillCode.MP3_FILL:
            self.event.on_next({"type": "wait_response_again", "data": ""})
        elif self.file_code == FillCode.TTS_FILL:
