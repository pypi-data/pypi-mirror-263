...
from the command line from '{os.getcwd()}'. If you are still having trouble, please contact Chalk support.""",
    ...
    def offline_query(
        ...
        if input is None:
            ...
            if isinstance(input, (list, tuple)):
                raise ValueError("A single input must be provided if not using multiple computers")
            query_input = _to_offline_query_input(input, cast(None, input_times))
        else:
            if not isinstance(input, (list, tuple)):
                raise ValueError("A sequence of inputs (one per computer) must be provided if using multiple computers")
            query_input = tuple(
    ...
    def recompute_dataset(
        ...
        req = CreateOfflineQueryJobRequest(
            ...
            dataset_name=dataset_name,
            branch=branch,
            recompute_features=features if features is not None else True,
            store_plan_stages=store_plan_stages,
            correlation_id=correlation_id,
            explain=explain,
            tags=tags,
    ...
    def await_operation_completion(
        ...
        def _await_operation(computer_id: int):
            ProgressService(
        ...
        futures = DEFAULT_IO_EXECUTOR.map(_await_operation, range(num_computers))
        for _ in futures:
            # Spinning the iterator to await completion
            pass
