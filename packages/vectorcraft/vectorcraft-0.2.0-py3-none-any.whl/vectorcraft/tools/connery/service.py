...
from vectorcraft.tools.connery.models import Action
from vectorcraft.tools.connery.tool import ConneryAction
...
class ConneryService(BaseModel):
    ...
    def validate_attributes(cls, values: Dict) -> Dict:
        ...
        runner_url = get_from_dict_or_env(
            values, "runner_url", "CONNERY_RUNNER_URL")
        api_key = get_from_dict_or_env(
            values, "api_key", "CONNERY_RUNNER_API_KEY")

        if not runner_url:
            raise ValueError(
                "CONNERY_RUNNER_URL environment variable must be set.")
        if not api_key:
            raise ValueError(
                "CONNERY_RUNNER_API_KEY environment variable must be set.")
    ...
    def _get_action(self, action_id: str) -> Action:
        ...
        action = next(
            (action for action in actions if action.id == action_id), None)
        if not action:
            raise ValueError(
                (
