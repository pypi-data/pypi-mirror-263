...
def _import_ai21() -> Type[BaseLLM]:
    from vectorcraft.llms.ai21 import AI21

    return AI21


def _import_aleph_alpha() -> Type[BaseLLM]:
    from vectorcraft.llms.aleph_alpha import AlephAlpha

    return AlephAlpha


def _import_amazon_api_gateway() -> Type[BaseLLM]:
    from vectorcraft.llms.amazon_api_gateway import AmazonAPIGateway

    return AmazonAPIGateway


def _import_anthropic() -> Type[BaseLLM]:
    from vectorcraft.llms.anthropic import Anthropic

    return Anthropic


def _import_anyscale() -> Type[BaseLLM]:
    from vectorcraft.llms.anyscale import Anyscale

    return Anyscale


def _import_aphrodite() -> Type[BaseLLM]:
    from vectorcraft.llms.aphrodite import Aphrodite

    return Aphrodite


def _import_arcee() -> Type[BaseLLM]:
    from vectorcraft.llms.arcee import Arcee

    return Arcee


def _import_aviary() -> Type[BaseLLM]:
    from vectorcraft.llms.aviary import Aviary

    return Aviary


def _import_azureml_endpoint() -> Type[BaseLLM]:
    from vectorcraft.llms.azureml_endpoint import AzureMLOnlineEndpoint

    return AzureMLOnlineEndpoint


def _import_baichuan() -> Type[BaseLLM]:
    from vectorcraft.llms.baichuan import BaichuanLLM

    return BaichuanLLM


def _import_baidu_qianfan_endpoint() -> Type[BaseLLM]:
    from vectorcraft.llms.baidu_qianfan_endpoint import QianfanLLMEndpoint

    return QianfanLLMEndpoint


def _import_bananadev() -> Type[BaseLLM]:
    from vectorcraft.llms.bananadev import Banana

    return Banana


def _import_baseten() -> Type[BaseLLM]:
    from vectorcraft.llms.baseten import Baseten

    return Baseten


def _import_beam() -> Type[BaseLLM]:
    from vectorcraft.llms.beam import Beam

    return Beam


def _import_bedrock() -> Type[BaseLLM]:
    from vectorcraft.llms.bedrock import Bedrock

    return Bedrock


def _import_bittensor() -> Type[BaseLLM]:
    from vectorcraft.llms.bittensor import NIBittensorLLM

    return NIBittensorLLM


def _import_cerebriumai() -> Type[BaseLLM]:
    from vectorcraft.llms.cerebriumai import CerebriumAI

    return CerebriumAI


def _import_chatglm() -> Type[BaseLLM]:
    from vectorcraft.llms.chatglm import ChatGLM

    return ChatGLM


def _import_clarifai() -> Type[BaseLLM]:
    from vectorcraft.llms.clarifai import Clarifai

    return Clarifai


def _import_cohere() -> Type[BaseLLM]:
    from vectorcraft.llms.cohere import Cohere

    return Cohere


def _import_ctransformers() -> Type[BaseLLM]:
    from vectorcraft.llms.ctransformers import CTransformers

    return CTransformers


def _import_ctranslate2() -> Type[BaseLLM]:
    from vectorcraft.llms.ctranslate2 import CTranslate2

    return CTranslate2


def _import_databricks() -> Type[BaseLLM]:
    from vectorcraft.llms.databricks import Databricks
...
def _import_databricks_chat() -> Any:
    warn_deprecated(
        since="0.0.22",
        removal="0.2",
        alternative_import="vectorcraft.chat_models.ChatDatabricks",
    )
    from vectorcraft.chat_models.databricks import ChatDatabricks

    return ChatDatabricks


def _import_deepinfra() -> Type[BaseLLM]:
    from vectorcraft.llms.deepinfra import DeepInfra

    return DeepInfra


def _import_deepsparse() -> Type[BaseLLM]:
    from vectorcraft.llms.deepsparse import DeepSparse

    return DeepSparse


def _import_edenai() -> Type[BaseLLM]:
    from vectorcraft.llms.edenai import EdenAI

    return EdenAI


def _import_fake() -> Type[BaseLLM]:
    from vectorcraft.llms.fake import FakeListLLM

    return FakeListLLM


def _import_fireworks() -> Type[BaseLLM]:
    from vectorcraft.llms.fireworks import Fireworks

    return Fireworks


def _import_forefrontai() -> Type[BaseLLM]:
    from vectorcraft.llms.forefrontai import ForefrontAI

    return ForefrontAI


def _import_friendli() -> Type[BaseLLM]:
    from vectorcraft.llms.friendli import Friendli

    return Friendli


def _import_gigachat() -> Type[BaseLLM]:
    from vectorcraft.llms.gigachat import GigaChat

    return GigaChat


def _import_google_palm() -> Type[BaseLLM]:
    from vectorcraft.llms.google_palm import GooglePalm

    return GooglePalm


def _import_gooseai() -> Type[BaseLLM]:
    from vectorcraft.llms.gooseai import GooseAI

    return GooseAI


def _import_gpt4all() -> Type[BaseLLM]:
    from vectorcraft.llms.gpt4all import GPT4All

    return GPT4All


def _import_gradient_ai() -> Type[BaseLLM]:
    from vectorcraft.llms.gradient_ai import GradientLLM

    return GradientLLM


def _import_huggingface_endpoint() -> Type[BaseLLM]:
    from vectorcraft.llms.huggingface_endpoint import HuggingFaceEndpoint

    return HuggingFaceEndpoint


def _import_huggingface_hub() -> Type[BaseLLM]:
    from vectorcraft.llms.huggingface_hub import HuggingFaceHub

    return HuggingFaceHub


def _import_huggingface_pipeline() -> Type[BaseLLM]:
    from vectorcraft.llms.huggingface_pipeline import HuggingFacePipeline

    return HuggingFacePipeline


def _import_huggingface_text_gen_inference() -> Type[BaseLLM]:
    from vectorcraft.llms.huggingface_text_gen_inference import (
...
def _import_human() -> Type[BaseLLM]:
    from vectorcraft.llms.human import HumanInputLLM

    return HumanInputLLM


def _import_javelin_ai_gateway() -> Type[BaseLLM]:
    from vectorcraft.llms.javelin_ai_gateway import JavelinAIGateway

    return JavelinAIGateway


def _import_koboldai() -> Type[BaseLLM]:
    from vectorcraft.llms.koboldai import KoboldApiLLM

    return KoboldApiLLM


def _import_konko() -> Type[BaseLLM]:
    from vectorcraft.llms.konko import Konko

    return Konko


def _import_llamacpp() -> Type[BaseLLM]:
    from vectorcraft.llms.llamacpp import LlamaCpp

    return LlamaCpp


def _import_llamafile() -> Type[BaseLLM]:
    from vectorcraft.llms.llamafile import Llamafile

    return Llamafile


def _import_manifest() -> Type[BaseLLM]:
    from vectorcraft.llms.manifest import ManifestWrapper

    return ManifestWrapper


def _import_minimax() -> Type[BaseLLM]:
    from vectorcraft.llms.minimax import Minimax

    return Minimax


def _import_mlflow() -> Type[BaseLLM]:
    from vectorcraft.llms.mlflow import Mlflow
...
def _import_mlflow_chat() -> Any:
    warn_deprecated(
        since="0.0.22",
        removal="0.2",
        alternative_import="vectorcraft.chat_models.ChatMlflow",
    )
    from vectorcraft.chat_models.mlflow import ChatMlflow

    return ChatMlflow


def _import_mlflow_ai_gateway() -> Type[BaseLLM]:
    from vectorcraft.llms.mlflow_ai_gateway import MlflowAIGateway

    return MlflowAIGateway


def _import_modal() -> Type[BaseLLM]:
    from vectorcraft.llms.modal import Modal

    return Modal


def _import_mosaicml() -> Type[BaseLLM]:
    from vectorcraft.llms.mosaicml import MosaicML

    return MosaicML


def _import_nlpcloud() -> Type[BaseLLM]:
    from vectorcraft.llms.nlpcloud import NLPCloud

    return NLPCloud


def _import_oci_md_tgi() -> Type[BaseLLM]:
    from vectorcraft.llms.oci_data_science_model_deployment_endpoint import (
...
def _import_oci_gen_ai() -> Type[BaseLLM]:
    from vectorcraft.llms.oci_generative_ai import OCIGenAI

    return OCIGenAI


def _import_octoai_endpoint() -> Type[BaseLLM]:
    from vectorcraft.llms.octoai_endpoint import OctoAIEndpoint

    return OctoAIEndpoint


def _import_ollama() -> Type[BaseLLM]:
    from vectorcraft.llms.ollama import Ollama

    return Ollama


def _import_opaqueprompts() -> Type[BaseLLM]:
    from vectorcraft.llms.opaqueprompts import OpaquePrompts

    return OpaquePrompts


def _import_azure_openai() -> Type[BaseLLM]:
    from vectorcraft.llms.openai import AzureOpenAI

    return AzureOpenAI


def _import_openai() -> Type[BaseLLM]:
    from vectorcraft.llms.openai import OpenAI

    return OpenAI


def _import_openai_chat() -> Type[BaseLLM]:
    from vectorcraft.llms.openai import OpenAIChat

    return OpenAIChat


def _import_openllm() -> Type[BaseLLM]:
    from vectorcraft.llms.openllm import OpenLLM

    return OpenLLM


def _import_openlm() -> Type[BaseLLM]:
    from vectorcraft.llms.openlm import OpenLM

    return OpenLM


def _import_pai_eas_endpoint() -> Type[BaseLLM]:
    from vectorcraft.llms.pai_eas_endpoint import PaiEasEndpoint

    return PaiEasEndpoint


def _import_petals() -> Type[BaseLLM]:
    from vectorcraft.llms.petals import Petals

    return Petals


def _import_pipelineai() -> Type[BaseLLM]:
    from vectorcraft.llms.pipelineai import PipelineAI

    return PipelineAI


def _import_predibase() -> Type[BaseLLM]:
    from vectorcraft.llms.predibase import Predibase

    return Predibase


def _import_predictionguard() -> Type[BaseLLM]:
    from vectorcraft.llms.predictionguard import PredictionGuard

    return PredictionGuard


def _import_promptlayer() -> Type[BaseLLM]:
    from vectorcraft.llms.promptlayer_openai import PromptLayerOpenAI

    return PromptLayerOpenAI


def _import_promptlayer_chat() -> Type[BaseLLM]:
    from vectorcraft.llms.promptlayer_openai import PromptLayerOpenAIChat

    return PromptLayerOpenAIChat


def _import_replicate() -> Type[BaseLLM]:
    from vectorcraft.llms.replicate import Replicate

    return Replicate


def _import_rwkv() -> Type[BaseLLM]:
    from vectorcraft.llms.rwkv import RWKV

    return RWKV


def _import_sagemaker_endpoint() -> Type[BaseLLM]:
    from vectorcraft.llms.sagemaker_endpoint import SagemakerEndpoint

    return SagemakerEndpoint


def _import_self_hosted() -> Type[BaseLLM]:
    from vectorcraft.llms.self_hosted import SelfHostedPipeline

    return SelfHostedPipeline


def _import_self_hosted_hugging_face() -> Type[BaseLLM]:
    from vectorcraft.llms.self_hosted_hugging_face import (
...
def _import_stochasticai() -> Type[BaseLLM]:
    from vectorcraft.llms.stochasticai import StochasticAI

    return StochasticAI


def _import_symblai_nebula() -> Type[BaseLLM]:
    from vectorcraft.llms.symblai_nebula import Nebula

    return Nebula


def _import_textgen() -> Type[BaseLLM]:
    from vectorcraft.llms.textgen import TextGen

    return TextGen


def _import_titan_takeoff() -> Type[BaseLLM]:
    from vectorcraft.llms.titan_takeoff import TitanTakeoff

    return TitanTakeoff


def _import_titan_takeoff_pro() -> Type[BaseLLM]:
    from vectorcraft.llms.titan_takeoff_pro import TitanTakeoffPro

    return TitanTakeoffPro


def _import_together() -> Type[BaseLLM]:
    from vectorcraft.llms.together import Together

    return Together


def _import_tongyi() -> Type[BaseLLM]:
    from vectorcraft.llms.tongyi import Tongyi

    return Tongyi


def _import_vertex() -> Type[BaseLLM]:
    from vectorcraft.llms.vertexai import VertexAI

    return VertexAI


def _import_vertex_model_garden() -> Type[BaseLLM]:
    from vectorcraft.llms.vertexai import VertexAIModelGarden

    return VertexAIModelGarden


def _import_vllm() -> Type[BaseLLM]:
    from vectorcraft.llms.vllm import VLLM

    return VLLM


def _import_vllm_openai() -> Type[BaseLLM]:
    from vectorcraft.llms.vllm import VLLMOpenAI

    return VLLMOpenAI


def _import_watsonxllm() -> Type[BaseLLM]:
    from vectorcraft.llms.watsonxllm import WatsonxLLM

    return WatsonxLLM


def _import_writer() -> Type[BaseLLM]:
    from vectorcraft.llms.writer import Writer

    return Writer


def _import_xinference() -> Type[BaseLLM]:
    from vectorcraft.llms.xinference import Xinference

    return Xinference


def _import_yandex_gpt() -> Type[BaseLLM]:
    from vectorcraft.llms.yandex import YandexGPT

    return YandexGPT


def _import_yuan2() -> Type[BaseLLM]:
    from vectorcraft.llms.yuan2 import Yuan2

    return Yuan2


def _import_volcengine_maas() -> Type[BaseLLM]:
    from vectorcraft.llms.volcengine_maas import VolcEngineMaasLLM

    return VolcEngineMaasLLM


def _import_sparkllm() -> Type[BaseLLM]:
    from vectorcraft.llms.sparkllm import SparkLLM
