...
class OneNoteLoader(BaseLoader, BaseModel):
    ...
    settings: _OneNoteGraphSettings = Field(
        default_factory=_OneNoteGraphSettings)
        ...
        if self.object_ids is not None:
            ...
            while request_url != "":
                response = requests.get(
                    request_url, headers=self._headers, timeout=10)
    ...
    def _auth(self) -> None:
        ...
        if self.auth_with_token:
            ...
            authorization_code = authorization_url.split("code=")[
                1].split("&")[0]
    ...
    def _url(self) -> str:
        ...
        if len(filter_list) > 0:
            query_params_list.append(
                "$filter=" + "%20and%20".join(filter_list))
