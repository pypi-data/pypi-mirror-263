...
class GoogleDriveLoader(BaseLoader, BaseModel):
    ...
    def validate_inputs(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if file_types:
            ...
            values["file_types"] = [
                full_form(file_type) for file_type in file_types]
        return values
    ...
    def _load_documents_from_folder(
        ...
        if file_types:
            _files = [f for f in files if f["mimeType"]
                      in file_types]  # type: ignore
        else:
            _files = files
    ...
    def _fetch_files_recursive(
        ...
        for file in files:
            if file["mimeType"] == "application/vnd.google-apps.folder":
                if self.recursive:
                    returns.extend(
                        self._fetch_files_recursive(service, file["id"]))
            else:
                returns.append(file)
