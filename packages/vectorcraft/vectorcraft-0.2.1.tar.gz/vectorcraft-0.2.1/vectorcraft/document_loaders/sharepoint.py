...
from vectorcraft.document_loaders.base_o365 import (
    O365BaseLoader,
    _FileType,
)
...
class SharePointLoader(O365BaseLoader):
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        if not isinstance(drive, Drive):
            raise ValueError(
                f"There isn't a Drive with id {self.document_library_id}.")
        ...
        if self.folder_path:
            ...
            if not isinstance(target_folder, Folder):
                raise ValueError(
                    f"There isn't a folder with path {self.folder_path}.")
