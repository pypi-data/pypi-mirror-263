...
from vectorcraft.document_loaders.parsers.pdf import (
...
from vectorcraft.document_loaders.unstructured import UnstructuredFileLoader
...
class UnstructuredPDFLoader(UnstructuredFileLoader):
    ...
    Examples
    --------
    from vectorcraft.document_loaders import UnstructuredPDFLoader
...
class PyPDFLoader(BasePDFLoader):
    ...
    def __init__(
        ...
        self.parser = PyPDFParser(
            password=password, extract_images=extract_images)
        ...
        for i in items:
            if i.is_file():
                if self._is_visible(i.relative_to(p)) or self.load_hidden:
                    try:
                        loader = PyPDFLoader(
                            str(i), extract_images=self.extract_images)
...
class MathpixPDFLoader(BasePDFLoader):
    ...
    def send_pdf(self) -> str:
        ...
        if "error" in response_data:
            raise ValueError(
                f"Mathpix request failed: {response_data['error']}")
    ...
    def load(self) -> List[Document]:
        ...
        metadata = {"source": self.source,
                    "file_path": self.source, "pdf_id": pdf_id}
