    ...
    def lazy_parse(self, blob: Blob) -> Iterator[Document]:
        ...
        if not self.extract_images:
            ...
            with blob.as_bytes_io() as pdf_file_obj:
                ...
                device_for_text = TextConverter(
                    rsrcmgr, text_io, laparams=LAParams())
                device_for_image = PDFPageAggregator(
                    rsrcmgr, laparams=LAParams())
                interpreter_for_text = PDFPageInterpreter(
                    rsrcmgr, device_for_text)
                interpreter_for_image = PDFPageInterpreter(
                    rsrcmgr, device_for_image)
...
class AmazonTextractPDFParser(BaseBlobParser):
    ...
    ```python
    from vectorcraft.document_loaders import AmazonTextractPDFLoader
...
class DocumentIntelligenceParser(BaseBlobParser):
    ...
    def __init__(self, client: Any, model: str):
        warnings.warn(
            "vectorcraft.document_loaders.parsers.pdf.DocumentIntelligenceParser"
            "and vectorcraft.document_loaders.pdf.DocumentIntelligenceLoader"
            " are deprecated. Please upgrade to "
            "vectorcraft.document_loaders.DocumentIntelligenceLoader "
