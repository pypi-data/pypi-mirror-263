...
from vectorcraft.callbacks.utils import (
...
def analyze_text(
    ...
    if nlp is not None:
        ...
        dep_out = spacy.displacy.render(
            doc, style="dep", jupyter=False, page=True)

        ent_out = spacy.displacy.render(
...
class MlflowLogger:
    ...
    def __init__(self, **kwargs: Any):
        ...
        if "DATABRICKS_RUNTIME_VERSION" in os.environ:
            ...
            if run_id := kwargs.get("run_id"):
                ...
                self.mlf_exp = self.mlflow.get_experiment_by_name(
                    experiment_name)
                if self.mlf_exp is not None:
                    self.mlf_expid = self.mlf_exp.experiment_id
                else:
                    self.mlf_expid = self.mlflow.create_experiment(
                        experiment_name)
    ...
    def langchain_artifact(self, chain: Any) -> None:
        self.mlflow.langchain.log_model(
            chain, "langchain-model", run_id=self.run_id)
...
class MlflowCallbackHandler(BaseMetadataCallbackHandler, BaseCallbackHandler):
    ...
    def on_llm_start(
        ...
        for idx, prompt in enumerate(prompts):
            ...
            self.mlflg.jsonf(
                prompt_resp, f"llm_start_{llm_starts}_prompt_{idx}")
        ...
        for generations in response.generations:
            for idx, generation in enumerate(generations):
                ...
                if "entities" in generation_resp:
                    ...
                    self.mlflg.html(entities, "ent-" +
                                    hash_string(generation.text))
    ...
    def on_retriever_end(
        ...
        **kwargs: Any,
        ...
        resp.update({"action": "on_retriever_end",
    ...
    def _create_session_analysis_df(self) -> Any:
        ...
        pd = import_pandas()
        on_llm_start_records_df = pd.DataFrame(
            self.records["on_llm_start_records"])
        on_llm_end_records_df = pd.DataFrame(
            self.records["on_llm_end_records"])
        ...
        session_analysis_df = pd.concat(
            [llm_input_prompts_df, llm_outputs_df], axis=1)
    ...
    def flush_tracker(self, langchain_asset: Any = None, finish: bool = False) -> None:
        pd = import_pandas()
        self.mlflg.table("action_records", pd.DataFrame(
        ...
        if self._contain_llm_records():
            ...
            self.mlflg.table("session_analysis",
                             pd.DataFrame(session_analysis_df))
        ...
        if langchain_asset:
            ...
            if "langchain.chains.llm.LLMChain" in str(type(langchain_asset)):
                ...
                langchain_asset_path = str(
                    Path(self.temp_dir.name, "model.json"))
                try:
