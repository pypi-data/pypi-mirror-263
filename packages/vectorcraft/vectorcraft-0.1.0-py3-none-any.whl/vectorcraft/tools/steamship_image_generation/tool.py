...
from vectorcraft.tools.steamship_image_generation.utils import make_image_public
...
class SteamshipImageGenerationTool(BaseTool):
    ...
    def validate_size(cls, values: Dict) -> Dict:
        if "size" in values:
            ...
            if size not in SUPPORTED_IMAGE_SIZES[model_name]:
                raise RuntimeError(
                    f"size {size} is not supported by {model_name}")
    ...
    def _run(
        ...
        image_generator = self.steamship.use_plugin(
            plugin_handle=self.model_name.value, config={
                "n": 1, "size": self.size}
        )
