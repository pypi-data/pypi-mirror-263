...
from vectorcraft.llms.utils import enforce_stop_tokens
from vectorcraft.utilities.anthropic import (
...
class LLMInputOutputAdapter:
    ...
    def prepare_output(cls, provider: str, response: Any) -> dict:
        if provider == "anthropic":
            ...
            if provider == "ai21":
                text = response_body.get("completions")[
                    0].get("data").get("text")
...
class BedrockBase(BaseModel, ABC):
    ...
    def _prepare_input_and_invoke(
        ...
        **kwargs: Any,
        ...
        services_trace = self._get_bedrock_services_signal(
    ...
    def _prepare_input_and_invoke_stream(
        ...
        **kwargs: Any,
        ...
        if stop:
            ...
            _model_kwargs[self.provider_stop_sequence_key_name_map.get(
                provider)] = stop
        ...
        for chunk in LLMInputOutputAdapter.prepare_output_stream(
            ...
            self._get_bedrock_services_signal(
                chunk.generation_info)  # type: ignore[arg-type]
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        if model_id.startswith("anthropic.claude-3"):
            raise ValueError(
                ...
                "Please use `from vectorcraft.chat_models import BedrockChat` "
                "instead."
            )
    ...
    async def _acall(
        ...
        **kwargs: Any,
        ...
        if not self.streaming:
            raise ValueError(
                "Streaming must be set to True for async operations. ")
