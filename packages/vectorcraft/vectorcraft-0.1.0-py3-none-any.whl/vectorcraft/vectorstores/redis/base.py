...
from vectorcraft.utilities.redis import (
    _array_to_buffer,
    _buffer_to_array,
    check_redis_module_exist,
    get_client,
)
from vectorcraft.vectorstores.redis.constants import (
    REDIS_REQUIRED_MODULES,
    REDIS_TAG_SEPARATOR,
)
from vectorcraft.vectorstores.utils import maximal_marginal_relevance
...
if TYPE_CHECKING:
    ...
    from vectorcraft.vectorstores.redis.filters import RedisFilterExpression
    from vectorcraft.vectorstores.redis.schema import RedisModel
...
class Redis(VectorStore):
    ...
    .. code-block:: python
        ...
        from vectorcraft.vectorstores import Redis
        from vectorcraft.embeddings import OpenAIEmbeddings
    ...
    schema according to the following rules:
        ...
        - All lists of strings are indexed as tag fields (joined by
            vectorcraft.vectorstores.redis.constants.REDIS_TAG_SEPARATOR)
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._schema = self._get_schema_with_defaults(
    ...
    def from_texts_return_keys(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            from vectorcraft.vectorstores.redis.schema import read_schema
        ...
        **kwargs: Any,
        ...
        for i, text in enumerate(texts):
            ...
            metadata = _prepare_metadata(
                metadata) if clean_metadata else metadata
            pipeline.hset(
                key,
                mapping={
    ...
    def similarity_search_by_vector(
        ...
        **kwargs: Any,
        ...
        for result in results.docs:
            ...
            docs.append(
                Document(page_content=getattr(
                    result, content_key), metadata=metadata)
            )
        return docs
        ...
        prefetch_embeddings = [
            _buffer_to_array(
                cast(
                    bytes,
                    self.client.hget(
                        prefetch_id, self._schema.content_vector_key),
                ),
    ...
    def _get_schema_with_defaults(
        ...
        from vectorcraft.vectorstores.redis.schema import (
            RedisModel,
            read_schema,
        )
