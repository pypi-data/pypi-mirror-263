...
class MomentoVectorIndex(VectorStore):
    ...
    Example:
        .. code-block:: python
            ...
            from vectorcraft.vectorstores import MomentoVectorIndex
            from momento import (
                CredentialProvider,
    ...
    def __validate_distance_strategy(distance_strategy: DistanceStrategy) -> None:
        if distance_strategy not in [
            ...
            raise ValueError(
                f"Distance strategy {distance_strategy} not implemented.")
    ...
    def from_texts(
        ...
        **kwargs: Any,
        ...
        if "client" in kwargs:
            ...
            api_key = supplied_api_key or get_from_env(
                "api_key", "MOMENTO_API_KEY")
            client = PreviewVectorIndexClient(
                configuration=VectorIndexConfigurations.Default.latest(),
                credential_provider=CredentialProvider.from_string(api_key),
            )
        vector_db = cls(embedding=embedding, client=client,
                        **kwargs)  # type: ignore
