...
from vectorcraft.graphs.graph_document import GraphDocument, Node, Relationship
from vectorcraft.graphs.graph_store import GraphStore
...
class GremlinGraph(GraphStore):
    ...
    *Implementation details*:
    ...
    def refresh_schema(self) -> None:
        ...
        """
        vertex_schema = self.client.submit(
            "g.V().label().dedup()").all().result()
        edge_schema = self.client.submit(
            "g.E().label().dedup()").all().result()
        vertex_properties = (
            self.client.submit(
    ...
    def add_graph_documents(
        ...
        for document in graph_documents:
            if include_source:
                ...
                doc_id = hashlib.md5(
                    document.source.page_content.encode()).hexdigest()
                doc_node = self.add_node(
                    Node(id=doc_id, type="Document",
                         properties=doc_props), node_cache
                )
