...
from vectorcraft.utils.google import get_client_info
from vectorcraft.vectorstores.utils import (
    DistanceStrategy,
...
# Do not check for index more often that this.
...
class BigQueryVectorSearch(VectorStore):
    ...
    def _initialize_table(self) -> Any:
        ...
        if self.doc_id_field not in columns:
            ...
            schema.append(
                bigquery.SchemaField(
                    name=self.doc_id_field, field_type="STRING")
            )
        elif (
            ...
            raise ValueError(
                f"Column {self.doc_id_field} must be of " "STRING type")
        if self.metadata_field not in columns:
            changed_schema = True
            schema.append(
                bigquery.SchemaField(
                    name=self.metadata_field, field_type="JSON")
            )
        elif (
        ...
        if self.content_field not in columns:
            ...
            schema.append(
                bigquery.SchemaField(
                    name=self.content_field, field_type="STRING")
            )
        elif (
            ...
            raise ValueError(
                f"Column {self.content_field} must be of " "STRING type")
        if self.text_embedding_field not in columns:
            ...
            columns[self.text_embedding_field].field_type not in (
                "FLOAT", "FLOAT64")
        ...
        if changed_schema:
            self._logger.debug("Updated table `%s` schema.",
                               self.full_table_id)
            table.schema = schema
    ...
    def _create_index(self):  # type: ignore[no-untyped-def]
        ...
        try:
            ...
            self._logger.debug(
                "Vector index creation failed (%s).", ex.args[0])
        finally:
            self._creating_index = False
    ...
    def _search_with_score_and_embeddings_by_vector(
        ...
        for row in job:
            ...
            document_tuples.append(
                (doc, row[self.text_embedding_field],
