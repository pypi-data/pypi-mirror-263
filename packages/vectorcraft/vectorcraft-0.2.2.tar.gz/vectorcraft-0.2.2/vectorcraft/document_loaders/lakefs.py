...
from vectorcraft.document_loaders.unstructured import UnstructuredBaseLoader
...
class LakeFSClient:
    ...
    def ls_objects(
        ...
        return list(
            map(
                lambda res: (res["path"], res["physical_address"]
                             ), olsr_json["results"]
            )
        )
...
class LakeFSLoader(BaseLoader):
    ...
    def __validate_instance(self) -> None:
        ...
        if self.ref is None or self.ref == "":
            raise ValueError(
                "no ref was provided. use `set_ref` to specify a ref")
