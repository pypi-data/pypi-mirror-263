...
from vectorcraft.document_loaders.base import BaseLoader
...
class ConfluenceLoader(BaseLoader):
    ...
    Example:
        .. code-block:: python
            ...
            from vectorcraft.document_loaders import ConfluenceLoader
        ...
        *,
        ...
        if session:
            self.confluence = Confluence(
                url=url, session=session, **confluence_kwargs)
        elif oauth2:
            self.confluence = Confluence(
    ...
    def _lazy_load(self, **kwargs: Any) -> Iterator[Document]:
        ...
        include_attachments = self._resolve_param(
            "include_attachments", kwargs)
        ...
        keep_markdown_format = self._resolve_param(
            "keep_markdown_format", kwargs)
    ...
    def is_public_page(self, page: dict) -> bool:
        ...
        restrictions = self.confluence.get_all_restrictions_for_content(
            page["id"])
    ...
    def process_page(
        ...
        if include_attachments:
            attachment_texts = self.process_attachment(
                page["id"], ocr_languages)
        else:
            attachment_texts = []
        ...
        if keep_markdown_format:
            ...
            text = markdownify(content, heading_style="ATX") + \
        ...
        for attachment in attachments:
            ...
            try:
                if media_type == "application/pdf":
                    text = title + \
                        self.process_pdf(absolute_url, ocr_languages)
                elif (
                    ...
                    text = title + \
                        self.process_image(absolute_url, ocr_languages)
                elif (
                    ...
                    text = title + \
                        self.process_svg(absolute_url, ocr_languages)
                else:
                    continue
                texts.append(text)
