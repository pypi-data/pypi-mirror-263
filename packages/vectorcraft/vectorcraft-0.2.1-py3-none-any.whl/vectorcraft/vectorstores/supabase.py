...
class SupabaseVectorStore(VectorStore):
    ...
    .. code-block:: python
        ...
        from langchain_core.documents import Document
        from vectorcraft.vectorstores import SupabaseVectorStore
    ...
    def _add_vectors(
        ...
        for i in range(0, len(rows), chunk_size):
            ...
            result = client.from_(table_name).upsert(
                chunk).execute()  # type: ignore
    ...
    def delete(self, ids: Optional[List[str]] = None, **kwargs: Any) -> None:
        ...
        for row in rows:
            self._client.from_(self.table_name).delete().eq(
                "id", row["id"]).execute()
