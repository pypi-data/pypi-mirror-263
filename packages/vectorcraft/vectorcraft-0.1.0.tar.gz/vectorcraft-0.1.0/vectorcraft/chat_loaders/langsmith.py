...
from vectorcraft.chat_loaders.base import BaseChatLoader
...
class LangSmithRunChatLoader(BaseChatLoader):
    ...
    def _load_single_chat_session(self, llm_run: "Run") -> ChatSession:
        ...
        """
        chat_session = LangSmithRunChatLoader._get_messages_from_llm_run(
            llm_run)
    ...
    def _get_messages_from_llm_run(llm_run: "Run") -> ChatSession:
        ...
        if llm_run.run_type != "llm":
            raise ValueError(
                f"Expected run of type llm. Got: {llm_run.run_type}")
        if "messages" not in llm_run.inputs:
            raise ValueError(
                f"Run has no 'messages' inputs. Got {llm_run.inputs}")
...
class LangSmithDatasetChatLoader(BaseChatLoader):
    ...
    def lazy_load(self) -> Iterator[ChatSession]:
        ...
        :return: Iterator of chat sessions containing messages.
        """
        from vectorcraft.adapters import openai as oai_adapter  # noqa: E402
