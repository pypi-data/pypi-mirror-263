...
from vectorcraft.callbacks.utils import import_pandas
...
class FiddlerCallbackHandler(BaseCallbackHandler):
    def __init__(
        ...
        self.fiddler_client = self.fdl.FiddlerApi(
            url, org_id=org, auth_token=api_key)
    ...
    def _publish_events(
        ...
        if token_usage_dict:
            for key, value in token_usage_dict.items():
                df[key] = [value] * \
                    prompt_count if isinstance(value, int) else value

        try:
            self.fiddler_client.publish_events_batch(
                self.project, self.model, df)
        except Exception as e:
            print(  # noqa: T201
