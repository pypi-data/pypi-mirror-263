...
class _KdtSuggestContext(BaseModel):
    ...
    def to_system_str(self) -> str:
        ...
        if self.description:
            lines.append(
                f"COMMENT ON TABLE {self.table} IS '{self.description}';")
        ...
        for idx, line in enumerate(lines):
            ...
            if role == "system":
                if system is not None:
                    raise ValueError(
                        f"Only one system token allowed in: {line}")
                ...
                if user_message is None:
                    raise Exception(
                        f"Found assistant token without user token: {line}")
    ...
    * An LLM context that specifies the tables and samples to use for inferencing.
    ...
    Example:
        .. code-block:: python
            ...
            from vectorcraft.chat_models.kinetica import KineticaChatLLM
    ...
    Example:
        .. code-block:: python
            ...
            from vectorcraft.chat_models.kinetica import (
    ...
    def load_messages_from_context(self, context_name: str) -> List:
        ...
        dict_messages.append(
            dict(role="system", content=payload.get_system_str()))
    ...
    def load_messages_from_datafile(cls, sa_datafile: Path) -> List[BaseMessage]:
        ...
        datafile_dict = _KineticaLlmFileContextParser.parse_dialogue_file(
            sa_datafile)
    ...
    def _convert_dict_to_messages(cls, sa_data: Dict) -> List[BaseMessage]:
        ...
        result_list.extend([cls._convert_message_from_dict(m)
                           for m in messages])
        return result_list
