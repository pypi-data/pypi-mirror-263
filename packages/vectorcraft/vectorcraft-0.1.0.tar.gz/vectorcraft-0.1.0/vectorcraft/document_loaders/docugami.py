...
class DocugamiLoader(BaseLoader, BaseModel):
    ...
    def validate_local_or_remote(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if values.get("file_paths") and values.get("docset_id"):
            raise ValueError(
                "Cannot specify both file_paths and remote API docset_id")

        if not values.get("file_paths") and not values.get("docset_id"):
            raise ValueError(
                "Must specify either file_paths or remote API docset_id")

        if values.get("docset_id") and not values.get("access_token"):
            raise ValueError(
                "Must specify access token if using remote API docset_id")
    ...
    def _parse_dgml(
        ...
        for dg_chunk in dg_chunks:
            ...
            if chunk_id:
                ...
                if dg_chunk.parent:
                    framework_parent_chunk = _build_framework_chunk(
                        dg_chunk.parent)
            ...
            if artifact_name == "report-values.xml" and artifact_url and artifact_doc:
                ...
                if response.ok:
                    ...
                    for entry in entries:
                        heading = entry.xpath(
                            "./pr:Heading", namespaces=ns)[0].text
                        value = " ".join(
                            entry.xpath(
                                "./pr:Value", namespaces=ns)[0].itertext()
                        ).strip()
    ...
    def load(self) -> List[Document]:
        ...
        if self.access_token and self.docset_id:
            # Remote mode
            _document_details = self._document_details_for_docset_id(
                self.docset_id)
            ...
            _project_details = self._project_details_for_docset_id(
                self.docset_id)
                ...
                for project in _project_details:
                    ...
                    for file_id in metadata:
                        if file_id not in combined_project_metadata:
                            ...
                            combined_project_metadata[file_id].update(
                                metadata[file_id])
