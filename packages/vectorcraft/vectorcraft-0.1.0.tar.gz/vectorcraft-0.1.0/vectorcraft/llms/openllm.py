...
class OpenLLM(LLM):
    ...
    Example running an LLM model locally managed by OpenLLM:
        .. code-block:: python
            ...
            from vectorcraft.llms import OpenLLM
            llm = OpenLLM(
                model_name='flan-t5',
    ...
    def __init__(
        ...
        **llm_kwargs: Any,
        ...
        if server_url is not None:
            logger.debug(
                "'server_url' is provided, returning a openllm.Client")
            assert (
    ...
    def runner(self) -> openllm.LLMRunner:
        ...
        if self._runner is None:
            raise ValueError(
                "OpenLLM must be initialized locally with 'model_name'")
        return self._runner
        ...
        if self._client is not None:
            ...
            try:
                self.llm_kwargs.update(
                    json.loads(
                        self._runner.identifying_params["configuration"])
                )
