...
class NeuralDBVectorStore(VectorStore):
    ...
    Example:
        .. code-block:: python
            ...
            from vectorcraft.vectorstores import NeuralDBVectorStore
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        temp = tempfile.NamedTemporaryFile(
            "w", delete=False, delete_on_close=False)  # type: ignore[call-overload]
        df.to_csv(temp)
        source_id = self.insert([ndb.CSV(temp.name)], **kwargs)[0]
        offset = self.db._savable_state.documents.get_source_by_id(source_id)[
            1]
