...
class ElasticVectorSearch(VectorStore):
    ...
    Example:
        .. code-block:: python
            ...
            from vectorcraft.vectorstores import ElasticVectorSearch
    ...
    def client_search(
        ...
        if version_num >= 8:
            response = client.search(
                index=index_name, query=script_query, size=size)
        else:
            response = client.search(
...
class ElasticKnnSearch(VectorStore):
    ...
    def knn_search(
        ...
        if not source and (
            fields is None or not any(
                page_content in field for field in fields)
        ):
            raise ValueError(
    ...
    def from_texts(
        ...
        **kwargs: Any,
        ...
        knnvectorsearch.add_texts(
            texts, model_id=model_id, dims=dims, **optional_args)
