...
class TileDB(VectorStore):
    ...
    Example:
        .. code-block:: python
            ...
            from vectorcraft import TileDB
        ...
        *,
        ...
        for idx, score in zip(ids, scores):
            ...
            if doc is None or len(doc["text"]) == 0:
                raise ValueError(
                    f"Could not find document for id {idx}, got {doc}")
            ...
            if pickled_metadata is not None:
                metadata = pickle.loads(
                    np.array(pickled_metadata.tolist()).astype(
                        np.uint8).tobytes()
                )
        ...
        docs_array.close()
        docs = [(doc, score)
                for doc, score in docs if score <= score_threshold]
        return docs[:k]
    ...
    def similarity_search_with_score_by_vector(
        ...
        **kwargs: Any,
        ...
        d, i = self.vector_index.query(
            np.array([np.array(embedding).astype(np.float32)]
                     ).astype(np.float32),
    ...
    def create(
        ...
        *,
        ...
        with tiledb.scope_ctx(ctx_or_config=config):
            ...
            text_attr = tiledb.Attr(
                name="text", dtype=np.dtype("U1"), var=True)
            attrs = [text_attr]
            if metadatas:
                metadata_attr = tiledb.Attr(
                    name="metadata", dtype=np.uint8, var=True)
