...
class CometCallbackHandler(BaseMetadataCallbackHandler, BaseCallbackHandler):
        ...
        try:
            if hasattr(langchain_asset, "save"):
                ...
                self.experiment.log_model(
                    model_name, str(langchain_asset_path))
    ...
    def _log_session(self, langchain_asset: Optional[Any] = None) -> None:
        try:
            llm_session_df = self._create_session_analysis_dataframe(
                langchain_asset)
            # Log the cleaned dataframe as a table
            self.experiment.log_table(
                "langchain-llm-session.csv", llm_session_df)
        except Exception:
            self.comet_ml.LOGGER.warning(
        ...
        try:
            metadata = {"langchain_version": str(vectorcraft.__version__)}
