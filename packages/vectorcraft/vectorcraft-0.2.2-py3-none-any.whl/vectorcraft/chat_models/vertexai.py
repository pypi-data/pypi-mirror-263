...
from vectorcraft.llms.vertexai import (
    _VertexAICommon,
    is_codey_model,
    is_gemini_model,
)
    ...
    for i, message in enumerate(history):
        ...
        if i == 0 and isinstance(message, SystemMessage):
            ...
            vertex_message = ChatMessage(
                content=message.content, author="user")
            vertex_messages.append(vertex_message)
        else:
            raise ValueError(
    ...
    def _convert_to_prompt(part: Union[str, Dict]) -> Part:
        ...
        if part["type"] == "text":
            ...
            if path.startswith("gs://"):
                ...
                encoded: Any = re.search(
                    r"data:image/\w{2,4};base64,(.*)", path)
                if encoded:
