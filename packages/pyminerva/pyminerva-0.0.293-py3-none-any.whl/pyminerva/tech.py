...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    if tech_type in tech_list_osilo:  # 오실로스코프형 그래프
        ...
        downward_true = check_go_downward(_df, tech_type)
        upward_true = check_go_upward(_df, tech_type)
        ...
        if not long_1.empty:
            print(long_1.iloc[-1]['pivot'])
        if not long_2.empty:            
            print(long_2.iloc[-1]['pivot'])
        if not long_3.empty:            
            print(long_3.iloc[-1]['pivot'])
        if not long_4.empty:            
            print(long_4.iloc[-1]['pivot'])

        if not short_1.empty:
            print(short_1.iloc[-1]['pivot'])
        if not short_2.empty:            
            print(short_2.iloc[-1]['pivot'])
        if not short_3.empty:            
            print(short_3.iloc[-1]['pivot'])
        if not short_4.empty:            
            print(short_4.iloc[-1]['pivot'])            
