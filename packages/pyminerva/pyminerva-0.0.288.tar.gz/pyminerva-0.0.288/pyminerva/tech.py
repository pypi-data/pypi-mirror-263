...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    if tech_type in tech_list_osilo:  # 오실로스코프형 그래프
        ...
        _df['pivot'] = np.where(((_df[tech_type][-1:].values < 0) & (check_go_downward(_df, tech_type))), -1, _df['pivot'])

        # Short-분할매수 중단
        # 현재 상태값이 (-)이지만, 최근 3일간의 5일 이동평균값의 각도가 +- 10% 범주내이면 Holding = 0
        _df['pivot'] = np.where(((_df[tech_type][-1:].values < 0) & (not check_go_downward(_df, tech_type))), -2, _df['pivot'])

        # Short-분할매도 시작
        # 현재 상태값이 (-)이면서, 오늘부터 최근 3일간의 5일 이동평균값의 각도가 +10% 이상이면 분할매도 시작
        _df['pivot'] = np.where((_df[tech_type] < 0) & (check_go_upward(_df, tech_type)), -3, _df['pivot'])
        ...
        # Long-여전히 분할매수
        # 현재 상태값이 (+)이면서, 오늘부터 최근 3일간의 5일 이동평균값의 각도가 +10% 이상이면 여전히 분할매수 단계
        _df['pivot'] = np.where(((_df[tech_type][-1:].values > 0) & (check_go_upward(_df, tech_type))), 1, _df['pivot'])

        # Long-분할매수 중단
        # 현재 상태값이 (+)이지만, 최근 3일간의 5일 이동평균값의 각도가 +- 10% 범주내이면 Holding = 0
        _df['pivot'] = np.where(((_df[tech_type][-1:].values > 0) & (not check_go_upward(_df, tech_type))), 2, _df['pivot'])

        # Long-분할매도 시작
        # 현재 상태값이 (+)이면서, 오늘부터 최근 3일간의 5일 이동평균값의 각도가 -10% 이상이면 분할매도 시작
        _df['pivot'] = np.where((_df[tech_type] > 0) & (check_go_downward(_df, tech_type)), 3, _df['pivot'])
        ...
        _1 = _df[_df['pivot'] == 1] # 분할매수 시작
        _2 = _df[_df['pivot'] == 2] # 분할매수 마무리
        _3 = _df[_df['pivot'] == 3] # 분할매도 시작
        _4 = _df[_df['pivot'] == 4] # 분할매도 마무리
