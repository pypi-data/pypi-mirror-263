...
def take_tech_signal_type(tech_type):
...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    for group, techs  in TECH_LIST.items():  # tech+type 의 총 갯수를 확인.
        for tech in techs:
    ...
    signal = take_tech_signal_type(tech_type)
    # print(signal)
    ...
    plt.title(f"{ticker}: {tech_type} / {latest_date}  {decision} ", fontdict={'fontsize':20, 'color':'g'})    
