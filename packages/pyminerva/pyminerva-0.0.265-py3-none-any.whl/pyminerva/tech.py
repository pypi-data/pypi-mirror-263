...
def find_current_slope(df, tech_type):
    result = 0



    try:
        current_slope = base.trend_detector(df, 'SellAndBuy', base.get_working_day_before(5))
    except Exception as e:  # lastest_date 가 짧아 np.polyfit 함수 오류로 인한 수작업 기울기 구하기.
        current_slope = (df[tech_type] - df[tech_type].shift(3)) / 3

    if tech_type in ['kvo', 'adosc', 'pvt', 'obv']:
        print('        ', 'volume: ', current_slope)
    else:
        print('        ', 'slope: ', current_slope)
    result = current_slope
    return result
...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    if tech_type in ['sma','ema','macd','adx','ppo','psar','ichmoku','stoch','roc','ao',
        ...
        '''
        # tech-type 의 그래프의 다시 5일 평균이동값: 미분의 미분값이랄까.... tech-type 의 기울기...
        _df['tech_type_ewm'] = _df[tech_type].ewm(span=5, adjust=False, min_periods=5).mean()
        # 현재 시점의 기울기
        current_slope = find_current_slope(_df, tech_type)

        # Here is Buttom, 분할매수 시작, slope 이 마이너스(-)에서 0 로 전환된 날 리포트 : 2
        _df['pivot'] = np.where((current_slope <= abs(permit_range)) & (_df['tech_type_ewm'].shift(1) < _df['tech_type_ewm']) & \
                                _df['tech_type'][-1:].values < 0, 2, 0)
        # 분할매수 끝, slope 이 플러스(+) 인 상태에서, 이동평균값이 기준선 0 위로 전환된 날 리포트 : 1 
        _df['pivot'] = np.where((current_slope > permit_range) & (_df['tech_type_ewm'].shift(1) < _df['tech_type_ewm']) & \
        ...
        _df['pivot'] = np.where((current_slope < permit_range*(-1)) & (_df['tech_type_ewm'].shift(1) > _df['tech_type_ewm']) & \
                                _df['tech_type'][-1:].values > 0, -1, _df['pivot'])
        ...
        if tech_type == 'bbands':  # 
            ...
            _df['pivot'] = np.where((_df[tech_type] >= _df['BBU_5_2.0']) & (_df[tech_type].shift(1) >= _df['BBU_5_2.0']).shift(1) & \
                                    (current_slope <= abs(permit_range)), -1, _df['pivot'])
            # 분할매수 시작은 Buttom 에서 시작해서 기준선 하단 통과시 매수 마무리
            _df['pivot'] = np.where((_df[tech_type] <= _df['BBL_5_2.0']) & (current_slope <= abs(permit_range)) & \
            ...
            _df['pivot'] = np.where((_df[tech_type] >= _df['DCU_20_20']) & (_df[tech_type].shift(1) >= _df['DCU_20_20']).shift(1) & \
                                    (current_slope <= abs(permit_range)), -1, _df['pivot'])
            # 분할매수 시작은 Buttom 에서 시작해서 기준선 하단 통과시 매수 마무리
            _df['pivot'] = np.where((_df[tech_type] <= _df['DCL_20_20']) & (current_slope <= abs(permit_range)) & \
            ...
            _df['pivot'] = np.where((_df[tech_type] >= _df['KCUe_20_2']) & (_df[tech_type].shift(1) >= _df['KCUe_20_2']).shift(1) & \
                                    (current_slope <= abs(permit_range)), -1, _df['pivot'])
            # 분할매수 시작은 Buttom 에서 시작해서 기준선 하단 통과시 매수 마무리
            _df['pivot'] = np.where((_df[tech_type] <= _df['KCLe_20_2']) & (current_slope <= abs(permit_range)) & \
