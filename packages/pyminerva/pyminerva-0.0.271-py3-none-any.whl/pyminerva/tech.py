...
def find_current_slope_degree(df, tech_type):
    result = 0
    # 3일이동평균의 미분값
    _5ewm = df[tech_type].ewm(span=5, adjust=False, min_periods=5).mean()
    current_slope_degree = np.degrees(np.arctan((_5ewm - _5ewm.shift(3)).diff())[-1:])
    ...
    print('current_slope_degree', current_slope_degree)
    print(df[tech_type][-1:].values)
    print(df[tech_type])
    print(df[tech_type_ewm])
...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    if tech_type in ['sma','ema','macd','adx','ppo','psar','ichmoku','stoch','roc','ao',
        ...
        print('current_slope_degree', current_slope_degree)
        print(_df[tech_type][-1:].values)
        print(_df[tech_type])
        print(_df['tech_type_ewm'])

        # Here is graph Buttom, 분할매수 시작, slope 이 마이너스(-)에서 0 로 전환된 날 리포트 : 2
        ...
        _df['pivot'] = np.where((current_slope_degree > permit_range) & (_df['tech_type_ewm'].shift(3) < _df['tech_type_ewm']) & \
                                _df['tech_type'][-1:].values < 0, 1, _df['pivot'])

        # Here is graph Top, 분할매도 시작, slope 이 플러스(+)에서 0 으로 전환된 날 리포트 : -2
