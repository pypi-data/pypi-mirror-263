...
def find_tech_graph_type(TECH_LIST):
    ...
    if len(find_tech_raw_list) == len(tech_list_osilo) + len(tech_list_band):
        pass
    else:
        base.logger2.warning(f'TECH_LIST 와 tech_graph_type is unmatched.')
...
def find_current_slope_degree(df, tech_type):
    ...
    if tech_type in ['kvo', 'adosc', 'pvt', 'obv']:
        print('        ', 'volume: ', current_slope_degree)
    else:
        print('        ', 'slope: ', current_slope_degree)
    result = current_slope_degree
...
def check_top(df, tech_type):
    ...
    print(ma_previous_close)
    print(df_previous_volume)
...
def find_decision_msg(tech_type, _1, _2, _3, _4):
    ...
    if _1.empty & _2.empty & _3.empty & _4.empty:
        ...
        # latest date 가 _1, _2, _3, _4 중 하나에서 발견되었다면, 분할매수 시작/마무리, 분할매도 시작/마무리 단계중이란 의미.
        try:
            if latest_date == _1.index[-1]:  # 분할매수 시작
                decision = 2
                if tech_howto == 'trend':
                    decision_msg = '추세상승, 분할매수 시작'
                elif tech_howto == 'overBoughtSold':
                    decision_msg = '과매도, 분할매수 시작'
                elif tech_howto == 'trendReversal':
                    decision_msg = '상승반전, 분할매수 시작'
                elif tech_howto == 'kReversal':
                    decision_msg = 'k 상승반전 분할매수 시작'
                else:
        ...
        try:    
            if latest_date == _2.index[-1]:  # 분할매수 마무리
                decision = 1
                if tech_howto == 'trend':
                    decision_msg = '상승추세 유지하지만 분할매수 마무리 '
                elif tech_howto == 'overBoughtSold':
                    decision_msg = '적정매도매수 분할매수 마무리'
                elif tech_howto == 'trendReversal':
                    decision_msg = '반전없음, 분할매수 마무리'
                elif tech_howto == 'kReversal':
                    decision_msg = 'k 반전없음, 분할매수 마무리'
                else:
                    decision_msg = '분할매수 마무리'
        except IndexError as e:
            pass
        ...
        try:
            if latest_date == _3.index[-1]:  # 분할매도 시작
                decision = -2
                if tech_howto == 'trend':
                    decision_msg = '추세하락, 분할매도 시작'
                elif tech_howto == 'overBoughtSold':
                    decision_msg = '과매수, 분할매도 시작'
                elif tech_howto == 'trendReversal':
                    decision_msg = '하락반정, 분할매도 시작'
                elif tech_howto == 'kReversal':
                    decision_msg = 'k 하락반전 분할매도 시작'
                else:
        ...
        try:
            if latest_date == _4.index[-1]:  # 분할매도 마무리
                decision = -1
                if tech_howto == 'trend':
                    decision_msg = '하락추세 유지하지만 분할매도 마무리'
                elif tech_howto == 'overBoughtSold':
                    decision_msg = '적정매수매도 분할매도 마무리'
                elif tech_howto == 'trendReversal':
                    decision_msg = '반전없음, 분할매도 마무리'
                elif tech_howto == 'kReversal':
                    decision_msg = 'k 반전없음, 분할매도 마무리'            
                else:
                    decision_msg = '분할매도 마무리'
        except IndexError as e:
            pass
...
def get_tech_yf_hist(ticker, window):
    ...
    try:
        df_hist['feature'] = signal.detrend(df_hist['Close'])  # 통계 정확성을 위한 주기성 시그널 제거용
            ...
            time.sleep(3.1)
            base.logger2.error(f'{ticker} get_tech_yf_hist Value error 1: {e}')  # get_tech_yf_hist Value error 1: array must not contain infs or NaNs: 채권에서처럼 detrend 를 만들수 없는 오류
            df_hist = obj_yf_ticker.history(period='36mo') # test: 10mo, real: 36mo
...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    if tech_type in tech_list_osilo:  # 오실로스코프형 그래프
        ...
        # 이전에 널리 알려져있는 신호포착 방법
          _df['pivot'] = np.where((_df[tech_type] > base_limit) & (_df[tech_type].shift(3) <= base_limit), 1, 0)  # Buy signal
          _df['pivot'] = np.where((_df[tech_type] < base_limit) & (_df[tech_type].shift(3) >= base_limit), -1, _df['pivot']) # Sell signal

        # 새로이 판단하고 있는 매수매도 신호포착 방법
        ...
        # 현재 시점의 기울기 각도
        # current_slope_degree = find_current_slope_degree(_df, tech_type)

        # 분할매수 시작, Here is graph Buttom, slope 이 마이너스(-)에서 0 로 전환된 날 리포트 : 2
        # slope dgree 가 0 허용치안에 포함되고, 5일 이동평균값의 3일 이전값이 오늘값보다 크고(하향추세),오늘의 값이 마이너스(-) 상태일때
        _df['pivot'] = np.where((_df[tech_type][-1:].values < 0) & (_df[tech_type].shift(1) > 0 & _df[tech_type] < 0) and check_buttom(_df, tech_type), 2, 0)
        # _df['pivot'] = np.where(check_buttom(_df, tech_type) and ((_df['tech_type_ewm'].shift(3) > _df['tech_type_ewm']) & _df[tech_type][-1:].values < 0), 2, 0)
        # 분할매수 끝, slope dgree 가 플러스(+) 인 상태에서, 5일 이동평균값의 3일 이전값이 오늘값보다 작고(상향추세), 오늘의 값이 기준선 0 위로 전환된 날 리포트 : 1 
        _df['pivot'] = np.where((_df[tech_type] > 0) & (_df[tech_type].shift(1) < 0 ), 1, _df['pivot'])
        # _df['pivot'] = np.where((_df[tech_type] > 0) & (_df[tech_type] > 0) & (_df[tech_type].shift(1) < 0 ), 1, _df['pivot'])
        # 분할매도 시작, Here is graph Top, slope 이 플러스(+)에서 0 으로 전환된 날 리포트 : -2
        # slope dgree 가 0 허용치안에 포함되고, 5일 이동평균값의 3일 이전값이 오늘값보다 작고(상향추세),오늘의 값이 플러스(+) 상태일때
        _df['pivot'] = np.where(_df[tech_type][-1:].values > 0 & (_df[tech_type].shift(1) < 0 & _df[tech_type] > 0) and check_top(_df, tech_type) > 0, -2, _df['pivot'])
        # _df['pivot'] = np.where(check_top(_df, tech_type) and ((_df['tech_type_ewm'].shift(3) < _df['tech_type_ewm']) & _df[tech_type][-1:].values > 0), -2, _df['pivot'])
        # 분할매도 끝, slope dgree 가 마이너스(-) 인 상태에서, 5일 이동평균값의 3일 이전값이 오늘값보다 크고(하향추세), 오늘의 값이 기준선 0 아래로 전환되는 상태
        _df['pivot'] = np.where((_df[tech_type] < 0) & (_df[tech_type].shift(1) > 0), -1, _df['pivot'])
        # _df['pivot'] = np.where((_df[tech_type] < 0) & (_df[tech_type].shift(1) > 0) & (_df[tech_type] < 0), -1, _df['pivot'])
        ...
        if tech_type == 'bbands':  # 
            ...
            _df['pivot'] = np.where((_df[tech_type] >= _df['BBU_5_2.0']) & (_df[tech_type].shift(3) <= _df['BBU_5_2.0']).shift(3), -2, 0)
            # 분할매도 마무리
            _df['pivot'] = np.where((_df[tech_type] >= _df['BBU_5_2.0']) & (_df[tech_type].shift(3) >= _df['BBU_5_2.0']).shift(3) & \
                                    (current_slope_degree <= abs(permit_range)), -1, _df['pivot'])
            ...
            _df['pivot'] = np.where((_df[tech_type] <= _df['BBL_5_2.0']) & (current_slope_degree <= abs(permit_range)) & \
                                    (_df[tech_type].shift(3) <= _df['BBL_5_2.0']).shift(3), 2, _df['pivot'])
            # 분할매수 마무리
            _df['pivot'] = np.where((_df[tech_type] <= _df['BBL_5_2.0']) & (_df[tech_type].shift(3) <= _df['BBL_5_2.0']).shift(3), 1, _df['pivot'])            
            ...
            # print(_df)
            # 분할매도는 상단기준선 넘으면서 바로 시작 (Not top) 해서 top 에서 매도 마무리
            _df['pivot'] = np.where((_df[tech_type] >= _df['DCU_20_20']) & (_df[tech_type].shift(3) <= _df['DCU_20_20']).shift(3), -2, 0)
            # 분할매도 마무리
            ...
            _df['pivot'] = np.where((_df[tech_type] <= _df['DCL_20_20']) & (current_slope_degree <= abs(permit_range)) & \
                                    (_df[tech_type].shift(3) <= _df['DCL_20_20']).shift(3), 2, _df['pivot'])
            # 분할매수 마무리
            _df['pivot'] = np.where((_df[tech_type] <= _df['DCL_20_20']) & (_df[tech_type].shift(3) <= _df['DCL_20_20']).shift(3), 1, _df['pivot'])  
        elif tech_type == 'kc':  # 
            plt.plot(_df.index, _df['KCUe_20_2'], color='orange')  # Upper line
            plt.plot(_df.index, _df['KCLe_20_2'], color='orange')  # Lower line
            # print(_df)
            # 분할매도는 상단기준선 넘으면서 바로 시작 (Not top) 해서 top 에서 매도 마무리
            _df['pivot'] = np.where((_df[tech_type] >= _df['KCUe_20_2']) & (_df[tech_type].shift(3) <= _df['KCUe_20_2']).shift(3), -2, 0)
            # 분할매도 마무리
            ...
            _df['pivot'] = np.where((_df[tech_type] <= _df['KCLe_20_2']) & (current_slope_degree <= abs(permit_range)) & \
                                    (_df[tech_type].shift(3) <= _df['KCLe_20_2']).shift(3), 2, _df['pivot'])
            # 분할매수 마무리
            _df['pivot'] = np.where((_df[tech_type] <= _df['KCLe_20_2']) & (_df[tech_type].shift(3) <= _df['KCLe_20_2']).shift(3), 1, _df['pivot'])  
        else:
            ...
            _df['pivot'] = np.where((current_slope_degree <= abs(permit_range)) & (_df['tech_type_ewm'].shift(3) < _df['tech_type_ewm']) & \
                                    _df[tech_type] < 0, 2, 0)
            # 분할매수 끝, slope 이 플러스(+) 인 상태에서, 이동평균값이 기준선 0 위로 전환된 날 리포트 : 1 
            _df['pivot'] = np.where((current_slope_degree > permit_range) & (_df['tech_type_ewm'].shift(3) < _df['tech_type_ewm']) & \
                                    _df[tech_type] < 0, 1, _df['pivot'])

            # Here is Top, 분할매도 시작, slope 이 플러스(+)에서 0 으로 전환된 날 리포트 : -2
            _df['pivot'] = np.where((current_slope_degree <= abs(permit_range)) & (_df['tech_type_ewm'].shift(3) > _df['tech_type_ewm']) & \
                                    _df[tech_type] > 0, -2, _df['pivot'])
            # 분할매도 끝, slope 이 마이너스(-)인 상태에서 , 이동평균값이 기준선 0 아래로 전환된 날 리포트: -1
            _df['pivot'] = np.where((current_slope_degree < permit_range*(-1)) & (_df['tech_type_ewm'].shift(3) > _df['tech_type_ewm']) & \
                                    _df[tech_type] > 0, -1, _df['pivot'])


            _df['pivot'] = np.where((_df[tech_type] >= h_limit) & (_df[tech_type].shift(3) <= h_limit), 1, 0)  # Sell signal
            _df['pivot'] = np.where((_df[tech_type] <= l_limit) & (_df[tech_type].shift(3) >= l_limit), -1, _df['pivot']) # Buy signal
        ...
        base.logger2.warning(f'{tech_type} is not found in tech_graph_type')


    decision, decision_msg, latest_date = find_decision_msg(tech_type, _1, _2, _3, _4)
