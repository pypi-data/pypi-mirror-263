...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    if tech_type in ['sma','ema','macd','adx','ppo','psar','ichmoku','stoch','roc','ao',
        ...
        _df['pivot'] = np.where(check_buttom(_df) and ((_df['tech_type_ewm'].shift(3) > _df['tech_type_ewm']) & _df[tech_type][-1:].values < 0), 2, 0)
        ...
        _df['pivot'] = np.where(check_top(_df) and ((_df['tech_type_ewm'].shift(3) < _df['tech_type_ewm']) & _df[tech_type][-1:].values > 0), -2, _df['pivot'])
