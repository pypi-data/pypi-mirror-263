...
def find_decision_long_msg(tech_type, long_1, long_2, long_3, long_4):
    ...
    if long_1.empty & long_2.empty & long_3.empty & long_4.empty:
        ...
        try:
            ...
            base.logger2.error(f'Decision 1 index error.')
            pass
        ...
        try:    
            ...
            base.logger2.error(f'Decision 2 index error.')            
            pass
        ...
        try:
            if latest_date == long_3.index[-1]:  # 분할매도 시작
                ...
                if tech_howto == 'trend':
                    ...
                    decision_msg = '분할매도 시작, 하락반정'
                elif tech_howto == 'kReversal':
                    decision_msg = '분할매도 시작, k 하락반전'
                else:
                    decision_msg = '분할매도 시작'
        except IndexError as e:
            base.logger2.error(f'Decision 3 index error.')            
            pass
        ...
        try:
            ...
            base.logger2.error(f'Decision 4 index error.')            
            pass
...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    if (latest_date == short_1.index[-1]) or (latest_date == short_2.index[-1]) or\
    (latest_date == short_3.index[-1]) or (latest_date == short_4.index[-1]):
