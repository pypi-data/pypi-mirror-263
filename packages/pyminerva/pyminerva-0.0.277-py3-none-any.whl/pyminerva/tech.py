...
def check_top(df):
    ...
    increase_degree = np.degrees(np.arctan((ma_1[after_5_days][-1] - ma_1[after_5_days][0])))
...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    if tech_type in ['sma','ema','macd','adx','ppo','psar','ichmoku','stoch','roc','ao',
        ...
        _df['pivot'] = np.where(check_buttom(_df) and (_df['tech_type_ewm'].shift(3) > _df['tech_type_ewm']) and _df['tech_type'][-1:].values < 0, 2, 0)
        # 분할매수 끝, slope dgree 가 플러스(+) 인 상태에서, 5일 이동평균값의 3일 이전값이 오늘값보다 작고(상향추세), 오늘의 값이 기준선 0 위로 전환된 날 리포트 : 1 
        _df['pivot'] = np.where((_df['tech_type'] > 0) and (_df['tech_type'] > 0) and (_df['tech_type'].shift(1) < 0 ), 1, _df['pivot'])
        ...
        _df['pivot'] = np.where(check_top(_df) and (_df['tech_type_ewm'].shift(3) < _df['tech_type_ewm']) and _df['tech_type'][-1:].values > 0, -2, _df['pivot'])
        # 분할매도 끝, slope dgree 가 마이너스(-) 인 상태에서, 5일 이동평균값의 3일 이전값이 오늘값보다 크고(하향추세), 오늘의 값이 기준선 0 아래로 전환되는 상태
        _df['pivot'] = np.where((_df['tech_type'] < 0) and (_df['tech_type'].shift(1) > 0) and (_df['tech_type'] < 0), -1, _df['pivot'])
