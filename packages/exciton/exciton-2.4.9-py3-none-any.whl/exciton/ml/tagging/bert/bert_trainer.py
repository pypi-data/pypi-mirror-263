...
from .bert import Tagging_Model_BERT


class Tagging_Trainer(Tagging_Model_BERT, End2EndBase):
    """Tagging trainer

    Args:
        n_labels (str): number of labels.
    ...
    def __init__(
        ...
        Tagging_Model_BERT.__init__(
            self, n_labels=n_labels, drop_rate=drop_rate, device=device
        )
    ...
    def _build_batch(self, batch_raw: List[Any]):
        ...
        """
        Tagging_Model_BERT._build_batch(self, batch_raw=batch_raw)
        labels_arr = []
    ...
    def _build_pipeline(self):
        ...
        logits = logits.contiguous().view(-1, self.n_labels)
        loss = self.loss_criterion(logits, self.batch_data["labels"].view(-1))
        return loss

    def build_modules(self):
        Tagging_Model_BERT.build_modules(self)
