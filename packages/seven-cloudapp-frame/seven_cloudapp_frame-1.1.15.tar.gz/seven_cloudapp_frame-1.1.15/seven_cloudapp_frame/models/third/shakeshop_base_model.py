...
@LastEditTime: 2024-03-21 14:09:15
...
class ShakeShopBaseModel():
    ...
    def get_user_order_list(self, open_id, size=100, page=0, order_status=None, create_time_start=None, create_time_end=None, open_id_type="douyin", is_log=False):
        ...
        try:
            ...
            params["create_time_start"] = create_time_start if create_time_start else TimeHelper.get_now_timestamp() - 90 * 24 * 60 * 60
            params["create_time_end"] = create_time_end if create_time_end else TimeHelper.get_now_timestamp()
            ...
            while has_next:
                ...
                if response and response.get("code") == 10000 and "data" in response:
                    response_data = SevenHelper.json_loads(response)
                    all_order.append(response_data["data"]["shop_order_list"])
    ...
    def product_search_list(self, page=0, size=100, name="", product_id=[], status=-1, product_type=-1, custom_params={}, field="product_id,name,img,status,product_type,outer_product_id", is_log=False):
        """
