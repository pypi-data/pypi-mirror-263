...
class FCPaint(object):
	...
	def beginPaint(self, rect, pRect):
		...
		if self.gdiPlusPaint != None:
			...
			self.drawHDC = win32gui.CreateCompatibleDC(self.hdc)
			win32gui.SetBkMode(self.drawHDC, TRANSPARENT)
			win32gui.SetGraphicsMode(self.drawHDC, GM_ADVANCED)
			self.memBM = win32gui.CreateCompatibleBitmap(self.hdc, int(rect.right - rect.left),  int(rect.bottom - rect.top))
			win32gui.SelectObject(self.drawHDC, self.memBM)
			self.moveTo = False
			self.innerHDC = self.drawHDC
			self.innerBM = self.memBM
		self.offsetX = 0
		self.offsetY = 0
	#绘制线
	#color:颜色 
	...
	def drawLine(self, color, width, style, x1, y1, x2, y2):
		if self.gdiPlusPaint != None:
			...
			hOldPen = win32gui.SelectObject(self.innerHDC, hPen)
			win32gui.MoveToEx(self.innerHDC, int((x1 + self.offsetX) * self.scaleFactorX), int((y1 + self.offsetY) * self.scaleFactorY))
			win32gui.LineTo(self.innerHDC, int((x2 + self.offsetX) * self.scaleFactorX), int((y2 + self.offsetY) * self.scaleFactorY))
			win32gui.SelectObject(self.innerHDC, hOldPen)
		...
		if len(apt) > 1:
			if self.gdiPlusPaint != None:
				...
				for i in range(0,len(apt)):
					...
					apt[i] = (int(x), int(y))
				win32gui.Polyline(self.innerHDC, apt)
				win32gui.SelectObject(self.innerHDC, hOldPen)
				win32gui.DeleteObject(hPen)
	#绘制多边形
	#color:颜色 
			...
			if self.gdiPlusPaint != None:
				...
				for i in range(0,len(apt)):
					...
					if self.scaleFactorX != 1 or self.scaleFactorY != 1:
						...
						y = self.scaleFactorY * y
					if i == 0:
						win32gui.MoveToEx(self.innerHDC, int(x), int(y))
					else:
						win32gui.LineTo(self.innerHDC, int(x), int(y))
					if i == len(apt) - 1:
						fx,fy = apt[0]
						win32gui.LineTo(self.innerHDC, int(fx), int(fy))
				win32gui.SelectObject(self.innerHDC, hOldPen)
				win32gui.DeleteObject(hPen)
	#绘制矩形 
	#color:颜色 
	...
	def drawRect(self, color, width, style, left, top, right, bottom):
		if self.gdiPlusPaint != None:
			...
			hPen = win32gui.CreatePen(PS_SOLID, int(wd), toColor(color)) 
			hOldPen = win32gui.SelectObject(self.innerHDC, hPen)
			win32gui.MoveToEx(self.innerHDC, int((left + self.offsetX) * self.scaleFactorX), int((top + self.offsetY) * self.scaleFactorY))
			win32gui.LineTo(self.innerHDC, int((right + self.offsetX) * self.scaleFactorX), int((top + self.offsetY) * self.scaleFactorY))
			win32gui.LineTo(self.innerHDC, int((right + self.offsetX) * self.scaleFactorX), int((bottom + self.offsetY) * self.scaleFactorY))
			win32gui.LineTo(self.innerHDC, int((left + self.offsetX) * self.scaleFactorX), int((bottom + self.offsetY) * self.scaleFactorY))
			win32gui.LineTo(self.innerHDC, int((left + self.offsetX) * self.scaleFactorX), int((top + self.offsetY) * self.scaleFactorY))
			#win32gui.Rect(self.drawHDC, int((left + self.offsetX) * self.scaleFactorX), int((top + self.offsetY) * self.scaleFactorY), int((right + self.offsetX) * self.scaleFactorX), int((bottom + self.offsetY) * self.scaleFactorY))
			win32gui.SelectObject(self.innerHDC, hOldPen)
			win32gui.DeleteObject(hPen)
	#绘制矩形 
	#color:颜色 
	...
	def drawEllipse(self, color, width, style, left, top, right, bottom):
		if self.gdiPlusPaint != None:
			...
			hPen = win32gui.CreatePen(PS_SOLID, int(wd), toColor(color)) 
			hOldPen = win32gui.SelectObject(self.innerHDC, hPen)
			xLeft = int((left + self.offsetX) * self.scaleFactorX)
			yTop = int((top + self.offsetY) * self.scaleFactorY)
			xRight = int((right + self.offsetX) * self.scaleFactorX)
			yBottom = int((bottom + self.offsetY) * self.scaleFactorY)
			xStart = xLeft
			yStart = int(yTop + (yBottom - yTop) / 2)
			xEnd = xLeft
			yEnd = yStart
			if xLeft == xRight or yTop == yBottom:
				win32gui.MoveToEx(self.innerHDC, int((xLeft + self.offsetX) * self.scaleFactorX), int((yTop + self.offsetY) * self.scaleFactorY))
				win32gui.LineTo(self.innerHDC, int((xRight + self.offsetX) * self.scaleFactorX), int((yBottom + self.offsetY) * self.scaleFactorY))
			else:
				win32gui.Arc(self.innerHDC, xLeft, yTop, xRight, yBottom, xStart, yStart, xEnd, yEnd)
			win32gui.SelectObject(self.innerHDC, hOldPen)
			win32gui.DeleteObject(hPen)
	#绘制文字大小 
	#text:文字 
	...
	def drawText(self, text, color, font, x, y):
		if self.gdiPlusPaint != None:
			...
			fontSize = float(font.split(",")[1]) + 7
			if self.hOldFont != None:
				win32gui.SelectObject(self.innerHDC, self.hOldFont)
				self.hOldFont = None
			lf = win32gui.LOGFONT()
			lf.lfFaceName = self.systemFont
			self.fontSize = fontSize
			lf.lfHeight = int(self.fontSize)
			#lf.lfWeight = 700
			self.hFont = win32gui.CreateFontIndirect(lf)
			self.hOldFont = win32gui.SelectObject(self.innerHDC, self.hFont)
			win32gui.SelectObject(self.innerHDC, self.hFont)
			win32gui.SetTextColor(self.innerHDC, toColor(color))
			fontSize = self.textSize(text,font)
			pyRect = (int((x + self.offsetX) * self.scaleFactorX), int((y + self.offsetY) * self.scaleFactorY), int((x + fontSize.cx + self.offsetX) * self.scaleFactorX), int((y + fontSize.cy + self.offsetY) * self.scaleFactorY))
			win32gui.DrawText(self.innerHDC, text, len(text), pyRect, DT_NOPREFIX|DT_WORD_ELLIPSIS|0)
	#结束绘图
	def endPaint(self):
		if self.gdiPlusPaint != None:
			...
			if self.clipRect != None:
				win32gui.BitBlt(self.hdc, int(self.clipRect.left * self.scaleFactorX), int(self.clipRect.top * self.scaleFactorY), int((self.clipRect.right - self.clipRect.left) * self.scaleFactorX), int((self.clipRect.bottom - self.clipRect.top) * self.scaleFactorY), self.drawHDC, int(self.clipRect.left * self.scaleFactorX), int(self.clipRect.top * self.scaleFactorY), SRCCOPY)
			else:
				win32gui.BitBlt(self.hdc, 0, 0, self.size.cx, self.size.cy, self.drawHDC, 0, 0, SRCCOPY)
			if self.drawHDC != None:
				win32gui.DeleteDC(self.drawHDC)
				self.drawHDC = None
			if self.memBM != None:
				win32gui.DeleteObject(self.memBM)
				self.memBM = None
	#填充矩形 
	#color:颜色
	#left:左侧坐标 
	...
	def fillRect(self, color, left, top, right, bottom):
		if self.gdiPlusPaint != None:
			...
			win32gui.SelectObject(self.innerHDC, brush)
			pyRect = (int((left + self.offsetX) * self.scaleFactorX), int((top + self.offsetY) * self.scaleFactorY), int((right + self.offsetX + 1) * self.scaleFactorX), int((bottom + self.offsetY + 1) * self.scaleFactorY))
			win32gui.FillRect(self.innerHDC, pyRect, brush)
		...
		if len(apt) > 1:
			if self.gdiPlusPaint != None:
				...
				for i in range(0,len(apt)):
					...
					if self.scaleFactorX != 1 or self.scaleFactorY != 1:
						x = self.scaleFactorX * x
						y = self.caleFactorY * y
					apt[i] = (int(x), int(y))
				win32gui.Polygon(self.innerHDC, apt)
				win32gui.DeleteObject(brush)
	#填充椭圆 
	#color:颜色
	#left:左侧坐标 
	...
	def fillEllipse(self, color, left, top, right, bottom):
		if self.gdiPlusPaint != None:
			...
			brush = win32gui.CreateSolidBrush(toColor(color))
			win32gui.SelectObject(self.innerHDC, brush)
			pyRect = (int((left + self.offsetX) * self.scaleFactorX), int((top + self.offsetY) * self.scaleFactorY), int((right + self.offsetX) * self.scaleFactorX), int((bottom + self.offsetY) * self.scaleFactorY))
			win32gui.Ellipse(self.innerHDC, int((left + self.offsetX) * self.scaleFactorX), int((top + self.offsetY) * self.scaleFactorY), int((right + self.offsetX) * self.scaleFactorX), int((bottom + self.offsetY) * self.scaleFactorY))
			win32gui.DeleteObject(brush)
	#填充饼图
	#color:颜色
	#left:左侧坐标 
	...
	def fillPie(self, color, left, top, right, bottom, startAngle, sweepAngle):
		if self.gdiPlusPaint != None:
			...
			win32gui.Pie(self.innerHDC, int((left + self.offsetX) * self.scaleFactorX), int((top + self.offsetY) * self.scaleFactorY), int((right + self.offsetX) * self.scaleFactorX), int((bottom + self.offsetY) * self.scaleFactorY), int((x1 + self.offsetX) * self.scaleFactorX), int((y1 + self.offsetY) * self.scaleFactorY), int((x2 + self.offsetX) * self.scaleFactorX), int((y2 + self.offsetY) * self.scaleFactorY))
			win32gui.DeleteObject(brush)
	#设置偏移量
	#offsetX:横向偏移 
	#offsetY:纵向偏移
	...
	def textSize(self, text, font):
		if self.gdiPlusPaint != None:
			...
			self.hOldFont = win32gui.SelectObject(self.innerHDC, self.hFont)
			win32gui.SelectObject(self.innerHDC, self.hFont)	
			cx, cy = win32gui.GetTextExtentPoint32(self.innerHDC, text)
			return FCSize(cx,cy)
	#绘制矩形 
	#text文字 
	#color:颜色 
	...
	def drawTextAutoEllipsis(self, text, color, font, left, top, right, bottom):
		if self.gdiPlusPaint != None:
			...
			win32gui.SetTextColor(self.innerHDC, toColor(color))
			fontSize = self.textSize(text,font)
			pyRect = (int((left + self.offsetX) * self.scaleFactorX), int((top + self.offsetY) * self.scaleFactorY), int((right + fontSize.cx + self.offsetX) * self.scaleFactorX), int((bottom + fontSize.cy + self.offsetY) * self.scaleFactorY))
			win32gui.DrawText(self.innerHDC, text, len(text), pyRect, DT_NOPREFIX|DT_WORD_ELLIPSIS|0)
	#设置裁剪
	#left:左侧坐标 
	#top:上方坐标 
	...
	def beginClip(self, rect):
		if self.gdiPlusPaint == None:
			self.innerHDC = win32gui.CreateCompatibleDC(self.drawHDC)
			win32gui.SetGraphicsMode(self.innerHDC, GM_ADVANCED)
			win32gui.SetBkMode(self.innerHDC, TRANSPARENT)
			self.offsetX = 0
			self.offsetY = 0
			self.innerBM = win32gui.CreateCompatibleBitmap(self.drawHDC, int((rect.right - rect.left) * self.scaleFactorX),  int((rect.bottom - rect.top) * self.scaleFactorY))
			win32gui.SelectObject(self.innerHDC, self.innerBM)
	...
	def endClip(self, rect, clipRect):	
		if self.gdiPlusPaint == None:
			...
			if self.hFont != None:
				...
				self.hFont = None
			win32gui.StretchBlt(self.drawHDC, int(clipRect.left * self.scaleFactorX), int(clipRect.top * self.scaleFactorY), int((clipRect.right - clipRect.left) * self.scaleFactorX), int((clipRect.bottom - clipRect.top) * self.scaleFactorY), self.innerHDC, int((clipRect.left - rect.left) * self.scaleFactorX), int((clipRect.top - rect.top) * self.scaleFactorY), int((clipRect.right - clipRect.left) * self.scaleFactorX), int((clipRect.bottom - clipRect.top) * self.scaleFactorY), 13369376)
			if self.innerHDC != None:
				win32gui.DeleteObject(self.innerHDC)
				self.innerHDC = None
			if self.innerBM != None:
				win32gui.DeleteObject(self.innerBM)
				self.innerBM = None
			self.innerHDC = self.drawHDC
			self.innerBM = self.memBM
...
class GdiPlusPaint(object):
	...
	def setAttribute(self, name, atrName, atrValue):
		return self.gdiPlus.setAttribute(self.gID, c_char_p(name.encode(self.encoding)), c_char_p(atrName.encode(self.encoding)), c_char_p(atrValue.encode(self.encoding)))
	#获取属性
	#name 名称
	#atrName 属性名称
	#data 返回数据 create_string_buffer(1024000)
	def getAttribute(self, name, atrName, data):
		return self.gdiPlus.getAttribute(self.gID, c_char_p(name.encode(self.encoding)), c_char_p(atrName.encode(self.encoding)), data)
	#获取属性
	#name 名称
    #left 左侧坐标
    #top 顶部左标
	...
	#name 名称
	#number 分片数量
	def initKv(self, path, name, number):
		return self.gdiPlus.initKv(c_char_p(path.encode(self.encoding)), c_char_p(name.encode(self.encoding)), c_int(number))
	#初始化内存库，单线程模式
	#path 路径
	#name 名称
	#number 分片数量
	def initKv2(self, path, name, number):
		return self.gdiPlus.initKv2(c_char_p(path.encode(self.encoding)), c_char_p(name.encode(self.encoding)), c_int(number))
	#关闭内存库
	#kID ID
	def closeKv(self, kID):
		return self.gdiPlus.closeKv(kID)
	#关闭所有内存库
	def closeAllKvs(self):
		return self.gdiPlus.closeAllKvs()
	#获取内存数据流
	#kID ID
	#key 键
	def getKv(self, kID, key):
		recvData = create_string_buffer(102400)
		self.gdiPlus.getKv(kID, c_char_p(key.encode(self.encoding)), recvData)
		return str(recvData.value, encoding=self.encoding)
	#保存键值
	#kID ID
	#key 键
	#value 值
	def putKv(self, kID, key, value):
		return self.gdiPlus.putKv(kID, c_char_p(key.encode(self.encoding)), c_char_p(value.encode(self.encoding)))
	#移除键值
	#kID ID
	#key 键
	def removeKv(self, kID, key):
		return self.gdiPlus.removeKv(kID, key)
	#设置光标
	#cursor 光标
	def setCursor(self, cursor):
		return self.gdiPlus.setCursor(self.gID, c_char_p(cursor.encode(self.encoding)))
	#移除原生视图
	#name 名称
	def removeView(self, name):
		return self.gdiPlus.removeView(self.gID, c_char_p(name.encode(self.encoding)))
...
class FCChart(FCView):
	def __init__(self):
		...
		self.showCrossLine = True #是否显示十字线
		...
		self.vScaleType = "standard" #纵轴的类型 log10代表指数坐标
...
def clearViewInputs(views):
	for i in range(0, len(views)):
		view = views[i]
		if view.exView:
			if view.paint.useGdiPlus and len(view.viewName) > 0:
				view.exView = False
					...
					if gridColumn.visible:
						if gridColumn.frozen == False:
							...
							if cRect.right >= 0 and cRect.left < grid.size.cx:
								if grid.paint.onPaintGridCell != None:
...
def drawGridScrollBar(grid, paint, clipRect):
	...
	if grid.headerHeight > 0:
		...
		for gridColumn in grid.columns:
			if gridColumn.visible:
				if gridColumn.frozen == False:
					if grid.paint.onPaintGridColumn != None:
					...
					if firstPoint.x >= cLeft and firstPoint.x <= cLeft + gridColumn.width:
						for j in range(0, len(grid.columns)):
							...
							if tColumn == gridColumn:
								...
								tColumn.sort = "none"
						if grid.paint.onClickGridColumn != None:
						...
						if gridColumn.frozen:
							...
							if cRect.right >= 0 and cRect.left < grid.size.cx:
								if firstPoint.x >= cRect.left and firstPoint.x <= cRect.right and firstPoint.y >= cRect.top and firstPoint.y <= cRect.bottom:
									...
									if grid.paint.onClickGridCell != None:
				...
				for j in range(0,len(row.cells)):
					...
					if treeColumn.visible:
						...
						if cRect.right >= 0 and cRect.left < tree.size.cx:
							if tree.paint.onPaintTreeNode != None:
			...
			if firstPoint.y >= cTop and firstPoint.y <= cTop + tree.rowHeight:
				...
				if len(node.childNodes) > 0:
					...
					if firstPoint.x < wLeft:
						...
						break
				if tree.paint.onClickTreeNode != None:
...
def getChartIndex(chart, mp):
	...
	if mp.x <= 0:
		return 0
	intX = int(mp.x - chart.leftVScaleWidth - chart.hScalePixel)
...
def calcChartIndicator(chart):
	...
	if chart.paint.onCalculateChartMaxMin != None:
...
def zoomOutChart(chart):
	if chart.autoFillHScale == False:
		...
		if chart.showCrossLine:
			...
			if newX > oldX:
				while chart.lastVisibleIndex < len(chart.data) - 1:
...
def drawChartPlot(chart, pPaint, clipRect):
	if len(chart.plots) > 0:
		paint = None
		if pPaint.gdiPlusPaint != None:
			paint = pPaint
			divHeight = getCandleDivHeight(chart)
			paint.setClip(chart.leftVScaleWidth, 0, chart.size.cx, divHeight)
		else:
			paint = FCPaint()
			paint.drawHDC = pPaint.innerHDC
			paint.memBM = pPaint.innerBM
			paint.scaleFactorX = pPaint.scaleFactorX
			paint.scaleFactorY = pPaint.scaleFactorY
			divHeight = getCandleDivHeight(chart)
			cRect = FCRect(chart.leftVScaleWidth, 0, chart.size.cx, divHeight)
		...
		if paint.gdiPlusPaint == None:
			...
			win32gui.StretchBlt(paint.drawHDC, int(cRect.left * paint.scaleFactorX), int(cRect.top * paint.scaleFactorY), int((cRect.right - cRect.left) * paint.scaleFactorX), int((cRect.bottom - cRect.top) * paint.scaleFactorY), paint.innerHDC, int((cRect.left - rect.left) * self.scaleFactorX), int((cRect.top - rect.top) * self.scaleFactorY), int((cRect.right - cRect.left) * self.scaleFactorX), int((cRect.bottom - cRect.top) * self.scaleFactorY), SRCPAINT)
			if paint.innerHDC != None:
				win32gui.DeleteObject(paint.innerHDC)
				paint.innerHDC = None
			if paint.innerBM != None:
				win32gui.DeleteObject(paint.innerBM)
				paint.innerBM = None
		else:
...
def touchMoveChart(chart, firstTouch, firstPoint, secondTouch, secondPoint):
	...
	if firstTouch and secondTouch:
		...
		if chart.lastVisibleIndex + subIndex > len(chart.data) - 1:
			subIndex = len(chart.data) - 1 - chart.lastIndexCache
		elif chart.firstVisibleIndex + subIndex < 0:
...
def drawChartCrossLine(chart, paint, clipRect):
	...
	if volDivHeight > 0:
		...
		drawColors = []
		if len(chart.data) > 0:
			drawTitles.append("VOL " + toFixed(chart.data[crossLineIndex].volume, chart.volDigit))
...
def drawChartStock(chart, paint, clipRect):
	if chart.data != None and len(chart.data) > 0:
		...
		if paint.gdiPlusPaint != None:
			...
			newPaint = FCPaint()
			newPaint.drawHDC = paint.innerHDC
			newPaint.memBM = paint.innerBM
			newPaint.scaleFactorX = paint.scaleFactorX
			newPaint.scaleFactorY = paint.scaleFactorY
			divHeight = getCandleDivHeight(chart)
			cRect = FCRect(chart.leftVScaleWidth, 0, chart.size.cx, divHeight)
			...
			if isTrend == False:
				...
				if hasMinTag == False:
					if low == chart.candleMin:
						...
						hasMinTag = True
		if newPaint.gdiPlusPaint == None:
				rect = FCRect(0, 0, cRect.right - cRect.left, cRect.bottom - cRect.top)
				win32gui.StretchBlt(newPaint.drawHDC, int(cRect.left * paint.scaleFactorX), int(cRect.top * paint.scaleFactorY), int((cRect.right - cRect.left) * paint.scaleFactorX), int((cRect.bottom - cRect.top) * paint.scaleFactorY), newPaint.innerHDC, int((cRect.left - rect.left) * paint.scaleFactorX), int((cRect.top - rect.top) * paint.scaleFactorY), int((cRect.right - cRect.left) * paint.scaleFactorX), int((cRect.bottom - cRect.top) * paint.scaleFactorY), SRCPAINT)
				if newPaint.innerHDC != None:
					win32gui.DeleteObject(newPaint.innerHDC)
					newPaint.innerHDC = None
				if newPaint.innerBM != None:
					win32gui.DeleteObject(newPaint.innerBM)
					newPaint.innerBM = None
		else:
...
def invalidate(paint):
	if paint.onInvalidate:
		...
		hDC = win32gui.GetDC(paint.hWnd)
		paint.hdc = hDC
		paint.clipRect = None
		rect = win32gui.GetClientRect(paint.hWnd)
		paint.size = FCSize(rect[2] - rect[0], rect[3] - rect[1])
		...
		paint.endPaint()
		win32gui.ReleaseDC(paint.hWnd, hDC)
		showOrHideInput(paint, paint.views)
...
def showOrHideInput(paint, views):
	for i in range(0, len(views)):
		...
		if len(view.views) > 0:
			showOrHideInput(paint, view.views)
		if view.hWnd != None:
			clX = clientX(view)
			clY = clientY(view)
			relativeRect = FCRect(clX * paint.scaleFactorX, clY * paint.scaleFactorY, (clX + view.size.cx) * paint.scaleFactorX, (clY + view.size.cy) * paint.scaleFactorY)
			if view.clipRect != None:
				relativeRect = FCRect(view.clipRect.left * paint.scaleFactorX, view.clipRect.top * paint.scaleFactorY, view.clipRect.right * paint.scaleFactorX, view.clipRect.bottom * paint.scaleFactorY)
			if paintVisible:
				if win32gui.IsWindowVisible(view.hWnd) == False:
					win32gui.ShowWindow(view.hWnd, SW_SHOW)
				win32gui.MoveWindow(view.hWnd, int(relativeRect.left), int(relativeRect.top), int(relativeRect.right - relativeRect.left), int(relativeRect.bottom - relativeRect.top), True)
			else:
				if win32gui.IsWindowVisible(view.hWnd):
					win32gui.ShowWindow(view.hWnd, SW_HIDE)
...
def updateViewDefault(views):
	for i in range(0,len(views)):
		...
		if view.parent != None and view.parent.viewType != "split":
			if view.dock == "fill":
				view.location = FCPoint(0, 0)
				view.size = FCSize(view.parent.size.cx, view.parent.size.cy)
			elif view.dock == "left":
				view.location = FCPoint(0, 0)
				view.size = FCSize(view.size.cx, view.parent.size.cy)
			elif view.dock == "top":
				view.location = FCPoint(0, 0)
				view.size = FCSize(view.parent.size.cx, view.size.cy)
			elif view.dock == "right":
				view.location = FCPoint(view.parent.size.cx - view.size.cx, 0)
				view.size = FCSize(view.size.cx, view.parent.size.cy)
			elif view.dock == "bottom":
				view.location = FCPoint(0, view.parent.size.cy - view.size.cy)
				view.size = FCSize(view.parent.size.cx, view.size.cy)
...
def getViewAttribute(view, atrName):
	viewText = ""
	if view.paint.useGdiPlus:
		recvData = create_string_buffer(102400)
		view.paint.gdiPlusPaint.getAttribute(view.viewName, atrName, recvData)
		viewText = str(recvData.value, encoding=view.paint.gdiPlusPaint.encoding)
	else:
		viewText = getHWndText(view.hWnd)
	return viewText
	...
	if calendar.mode == "day":
		...
		for i in range(0,dayButtonsSize):
			...
			if dayButton.visible:
				if paint.onPaintCalendarDayButton != None:
		...
		for i in range(0,monthButtonsSize):
			...
			if monthButton.visible:
				if paint.onPaintCalendarMonthButton != None:
		...
		for i in range(0,yearButtonsSize):
			...
			if yearButton.visible:
				if paint.onPaintCalendarYearButton != None:
		...
		for i in range(0,yearFCButtonmSize):
			...
			if yearButton.visible:
				if paint.onPaintCalendarYearButton != None:
					...
					drawYearButton(yearButton, paint)
	if paint.onPaintHeadDiv != None:
		paint.onPaintHeadDiv(calendar.headDiv, paint)
	else:
	...
	for child in node:
		...
		if view != None:
			...
			if typeStr == "splitlayout":
				...
				if paint.useGdiPlus and len(view.viewName) > 0:
					view.exView = True
					paint.init()
					paint.gdiPlusPaint.createView(view.viewType, view.viewName)
					if view.paint.defaultUIStyle == "dark":
						paint.gdiPlusPaint.setAttribute(view.viewName, "backcolor", "rgb(0,0,0)")
						paint.gdiPlusPaint.setAttribute(view.viewName, "bordercolor", "rgb(100,100,100)")
						paint.gdiPlusPaint.setAttribute(view.viewName, "textcolor", "rgb(255,255,255)")
					elif view.paint.defaultUIStyle == "light":
						paint.gdiPlusPaint.setAttribute(view.viewName, "backcolor", "rgb(255,255,255)")
						paint.gdiPlusPaint.setAttribute(view.viewName, "bordercolor", "rgb(150,150,150)")
						paint.gdiPlusPaint.setAttribute(view.viewName, "textcolor", "rgb(0,0,0)")
					for key in child.attrib:
						name = key.lower()
						value = child.attrib[key]
						paint.gdiPlusPaint.setAttribute(view.viewName, name, value)
				else:
					view.hWnd = win32gui.CreateWindowEx(0, "Edit", view.viewName, WS_VISIBLE|WS_CHILD|SS_CENTERIMAGE, 0, 0, 100, 30, paint.hWnd, 0, 0, None)
					win32gui.ShowWindow(view.hWnd, SW_HIDE)
					s = win32gui.GetWindowLong(view.hWnd, GWL_EXSTYLE)
					win32gui.SetWindowLong(view.hWnd, GWL_EXSTYLE, s|ES_CENTER)
					setHWndText(view.hWnd, view.text)
			elif nodeName == "calendar":
				initCalendar(view)
...
def WndProcDefault(paint, hwnd, msg, wParam, lParam):
	...
	if hwnd == paint.hWnd:
		...
		#大小改变
			...
			rect = win32gui.GetClientRect(paint.hWnd)
			wWidth = rect[2] - rect[0]
			wHeight = rect[3] - rect[1]
		...
		#输入法事件
			...
			if paint.useGdiPlus and paint.gdiPlusPaint != None:
...
def hideWindow(paint):
	global showWindowState
	showWindowState = 1
	win32gui.ShowWindow(paint.hWnd, SW_HIDE)
...
def showWindow(paint):
	...
	if showWindowState == 1:
		win32gui.ShowWindow(paint.hWnd, SW_SHOWMAXIMIZED)
	else:
		win32gui.ShowWindow(paint.hWnd, SW_SHOW)
	win32gui.UpdateWindow(paint.hWnd)
	showWindowState = 0
...
def createMainWindow(paint, title, wndProc):
	...
	if showWindowSize.cx > 0 and showWindowSize.cy > 0:
		hwnd = win32gui.CreateWindow(reg, title, WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN, showWindowLocation.x, showWindowLocation.y, showWindowSize.cx, showWindowSize.cy, 0, 0, 0, None)
	else:
		hwnd = win32gui.CreateWindow(reg, title, WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, 0, 0, 0, None)
	paint.hWnd = hwnd
