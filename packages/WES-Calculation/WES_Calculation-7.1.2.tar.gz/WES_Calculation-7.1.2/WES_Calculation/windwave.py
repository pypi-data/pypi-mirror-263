...
LastEditTime : 2024-03-19 09:22:44
LastEditors  : <BDFD>
Description  : 
...
def windwave(o1, ad, X, U10k, atm, atr, wdu, o2, beta, slc, o5, o4, xs, d0, Ksb, xlook, o6):
    # Secondary
    ...
    if atm != atr:
        ...
        section1.extend(['已知风速的时距并非指定时距(='+str(round(atr, 2))+'min)，该已知风速需要适当转换以获取指定时距的计算风速。'])
    else:
    ...
    if o2 == 'Yes' and o1 == 1:
        nn = 0
        print('')
        print('---根据水深剖面逐段近似推算风壅增水的参考方法---')

        nn = nn+1
        if atm != atr:
            print(
                nn, '. 已知风速的时距并非所需时距，在此采用CEM (2015)中图II-2-1的近似转换方法获取所需时距的风速值作为计算风速，结果见以下CEM部分。')
        else:
            print(nn, '. 已知风速的时距等于所需时距，故已知风速即为计算风速。')
        ...
        if o5 == 'Yes' and o6 == 1:
            if o4 == 1:
                ...
                nn = nn+1
                print(nn, '. 由水深剖面得到的平均水深（静水位下，无风壅增水）=',
                      round(d0[0]/2, 3), 'm。')
            ...
            if o4 == 2:
                ...
                nn = nn+1
                print(nn, '. 由水深剖面得到的平均水深（静水位下，无风壅增水）=', round(adb, 3), 'm。')
            ...
            nn = nn+1
            if xlook > 0:
                print(nn, '. 静水位接岸处的风壅增水=', round(ws0, 3), 'm。在距离静水位接岸处',
                      round(xlook, 3), 'm的位置，风壅增水=', round(wslook, 3), 'm。')
            else:
                print(nn, '. 在距离静水位接岸处', round(xlook, 3),
                      'm的位置，风壅增水=', round(wslook, 3), 'm。')

            if ws0 > slc:
                nn = nn+1
                print(nn, '. 警告：在该风壅增水作用下，理论上静水位接岸处的水面已超过堤防顶沿。')
        else:
            nn = nn+1
            print(nn, '. 水深剖面数据缺乏或不恰当，无法近似推算风壅增水')
    ...
    if atr != atm:
        ...
        if ad == None:
            ...
            if ad/U10a**2 > 0.2:
                dstate1 = '深水'
                section3.extend(['. 按第7.2.1条的判别标准：d/U\u2081\u2080\u00b2=',
                                 round(ad/U10a**2, 3), '>0.2，故水域属于深水。'])
            else:
                dstate1 = '浅水'
                section3.extend(['. 按第7.2.2条的判别标准：d/U\u2081\u2080\u00b2=',
                                 round(ad/U10a**2, 3), '<=0.2，故水域属于浅水。'])
                section3.extend(
        ...
        if 9.81*X*1000/U10a**2 >= 34700:  # 充分成长状态
            ...
            if dstate1 == '深水':
                section3.extend(['. 按第7.2.1条的判别标准：gF/U\u2081\u2080\u00b2=',
                                round(9.81*X*1000/U10a**2, 3), '>=34700，故波浪达到充分成长状态。'])
                section3.extend(['. 在充分成长状态下，按公式(7.2.1-5)求得Hs=', round(Hsf, 3),
                                'm，根据gF/U\u2081\u2080\u00b2=34700按公式(7.2.1-2)求得Ts=', round(Tsf, 2), 's'])
        else:
            if X > Fmin1/1000:  # 过渡状态
                Hs1 = U10a**2*0.0055*(9.81*Fmin1/U10a**2)**.35/9.81
                Ts1 = U10a*0.55*(9.81*Fmin1/U10a**2)**.233/9.81
                wstate1 = '过渡'
                section3.extend(['. 由实际风时和计算风速，按公式(7.2.1-3)求得Fmin=', round(Fmin1/1000, 1),
                                'km。实际风距>Fmin，故风浪处于过渡状态。由计算风速和Fmin可定义一个定常状态，以推算风浪要素。'])

                if dstate1 == '浅水':
                    section3.extend(['. 按公式(7.2.1-1)求得深水的Hs=', round(Hs1, 3),
                                     'm，按公式(7.2.1-2)求得深水的Ts=', round(Ts1, 2), 's'])
                    Kt = math.tanh(30*(9.81*ad/U10a**2)**0.8 /
                                   (9.81*Fmin1/U10a**2)**0.35)
                    Hs1 = Hs1*Kt
                    Ts1 = Ts1*Kt**(2/3)
                    section3.extend(
                        ['. 采用Fmin，求得公式(7.2.2-1)中的tanh项，即浅水中的波高折减系数Kt:', round(Kt, 2)])
                    section3.extend(
                        ['. 按公式(7.2.2-1)求得Hs=', round(Hs1, 3),
                         'm，按公式(7.2.2-2)求得Ts=', round(Ts1, 2), 's'])
                else:
                    section3.extend(
                        ['. 按公式(7.2.1-1)求得Hs=', round(Hs1, 3),
                         'm，按公式(7.2.1-2)求得Ts=', round(Ts1, 2), 's'])
                ...
                wstate1 = '定常'
                section3.extend(
                    ['. 由实际风时和计算风速，按公式(7.2.1-3)求得Fmin=',
                     round(Fmin1/1000, 1), 'km。实际风距<=Fmin，故风浪处于定常状态。'])
                ...
                if dstate1 == '浅水':
                    ...
                    Ts1 = Ts1*KF**(2/3)
                    section3.extend(
                        ['. 公式(7.2.2-1)中的tanh项即为浅水中的波高折减系数KF:', round(KF, 2)])
                    section3.extend(
                        ['. 按公式(7.2.2-1)求得Hs=', round(Hs1, 3),
                         'm，按公式(7.2.2-2)求得Ts=', round(Ts1, 2), 's'])
                else:
        ...
        if o1 != 1:
            ...
            if o2 == 'Yes':
                if ad != None:
                    ...
                    if ws1 > slc:
                        section3.extend(['. 按第7.2.8条，静水位接岸处的风壅增水=', round(
                            ws1, 3), 'm。（警告：在该风壅增水作用下，理论上水面已超过堤防顶沿。）'])
                    else:
                        section3.extend(['. 按第7.2.8条，静水位接岸处的风壅增水=',
                                         round(ws1, 3), 'm。'])
                else:
                    ws1 = None
    ...
    if atr == atm and ad != None and atr == 10:
        ...
        if tmin4 <= wdu:  # a fetch-limited condition occurs
            ...
            section4.extend(['. 由计算风速和推算的Ta，按公式(C.1.2-3)求得tmin=',
                             round(tmin4/60, 2), 'h。实际风时>=tmin，故风浪处于定常状态。'])
            section4.extend(['. 按公式(C.1.2-1)求得Ha=', round(Ha4, 3),
                             'm，按公式(C.1.2-2)求得Ta=', round(Ta4, 2), 's'])

        else:  # a duration-limited condition occurs
            section4.extend(['. 由计算风速和推算的Ta，按公式(C.1.2-3)求得tmin=',
                             round(tmin4/60, 2), 'h。实际风时<tmin，故风浪处于过渡状态。'])
            wstate4 = '过渡'
            if X <= 100:
                section4.extend(
                ...
                section4.extend(['. 由实际风时和计算风速，按公式(C.1.2-3)求得Ta=', round(Ta4, 2),
                                 's，按公式(C.1.2-2)求得Ha=', round(Ha4, 3), 'm'])
    ...
    if o1 == 1 or o1 == 2:
        ...
        if atr == atm and ad != None and atr == 10:
            ...
            if wstate4 == '过渡' and X > 100:
                ...
                section5.extend(['. 由实际风时和计算风速，按公式(C.0.4-1)求得Ta=', round(Ta5, 2),
                                 's，按公式(C.0.4-2)求得Ha=', round(Ha5, 3), 'm'])

            Ts5 = 1.15*Ta5
            section5.extend(
                ['. 按公式(C.0.1-2)，Ts = 1.15Ta =', round(Ts5, 2), 's'])
        else:
            Ha5 = None
            Ta5 = None
            section5.extend(['. 风浪要素推算结果见《堤防工程设计规范》部分。'])
    ...
    if atm != atr:
        section6.extend(['. 按图3-13，1-h时距的U\u2081\u2080=', round(att(U10k, atm, 60), 3),
                         'm/s，而', str(round(atr, 1))+'-min时距的U\u2081\u2080=', round(U10a, 3), 'm/s'])
    ...
    if ad == None or (ad != None and ad > 90):
        ...
        section6.extend(['. 由实际风距按公式(3-35)试算求得tmin=', round(tmin2/60, 2),
                         'h，以此作为时距的U\u2081\u2080=', round(U10ae, 3), 'm/s'])  # （该风速即为计算风速）')
        ...
        if tmin2 > wdu:  # a duration-limited condition occurs
            ...
            section6.extend(['. 以实际风时作为时距的风速即为计算风速(=', round(U10ad, 3),
                             'm/s)。按公式(3-28a)，根据计算风速求得风应力因子Ua=', round(U10aad, 3)])
            section6.extend(['. 根据实际风时和Ua按公式(3-35)求得Fmin(=', round(Fmin2/1000, 3),
                             'km)。由Ua和Fmin可定义一个定常状态，可按公式(3-33)和(3-34)推算风浪要素。'])
            ...
            section6.extend(['. 以tmin为时距的风速即为计算风速(=', round(U10ae, 3),
                             'm/s)。按公式(3-28a)，根据计算风速求得风应力因子Ua=', round(U10aa, 3)])
            ...
            if Hm02 > Hm02f:
                ...
                section6.extend(['. 按公式(3-33)得到的Hm0=', round(Hm02, 3), 'm'])
            ...
            if Tm2 > Tm2f:
                ...
                section6.extend(['. 按公式(3-34)得到的Tp=', round(Tm2, 2), 's'])
    ...
    if ad != None:
        ...
        if 1.5 <= ad <= 90:
            ...
            section6.extend(['. 由实际风距按公式(3-40)和(3-41)试算求得tmin=', round(tmin2/60, 2),
                             'h，以此作为时距的U\u2081\u2080=', round(U10ae, 3), 'm/s'])  # （该风速即为计算风速）')
            ...
            if tmin2 > wdu:  # a duration-limited condition occurs
                ...
                Tm2 = Tm2lim
                section6.extend(['. 实际风时<tmin，故风浪处于过渡状态。'])
                section6.extend(['. 以实际风时作为时距的U\u2081\u2080即为计算风速(=', round(
                    U10ad, 3), 'm/s)。按公式(3-28a)，根据计算风速求得风应力因子Ua=', round(U10aad, 3)])
                section6.extend(['. 由实际风时按公式(3-41)求得Tp=', round(Tm2, 2),
                                 's，再结合公式(3-40)求得Fmin(=', round(Xu, 3), 'km)，最后按公式(3-39)求得Hm0。'])
                ...
                wstate2 = '定常'
                section6.extend(['. 实际风时>=tmin，故风浪处于定常状态。'])
                section6.extend(['. 以tmin为时距的U\u2081\u2080即为计算风速(=', round(
                    U10ae, 3), 'm/s)。按公式(3-28a)，根据计算风速求得风应力因子Ua=', round(U10aa, 3)])
                section6.extend(['. 由实际风距按公式(3-40)求得Tp=', round(Tm2, 2),
                                 's，最后按公式(3-39)求得Hm0。'])

            if ad/Tm2**2 > 0.78:
                dstate2 = '深水'
                section6.extend(['. 水深在[1.5 m,90 m]范围内。按图3-27至图3-36中的标准：d/Tp\u00b2=',
                                 round(ad/Tm2**2, 2), '>0.78。故水域仍属于深水。'])
            else:
                dstate2 = '浅水'
                section6.extend(['. 水深在[1.5 m,90 m]范围内。按图3-27至图3-36中的标准：d/Tp\u00b2=',
                                 round(ad/Tm2**2, 2), '<=0.78。故水域属于浅水。'])
    ...
    if atm != atr:
        section7.extend(['. 按图II-2-1，1-h时距的U\u2081\u2080=', round(att(U10k, atm, 60), 3),
                         'm/s，而', str(round(atr, 1))+'-min时距的U\u2081\u2080=', round(U10a, 3), 'm/s（计算风速）'])
    ...
    section7.extend(['. 按第II-2-48页，风阻系数CD=', round(CD, 6),
                     '，摩擦风速U*=', round(Uf, 3), 'm/s'])
    ...
    section7.extend(['. 限制条件：按公式(II-2-30)，充分成长状态下的上限波高（定义不明）=', round(Hinf, 3),
                     'm。按公式(II-2-36)，对应的风距=', round(XHinf, 3), 'km，Tp=', round(TmHinf, 2), 's'])

    Hm0f = 211.5*Uf**2/9.81  # by Eq.(II-2-37)
    Tmf = 239.8*Uf/9.81  # by Eq.(II-2-37)
    section7.extend(['. 限制条件：按公式(II-2-37)，充分成长状态下的波浪要素：Hm0=',
                     round(Hm0f, 3), 'm，Tp=', round(Tmf, 2), 's'])

    if ad != None:
        Tmad = 9.78*(ad/9.81)**0.5  # by Eq.(II-2-39)
        Hm0d = 0.5*ad  # On Page II-2-58
        section7.extend(
            ['. 限制条件：按公式(II-2-39)，水深限制下的最大Tp=', round(Tmad, 2), 's'])
        section7.extend(
            ['. 限制条件：按第II-2-58页，水深限制下的最大Hm0 = 0.5d =', round(Hm0d, 2), 'm'])

    if wdu >= tmin3:
        wstate3 = '定常'
        Fa = X  # assumed fetch for the fetch-limited condition
        section7.extend(['. 由实际风距和计算风速，按公式(II-2-35)求得tmin=',
                         round(tmin3/60, 2), 'h。实际风时>=tmin，故风浪处于定常状态。'])

    else:
        wstate3 = '过渡'
        section7.extend(['. 由实际风距和计算风速，按公式(II-2-35)求得tmin=',
                         round(tmin3/60, 2), 'h。实际风时<tmin，故风浪处于过渡状态。'])
        if ad != None:
        ...
        section7.extend(
            ['. 由实际风时和计算风速，按公式(II-2-35)求得Fmin=', round(Xfl, 3), 'km'])
        section7.extend(['. 各种限制条件的Tp中，最小值为', round(Tmmin, 2), 's，按公式(II-2-35)求得对应的风距=',
                         round((9.81*Tmmin/Uf/0.651)**3*Uf**2/9.81/1000, 3), 'km'])
        section7.extend(['. 综上所述，Fmin最终取', round(Fa, 3),
    ...
    section7.extend(['. 按公式(II-2-36)推算Hm0和Tp，并考虑各种限制条件施加的上限值，最终结果：Hm0=',
                     round(Hm03, 3), 'm，Tp=', round(Tm3, 2), 's'])
    ...
    section8 += (("推算方法", "tmin(h)", "Ha(m)", "Hs(m)",
          'Ta(s)', 'Ts(s)', 'Tp(s)', '风浪状态', '水深条件', '风壅增水(m)'))

    if Hs1 != None:
        if ws1 == None:
            section8 += (('JTS 145-2015', round(tmin1/60, 2), '',
                  round(Hs1, 3), '', round(Ts1, 1), '', wstate1, dstate1, ''))
        else:
            section8 += (('JTS 145-2015', round(tmin1/60, 2), '',
                  round(Hs1, 3), '', round(Ts1, 1), '', wstate1, dstate1, round(ws1, 3)))
    else:
        section8 += (('JTS 145-2015', '', '', '', '', '', '', '', '', ''))

    if Ha4 != None:
        if ws2 == None:
            section8 += (('GB 50286-2013', round(tmin4/60, 2),
                  round(Ha4, 3), '', round(Ta4, 1), '', '', wstate4, '无定义', ''))
        else:
            section8 += (('GB 50286-2013', round(tmin4/60, 2), round(Ha4,
                  3), '', round(Ta4, 1), '', '', wstate4, '无定义', round(ws2, 3)))
    else:
        section8 += (('GB 50286-2013', '', '', '', '', '', '', '', '', ''))

    if Ha5 != None:
        section8 += (('GB/T 51015-2014', '', round(Ha5, 3), '',
              round(Ta5, 1), round(Ts5, 1), '', '不判别', '无定义', ''))
    else:
        section8 += (('GB/T 51015-2014', '', '', '', '', '', '', '', '', ''))

    if Hm02 != None:
        section8 += (('SPM 1984', round(tmin2/60, 2), '', round(Hm02,
              3), '', round(Ts2, 1), round(Tm2, 1), wstate2, dstate2, ''))
    else:
        section8 += (('SPM 1984', '', '', '', '', '', '', '', '', ''))

    if Hm03 != None:
        section8 += (('CEM 2015', round(tmin3/60, 2), '',
              round(Hm03, 3), '', '', round(Tm3, 1), wstate3, '无定义', ''))
    else:
        section8 += (('CEM 2015', '', '', '', '', '', '', '', '', ''))
    ...
    return heading, section1, section3, section4, section5, section6, section7, section8, ending
