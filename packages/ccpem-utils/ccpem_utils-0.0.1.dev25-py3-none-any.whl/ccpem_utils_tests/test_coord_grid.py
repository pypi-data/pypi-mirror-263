...
from ccpem_utils.scripts import (
    shift_model_origin_zero,
    shift_model_back_origin_nonzero,
)
...
class MapParseTests(unittest.TestCase):
    ...
    def test_run_subprocess_shift_map_model_zero(self):
        ...
        subprocess.call(
            [
                "python3 "
                + os.path.realpath(shift_model_origin_zero.__file__)
                + " -m "
    ...
    def test_run_subprocess_shift_model_back_nonzero(self):
        ...
        subprocess.call(
            [
                "python3 "
                + os.path.realpath(shift_model_back_origin_nonzero.__file__)
                + " -m "
                + map_input
                + " -p "
                + model_input
