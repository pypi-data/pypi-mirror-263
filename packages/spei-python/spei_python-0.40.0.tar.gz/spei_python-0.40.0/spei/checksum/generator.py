...
class ChecksumGeneratorDefault(ChecksumGenerator):
    def format_data(self, message: Dict[AnyStr, Any]):
        message_data = (
            self._fecha_to_bytearray(message['op_fecha_oper']),
            self._clave_to_bytearray(message['op_ins_clave_ord']),
            self._clave_to_bytearray(message['op_ins_clave_ben']),
            self._to_byte_array(message['op_cve_rastreo'], add_zero_byte=True),
            self._monto_to_bytearray(message['op_monto']),
        )
...
class ChecksumGeneratorOrigin(ChecksumGenerator):
    def format_data(self, message: Dict[AnyStr, Any]):
        message_data = (
            ...
            self._to_byte_array(message['op_cve_rastreo'], add_zero_byte=True),
            self._monto_to_bytearray(message['op_monto']),
            self._to_byte_array(message['op_cuenta_ord'], add_zero_byte=True),
        )
        return self._list_to_bytes(message_data)
...
class ChecksumGeneratorBeneficiary(ChecksumGenerator):
    def format_data(self, message: Dict[AnyStr, Any]):
        message_data = (
            ...
            self._to_byte_array(message['op_cve_rastreo'], add_zero_byte=True),
            self._monto_to_bytearray(message['op_monto']),
            self._to_byte_array(message['op_cuenta_ben'], add_zero_byte=True),
        )
        return self._list_to_bytes(message_data)
...
class ChecksumGeneratorBeneficiaryAndAdditionalBeneficiary(ChecksumGenerator):
    def format_data(self, message: Dict[AnyStr, Any]):
        message_data = (
            ...
            self._monto_to_bytearray(message['op_monto']),
            self._to_byte_array(message['op_cuenta_ben'], add_zero_byte=True),
            self._to_byte_array(message['op_cuenta_ben_2'], add_zero_byte=True),
        )
        return self._list_to_bytes(message_data)
