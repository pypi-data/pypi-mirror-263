...
from kya_newsreader.news_url_getter import NewsUrlGetter
import kya_newsreader.constant
from kya_newsreader.exceptions import HTMLValidationException, HTMLDownloadException
...
class NewsParser:
    def create_driver_options(self):
        # If the crx file 'kya_newsreader/paywall_ext.crx' is not found, download it from github
        # Url: https://github.com/xwkya/NewsScraping/blob/master/kya_newsreader/paywall_ext.crx
        extension_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "paywall_ext.crx")

        if not os.path.exists(extension_path):
            # Download the extension from PyPI
            extension_url = 'https://github.com/xwkya/NewsScraping/raw/master/kya_newsreader/paywall_ext.crx'
            response = requests.get(extension_url)
            
            # Create the cache folder if it doesn't exist
            cache_folder = 'extension_cache'
            os.makedirs(cache_folder, exist_ok=True)
            
            # Save the extension to the cache folder
            extension_path = os.path.join(cache_folder, 'paywall_ext.crx')
    ...
    def validate_html(self, html: str, url: str) -> bool:
        ...
        """
        banned_strings = ["This page appears when Google automatically detects requests coming from your computer", "Partner content: This content", "Error 404", "security service to protect itself from online attacks", "understand that not everyone can afford to pay for expensive news subscriptions", "products or services. Here is a list of our partners", "enable javascript", "supports javascript", "We suspect you are using an automated system", "bad bot", "Our engineers are working quickly to resolve the issue."]
        ...
        if len(article.text) < 120\
            or sum([1 if banned_string in article_text else 0 for banned_string in banned_strings]) > 0:
    ...
    def download_article_with_google_cache(self, url: str, driver: webdriver.Chrome) -> str:
        ...
        """
        try:
            driver.get(kya_newsreader.constant.GOOGLE_CACHE + url)
    ...
    def get_news(self, topic: str) -> List[Article]:
        ...
        for urlAndHtml in urls:
            ...
            if article.publish_date is None:
                article.publish_date = urlAndHtml.date
