...
def generate_kirby_conf(table_raw_name=None, table_master_name=None, kirby_type=None, target_staging_path=None):
    ...
    if kirby_type == "raw":
        ...
        table_dict_conf[table_raw_name]["kirby"]["transformations"].append(run_id_dict)
        table_dict_conf[table_raw_name]["kirby"]["transformations"].append(job_name_dict)
        ...
        for field in txt_json["fields"]:
            ...
            if str(name).lower() not in ("audtiminsert_date", "cutoff_date"):
                if str(legacy_name).lower() not in ("calculated", None):
                    trim_columns_list.append(name)
                    rename_col_dict[name] = name
