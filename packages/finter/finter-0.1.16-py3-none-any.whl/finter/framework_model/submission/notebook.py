...
class NotebookSubmissionHelper:
    ...
    def __init__(
        ...
        try:
            self.benchmark = ModelUniverseConfig[model_universe.upper()].get_benchmark(
                benchmark=benchmark
            )
            assert self.benchmark in DefaultBenchmark.available_options() + [False], (
                f"Invalid benchmark: {self.benchmark}. Available options: "
                f"{DefaultBenchmark.available_options()}, False"
            )
        except KeyError:
            logger.error(
                f"Invalid benchmark: {self.benchmark}. Available options: {DefaultBenchmark.available_options()}"
            )
            raise
