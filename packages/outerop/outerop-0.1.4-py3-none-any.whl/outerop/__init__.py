...
class Outerop:
    ...
    async def get_prompt(self, prompt_uuid: str, environment: str, version: Union[int, str]):
        ...
        try:
            ...
            response = self.client.get(f"/api/v1/prompt/{prompt_uuid}/{environment}/{version}")
            response.raise_for_status()
    ...
    async def chat(
        ...
        prompt = await self.get_prompt(prompt_uuid, environment, version)
        ...
        try:
            ...
            if prompt["model_config"]["provider"] == "mistral":
                ...
                result = await self.mistral.chat(
            ...
            if isinstance(e, OpenAI.APIError):
                raise Exception(f"Request failed with status code {e.code}: {e.message}")
            else:
                raise Exception(f"Request failed: {e}")
