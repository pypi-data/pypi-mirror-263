...
def test_clusterInfo(path, temp_folder,save_path_test):
    ...
    try:
        ...
        new_row = pd.DataFrame({'File': f"{raw_path}", 'competability': "can be merged"}, ignore_index=True)

        print(f"{raw_path} merge complete")
    except AssertionError:
        new_row = pd.DataFrame({'File': f"{raw_path}", 'competability': "can not be merged"}, ignore_index=True)
