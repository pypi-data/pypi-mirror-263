...
%s%s""" % (
    ...
    ps1_clean_up_1 = compile(r'\$\{.*\}')
    ps1_clean_up_2 = compile(r'\$\(.*\)')
    ps1_exit_code = compile(r"\$\?")
    ps1_hostname_sort = compile(r"\\h")
    ps1_hostname = compile(r"\\H")
    ps1_date = compile(r"\\d")
    ps1_shell = compile(r"\\s")
    ps1_username = compile(r"\\u")
    ps1_shell_version = compile(r"\\v")
    ps1_shell_release = compile(r"\\V")
    ps1_working_directory = compile(r"\\w")
    ps1_working_directory_basename = compile(r"\\W")
    ps1_prompt_sign = compile(r"\\\$")
    ps1_newline = compile(r"\\n")
    ps1_carriage_return = compile(r"\\r")
    ps1_bell = compile(r"\\a")
    ps1_time_24_hour = compile(r"\\t")
    ps1_time_12_hour = compile(r"\\T")
    ps1_time_am_pm = compile(r"\\@")
    ps1_begin_a_sequence_of_non_printing_characters = compile(r"\\\[\\033")
    ps1_end_a_sequence_of_non_printing_characters = compile(r"\\\]")
    ps1_virtual_env = compile(r"\$VIRTUAL_ENV")
    ...
    def load_alias(self):
        file = expanduser("~/.glxsh_alias")
        if exists(file):
            with open(file, "r") as alias_file:
    ...
    def prompt(self):
        ...
        if self.environ.get("PS1"):
            ...
            tmp_value = self.ps1_working_directory.sub(getcwd().replace("%s/" % self.environ.get("HOME"), "~/"),
                                                       tmp_value)
    ...
    def run_simple_command(self, cmd, arg, line):
        if hasattr(self, "do_%s" % cmd):
            ...
            try:
                ...
                pass
            try:
                pr = subprocess.run(line.split(" "), start_new_session=True, env=self.environ)
