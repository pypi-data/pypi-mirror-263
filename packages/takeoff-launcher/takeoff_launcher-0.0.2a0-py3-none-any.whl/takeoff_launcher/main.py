...
def start():
    ...
    if exit_code == 0:
        ...
        console.print("[bold yellow]:warning: No license key found[/]")
...
@main.command(help="List all running takeoff servers.")
def list():
    # ─────────────────────────────────── check docker environment ─────────────────────────────────── #
    docker_installed = check_docker_environment()
    if not docker_installed:
...
@main.command(help="Shut down a running takeoff server.")
def down(
