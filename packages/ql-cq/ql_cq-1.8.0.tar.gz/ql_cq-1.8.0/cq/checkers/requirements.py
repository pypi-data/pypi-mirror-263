...
class MinVersion(packaging.version._BaseVersion):  # pylint: disable=protected-access
	def __init__(self) -> None:
		self._key: MinVersion = self  # type: ignore[assignment]
...
class MaxVersion(packaging.version._BaseVersion):  # pylint: disable=protected-access
	def __init__(self) -> None:
		self._key: MaxVersion = self  # type: ignore[assignment]
...
class VersionRange:
	def __init__(
		self,
		lower: packaging.version._BaseVersion,
		upper: packaging.version._BaseVersion,
	...
	def from_specs(cls, specs: List[Tuple[str, str]]) -> 'VersionRange':
		lower = MIN_VERSION  # type: packaging.version._BaseVersion
		upper = MAX_VERSION  # type: packaging.version._BaseVersion
		lower_inclusive = True
		upper_inclusive = True
		for sign, version_str in specs:
			if sign == '==':
				lower = packaging.version.parse(version_str)
				upper = packaging.version.parse(version_str)
	...
	def issubset(self, other: 'VersionRange') -> bool:
		...
		return lower_condition and upper_condition and overlap_condition
...
class RequirementsSetupCompatiblityChecker(cq.checker.Checker):
	...
	def _create_requirements_dict(source: str) -> Dict[str, List[Tuple[str, str]]]:
		...
		for req in requirements.parse(source):
			result[req.name] = req.specs  # type: ignore[index, assignment]
		return result
