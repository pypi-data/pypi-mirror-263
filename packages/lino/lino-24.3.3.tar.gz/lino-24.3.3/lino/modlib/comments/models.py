...
class Comment(CreatedModified, UserAuthored, Controllable, ElasticSearchable,
    ...
    def as_story_item(self, ar, indent=0):
        def storypar(s):
            ...
            if indent > 0:
                ...
                return """<p style="{}">{}</p>""".format(style, s)
            return """<p>{}</p>""".format(s)
    ...
    def unused_as_story_item(self, ar, indent=0):
    ...
    def get_user_queryset(cls, user):
        ...
        if not user.user_type.has_required_roles([CommentsStaff]):
            filters = []
...
class Reaction(CreatedModified, UserAuthored, DateRangeObservable):
    ...
    def as_summary_row(self, ar, **kw):
        return self.emotion.button_text
