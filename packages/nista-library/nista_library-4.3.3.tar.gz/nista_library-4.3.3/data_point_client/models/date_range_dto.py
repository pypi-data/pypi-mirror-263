...
class DateRangeDTO:
    ...
    def to_dict(self, time_zone: ZoneInfo) -> Dict[str, Any]:
        first_entry: Union[Unset, str] = UNSET
        if not isinstance(self.first_entry, Unset):
            first_entry = DateRangeDTO.to_utc(self.first_entry, time_zone)

        last_entry: Union[Unset, str] = UNSET
        if not isinstance(self.last_entry, Unset):
            last_entry = DateRangeDTO.to_utc(self.last_entry, time_zone)
    ...
    @staticmethod
    def to_utc(entry: datetime.datetime, time_zone: ZoneInfo) -> str:
        zone = entry.replace(tzinfo=time_zone)
        zone = zone.astimezone(datetime.timezone.utc)
        return zone.strftime("%Y-%m-%dT%H:%M:%SZ")
