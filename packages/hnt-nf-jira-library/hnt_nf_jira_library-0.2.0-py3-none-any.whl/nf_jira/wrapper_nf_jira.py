...
class wrapper_jira:
    ...
    def _get_nf_jira(self, issue_id: str):
        try:
            ...
            attachment['CnpjFornecedor'] = form_data["state"]["answers"]["3"]["text"] if "3" in form_data["state"]["answers"] else attachment.get('CnpjFornecedor')
            attachment['RazaoSocialFornecedor'] = form_data["state"]["answers"]["4"]["text"] if "4" in form_data["state"]["answers"] else attachment.get('RazaoSocialFornecedor')
            attachment['CnpjCliente'] = form_data["state"]["answers"]["5"]["text"] if "5" in form_data["state"]["answers"] else attachment.get('CnpjCliente')
            attachment['NotaFiscal'] = form_data["state"]["answers"]["7"]["text"] if "7" in form_data["state"]["answers"] else attachment.get('NotaFiscal')
            attachment['NumeroFaturaFornecedor'] = form_data["state"]["answers"]["8"]["text"] if "8" in form_data["state"]["answers"] else attachment.get('NumeroFaturaFornecedor')
            attachment['DataEmissao'] = form_data["state"]["answers"]["9"]["date"] if "9" in form_data["state"]["answers"] else attachment.get('DataEmissao')
            attachment['DataVencimento'] = form_data["state"]["answers"]["10"]["date"] if "10" in form_data["state"]["answers"] else attachment.get('DataVencimento')
            attachment['Valor'] = form_data["state"]["answers"]["11"]["text"] if "11" in form_data["state"]["answers"] else attachment.get('Valor')
            attachment['ChaveAcessoNotaFiscal'] = form_data["state"]["answers"]["12"]["text"] if "12" in form_data["state"]["answers"] else attachment.get('ChaveAcessoNotaFiscal')
            attachment['numero_log'] = form_data["state"]["answers"]["13"]["text"] if "13" in form_data["state"]["answers"] else attachment.get('numero_log')
            attachment['data_procmto'] = form_data["state"]["answers"]["14"]["date"] if "14" in form_data["state"]["answers"] else attachment.get('data_procmto')
            attachment['hora_procmto'] = form_data["state"]["answers"]["15"]["time"] if "15" in form_data["state"]["answers"] else attachment.get('hora_procmto')
            attachment[nf_type]['DataLeituraAnterior'] = form_data["state"]["answers"]["16"]["date"] if "16" in form_data["state"]["answers"] else attachment[nf_type].get('DataLeituraAnterior')
            attachment[nf_type]['DataLeituraAtual'] = form_data["state"]["answers"]["17"]["date"] if "17" in form_data["state"]["answers"] else attachment[nf_type].get('DataLeituraAtual')
            attachment['DataReferencia'] = form_data["state"]["answers"]["18"]["text"] if "18" in form_data["state"]["answers"] else attachment.get('DataReferencia')

            jira_info = {
                'issue_id': issue_id,
                'form_id': automacao_form_id
            }
    ...
    def update_jira_form(self, issue, data):
        ...
        answers = {
            "answers": {
                "4": {
                    "text": data['cod_fatura'] if 'cod_fatura' in data else None ##Codigo SAP FV60
                },
                "5": {
                    "text": data['status_liberacao'] if 'status_liberacao' in data else None ##Status de Aprovação
                },
                "7": {
                    "text": data['cod_miro'] if 'cod_miro' in data else None ##Codigo SAP Miro
                },
                "6": {
                    "text": data['data_liberacao'] if 'data_liberacao' in data else None ##Data de Aprovação
                },
                "1": {
                    "text": data['cod_nota_pedido'] if 'cod_nota_pedido' in data else None ##Codigo SAP Nota de Pedido
                }
            }
        }
