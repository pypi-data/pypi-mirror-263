        ...
        if self._initialized and state.is_world_process_zero:
            ...
            if self.upload_model:
                with TemporaryDirectory() as temp_dir:
                    ...
                    if response is None:
                        # if model was not created successfully, the function returns None
                        logger.exception(
                            f"Could not create model ({self.repository_name}). You may have tried to upload model in VESSL Run, which is not supported yet."
                        )
                    else:
                        model_number = response.number
                        for file_path in glob.glob(f"{temp_dir}/*"):
                            upload_model_volume_file(
                                self.repository_name, model_number, file_path, "/"
                            )
