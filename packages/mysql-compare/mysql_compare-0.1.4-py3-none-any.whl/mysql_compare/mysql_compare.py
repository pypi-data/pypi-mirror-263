...
class MysqlTableCompare:
    ...
    def result_worker(self, task_result_queue: queue.Queue):
        _batch_id = 0
        _tasks: list[tuple[int, int, list[dict], list[dict], int]] = []
        while True:
            task: tuple[int, int, list[dict], list[dict], int] = task_result_queue.get()
            if task is None:
                break
            ...
            while True:
                ...
                for i in range(len(_tasks) - 1, -1, -1):
                    ...
                    if batch_id == _batch_id:
                        ...
                        self.logger.debug(
                            f"threading release[{task_id}], progress: {round(self.processed_rows_number/max(1, self.source_table_rows_number)*100, 1)}%, {self.processed_rows_number}/{max(1, self.source_table_rows_number)}, elapsed time:{row_proc_etime}s"
                        )
    ...
    def worker(self, task_id: int, task_queue: queue.Queue, task_result_queue: queue.Queue):
        ...
        while True:
            ...
            if source_key_vals:
                for try_cnt in range(1, 11):
                    try:
                        _s_ts = time.time()
                        diff_rows = self.process_rows(task_id, src_conn, dst_conn, source_key_vals)
                        task_result_queue.put((task_id, batch_id, source_key_vals, diff_rows, get_elapsed_time(_s_ts)))
                        break
                    except Exception as e:
                        self.logger.error(f"threading running:[{task_id}] - {str(e)}.")
                        self.logger.error(f"threading running:[{task_id}] - retry: {try_cnt}/10")
    ...
    def process_rows(self, task_id: int, source_con: MySQLConnection, target_con: MySQLConnection, source_key_vals: list[dict]):
        _s_ts = time.time()
        _target_rows = get_table_rows_by_keys(target_con, self.dst_database, self.dst_table, self.source_table_keys, source_key_vals)
        self.logger.debug(f"threading running:[{task_id}] - target rows query elapsed time {get_elapsed_time(_s_ts,2)}s, count: {len(_target_rows)}.")

        _s_ts = time.time()
        _source_rows = get_table_rows_by_keys(source_con, self.src_database, self.src_table, self.source_table_keys, source_key_vals)
        self.logger.debug(f"threading running:[{task_id}] - source rows query elapsed time {get_elapsed_time(_s_ts,2)}s, count: {len(_source_rows)}.")
    ...
    def run(self) -> None:
        ...
        for t_id in range(self.parallel):
            t = threading.Thread(
                target=self.worker,
                args=(
                    t_id,
        ...
        for rows in self.get_full_table_orderby_keys(self.fetch_size, self.source_table_keys, _checkpoint_row):
            self.task_queue.put([_batch_id, rows])
            _batch_id += 1
