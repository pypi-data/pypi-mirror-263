...
class RDataFrame(TObject):
    ...
    def __init__(self, rdf=None, **kwargs):
    ...
    @semistaticmethod
    def _awkward_array(self, rdf, columns:Optional[List[str]]=None):
        if columns is None:
            columns = list(rdf.GetColumnNames())
        import awkward as ak
        array = ak.from_rdataframe(rdf, columns)
        return array

    def awkward_array(self, columns:Optional[List[str]]=None):
        rdf = self.rdf
        if rdf is None:
            RuntimeError('RDataFrame instance not initialized')
        return self._awkward_array(rdf, columns=columns)
