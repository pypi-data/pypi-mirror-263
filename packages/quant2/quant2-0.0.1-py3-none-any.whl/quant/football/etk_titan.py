...
def time_str2secs(text: str, year: str = None):
    ...
    arr_date = [f"0{s}" if len(s) < 2 else s for s in str_date.split("-")]
    arr_time = [f"0{s}" if len(s) < 2 else s for s in str_time.split(":")]
...
def wt20240320_event_data_parse(event_data: list, points: list):
    ...
    indexes = [np.abs(timestamps - point).argmin()
               for point in points[1:]] + [-1]
    for idx, point in zip(indexes[:-1], points[1:]):
        offset = timestamps[idx] - point
        if offset > 60 or offset < -60:
            print("\n[W] quant.football.etk_titan.wt20240320_event_data_parse():")
...
def wt20240320_odds_data_parse(odds_data: list, points: list, year: str):
    last_data = sorted(odds_data, key=lambda x: x["crawler_end_time"])[-1]

    segment_1x2, first_1x2 = wt20240320_stat_odds(
        last_data["胜平负"], points, year
    )
    segment_overunder, first_overunder = wt20240320_stat_odds(
        last_data["进球数"], points, year
    )

    return segment_1x2, segment_overunder, first_1x2 + first_overunder


def wt20240320_stat_odds(data: list, points: list, year: str):
    ...
    for idx, point in zip(indexes[:-1], points):
        offset = timestamps[idx] - point
        if offset > 60 or offset < -60:
            print("\n[W] quant.football.etk_titan.wt20240320_stat_odds():")
            print(f"  find nearest timestamp: ({offset=}).abs > 60 seconds.")
            print(f"  use: {time_secs2str(timestamps[idx])}")
            print(f"  to match: {time_secs2str(point)}")
            print(f"  index of: {points.index(point)}")
    ...
    for a, b in zip(indexes[1:], indexes[:-1]):
        out_segment += wt20240320_stat_x(
            [x[2] for x in data[a:b]], weights[a:b]
        )
        out_segment += wt20240320_stat_x(
            [x[3] for x in data[a:b]], weights[a:b]
        )
        out_segment += wt20240320_stat_x(
            [x[4] for x in data[a:b]], weights[a:b]
        )
...
def wt20240320_stat_x(data: list, weights: list = None):
    if len(data) < 1:
        return [0., 0., 0.]
...
def feature_selection_wt20240320(base_data: list, event_data: list, odds_data: list):
    ...
    out_event = wt20240320_event_data_parse(event_data, points)
    ...
    out_odds = wt20240320_odds_data_parse(odds_data, points, year)
    out_1x2, out_overunder, out_first = out_odds
...
def extract_match_feature(match_section: list, match_info: list):
    ...
    vals = feature_selection_wt20240320(base_data, event_data, odds_data)
