...
class Runpipe:
    ...
    def runpipe(self):
        ...
        if self.pairedreads:
            data_cmd += ['--pairedreads %s'%self.customize]
        if self.splitreads:
            data_cmd += [f'--splitreads {self.splitreads}']

        data_cmd = ' '.join(data_cmd)

        assembly_cmd = [
            "dnbc4tools vdj assembly",
            f"--name {self.name}",
            f"--outdir {self.outdir}",
            f"--threads {self.threads}",
            f"--coordinate {coordinate}",
        ...
        if self.pairedreads:
            assembly_cmd += ['--pairedreads %s'%self.customize]
        assembly_cmd = ' '.join(assembly_cmd)    

        #filter_cmd = 'dnbc4vdj filter --name %s  --outdir %s --genomeDir %s'\
        #    %(self.name,self.outdir,self.genomeDir) 
        
        #report_cmd = ['dnbc4vdj report --name %s --genomeDir %s --outdir %s --threads %s'
        ...
        if 'filter' in pipelist:
            cmdlist['filter'] = data_cmd
        if 'report' in pipelist:
            cmdlist['report'] = data_cmd
