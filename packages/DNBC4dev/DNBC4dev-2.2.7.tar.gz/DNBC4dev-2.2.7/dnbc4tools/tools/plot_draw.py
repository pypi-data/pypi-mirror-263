...
def merge_graph(selectbarcode: str, outdir: str) -> None:
    selectbarcode = pd.read_table(selectbarcode, sep='\t', header=None)
...
def violinatac_plot(singlecellsummary, outdir):
    ...
    sns.boxplot(
        ...
        showcaps=False)
    ax[0].axes.set_ylabel('')
    ax[0].axes.set_xlabel('fragments(Log10)')
    ...
    ax[1].axes.set_ylabel('')
    ax[1].axes.set_xlabel('TSS_Proportion')
    ...
    sns.boxplot(
        ...
        showcaps=False)
    ax[2].axes.set_ylabel('')
    ax[2].axes.set_xlabel('FRIP')

    fig.tight_layout()
    plt.savefig(
        '%s/summary.violin.png' % outdir,
        dpi=300,
        facecolor="white"
        )
