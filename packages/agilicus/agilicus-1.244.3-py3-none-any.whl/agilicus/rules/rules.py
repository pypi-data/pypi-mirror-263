...
def add_mfa_proof_rule(ctx, name, max_seconds, actions, purpose=None, **kwargs):
    token = context.get_token(ctx)
    apiclient = context.get_apiclient(ctx, token)
    kwargs = strip_none(kwargs)
...
def add_ruleset(ctx, name, trees, labels, org_id=None, scopes=None, **kwargs):
    token = context.get_token(ctx)
    apiclient = context.get_apiclient(ctx, token)
    org_id = get_org_from_input_or_ctx(ctx, org_id=org_id)
...
def replace_ruleset(
    ctx, ruleset_id, org_id, name=None, trees=None, labels=None, scopes=None, **kwargs
):
    token = context.get_token(ctx)
    apiclient = context.get_apiclient(ctx, token)
    org_id = get_org_from_input_or_ctx(ctx, **kwargs)
