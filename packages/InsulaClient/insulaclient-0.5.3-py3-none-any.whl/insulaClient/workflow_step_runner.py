...
class InsulaWorkflowStepRunner(object):
    ...
    def __init__(self,
        ...
        self.__max_parallel_jobs = self.__workflow_data_manager.config['max_parallel_jobs']
    ...
    def __translate_values(self, param: dict) -> list[StepResult]:
        ...
        for value in [param['values']] if isinstance(param['values'], str) else param['values']:
            job_results.extend(Placeholder(value, self.__workflow_data_manager).get_results())
    ...
    def __run_s3_step(self, step):
        ...
        s3_connections = self.__workflow_data_manager.requirements['connections'][step['connection']]
        ...
        if step['action'] == 'pull':
            for param in step['params']:
                if 'save_in' in param:
                    ...
                    # save_in_c = (self.__workflow_data_manager.parse_match(param['save_in']).get_param_changed())

                    # TODO: da sistemare!
                    save_in_c = (Placeholder(param['save_in'], self.__workflow_data_manager).get_results())
            ...
            for param in step['params']:
                if 'save_in' in param:
                    for file in self.__translate_values(param):
                        try:
                            # TODO: da sistemare!
                            # save_in_c = (self.__workflow_data_manager.parse_match(param['save_in'])
                            #              .get_param_changed())
                            save_in_c = (Placeholder(param['save_in'], self.__workflow_data_manager).get_results())
