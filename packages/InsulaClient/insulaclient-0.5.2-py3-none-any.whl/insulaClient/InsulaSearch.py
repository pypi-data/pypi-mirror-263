...
class InsulaSearch(object):
    ...
    def query(self, query: InsulaQuery, call_back=None, **kwargs):
        template = f'{self.__insula_api_config.get_search_api_path()}?{self.__page_for_page}&{str(query)}'
    ...
    def __load_data_from_url(self, url_search_metadata) -> dict:
        search_result = requests.get(url_search_metadata, headers=self.__insula_api_config.get_headers())
