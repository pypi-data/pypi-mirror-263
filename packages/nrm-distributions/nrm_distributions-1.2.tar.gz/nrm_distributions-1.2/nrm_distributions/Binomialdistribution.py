...
class Binomial(Distribution):
    ...
    def plot_bar(self):
        ...
        count_of_ones = len([x for x in self.data if x == 1])
        count_of_zeros = len([y for y in self.data if y == 0])
        plt.bar([0, 1], [count_of_zeros, count_of_ones])        
    ...
    def plot_bar_pdf(self):
        ...
        k_values = self.n
        probability = self.pdf
        plt.bar(k_values, probability)
        plt.xlabel('k')
    ...
    def __repr__(self):
        ...
        return f"mean {self.mean}, standard deviation {self.std_dev}, p {self.p}, n {self.n}"
