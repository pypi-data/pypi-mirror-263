...
class ConstraintCSPProblem:
    ...
    def var_has_type(self, var, etype):
        ...
        self.add_expr((var,), f"{var} == {etype!r}")
    ...
    def var_has_types(self, var, etypes):
        ...
        if len(etypes) == 1:
            ...
            cstr = f"{var} in {etypes} "
        self.add_expr((var,), cstr)

    def vars_have_same_types(self, varnames, types):
        self.add_expr(varnames, f"{'=='.join(varnames)} in {types}")
        ...
        for orred_expr in equalities:
            ...
            for vars, types in orred_expr:
                ...
                for t in types:
                    assert isinstance(t, str)
                if len(types) == 1:
                    anded.add(f"{'=='.join(vars)} == \"{types[0]}\"")
                else:
                    anded.add(f"{'=='.join(vars)} in {types}")
                for var in vars:
                    variables.add(var)
...
class GecodeCSPProblem:
    ...
    def pretty_print_ops(self, ops):
        if ops[0] in (_AND, _OR):
            ...
            return f"{self.ivariables[ops[1]]}=={self.ivalues[ops[2]]}"
        elif ops[0] == _EQV:
...
class ETypeResolver:
    ...
    def solve(
        ...
        if not sols:
            ...
            ex_msg = f'Unable to resolve variables types in "{rql}"'
            if self.debug:
                ex_msg += f"\n{constraints.get_output()}"
    ...
    def _visit(
        ...
        * constraints: a XxxCSPProblem object.
        """
        func = getattr(self, f"visit_{node.__class__.__name__.lower()}")
