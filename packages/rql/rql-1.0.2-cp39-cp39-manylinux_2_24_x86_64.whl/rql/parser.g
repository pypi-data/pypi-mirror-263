...
parser Hercule:
    ...
    token QMARK:       r'\?'
    token FUNCTION_CALL: r'[A-Z][A-Z0-9_]*\.[a-z_][a-zA-Z0-9_]+'
...
rule selection<<S>>: selection_function<<S>>
                     (  ',' selection_function<<S>>
                     )*


rule selection_function<<S>>: expr_add<<S>>                {{ S.append_selected(expr_add); }}
                              | FUNCTION_CALL (
                                 '\('function_args'\)'     {{ S.append_selected_method_call(FUNCTION_CALL, function_args); }}
                                 |                         {{ S.append_selected_attribute_access(FUNCTION_CALL); }}
                                 )

rule function_args: const             {{ args = [const] }}
                    ( ',' const       {{ args.append(const) }}
                    )*                {{ return args }}
                    |                 {{ return [] }}
