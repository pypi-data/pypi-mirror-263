...
BROKEROUTPUTBROKEROUTPUTSINNER_ONE_OF_SCHEMAS = [
...
class BrokerOutputBrokerOutputsInner(BaseModel):
    """
    ...
    oneof_schema_2_validator: Optional[CarrierOutput] = None
    # data type: SqlRaw
    oneof_schema_3_validator: Optional[SqlRaw] = None
    # data type: SyncResponse
    oneof_schema_4_validator: Optional[SyncResponse] = None
    actual_instance: Any
    one_of_schemas: List[str] = Field(
        BROKEROUTPUTBROKEROUTPUTSINNER_ONE_OF_SCHEMAS, const=True
    )
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = BrokerOutputBrokerOutputsInner.construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in BrokerOutputBrokerOutputsInner with oneOf schemas: AzureBlobStorageOutput, CarrierOutput, SqlRaw, SyncResponse. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in BrokerOutputBrokerOutputsInner with oneOf schemas: AzureBlobStorageOutput, CarrierOutput, SqlRaw, SyncResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> BrokerOutputBrokerOutputsInner:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> BrokerOutputBrokerOutputsInner:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into BrokerOutputBrokerOutputsInner with oneOf schemas: AzureBlobStorageOutput, CarrierOutput, SqlRaw, SyncResponse. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into BrokerOutputBrokerOutputsInner with oneOf schemas: AzureBlobStorageOutput, CarrierOutput, SqlRaw, SyncResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
