...
BROKERBROKEROUTPUTSINNER_ONE_OF_SCHEMAS = [
...
class BrokerBrokerOutputsInner(BaseModel):
    """
    ...
    one_of_schemas: List[str] = Field(
        BROKERBROKEROUTPUTSINNER_ONE_OF_SCHEMAS, const=True
    )
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = BrokerBrokerOutputsInner.construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in BrokerBrokerOutputsInner with oneOf schemas: AzureBlobStorageOutput, CarrierOutput, SqlRaw, SyncResponse. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in BrokerBrokerOutputsInner with oneOf schemas: AzureBlobStorageOutput, CarrierOutput, SqlRaw, SyncResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> BrokerBrokerOutputsInner:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> BrokerBrokerOutputsInner:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into BrokerBrokerOutputsInner with oneOf schemas: AzureBlobStorageOutput, CarrierOutput, SqlRaw, SyncResponse. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into BrokerBrokerOutputsInner with oneOf schemas: AzureBlobStorageOutput, CarrierOutput, SqlRaw, SyncResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
