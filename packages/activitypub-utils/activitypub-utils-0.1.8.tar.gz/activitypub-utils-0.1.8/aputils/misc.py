...
from .enums import AlgorithmType, SignatureAlgorithm
...
class HttpDate(datetime):
	"Datetime object with convenience methods to parsing and creating HTTP date strings"
	...
	@classmethod
	def parse(cls: type[HttpDate], date: str) -> HttpDate:
		"""
			Parse an HTTP date in string format

			:param date: String from an HTTP Date header
		"""
...
class JsonBase(dict):
	...
	def new_from_json(cls: type[JsonBase], data: str | bytes | dict | JsonBase) -> JsonBase:
		"""
			Parse an ActivityPub object

			.. deprecated:: 0.1.5
				Use :meth:`JsonBase.parse` instead

			:param data: ActivityPub object to parse
	...
	@classmethod
	def parse(cls: type[JsonBase], data: str | bytes | dict | JsonBase) -> JsonBase:
		"""
			Parse an ActivityPub object
...
class Signature:
	...
	def __init__(self,
		...
		self.headers: list[str] = headers # type: ignore
	...
	def __setattr__(self, key: str, value: Any) -> None:
		if key == "headers" and isinstance(value, str):
			...
			value = SignatureAlgorithm.parse(value)
	...
	def algorithm_type(self) -> AlgorithmType:
		...
		if self.algorithm.name == "HS2019":
			return AlgorithmType.HS2019

		if self.algorithm.name.startswith(("ECDSA", "RSA")):
