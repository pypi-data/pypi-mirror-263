...
from .misc import HttpDate, JsonBase
...
CONVERTERS: dict[type, tuple[Callable, Callable]] = {
	datetime: (HttpDate.parse, lambda v: v.strftime(HttpDate.FORMAT)),
...
class Message(JsonBase):
	...
	closed: Property[HttpDate] = Property("date")
	"``closed`` key"
	...
	created: Property[HttpDate] = Property("date")
	"``created`` key"

	current: Property[str] | Message = Property("object")
	"``current`` key"

	deleted: Property[HttpDate] = Property("date")
	"``deleted`` key"
	...
	end_time: Property[HttpDate] = Property("date")
	"``endTime`` key"
	...
	published: Property[HttpDate] = Property("date")
	"``published`` key"
	...
	start_time: Property[HttpDate] = Property("date")
	"``startTime`` key"
	...
	updated: Property[HttpDate] = Property("date")
	"``updated`` key"
	...
	def new(cls,
			obj_type: ObjectType,
			data: dict[str, Any],
			context: list[Any] | None = None) -> Message:
		"""
	...
	@classmethod
	def new_image(cls, url: str, mimetype: str | None = None) -> Message:
		"""
	...
	def add_image(self, key: str, url: str) -> None:
		"""
			...
			:param url: Location of the image
		"""
		self[key] = Message.new_image(url)
