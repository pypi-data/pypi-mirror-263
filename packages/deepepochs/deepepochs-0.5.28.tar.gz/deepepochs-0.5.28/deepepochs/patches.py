...
class TensorPatch(PatchBase):
    ...
    def forward(self):
        ...
        except (RuntimeError, TypeError): # 如果concat失败则不再concat，以应对复杂的模型输出
            preds = self.batch_preds[0] if len(self.batch_preds) == 1 else self.batch_preds
            targets = self.batch_targets[0] if len(self.batch_targets) == 1 else self.batch_targets
