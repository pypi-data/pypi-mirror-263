...
from relationalai.rel import sanitize
...
def read_resource_context(graph: Graph, name: str, **kwargs):
    source_steps = kwargs.get("source_steps", 1) + 2 # offset for the contextmanager to get back to user code
    ctx = Context(graph, **kwargs, source_steps=source_steps)
    with ctx:
