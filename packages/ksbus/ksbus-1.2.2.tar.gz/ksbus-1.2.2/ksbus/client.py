...
class Bus:
    def __init__(self, options, block=False):
        ...
        self.OnData = options.get('OnData', None)
        self.OnId = options.get('OnId', lambda data: None)
        self.id = options.get('id') or self.makeId(8)
        if block:
        ...
        try:
            ...
            async for message in self.conn:
                ...
                if self.OnData is not None:
                    self.OnData(obj)
                if "event_id" in obj:
                    self.Publish(obj["event_id"], {"success": "got the event", "id": self.id})
                if "to_id" in obj:
    ...
    def PublishToIDWaitRecv(self, id, topic, data, onRecv, onExpire):
        data["from"] = self.id
        data["id"] = id
        eventId = self.makeId(8)
        data["event_id"] = eventId
