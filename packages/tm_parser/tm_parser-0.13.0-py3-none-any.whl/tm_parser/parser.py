...
class Parser:
    def __init__(self, infile=None, config=Config, outfile=None, file_format=None):
        logger.warn(f"in parser, file_format = {file_format}")
        self.config = Config
        if infile:
    ...
    def dump(self, outfile=None, file_format=None):
        ...
        if file_format:
            logger.warn(f"provided file format {file_format}")
            filetype = file_format
        elif self.output_format:
            logger.warn(f"provided self.output_format: {self.output_format}")
            filetype = self.output_format
        elif file.split(".")[-1].lower() in ("yaml", "toml", "json"):
            logger.warn(f"using file ending: {file.split('.')[-1].lower()}")
            filetype = file.split(".")[-1].lower()
        else:
            raise ValueError("no output format specified")

        logger.warn(f"file format is {filetype}")
