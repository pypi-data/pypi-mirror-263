# coding: utf-8

"""
    API Docs

    Use the interactive documentation to explore the endpoints CDO has to offer

    The version of the OpenAPI document: 0.0.1
    Contact: cdo.tac@cisco.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from cdo_sdk_python.models.device_patch_input import DevicePatchInput

class TestDevicePatchInput(unittest.TestCase):
    """DevicePatchInput unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> DevicePatchInput:
        """Test DevicePatchInput
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `DevicePatchInput`
        """
        model = DevicePatchInput()
        if include_optional:
            return DevicePatchInput(
                name = 'My ASA',
                labels = cdo_sdk_python.models.labels.Labels(
                    grouped_labels = {"group1":["label-1","label-2"],"group2":["label-1","label-2"]}, 
                    ungrouped_labels = ["label-a","label-b","label-c"], )
            )
        else:
            return DevicePatchInput(
        )
        """

    def testDevicePatchInput(self):
        """Test DevicePatchInput"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
