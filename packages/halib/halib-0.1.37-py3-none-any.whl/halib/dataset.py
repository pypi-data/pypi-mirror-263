...
def split_dataset_cls(indir, outdir, val_size, seed, inplace, stratified_split, no_train):
    ...
    sss = ShuffleSplit(n_splits=1, test_size=val_size) if not stratified_split else StratifiedShuffleSplit(n_splits=1, test_size=val_size)
...
def reverse_split_ds(indir):
    ...
    all_train_files = fs.filter_files_by_extension(train_dir, ["jpg", "jpeg", "png", "bmp", "gif", "tiff"])
    all_val_files = fs.filter_files_by_extension(val_dir, ["jpg", "jpeg", "png", "bmp", "gif", "tiff"])
...
def main():
    ...
    if(outdir == "."):
        ...
        indir_parent_dir = os.path.dirname(
