...
from solana_transaction.src.transaction import Transaction
...
class _TokenCore:  # pylint: disable=too-few-public-methods
    ...
    def _create_mint_args(
        ...
        txn = Transaction(fee_payer=payer)
        txn.add(
            sp.create_account(
    ...
    def _create_account_args(
        ...
        # Construct transaction
        txn = Transaction(fee_payer=self.payer)
        txn.add(
            sp.create_account(
                sp.CreateAccountParams(
    ...
    def _transfer_args(
        ...
        txn = Transaction(fee_payer=self.payer).add(
            spl_token.transfer(
    ...
    def _freeze_account_args(
        ...
        txn = Transaction(fee_payer=fee_payer_keypair).add(
