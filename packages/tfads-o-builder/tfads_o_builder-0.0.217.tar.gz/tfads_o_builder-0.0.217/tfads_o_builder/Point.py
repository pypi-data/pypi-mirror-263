...
class Point:
    ...
    def __getattr__(self, called_set_fn_name=""):
        def default_function(*args, **kwargs):
            # If called_fn_name does not start with "set_", then return None
            if not called_set_fn_name.startswith("set"):
                raise AttributeError(
                    f"The attribute {called_set_fn_name} must start with 'set'"
                )

            # Trim the "set_" from the called_fn_name
            called_fn_name = called_set_fn_name[3:]
