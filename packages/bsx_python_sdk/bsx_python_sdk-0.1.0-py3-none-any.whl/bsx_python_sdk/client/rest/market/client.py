...
class MarketClient(AuthRequiredClient):
    ...
    def cancel_order(self, order_id: Optional[str], nonce: Optional[str]) -> CancelOrderResult:
        resp = self.delete("/order", {"order_id": order_id, "nonce": nonce})
        if resp.get(order_id) == order_id or resp.get(nonce, 0) == nonce:
            return CancelOrderResult(**resp)
        else:
            raise Exception(f"Cancel order failed. Order id: {order_id}, nonce: {nonce}. Response: {str(resp)}")
    ...
    def cancel_orders(self, params: CancelMultipleOrdersParams):
        ...
        resp = self.delete("/orders", payload)
    ...
    def cancel_all_orders(self, product_id: str) -> CancelMultipleOrdersResult:
        ...
        resp = self.delete("/orders/all", payload)
        return CancelMultipleOrdersResult(
            cancelled_orders=[
    ...
    def get_open_orders(self, product_id: str) -> OrderListingResult:
        ...
        return OrderListingResult(
            orders=[Order(**i) for i in resp.get("orders", [])]
        )
