...
openai_counter = meter.create_counter("openai")
anthropic_counter = meter.create_counter("anthropic")
dlp_counter = meter.create_counter("dlp")
...
class Obex:
	...
	def increase_counter(self, provider, dlp):
		if provider == "openai":
			openai_counter.add(1)
		if provider == "anthropic":
			anthropic_counter.add(1)
		if dlp:
			dlp_counter.add(1)
	...
	def protect(self, func):
		...
		def new_send(*args, **kwargs):
			...
			self.increase_counter(self.get_provider(request.url), dlp_check)
			if self.get_provider(request.url) == "openai":
				openai_counter.add(1)
			if dlp_check:
