...
import json
from .extract import DossierCompetence, get_content
...
class CV:
    ...
    def make_html(self, output_path_name, asset_path, anonimize: bool):
        """make cv in html"""
        content_filled = self.template_render(f"../../{asset_path}")
        if anonimize:
    ...
    def make_pdf(self, output_path_name, asset_path, anonimize: bool):
        """make cv in pdf"""
        content_filled = self.template_render(asset_path)
        if anonimize:
            content_filled = self.anonimize(content_filled)
        output_path_name = output_path_name + ".pdf"
        with open(f"{asset_path}/print.css", "rb") as css:
