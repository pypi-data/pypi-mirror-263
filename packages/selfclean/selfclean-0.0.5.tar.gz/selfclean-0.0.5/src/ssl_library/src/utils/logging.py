...
def visualize_worst_duplicate_ranking(
    ...
    for n_worst in range(imgs_to_visualize):
        ...
        if paths is not None:
            ax[n_worst, 0].set_title(os.path.basename(paths[i]))
        ax[n_worst, 1].set_title(f"cos dist: {sim:.4f}")
...
def calculate_scores_from_ranking(
    ...
    if show_scores:
        print(f"AUROC (%): {score_auc*100:.1f}")
        print(f"AP (%): {score_ap*100:.1f}")
        print(f"AUPRG (%): {score_auprg*100:.1f}")
        print(f"Percentage Pos. (%): {pi*100:.1f}")
    ...
    if show_plots:
        with plt.style.context(["science", "std-colors", "grid"]):
            ...
            fig.tight_layout()
            log_dict[f"{wandb_cat}evaluation/evaluation"] = wandb.Image(fig)
            if path is not None:
