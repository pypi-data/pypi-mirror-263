...
class huobi(baseSpotExchange.BaseSpotExchange):
    ...
    ## parsers
    # free and total are same as no segregation in api response
    def _parseBalance(self, balData):
        ...
        for balDataEle in balData["data"]["list"]:
            parsedBal["free"][balDataEle["currency"].upper()] = balDataEle.get("balance", None)
            parsedBal["total"][balDataEle["currency"].upper()] = balDataEle.get("balance", None)
        return parsedBal
    ...
    def _parseTrades(self, trades):
        def parseTrade(trade):
            ...
            parsedTrade["takerOrMaker"] = "taker" if trade["type"].endswith("market") else "maker"
            if "execTime" in trade:
                parsedTrade["timestamp"] = trade["finished-at"]
                parsedTrade["datetime"] = general.ts_to_datetime(parsedTrade["timestamp"])
            else: 
                parsedTrade["timestamp"] = None
                parsedTrade["datetime"] = None
            parsedTrade["feeCurrency"] = None
            parsedTrade["fee"] = trade["field-fees"]
            return parsedTrade
        parsedTradeList = []
    ...
    def fetch_my_trades(self, symbol=None, since=None, limit=None, params={}):
        apiUrl = '/v1/order/orders'
        queryParams = {"symbol": self._getSymbol(symbol), "states": "filled"}
        if since:
