...
class bingx(baseSpotExchange.BaseSpotExchange):
    ...
    def _parseTrades(self, trades):
        def parseTrade(trade):
            ...
            parsedTrade["side"] = None  # bingx doesnt prvide side
            parsedTrade["price"] = trade.get("price", None)
            parsedTrade["amount"] = trade.get("amount", None)
            parsedTrade["takerOrMaker"] = None # bingx doesnt return taker or maker role
            if "filledTime" in trade:
                parsedTrade["timestamp"] = general.datetime_to_ts_ms(parsedTrade["filledTime"])
                parsedTrade["datetime"] = parsedTrade["filledTime"]
            else: 
    ...
    def fetch_my_trades(self, symbol=None, since=None, limit=None, params={}):
        apiUrl = '/openApi/swap/v2/trade/allFillOrders'
        queryParams = {
            "endTs": params['end_time'] if 'end_time' in params else str(general.get_curr_timestamp_ms()),
            "startTs": since if since else str(general.get_1d_ago_ts_ms()),
            "symbol": self._getSymbol(symbol),
            "tradingUnit": "COIN",
            "timestamp": str(general.get_curr_timestamp_ms())
        }
        try:
            resp = self._send_request(method='GET', path=apiUrl, params=queryParams, payload=None)
