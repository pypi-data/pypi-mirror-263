...
from FastCNN.prx.TrainProxy2 import TrainProxy,_Eendflag
from FastCNN.prx.YoloV5TrainProxy import doTrain,_Yendflag
...
class TrainResource:
    ...
    def startTrainENet(self,projectid,modelid):
        if not self.isTraining():
            _thx = Thread(target=doTrain,args=(projectid,modelid))
            _thx.start()
    
    def startTrainYNet(self,projectid,modelid):
        if not self.isTraining():
            _thx = Thread(target=TrainProxy.startTrainModel,args=(projectid,modelid))
            _thx.start()
...
class FastCNNApi(Resource):
    ...
    def startTrainEfficient():
        ...
        insTrain.startTrainENet(projectid,modelid)
        rtn['success'] = True
    ...
    def startTrainYoloV5():
        ...
        if not modelid:
            ...
            return rtn
        print(rtn)
        insTrain.startTrainYNet(projectid,modelid)
        rtn['success'] = True
