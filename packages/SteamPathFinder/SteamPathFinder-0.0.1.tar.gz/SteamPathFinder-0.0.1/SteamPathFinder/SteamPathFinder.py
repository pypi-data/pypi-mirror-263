...
def get_vdf_file_path(steam_path):
    ...
    if not os.path.exists(vdf_file_path):
        print("ERROR: libraryfolders.vdf file does not exist.")
        return None
...
def get_steam_path():
    try:
        ...
        print("ERROR: Steam is not installed.")
        return None
    
    if not os.path.exists(steam_path):
        print("ERROR: The Steam path does not exist or is not accessible.")
        return None
...
def get_app_path(steam_path, app_id):
    ...
    try:
        ...
        print(f"ERROR: Unable to read the libraryfolders.vdf file. {e}")
        return None
        
    try:
        parsed = vdf.loads(data)
    except Exception as e:
        print(f"ERROR: Error parsing the VDF file. {e}")
        return None
    ...
    if app_path is None:
        print(f"ERROR: Could not find the installation path for app {app_id}.")
...
def get_game_path(steam_path, app_id, game_name):
    ...
    if not os.path.exists(game_path):
        print(f"ERROR: The game path does not exist. Check if the game name '{game_name}' is correct and the game is properly installed.")
        return None
