...
from censius.nlp.llm_service import LLMServices
...
class CensiusClient(object):
    def __init__(self, api_key, project_id):
        ...
        if project_id is None or type(project_id) != int:
    ...
    def register_dataset_llm(self, *args, **kwargs):
        response = LLMServices.register_dataset(
            kwargs["file_path"], kwargs["dataset_type"], kwargs["dataset_name"], self
        )

        return response

    def register_model_llm(self, *args, **kwargs):
        model_data = {
        ...
        response = LLMServices.register_model(
            kwargs["dataset_id"], model_data, self, kwargs["parent_model_id"]
        )

        return response

    def register_logs(self, *args, **kwargs):
        response = LLMServices.register_logs(
    ...
    # def register_bulk_logs_llm(self, *args, **kwargs):
    #     response = LLMServices.register_bulk_logs_llm(
    #         model_id=kwargs["model_id"], file_path=kwargs["file_path"], client_obj=self
    #     )
    #     return response
